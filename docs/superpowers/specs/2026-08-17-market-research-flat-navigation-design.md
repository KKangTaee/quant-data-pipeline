...
이번 변경은 경기 국면과 물가·정책을 다른 시장 환경 화면과 같은 깊이로 올리고, 모바일 탐색을 한 줄의 얇은 레일로 축소해 사용자가 본문에 더 빨리 도달하게 한다.
...
- family rail은 `시장 환경 | 지수 가치평가 | 종목 리서치` 3열을 유지한다.
- view rail은 2열 grid나 full-width button을 사용하지 않는다.
- view는 높이 32~36px의 한 줄 text tab으로 표시한다.
- 선택 view는 fill 대신 text weight, underline, subtle background만 사용한다.
- 360px 이상에서는 다섯 view가 한 줄에 최대한 맞고, 더 좁은 폭에서는 view rail만 horizontal swipe를 허용한다.
- scrollbar는 시각적으로 숨기되 native keyboard focus와 touch scroll은 보존한다.
...
- `app/web/streamlit_components/market_research_navigation/`: five-view navigation payload fixtures and compact mobile CSS
...
- Browser QA at desktop, 736px and 360px: selected state, one-line mobile view rail, horizontal overflow boundary, both direct routes, zero console errors
