...
두 패널은 하나의 `xDomain = [start, end]`를 사용한다.

- anchor/end: CNN과 AAII 관측 중 가장 최신 날짜
- `6M`: anchor에서 6개월 전
- `1Y`: anchor에서 12개월 전
- `공통 전체`: `max(CNN canonical start, AAII canonical start)`

AAII는 주간이므로 최신점이 CNN보다 며칠 앞에서 끝날 수 있지만 축의 시작과 끝은 동일하다. 빈 날짜를 보간하거나 직선을 최신일까지 연장하지 않는다.
...
1. 사용자는 기본 `6M`에서 두 그래프를 같은 시작일/종료일로 비교한다.
...
- 공통 전체의 시작은 두 canonical 시작일 중 늦은 날짜다.
