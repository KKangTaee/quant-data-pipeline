...
**Architecture:** Extend the deterministic observed-state transition record with persistent anchor metadata, normalize legacy metadata in the DB-only Overview service, and keep the 12-month payload while selecting four chart checkpoints in React. Freshness remains synchronous and fail-closed, but the read model and UI distinguish successful collection time, calculation cutoff, and actual source observation date.
...
- [x] **Step 1: Write failing initialization and promotion tests**
...
- [x] **Step 3: Implement minimal sequential provenance state**
...
- Produces: normalized anchor provenance labels plus `last_successful_collection_at` and `latest_source_observation_date` in `data_freshness`.
...
Run: `.venv/bin/python -m pytest tests/test_economic_cycle_service.py tests/test_economic_cycle_freshness.py -q`
...
- [x] **Step 3: Implement minimal helpers and markup**
...
- [x] **Step 4: Run Vitest and verify GREEN**
...
- [x] **Step 1: Run the full focused Python suite**

Run: `.venv/bin/python -m pytest tests/test_economic_cycle_observed_state_v1.py tests/test_economic_cycle_freshness.py tests/test_economic_cycle_service.py tests/test_economic_cycle_refresh.py tests/test_market_context_economic_cycle.py -q`

Expected: all tests pass.

- [x] **Step 2: Run static checks**

Run: `.venv/bin/python -m py_compile finance/economic_cycle_observed_state.py app/services/overview/economic_cycle.py app/services/overview/economic_cycle_freshness.py` and `git diff --check`

Expected: exit 0 with no whitespace errors.

- [x] **Step 3: Verify live Streamlit UI**
...
- [x] **Step 4: Review frozen asset boundary**
