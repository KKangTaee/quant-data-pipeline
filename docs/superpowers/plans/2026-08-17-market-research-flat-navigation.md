...
**Goal:** Flatten Market Research so 경기 국면 and 물가·정책 are sibling views and keep mobile subtabs in the same compact, wrapping style as desktop.
...
- Mobile subtabs must retain desktop pill styling and wrap naturally without page-level horizontal overflow.
...
### Task 3: A-style navigation and desktop-parity mobile subtabs
...
- Produces: desktop editorial rail and desktop-parity wrapping mobile rail.
...
- [ ] Remove the mobile two-column grid and mobile-only overrides so family/view rails reuse desktop styling and wrap naturally; mirror the contract in the Streamlit fallback CSS.
