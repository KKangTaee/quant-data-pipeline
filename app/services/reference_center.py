...
REFERENCE_CENTER_ITEMS: list[dict[str, Any]] = [
    _item(
        ...
        summary="시장 환경, 지수 가치평가, 종목 리서치를 한 흐름으로 읽습니다.",
        ...
        related_surfaces=["Market Research"],
        meaning="Market Research의 시장 환경·지수 가치평가·종목 리서치 관점을 함께 보며 현재 시장의 방향과 자료 한계를 구분하는 시작 흐름입니다.",
        impact="한 지표만으로 결론 내리지 않고 경기, 물가·정책, 선물, 심리, 일정, 지수, 변동 종목과 개별 기업 근거가 같은 이야기를 하는지 확인할 수 있습니다.",
        next_action="Market Research에서 경기 국면이나 물가·정책을 먼저 읽고 필요한 지수·종목 관점으로 이동합니다.",
        related_item_ids=[
    ...
    _item(
        ...
        kind="concept",
        category="Market Research 기능",
        title="Market Context",
        summary="시장 가격, 거시 조건, 자료 신뢰도를 합쳐 오늘의 맥락을 읽는 화면입니다.",
        aliases=["시장 맥락", "오늘의 시장 맥락"],
        keywords=["overview", "brief", "macro", "context"],
        related_surfaces=["Market Research"],
        meaning="여러 시장 근거가 현재 환경을 어떻게 설명하는지 요약하는 Market Research 관점입니다.",
        impact="개별 신호를 독립적인 매매 결론으로 오해하지 않고 공통 맥락에서 읽게 합니다.",
        next_action="핵심 결론과 자료 제한을 읽고 필요하면 Economic Cycle이나 다른 Market Research 관점을 확인합니다.",
    ...
    _item(
        item_id="feature.inflation_policy",
        kind="concept",
        category="Market Research 기능",
        title="Inflation / Policy",
        summary="Core PCE, FOMC 정책 경로, 금리와 조건부 S&P 500 스트레스를 분리해 읽습니다.",
        aliases=["물가·정책", "inflation policy", "금리 경로"],
        keywords=["core pce", "fomc", "policy", "yield", "equity stress"],
        related_surfaces=["Market Research"],
        meaning="저장된 Point-in-Time macro와 policy artifact로 물가·정책·금리·equity 조건부 시나리오를 읽는 관점입니다.",
        impact="경제 사이클 결과나 Shiller proxy를 임의 fallback으로 섞지 않고 READY / LIMITED / NOT_AVAILABLE 상태를 구분하게 합니다.",
        next_action="연말 Core PCE 상태, 다음 발표·FOMC 경로, 10년물 조건과 equity stress 제한 사유를 순서대로 확인합니다.",
        related_item_ids=["journey.market_understanding", "feature.economic_cycle", "concept.data_trust"],
        destination="overview",
    ),
    _item(
    ...
    _item(
        item_id="feature.sp500",
        kind="concept",
        category="Market Research 기능",
        title="S&P 500",
        summary="S&P 500 가치평가와 EPS 근거를 현재 시장 맥락의 지수 관점으로 읽습니다.",
        aliases=["에스앤피 500", "sp500", "index valuation"],
        keywords=["s&p 500", "valuation", "per", "eps", "cape"],
        related_surfaces=["Market Research"],
        meaning="S&P 500의 valuation, EPS source, 시나리오와 자료 제한을 지수 가치평가 family 안에서 보여주는 관점입니다.",
        impact="지수 숫자를 자동 목표가나 매매 신호로 해석하지 않고 source·기준일·actual/estimate 경계를 함께 보게 합니다.",
        next_action="최근 valuation 구간, EPS source 상태, 시나리오 제한과 Market Research의 다른 시장 환경 근거를 비교합니다.",
        related_item_ids=["journey.market_understanding", "feature.inflation_policy", "concept.data_trust"],
        destination="overview",
    ),
    _item(
    ...
    _item(
        item_id="feature.us_stock",
        kind="concept",
        category="Market Research 기능",
        title="U.S. Stock Research",
        summary="미국 개별 종목의 가격·재무·valuation 근거를 read-only로 조사합니다.",
        aliases=["개별 종목", "미국 주식", "stock research"],
        keywords=["ticker", "per", "financials", "valuation", "us stock"],
        related_surfaces=["Market Research"],
        meaning="선택 종목의 PER 상대가치, 전환 분석, 가격·재무 근거와 자료 gap을 분리해 보는 관점입니다.",
        impact="개별 종목 조사 결과를 자동 추천, validation PASS, monitoring decision으로 착각하지 않게 합니다.",
        next_action="종목을 선택하고 valuation readiness, 재무제표 source, 필요한 데이터 보강 여부를 확인합니다.",
        related_item_ids=["journey.market_understanding", "feature.market_movers", "concept.provider_coverage"],
        destination="overview",
    ),
    _item(
    ...
    _item(
        ...
        related_surfaces=["Market Research", "Backtest Analysis", "Practical Validation"],
    ...
    _item(
        ...
        related_surfaces=["Institutional Holdings", "Data Operations", "Practical Validation"],
    ...
    _item(
        ...
        next_action="결측 대상과 기간을 확인하고 Data Operations에서 해당 수집만 실행한 뒤 원래 화면으로 돌아갑니다.",
