...
WHY_IT_MOVED_SEC_PREVIEW_MAX_BYTES = 1_500_000
WHY_IT_MOVED_SEC_PREVIEW_SNIPPET_CHARS = 420
WHY_IT_MOVED_SEC_PREVIEW_MAX_SECTIONS = 8
...
def _normalize_sec_preview_filing(filing: Any) -> dict[str, Any]:
    if isinstance(filing, pd.Series):
        ...
        raw = {}
    url = (
        _clean_optional_text(raw.get("URL"))
        or _clean_optional_text(raw.get("Open"))
        or _clean_optional_text(raw.get("열기"))
    )
    form = _clean_optional_text(raw.get("Form")) or _clean_optional_text(raw.get("form"))
    filing_date = (
        _metadata_timestamp(raw.get("Filing Date"))
        or _metadata_timestamp(raw.get("filing_date"))
        or _metadata_timestamp(raw.get("filingDate"))
    )
    title = (
        _clean_optional_text(raw.get("Title"))
        or _clean_optional_text(raw.get("title"))
        or _clean_optional_text(raw.get("description"))
        or form
    )
    accession, document = _extract_sec_accession_document(url)
    return {
        "form": form,
        "filing_date": filing_date,
        "title": title,
        "official_url": url,
        "accession": _clean_optional_text(raw.get("Accession")) or accession,
        "document": _clean_optional_text(raw.get("Document")) or document,
    }


def _extract_sec_accession_document(url: str | None) -> tuple[str | None, str | None]:
    text = _clean_optional_text(url)
    if not text:
        return None, None
    parsed = urlparse(text)
    path = parsed.path or ""
    query_doc = parse_qs(parsed.query).get("doc")
    if query_doc:
        path = unquote(str(query_doc[0] or ""))
    parts = [part for part in path.split("/") if part]
    accession: str | None = None
    document: str | None = parts[-1] if parts else None
    for idx, part in enumerate(parts):
        if part.lower() == "data" and idx + 2 < len(parts):
...
def _parse_sec_html(html_text: str) -> Any:
    import warnings
...
def _sanitize_sec_html_lines(html_text: str) -> list[str]:
    ...
    for tag in list(soup.find_all(True)):
        ...
        if tag.has_attr("hidden") or "display:none" in style or "visibility:hidden" in style:
            tag.decompose()
            continue
        if tag_name in {"ix:header", "ix:hidden", "ix:references", "ix:resources", "xbrli:xbrl"}:
            tag.decompose()
            continue
...
_SEC_8K_ITEM_RE = re.compile(r"^item\s+\d{1,2}\.\d{2}\b", re.IGNORECASE)
_SEC_ITEM_RE = re.compile(r"^item\s+\d+[a-z]?\.", re.IGNORECASE)
_SEC_PART_RE = re.compile(r"^part\s+[ivxlcdm]+\b", re.IGNORECASE)


def _is_sec_preview_heading(line: str) -> bool:
    text = str(line or "").strip()
    if not text:
        return False
    if text.lower() == "table of contents":
        return True
    return bool(_SEC_8K_ITEM_RE.match(text) or _SEC_ITEM_RE.match(text) or _SEC_PART_RE.match(text))


def _bounded_preview_snippet(lines: list[str], start_idx: int, *, max_chars: int = WHY_IT_MOVED_SEC_PREVIEW_SNIPPET_CHARS) -> str:
    snippets: list[str] = []
    for line in lines[start_idx + 1 :]:
        if _is_sec_preview_heading(line):
            break
        snippets.append(line)
        joined = " ".join(snippets).strip()
        if len(joined) >= max_chars:
            return joined[:max_chars].rstrip() + "..."
    return " ".join(snippets).strip()[:max_chars]


def _toc_preview_snippet(lines: list[str], start_idx: int, *, max_chars: int = WHY_IT_MOVED_SEC_PREVIEW_SNIPPET_CHARS) -> str:
    joined = " | ".join(lines[start_idx + 1 : start_idx + 9]).strip()
    if len(joined) > max_chars:
        return joined[:max_chars].rstrip() + "..."
    return joined
...
def _section_has_substance(section: dict[str, Any]) -> bool:
    return bool(str(section.get("snippet") or "").strip())
...
def _append_or_replace_section(
    ...
    key = _section_key(str(section.get("heading") or ""))
    if not key:
        return
    existing = sections_by_key.get(key)
    if existing is None or (_section_has_substance(section) and not _section_has_substance(existing)):
        sections_by_key[key] = section


def _extract_8k_preview_sections(lines: list[str]) -> list[dict[str, Any]]:
    sections: list[dict[str, Any]] = []
    for idx, line in enumerate(lines):
        if not _SEC_8K_ITEM_RE.match(line):
            continue
        sections.append(
            {
                "kind": "8-K Item",
                "heading": line,
                "snippet": _bounded_preview_snippet(lines, idx),
            }
        )
        if len(sections) >= WHY_IT_MOVED_SEC_PREVIEW_MAX_SECTIONS:
            break
    return sections


def _extract_periodic_preview_sections(lines: list[str]) -> list[dict[str, Any]]:
    sections_by_key: dict[str, dict[str, Any]] = {}
    for idx, line in enumerate(lines):
        normalized = line.strip().lower()
        is_toc = normalized == "table of contents"
        is_part = bool(_SEC_PART_RE.match(line))
        is_item = bool(_SEC_ITEM_RE.match(line))
        if not (is_toc or is_part or is_item):
            continue
        if is_toc:
            heading = line
            snippet = _toc_preview_snippet(lines, idx)
            kind = "Table of Contents"
        elif is_part:
            heading = line.upper()
            snippet = _bounded_preview_snippet(lines, idx, max_chars=180)
            kind = "Part heading"
        else:
            heading = line
            snippet = _bounded_preview_snippet(lines, idx)
...
def parse_market_mover_sec_filing_preview(
    ...
    *,
    ...
    """Parse a bounded, session-only SEC filing preview without returning full filing text."""

    normalized = _normalize_sec_preview_filing(filing)
    form = str(normalized.get("form") or "").strip().upper()
    try:
        lines = _sanitize_sec_html_lines(str(html_text or ""))
    except Exception as exc:
        return _sec_preview_payload(
            ...
            fetched_at_utc=fetched_at_utc,
            sections=[],
            messages=[f"SEC filing preview parse failed: {exc}"],
        )
    if form.startswith("8-K"):
        sections = _extract_8k_preview_sections(lines)
        preview_type = "8-K item headings"
    elif form.startswith("10-Q") or form.startswith("10-K"):
        sections = _extract_periodic_preview_sections(lines)
        preview_type = "10-Q/10-K section locator"
    else:
...
def _fetch_sec_filing_html_document(url: str, user_agent: str | None, request_timeout: float) -> dict[str, Any]:
    import requests

    from finance.data.sec_delisting import DEFAULT_SEC_USER_AGENT
...
def _normalize_sec_preview_fetch_response(response: Any) -> dict[str, Any]:
    if isinstance(response, dict):
        return dict(response)
    if isinstance(response, bytes):
        return {"status_code": 200, "content_type": "text/html", "content_bytes": response}
    return {"status_code": 200, "content_type": "text/html", "text": str(response or "")}


def _sec_preview_response_text(response: dict[str, Any]) -> tuple[str, int]:
    content_bytes = response.get("content_bytes")
    if isinstance(content_bytes, bytes):
        text = content_bytes.decode("utf-8", errors="replace")
        return text, len(content_bytes)
    text = str(response.get("text") or "")
    return text, len(text.encode("utf-8", errors="replace"))


def _sec_preview_is_html(content_type: str, url: str) -> bool:
    normalized = str(content_type or "").lower()
    if not normalized:
        parsed_path = (urlparse(url).path or "").lower()
        return parsed_path.endswith((".htm", ".html")) or "/ix?" in url.lower()
    return "html" in normalized or "xhtml" in normalized


def fetch_market_mover_sec_filing_preview(
    filing: Any,
    *,
    html_fetcher: Callable[[str, str | None, float], Any] | None = None,
    user_agent: str | None = None,
    request_timeout: float = 8.0,
    max_bytes: int = WHY_IT_MOVED_SEC_PREVIEW_MAX_BYTES,
) -> dict[str, Any]:
    """Fetch one selected SEC filing and return only a bounded, session-only preview."""

    normalized = _normalize_sec_preview_filing(filing)
    official_url = str(normalized.get("official_url") or "").strip()
    if not official_url:
        return _sec_preview_payload(
            filing=filing,
            status="FAILED",
            messages=["Official SEC URL is required for filing preview."],
        )

    try:
        response = _normalize_sec_preview_fetch_response(
            (html_fetcher or _fetch_sec_filing_html_document)(official_url, user_agent, float(request_timeout))
        )
    except Exception as exc:
        return _sec_preview_payload(
            filing=filing,
            status="FAILED",
            fetched_at_utc=datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z"),
            messages=[f"SEC filing preview fetch failed: {exc}"],
        )

    fetched_at = str(response.get("fetched_at_utc") or "").strip() or datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")
    status_code = int(response.get("status_code") or 200)
    content_type = str(response.get("content_type") or "")
    text, byte_count = _sec_preview_response_text(response)
    if status_code >= 400:
        return _sec_preview_payload(
            filing=filing,
            status="FAILED",
            fetched_at_utc=fetched_at,
            messages=[f"SEC filing preview fetch failed with HTTP {status_code}."],
        )
    if not _sec_preview_is_html(content_type, official_url):
        return _sec_preview_payload(
            filing=filing,
            status="NON_HTML",
            fetched_at_utc=fetched_at,
            messages=[f"SEC filing preview supports HTML/iXBRL only; received non-HTML content type: {content_type or 'unknown'}."],
        )
    if byte_count > max(0, int(max_bytes)):
        return _sec_preview_payload(
            filing=filing,
            status="OVERSIZE",
            fetched_at_utc=fetched_at,
            messages=[f"SEC filing document is too large for session-only bounded preview ({byte_count} bytes). Use the official Open link."],
        )
    preview = parse_market_mover_sec_filing_preview(text, filing=filing, fetched_at_utc=fetched_at)
    if preview["status"] == "PARTIAL":
        preview["messages"].append("Use the official Open link for the full SEC filing.")
    return preview
