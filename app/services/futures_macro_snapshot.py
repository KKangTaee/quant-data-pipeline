...
def _compatible_row(row: dict[str, Any] | None, *, source_marker: str | None = None) -> bool:
    ...
    if str(row.get("algorithm_version") or "") != PATTERN_ALGORITHM_VERSION:
        return False
    return source_marker is None or str(row.get("source_marker") or "") == str(source_marker)
...
def materialize_overview_futures_macro_snapshot(
    *,
    ...
    write_fn: Callable[[dict[str, object]], int] | None = None,
    ...
    existing = read()
    if not force and _compatible_row(existing, source_marker=str(marker)):
        return {
            "status": "reused",
    ...
    materialized_at = (now_fn or (lambda: datetime.now().strftime("%Y-%m-%d %H:%M:%S")))()
    macro = build_macro()
    ...
    (write_fn or upsert_futures_macro_snapshot)(row)
    return {
        "status": "materialized",
        "source_marker": str(marker),
        "as_of_date": as_of_date,
