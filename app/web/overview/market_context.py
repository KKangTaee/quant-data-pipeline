...
def render_market_context_mode_selector(*, state: Any = None) -> str:
    ...
    current = normalize_market_context_mode(resolved_state.get(MARKET_CONTEXT_MODE_KEY))
    resolved_state[MARKET_CONTEXT_MODE_KEY] = current
    ...
    normalized = normalize_market_context_mode(selected)
    resolved_state[MARKET_CONTEXT_MODE_KEY] = normalized
    return normalized
