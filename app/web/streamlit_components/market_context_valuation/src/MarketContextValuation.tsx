...
type ValuationPayload = {
  ...
  multiple_regime: {
    ...
    period_sensitive?: boolean; basis_start?: string; basis_end?: string;
  ...
  earnings_scenario: {
    ...
    conservative?: { growth_pct: number; projected_eps: number };
    baseline?: { growth_pct: number; projected_eps: number };
    optimistic?: { growth_pct: number; projected_eps: number };
    limitation?: string;
  };
  index_scenario: {
...
function MultipleChart({ model }: { model: ValuationPayload["multiple_regime"] }) {
  ...
  if (!points.length) return <div className="empty">60개월 실제 PER 이력이 필요합니다.</div>;
...
function MarketContextValuation({ args }: Props) {
  ...
  return <main className="valuation-workbench" data-status={payload.status}>
    ...
    <section className="valuation-section"><div className="section-head"><div><span>그래프 1</span><h3>최근 5년 멀티플 구간</h3><p>월별 SPX ÷ 당시 TTM EPS의 log(PER) 분포</p></div><div className={`regime regime-${multiple.bucket || "blocked"}`}>{bucketLabel[multiple.bucket || ""] || "자료 확인 필요"}</div></div>
      ...
      <MultipleChart model={multiple} />{multiple.period_sensitive ? <p className="notice">3년과 5년 판정이 달라 기간 민감도가 큽니다.</p> : null}<p className="limitation">{multiple.limitation}</p>
    </section>
    <section className="valuation-section"><div className="section-head"><div><span>그래프 2</span><h3>FOMC 예상 실적 기반 지수 시나리오</h3><p>현재 actual TTM EPS에 SEP GDP·PCE 성장률을 적용</p></div><div className="release"><span>SEP 발표</span><strong>{earnings.release_date || "-"}</strong></div></div>
      <div className="metrics"><div><span>현재 TTM EPS</span><strong>{n(earnings.current_ttm_eps,2)}</strong></div><div><span>보수 EPS</span><strong>{n(earnings.conservative?.projected_eps,2)}</strong></div><div><span>기준 EPS</span><strong>{n(earnings.baseline?.projected_eps,2)}</strong></div><div><span>낙관 EPS</span><strong>{n(earnings.optimistic?.projected_eps,2)}</strong></div></div>
      {index.status === "READY" ? <ScenarioChart model={index} /> : <div className="empty">{index.reason || earnings.reason || "시나리오 근거를 확인해 주세요."}</div>}
      {index.gap_pct ? <div className="gap-row"><span>현재 대비 하단 {n(index.gap_pct.lower,1)}%</span><strong>기준 {n(index.gap_pct.baseline,1)}%</strong><span>상단 {n(index.gap_pct.upper,1)}%</span></div> : null}<p className="limitation">{index.limitation || earnings.limitation}</p>
    </section>
