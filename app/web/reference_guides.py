...
def _stage_timeline_rows() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "screen": "Operations > Selected Portfolio Dashboard",
            "title": "선정 row 읽기",
            "check": "Final Review에서 선정된 current decision을 read-only로 읽고 drift / recheck signal을 확인합니다.",
    ...
    return {
        "단일 후보": [
            ...
            },
            {
                ...
                "detail": "최종 선정 저장과 최종 메모는 Final Review decision에만 남깁니다. 보류 / 거절 / 재검토는 상태 안내입니다.",
                "screen": "Final Review",
            },
        ],
        "여러 후보 묶음": [
...
def _registry_detail_rows() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "담는 데이터": "선정 이후 별도 monitoring snapshot을 저장할 때 쓰는 current workflow 보조 ledger",
    ...
    with tabs[1]:
        ...
        with st.expander("1~4단계 전체를 어떻게 읽나", expanded=False):
            st.markdown(
                ...
                - `3`: Final Review에서 최종 후보 선정과 이유를 current decision으로 남긴다.
