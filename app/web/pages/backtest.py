...
def _render_backtest_panel_selector() -> str:
    ...
    st.caption("Backtest Analysis에서 후보 source를 만들고, Practical Validation에서 검증 근거를 만든 뒤, Final Review에서 Portfolio Monitoring 후보 여부를 판단합니다.")
    ...
    st.caption("과거 실행 기록은 `Operations > Archive: Backtest Runs`, 선정 후 모니터링 관리는 `Operations > Portfolio Monitoring`에서 확인합니다.")
...
def render_backtest_tab() -> None:
    ...
    with st.expander("Backtest 사용 안내", expanded=False):
        st.markdown(
            ...
            - `Final Review`: 검증 자료를 기준으로 selected-route gate까지 통과한 후보만 Portfolio Monitoring 후보로 저장합니다.
            - `Operations > Archive: Backtest Runs`: 저장된 실행 결과를 다시 보고, `Run Again` 또는 `Load Into Form`을 사용하는 운영 도구입니다.
            - `Operations > Portfolio Monitoring`: Final Review에서 저장된 selected decision을 읽어 모니터링 성과와 review signal을 확인합니다.
