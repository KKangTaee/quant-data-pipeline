...
- 체크 항목:
  - [ ] `Strategies` 아래 `Strategy Variants` 섹션에서 Annual / Quarterly variant를 고르면, 별도 버튼 없이 아래 `Strategy-Specific Advanced Inputs`가 즉시 바뀌는지
  - [ ] `Compare Period & Shared Inputs`에서 `Start Date`, `End Date`, `Timeframe`, `Option`이 한 곳에 모여 있어 공용 실행 입력으로 이해되는지
  - [ ] `Advanced Inputs` expander / form이 따로 보이지 않고, 전략별 세부 옵션이 `Strategy-Specific Advanced Inputs`라는 독립 섹션으로 읽히는지
