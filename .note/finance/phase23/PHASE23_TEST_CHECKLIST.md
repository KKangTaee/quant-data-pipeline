...
- 체크 항목:
  ...
  - [ ] `Backtest > History > Run Kind Filter = strategy_compare > Selected History Run > Saved Input & Context > Context`에서 quarterly compare 기록을 선택했을 때, 전략별 override 표에 `Rejected Slot Handling`, `Weighting`, `Risk-Off` 값이 남아 있는지
  - [ ] `Backtest > Compare & Portfolio Builder > Saved Portfolios > Inspect Saved Portfolio > Compare Context`에서 저장된 포트폴리오의 quarterly 전략 override에 `Rejected Slot Handling`, `Weighting`, `Risk-Off` 값이 남아 있는지
  - [ ] `Backtest > History > Run Kind Filter = single_strategy > Selected History Run > Saved Input & Context > Input`에서 single-strategy quarterly 기록을 선택했을 때, `strategy_key`나 전략 이름이 `Strict Quarterly Prototype`으로 보이고 annual처럼 보이지 않는지
  - [ ] `Backtest > History > Selected History Run > Actions For This History Run > Load Into Form`을 눌렀을 때 `Single Strategy`로 이동하고, `Quality / Value / Quality + Value Variant`가 quarterly로 유지되는지
  - [ ] `Load Into Form` 후 이동한 `Single Strategy` 화면의 quarterly form에서 `Portfolio Handling & Defensive Rules`를 열었을 때, `Rejected Slot Handling`, `Weighting`, `Risk-Off / Defensive Tickers` 값이 저장 기록과 동일하게 복원되는지
  - [ ] `Backtest > History > Selected History Run > Actions For This History Run > Run Again`을 누르면 저장 기록의 payload가 즉시 다시 실행되고 최신 single-strategy 결과 화면으로 이동하는지
  - [ ] `Load Into Form` 후 이동한 `Single Strategy` 화면 상단의 `Back To History`를 누르면 `History` panel로 돌아가는지
  - [ ] `Backtest > Compare & Portfolio Builder > Saved Portfolios > Inspect Saved Portfolio > Replay Saved Portfolio`를 실행했을 때 `Compare Results`와 `Weighted Portfolio Result`가 다시 표시되고, quarterly contract 값이 오류 없이 유지되는지
