# Phase 14 Backtest Reports

## 목적

이 폴더는 Phase 14 이후 current runtime 기준으로 다시 돌려본
결과 중심 backtest Markdown 문서를 모아두는 공간이다.

쉽게 말하면:

- Phase 14 calibration / UX / operator surface 정리 이후
- strict annual family가 현재 코드에서 어디까지 올라오는지
- 다시 확인한 결과를 모아두는 archive다.

## 먼저 볼 문서

- [QUALITY_STRICT_ANNUAL.md](/Users/taeho/Project/quant-data-pipeline-worktrees/phase/.note/finance/reports/backtests/strategies/QUALITY_STRICT_ANNUAL.md)
- [VALUE_STRICT_ANNUAL.md](/Users/taeho/Project/quant-data-pipeline-worktrees/phase/.note/finance/reports/backtests/strategies/VALUE_STRICT_ANNUAL.md)
- [QUALITY_VALUE_STRICT_ANNUAL.md](/Users/taeho/Project/quant-data-pipeline-worktrees/phase/.note/finance/reports/backtests/strategies/QUALITY_VALUE_STRICT_ANNUAL.md)

## 포함된 문서 성격

- current runtime refresh
- non-hold candidate search
- family별 practical candidate 정리

