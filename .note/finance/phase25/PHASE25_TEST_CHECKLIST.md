...
- 체크 항목:
  - [ ] Backtest panel에 `Pre-Live Review`가 보이는지
  - [ ] `Create From Current Candidate` 탭에서 current candidate 목록이 보이는지
  - [ ] 후보를 선택하면 `Promotion`, `Shortlist`, `Deployment`, `Default Status`가 보이는지
  - [ ] `Pre-Live Status`, `Operator Reason`, `Next Action`, `Review Date`를 확인하거나 수정할 수 있는지
  - [ ] 저장 전 JSON 초안이 보이고, 저장 버튼을 누르기 전에는 registry에 기록되지 않는다는 점이 이해되는지
  - [ ] `Save Pre-Live Record`를 누르면 `Pre-Live Registry` 탭에서 저장된 active record가 보이는지
  - [ ] 화면 문구가 live trading 승인이나 실제 투자 실행으로 오해되지 않는지
