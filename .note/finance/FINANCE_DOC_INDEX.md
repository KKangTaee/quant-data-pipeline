...
| Phase 30 | `active` | `not_ready_for_qa` | Product-flow 재정렬 + `backtest.py` 리팩토링 경계 검토 진행 중 |
...
- Phase 30 `Portfolio Proposal And Pre-Live Monitoring Surface`는 active 상태로 열렸지만,
  첫 작업은 portfolio proposal 기능 구현이 아니라 Phase 29 이후 기준의 사용 흐름 재정렬과 `backtest.py` 리팩토링 경계 검토다.
...
| Phase 30 | `active` | `not_ready_for_qa` | Product-flow 재정렬 + `backtest.py` 리팩토링 경계 검토 진행 중 | portfolio proposal / pre-live monitoring | `.note/finance/phase30/PHASE30_PORTFOLIO_PROPOSAL_AND_PRE_LIVE_MONITORING_SURFACE_PLAN.md` |
