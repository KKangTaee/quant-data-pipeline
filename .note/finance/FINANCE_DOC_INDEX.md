...
| Phase 29 | `active` | `manual_qa_pending` | Candidate Review Board + Result Handoff 구현 완료 |
...
- 현재는 Phase 29 `Candidate Review And Recommendation Workflow`에서 `Backtest > Candidate Review`와 Latest / History 후보 검토 초안 handoff를 QA하면 된다.
...
| Phase 29 | `active` | `manual_qa_pending` | Candidate Review Board + Result Handoff 구현 완료 | candidate review / recommendation workflow | `.note/finance/phase29/PHASE29_CANDIDATE_REVIEW_AND_RECOMMENDATION_WORKFLOW_PLAN.md` |
...
- Phase 29는 current candidate를 바로 투자 추천으로 확정하지 않고, 후보 검토 보드에서 읽은 뒤 compare / Pre-Live Review / 후보 검토 초안으로 넘기는 workflow를 만드는 phase다.
