...
- 현재 `Global Relative Strength`는 core/runtime smoke까지 완료된 상태이며,
  UI / compare / history / saved replay 체크 항목은 다음 Phase 24 구현 단위가 끝난 뒤 확인한다.
...
- 확인 위치:
  ...
  - 첫 신규 전략 family 화면
- 체크 항목:
  - [ ] `Backtest > Single Strategy > 첫 신규 전략 family`에서 전략 이름과 설명이 research note와 자연스럽게 연결되는지
  - [ ] `Backtest > Single Strategy > 첫 신규 전략 family > 핵심 입력`에서 universe, rebalance, signal, risk-off 관련 입력이 보이는지
  - [ ] `Backtest > Single Strategy > 첫 신규 전략 family > Run`을 실행했을 때 결과가 오류 없이 표시되는지
...
- 체크 항목:
  ...
  - [ ] `Backtest > Compare & Portfolio Builder > Strategy-Specific Advanced Inputs > 첫 신규 전략 box`에서 신규 전략 설정이 보이는지
  - [ ] `Backtest > History > Run Kind Filter = single_strategy > Selected History Run`에서 신규 전략 실행 기록이 전략 이름과 key를 잃지 않는지
  - [ ] `Backtest > History > Selected History Run > Load Into Form`을 눌렀을 때 신규 전략 입력값이 form에 복원되는지
  - [ ] `Backtest > History > Selected History Run > Run Again`을 눌렀을 때 신규 전략이 즉시 다시 실행되는지
