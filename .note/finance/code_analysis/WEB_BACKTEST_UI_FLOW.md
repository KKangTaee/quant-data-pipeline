...
| `app/web/pages/backtest_candidate_review.py` | Candidate Review / Candidate Packaging 화면 render logic |
| `app/web/pages/backtest_candidate_review_helpers.py` | Candidate Review 판단, Review Note / registry 변환, readiness score helper |
...
- `Candidate Draft`는 latest run 또는 history run을 후보처럼 읽는 저장 전 초안이며, Candidate Packaging 안에서 쓰인다.
...
| 우선순위 | 분리 후보 | 대표 책임 | 이유 |
|---|---|---|---|
| 1 | Candidate Review module | Candidate Packaging flow, review note save, registry draft UI, Pre-Live route check | 2차 완료: render flow는 `backtest_candidate_review.py`, 판단 / 변환 helper는 `backtest_candidate_review_helpers.py`로 분리 |
...
- Candidate Review는 `app/web/pages/backtest_candidate_review.py`와 `app/web/pages/backtest_candidate_review_helpers.py`로 분리되어, `backtest.py`에는 panel wrapper와 cross-panel handoff call만 남아 있다.
- Pre-Live UI, Compare prefill 변환, 일부 History / Saved Portfolio / Portfolio Proposal render logic, Streamlit session state는 아직 `app/web/pages/backtest.py`에 남아 있다.
