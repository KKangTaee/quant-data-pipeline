...
이 문서는 Streamlit Backtest 화면의 single strategy, compare, candidate review, Pre-Live, portfolio proposal, Operations-owned backtest history, saved portfolio 흐름을 설명한다.
...
| `app/web/backtest_history.py` | `Operations > Backtest Run History` page shell |
| `app/web/pages/backtest.py` | Backtest page entry, workflow navigation, shared helper, 아직 분리되지 않은 panel / history helper render logic |
...
| `app/web/backtest_pre_live_review_helpers.py` | Pre-Live status 추천, draft 변환, Portfolio Proposal 진입 readiness score helper |
...
Ingestion / Data Trust
  ...
  -> Compare
  -> Candidate Packaging
     -> Draft 확인 / Review Note 저장 / Registry 저장 / Pre-Live route 확인
...
- `Candidate Packaging`은 Draft 확인, Review Note 저장, Registry 저장, Pre-Live 진입 평가를 하나로 묶은 사용자-facing 6단계다.
...
- `Pre-Live 진입 평가`는 저장된 후보를 다시 읽고 `PRE_LIVE_READY`, `COMPARE_REVIEW_READY`, `BOARD_HOLD` 중 다음 경로를 정하는 Candidate Packaging의 최종 route 확인이다.
...
| `1~8 단계 실행 흐름` | 테스트에서 상용화 후보 검토까지의 단계별 흐름. 각 단계는 expander로 접어 읽는다 |
...
| 우선순위 | 분리 후보 | 대표 책임 | 이유 |
|---|---|---|---|
| 1 | Candidate Review module | Candidate Packaging flow, review note save, registry draft UI, Pre-Live route check | 완료: render flow는 `app/web/backtest_candidate_review.py`, 판단 / 변환 helper는 `app/web/backtest_candidate_review_helpers.py`로 분리. Streamlit auto page discovery를 피하려고 `pages/` 밖에 둔다 |
| 2 | Pre-Live Review module | Pre-Live draft, status / tracking plan, registry inspect | 완료: render flow는 `app/web/backtest_pre_live_review.py`, status / draft / readiness helper는 `app/web/backtest_pre_live_review_helpers.py`로 분리. Streamlit auto page discovery를 피하려고 `pages/` 밖에 둔다 |
...
- Backtest shell은 중복 제목을 제거하고, `Single Strategy -> Compare & Portfolio Builder -> Candidate Review -> Pre-Live Review -> Portfolio Proposal`을 주 workflow navigation으로 보여준다. `History`는 메인 흐름에서 제외하고 `Operations > Backtest Run History` page로 연다.
- Compare prefill 변환, 일부 History helper / Saved Portfolio / Portfolio Proposal render logic, Streamlit session state는 아직 `app/web/pages/backtest.py`에 남아 있다.
...
CURRENT_CANDIDATE_REGISTRY.jsonl
  ...
  -> 3. Pre-Live 진입 평가에서 후보 route 확인
  -> Open Selected Candidate In Pre-Live Review 또는 Open Compare Picker
```
...
Latest Backtest Run 또는 Operations > Backtest Run History selected record
  ...
  -> 3. Pre-Live 진입 평가에서 Candidate Packaging 종합 판단
  -> PRE_LIVE_READY면 Pre-Live Review로 이동
```

구분:

- Candidate Review는 후보를 투자 추천으로 확정하는 화면이 아니다.
- Candidate Review는 Candidate Packaging 작업 공간이며 한 화면 안에서 Draft 확인, Registry 저장, Pre-Live 진입 평가를 순서대로 처리한다.
...
CURRENT_CANDIDATE_REGISTRY.jsonl
  ...
  -> 7단계 Pre-Live 운영 점검 설명 확인
  -> 운영 후보 확인
     -> Candidate Packaging에서 넘어온 후보 자동 선택
     -> 또는 current candidate 직접 선택
  -> 운영 상태 / 추적 계획 결정
  -> Portfolio Proposal 진입 평가
