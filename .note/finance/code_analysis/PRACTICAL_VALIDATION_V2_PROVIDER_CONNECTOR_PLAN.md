...
2. `etf_operability_snapshot`, `etf_holdings_snapshot`, `etf_exposure_snapshot`, `macro_series_observation` schema 추가
...
- `app/web/backtest_practical_validation.py`
  ...
  - source map이 없는 holdings / exposure는 `connector mapping 필요`로 분리해, 단순 미수집 문제와 미구현 connector 문제를 구분
