...
- Phase 33에서는 `.note/finance/registries/PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl`에 append-only row를 저장하는 ledger 계약을 만들었다.
...
- `최종 검토 결과 기록`을 누르면 `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 append-only row가 남는다.
