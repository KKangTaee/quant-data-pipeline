...
현재 Phase 32는 `implementation_complete / manual_qa_pending` 상태다.
아래 항목을 확인한 뒤 문제가 없으면 Phase 32를 closeout할 수 있다.

## 사용 방법

- 아래 항목은 사용자가 직접 `[ ]`를 `[x]`로 바꾸며 확인한다.
- 특별한 사유가 없으면 주요 체크 항목이 완료된 뒤 Phase 32를 닫는다.
- 일부 항목을 나중으로 미루면 그 이유를 handoff에 짧게 남긴다.
...
- 체크 항목:
  ...
  - [ ] `Stress / Sensitivity Summary` table이 보이는지
  - [ ] 단일 후보에서는 portfolio-only stress가 `NOT_APPLICABLE`로 읽힐 수 있는지
  - [ ] preview를 열어도 proposal draft 저장이나 live approval이 자동 수행되지 않는지
...
- 체크 항목:
  ...
  - [ ] `Stress / Sensitivity Summary`에서 `Input Status`와 `Result Status`가 구분되어 보이는지
  - [ ] `Result Status = NOT_RUN`이 실패가 아니라 아직 실제 stress runner가 실행되지 않은 상태로 읽히는지
...
- 확인 위치:
  ...
  - `Validation` tab
- 체크 항목:
  - [ ] 저장 proposal validation 요약 표에 `Robustness Route`, `Robustness Score`, `Phase33 Handoff`가 보이는지
  - [ ] 저장 proposal detail의 Validation Pack에서도 같은 robustness preview가 보이는지
  - [ ] 저장 proposal detail에서 `Stress / Sensitivity Summary` table이 보이는지
...
- 체크 항목:
  - [ ] `Phase 33 Handoff` route panel이 보이는지
  - [ ] route가 `READY_FOR_PAPER_LEDGER_PREP`, `NEEDS_STRESS_INPUT_REVIEW`, `BLOCKED_FOR_PAPER_LEDGER` 중 하나로 읽히는지
  - [ ] source id, component weight, tracking benchmark, stress summary contract 준비 상태가 표로 보이는지
  - [ ] 이 handoff가 paper ledger 준비 가능성일 뿐, live approval이나 주문 지시가 아니라는 문구가 보이는지
...
- [ ] Phase32가 실제 기간 분할 백테스트나 parameter sweep을 이미 실행했다는 뜻으로 오해되지 않는지
...
- 체크 항목:
  - [ ] Phase 32가 `implementation_complete / manual_qa_pending`으로 표시되는지
  - [ ] 1~4번째 작업이 모두 완료로 표시되는지
  - [ ] Phase 32가 live approval / 최종 투자 선정 단계가 아니라는 경계가 문서와 UI에 남아 있는지
  - [ ] Phase 33에서 실제로 무엇을 만들지 next phase preparation에 쉽게 설명되어 있는지
