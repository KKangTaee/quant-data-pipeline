...
일부는 다음 phase에서 다룰 주제로 넘기는 것이 더 맞고,
...
|---|---|---|---|---|
| Phase 8 | `implementation_complete / manual_qa_pending` | `complete / superseded_by_later_phase` | quarterly prototype family와 diagnostics는 Phase 23 quarterly productionization에서 제품 흐름으로 흡수됨 | Phase 27 data diagnostics, Phase 28 quarterly parity에서 다룰 주제로 유지 |
| Phase 9 | `implementation_complete / manual_qa_pending` | `complete / superseded_by_later_phase` | strict coverage / promotion policy는 Phase 10, 12, 23, 25 흐름에 흡수됨 | Phase 27 coverage trust, Phase 28 promotion parity에서 다룰 주제로 유지 |
| Phase 12 | `practical_closeout / manual_qa_pending` | `complete / superseded_by_later_phase` | Real-Money first-pass hardening은 Phase 13~15와 Phase 25 Pre-Live로 이어졌음 | ETF PIT operability, richer benchmark, execution readiness는 Phase 27/28/30에서 다룰 주제 |
| Phase 13 | `implementation_complete / manual_qa_pending` | `complete / superseded_by_later_phase` | deployment readiness / probation 언어는 Phase 25 Pre-Live 운영 기록으로 재정의됨 | monthly review note, paper tracking은 Phase 30에서 다룰 주제 |
| Phase 14 | `practical_closeout / manual_qa_pending` | `complete / superseded_by_later_phase` | gate blocker / calibration 설계는 current candidate 및 Pre-Live 흐름의 배경 지식으로 남음 | threshold experiment, PIT operability, candidate review는 Phase 27~29에서 다룰 주제 |
| Phase 15 | `practical_closeout / manual_qa_pending` | `complete / superseded_by_later_phase` | candidate quality improvement와 strategy log 체계는 current candidate registry / backtest reports로 이어짐 | 후보 검토 표준화는 Phase 29, portfolio grouping은 Phase 30에서 다룰 주제 |
...
Phase 18을 쉽게 풀면,
먼저 끝낸 작업은 "선정된 종목 중 일부가 탈락했을 때 그 빈자리를 어떻게 처리할지"를 정한 것이다.
이 흐름은 Phase 19의 contract UI / 설명 정리까지 이어졌고, 현재 기준으로는 완료된 작업으로 본다.
남은 구조 아이디어는 버린 것이 아니라,
Phase 28에서 annual / quarterly / 신규 전략의 옵션 균형을 맞출 때 필요하면 다시 꺼내 볼 후보로 남긴다.

## Immediate blocker 판단

Phase 27로 넘어가기 전에 반드시 해결해야 하는 immediate blocker는 없다.

다만 Phase 27에서 다뤄야 할 데이터 / 백테스트 신뢰성 문제가 있다.
즉 "멈추고 과거 phase로 돌아가야 한다"가 아니라,
"다음 phase에서 제품 기능으로 정리해야 한다"가 더 정확하다.

## Phase 27에서 다룰 주제: Data Integrity And Backtest Trust Layer

Phase 27에서 우선 다룰 주제는 다음과 같다.

| 다룰 주제 | 어디서 나온 내용인가 | 왜 중요한가 |
|---|---|---|
...
## Phase 28에서 다룰 주제: Strategy Family Parity And Cadence Completion

Phase 28에서 다룰 주제는 다음과 같다.
...
## Phase 29에서 다룰 주제: Candidate Review And Recommendation Workflow

Phase 29에서 다룰 주제는 다음과 같다.
...
## Phase 30에서 다룰 주제: Portfolio Proposal And Pre-Live Monitoring Surface

Phase 30에서 다룰 주제는 다음과 같다.
...
각각 Phase 27~30에서 다룰 주제 또는 future option으로 정리하는 것이 맞다.
