...
- Phase 31~33에서 쌓은 portfolio risk, robustness / stress, paper observation 기준을 한 화면에서 읽는다.
- 단일 current candidate 또는 저장된 `Portfolio Proposal`을 최종 선정 검토 입력으로 사용한다.
...
사용자-facing main flow에서는 별도 `Save Paper Tracking Ledger`를 다시 누르게 하지 않고,
`Backtest > Final Review`에서 paper observation 기준을 final review record 안에 함께 남긴다.

## 왜 필요한가

- Phase 33까지는 paper tracking 시작 조건과 관찰 개념을 만들었지만, 최종 선정 decision은 아직 없다.
- paper observation 기준이 준비돼도, 그것은 "검토 가능"이지 자동 선정이 아니다.
...
- Portfolio Proposal 탭 안에 저장 버튼을 계속 늘리면 사용자가 각 저장의 의미를 이해하기 어렵다.

## 이 phase가 끝나면 좋은 점

- 사용자는 `Backtest > Final Review`에서 단일 후보 또는 저장된 proposal을 기반으로 최종 실전 후보 검토 화면을 열 수 있다.
- 최종 선정 / 보류 / 거절 / 재검토 판단이 어떤 근거로 내려졌는지 남는다.
- Phase 35는 선정 이후 리밸런싱, 중단, 축소, 재검토 운영 기준을 이어받을 수 있다.

## 이 phase에서 다루는 대상

- `Backtest > Final Review`의 최종 검토 surface
...
- Phase 33 paper observation 기준
- 기존 Paper Ledger registry row와 `phase34_handoff`
  - 호환성 / 운영 artifact로 유지
  - 이번 main flow의 필수 사용자 저장 단계는 아님
...
2. Decision evidence pack 계산 기준 추가
   - 쉽게 말하면: validation, stress, paper observation 기준, blocker, operator note를 한 묶음으로 읽는다.
   - 왜 필요한가: 최종 판단은 CAGR / MDD 하나만 보고 내리면 안 된다.
   - 기대 효과: 사용자가 "왜 선정했는가 / 왜 보류했는가"를 재현할 수 있다.
3. Final decision UI 추가
   - 쉽게 말하면: 단일 후보 또는 저장된 proposal을 고르고, 선정 / 보류 / 거절 / 재검토 route를 선택한다.
...
- `Decision Evidence`: 백테스트 성과, validation, robustness / stress, paper observation 기준, operator note를 합친 판단 근거다.
...
- paper observation 기준을 자동으로 최종 선정으로 승격하지 않는다.
- Portfolio Proposal 탭은 proposal draft 작성 / 저장에 집중하고, 최종 검토는 Final Review 탭에서 처리한다.
...
- Portfolio Proposal 탭 안에 Paper Ledger / Final Selection 저장 버튼이 다시 노출되지 않는지 QA에서 확인한다.

## 한 줄 정리

Phase 34는 검증과 paper observation 기준을 통과한 후보나 proposal을 `Final Review`에서 최종 실전 후보로 선정 / 보류 / 거절 / 재검토하는 decision pack을 만드는 단계다.
