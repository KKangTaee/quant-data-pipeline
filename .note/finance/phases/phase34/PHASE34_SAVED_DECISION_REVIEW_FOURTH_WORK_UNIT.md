...
이 문서는 네 번째 작업의 첫 구현 기준이다.
현재 main UI에서는 저장된 final review record가 source candidate / proposal,
inline paper observation, selected components, Phase35 handoff를 함께 보여준다.
`source paper ledger`는 legacy row가 있을 때의 호환 field로만 읽는다.
...
- source candidate / proposal, observation, selected components가 연결되어 보인다.
