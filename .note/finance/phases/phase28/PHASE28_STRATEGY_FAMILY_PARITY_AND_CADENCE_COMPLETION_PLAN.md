...
2. history / load / replay parity 확인
   ...
   - 현재 상태: `History Replay / Load Parity Snapshot`과 `Saved Portfolio Replay / Load Parity Snapshot`으로 저장 기록의 재진입 상태를 먼저 볼 수 있게 했다.
...
3. Real-Money / Guardrail parity 범위 결정
   ...
   - 현재 결론: quarterly prototype에는 annual strict 수준의 Real-Money / Guardrail surface를 억지로 붙이지 않는다.
   - 현재 상태: compare / history / saved portfolio에 `Real-Money / Guardrail Scope` 표를 추가해 전략별 지원 범위를 같은 언어로 보여준다.
...
2. History / Load / Replay 보존 확인
   ...
   - 두 번째 작업 단위에서 `Backtest > History`에 parity snapshot을 추가했다.
   - 세 번째 작업 단위에서 `Saved Portfolios`에도 replay / load parity snapshot을 추가했다.
3. Data Trust Summary 확장 범위 결정
   - Phase 27의 Data Trust Summary를 compare / saved replay / ETF 전략군에서 어디까지 공통화할지 정한다.
   - 네 번째 작업 단위에서 compare 결과와 weighted portfolio 결과에 component별 data trust surface를 추가했다.
4. Real-Money / Guardrail parity 결정
   - annual strict, quarterly prototype, ETF 전략의 실전 검증 surface 차이를 표준화한다.
   - 다섯 번째 작업 단위에서 annual strict, quarterly prototype, GRS, GTAA, Risk Parity Trend, Dual Momentum, Equal Weight의 Real-Money / Guardrail scope를 표로 구분했다.
...
- `Backtest > History`에서 선택한 저장 기록 아래 `History Replay / Load Parity Snapshot`이 보이는지
- `Backtest > Compare & Portfolio Builder > Saved Portfolios`에서 선택한 저장 포트폴리오 아래 `Saved Portfolio Replay / Load Parity Snapshot`이 보이는지
- `Backtest > Compare & Portfolio Builder > Strategy Comparison > Data Trust`에서 component별 data trust가 보이는지
- `Weighted Portfolio Result > Component Data Trust`에서 구성 전략별 데이터 상태가 보이는지
- annual strict와 quarterly prototype의 Real-Money / Guardrail 차이가 화면에서 이해되는지
- `Strategy Comparison > Real-Money / Guardrail`에서 strategy별 검증 범위가 보이는지
- `History Real-Money / Guardrail Scope`와 `Saved Portfolio Real-Money / Guardrail Scope`에서 저장 / 재실행 전 검증 범위가 이해되는지
