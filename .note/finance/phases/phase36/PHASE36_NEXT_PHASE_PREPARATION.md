...
2. drift가 커졌을 때 read-only preview를 실제 alert persistence / stop / re-review workflow와 연결할 것인가
...
- 다음 phase는 실제 계좌 연결 여부를 결정하거나, drift preview를 저장 가능한 risk alert / review workflow로 연결할지 정해야 한다.
...
- 운영 대시보드와 drift check 입력 계약, read-only alert preview가 생겼으므로 다음 질문은 실제 holding source와 저장 가능한 경고 체계를 어떻게 연결할지다.
