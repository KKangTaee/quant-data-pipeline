...
목표 비중 대비 현재 보유 상태 차이는 `Allocation Check`에서 추가로 확인한다.
...
- summary cards, source 설명, selected portfolio table, filter, selected row detail, Performance Recheck, target allocation, operator context, audit expander를 제공한다.
...
- component contribution, strongest periods, weakest periods를 보여준다.
...
### 5. Allocation Check / Drift Check

- drift check는 기본 화면이 아니라 `Allocation Check` 고급 영역으로 이동했다.
