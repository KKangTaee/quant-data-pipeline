...
이 두 번째 작업 당시에는 저장소와 append 기능을 구현하지 않았다.
네 번째 작업 이후 기본 저장 위치는
`.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl`로 구현되었다.

아래는 이후 UI / 저장소 구현이 맞춰야 하는 row 계약이다.
...
## 이 두 번째 작업에서 아직 하지 않았던 것

- `.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 파일 생성
...
- 세 번째 작업에서 registry JSONL I/O helper를 먼저 분리했다.
- 네 번째 작업에서 Portfolio Proposal Draft UI와 append-only proposal helper를 구현했다.
- 다섯 번째 작업에서 Proposal Monitoring Review surface를 구현했다.
- portfolio optimizer, live readiness approval, broker / order integration은 여전히 하지 않는다.

## 다음 작업

다음 작업은 둘 중 하나가 자연스럽다.

1. Paper / Pre-Live tracking feedback을 proposal review에 연결한다.
2. Candidate Review / Pre-Live / History / Saved Portfolio의 추가 모듈 분리를 진행한다.
