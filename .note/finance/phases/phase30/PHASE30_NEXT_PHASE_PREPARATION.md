...
현재 Phase 30은 구현 완료 / manual QA 대기 상태다.
따라서 이 문서는 QA 이후 closeout 또는 다음 phase 방향을 정할 때 볼 handoff 초안이다.
...
- 여섯 번째 작업으로 `Backtest > Portfolio Proposal > Pre-Live Feedback` surface를 추가했다.
- 일곱 번째 작업으로 `Backtest > Portfolio Proposal > Paper Tracking Feedback` surface를 추가했다.
...
4. `backtest.py`의 Candidate Review / Pre-Live / History / Saved Portfolio UI 경계를 어떤 순서로 분리할 것인가
...
4. Special refactor task
   - Phase 30 QA와 별도로 Candidate Review / Pre-Live / History / Saved Portfolio의 `backtest.py` 모듈 분리 경계를 실행한다.

## 추천 다음 방향

아직은 Phase 30 사용자 QA 전이므로 다음 phase를 확정하지 않는다.
Phase 30 checklist QA가 끝난 뒤,
Live Readiness / Final Approval phase를 열지,
또는 먼저 `backtest.py` special refactor task를 열지 판단한다.
...
- Phase 30의 여섯 번째 작업은 Portfolio Proposal Pre-Live Feedback 구현이었다.
- Phase 30의 일곱 번째 작업은 Portfolio Proposal Paper Tracking Feedback 구현이었다.
- 다음 구현 전에 `PHASE30_TEST_CHECKLIST.md`, `PHASE30_CURRENT_CHAPTER_TODO.md`, Portfolio Proposal 계약 문서, fourth/fifth/sixth/seventh work-unit 문서, Portfolio Proposal registry guide를 먼저 확인한다.
