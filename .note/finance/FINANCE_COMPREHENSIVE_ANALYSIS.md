...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-04 / Phase 34 manual QA closeout and Phase 35 Post-Selection Operating Guide kickoff
...
| Web Runtime / UI | Streamlit에서 Overview dashboard, single / compare / candidate review / Pre-Live 운영 기록 / Operations history / Candidate Library / saved replay / Portfolio Proposal / Final Review를 실행한다 | `app/web/runtime/*`, `app/web/overview_dashboard*.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history*.py`, `app/web/backtest_candidate_library*.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py`, `app/web/backtest_final_review*.py` |
...
| Phase 35 | Post-Selection Operating Guide | Phase34에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 기록된 final review record를 읽어 리밸런싱 / 중단 / 축소 / 재검토 운영 기준을 만드는 active phase로 시작했으며 아직 구현 전 / QA 전 |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal, Final Review는 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal은 단일 후보 직행 평가와 다중 후보 proposal draft 작성 / feedback에 집중한다. Final Review는 단일 후보 또는 saved proposal을 같은 검증 언어로 읽어 validation, robustness / stress 질문, paper observation 기준, 최종 선정 / 보류 / 거절 / 재검토 판단과 Phase35 handoff를 남긴다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Web UI | `app/web/overview_dashboard.py`, `app/web/overview_dashboard_helpers.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_library.py`, `app/web/backtest_candidate_library_helpers.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py`, `app/web/backtest_final_review.py`, `app/web/backtest_final_review_helpers.py` | Overview 후보 / funnel / next action dashboard, Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal draft / Final Review validation / paper observation / final decision 주 흐름과 Operations Backtest Run History / Candidate Library / Saved Weighted Portfolio 보조 재현 화면, 공용 result/status/route UI component |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Final Review validation / robustness / paper observation, Final Portfolio Selection Decision 저장 / review 단계까지 확장됐다.
...
16. Phase 35는 active / not_ready_for_qa 상태로 시작했다. Phase34에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 기록된 final review record를 주문으로 보내지 않고, 리밸런싱 / 중단 / 축소 / 재검토 운영 기준으로 바꾸는 `Post-Selection Operating Guide`를 만드는 단계다. 아직 Phase35 구현이나 QA handoff는 시작하지 않았다.
...
| Phase 35 Post-Selection Operating Guide | Phase34에서 최종 선정된 후보를 바로 주문으로 연결하지 않고, 실제 운영 전 리밸런싱 / 중단 / 축소 / 재검토 기준으로 정리하는 phase다. 현재는 active / not_ready_for_qa 상태이며 첫 작업은 운영 policy contract 정리다. | `phases/phase35/`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
