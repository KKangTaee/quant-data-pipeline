...
| Phase 36 Final Selected Portfolio Monitoring And Rebalance Operations | Final Review에서 선정된 포트폴리오를 `Operations > Selected Portfolio Dashboard`에서 읽고, 사용자 지정 최신 기간 / 가상 투자금 기준 성과 재검증과 optional Actual Allocation을 read-only로 확인하는 phase다. manual QA 대기 중이다. | `phases/phase36/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
