...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-04-22 / Phase 26 manual QA closeout
...
9. Phase 28은 annual strict, quarterly strict, price-only ETF 전략의 지원 범위와 cadence 차이를 `Strategy Capability Snapshot`으로 보이게 만들고, history / saved portfolio 재진입 가능성, compare / weighted data trust, Real-Money / Guardrail scope까지 확인하게 만든 상태다.
10. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| Phase 28 Strategy family parity | annual / quarterly / 신규 전략이 UI, payload, history, saved replay, Real-Money, Guardrail에서 의도된 수준으로 맞아야 하며, capability snapshot, history / saved portfolio replay/load parity snapshot, compare / weighted data trust, Real-Money / Guardrail scope surface를 추가했다. | `code_analysis/STRATEGY_IMPLEMENTATION_FLOW.md`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `phase28/` |
