...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 32 Robustness / Stress Validation Pack implementation complete
...
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 문서 |
...
| Phase 32 | Robustness / Stress validation pack | Phase 31 Validation Pack 아래에 robustness preview, stress summary contract / table, Phase33 handoff를 추가해 period / contract / benchmark / 성과 snapshot / compare evidence가 stress 검증과 paper ledger 준비에 충분한지 읽게 만들었으며, 아직 실제 stress sweep engine이나 최종 선정은 아님 |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Portfolio Risk / Live Readiness Validation Pack, Robustness / Stress Validation Pack까지 확장됐다.
...
13. Phase 32는 implementation_complete / manual_qa_pending 상태다. Validation Pack 아래에 `Robustness / Stress Validation Preview`, `Stress / Sensitivity Summary`, `Phase 33 Handoff`를 추가해 stress 실행 전 입력 gap, stress row 계약, paper ledger 준비 상태를 보여준다. 아직 실제 기간 분할 backtest, benchmark sensitivity, parameter sweep engine, paper ledger 저장, 최종 투자 선정은 아니다.
14. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| Phase 32 Robustness / Stress Validation Pack | 좋은 결과가 특정 기간 / benchmark / parameter 조건에만 의존한 것인지 확인하기 위한 단계다. Validation Pack 아래에 robustness input preview, stress / sensitivity summary, Phase33 paper ledger handoff를 추가했고 사용자 QA 대기 상태다. | `phases/phase32/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Portfolio Risk Validation Pack / Robustness-Stress Validation Pack, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
