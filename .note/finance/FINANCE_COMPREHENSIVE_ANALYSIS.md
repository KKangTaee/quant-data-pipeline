...
- Phase 18 first slice에서는 same strict annual family에
  ...
  - `market regime`, `underperformance guardrail`, `drawdown guardrail`
    같은 full risk-off는 이 lane의 fill 대상이 아니며,
  ...
  - Phase 19 third slice에서는
    ...
    - `Risk-Off Contract`는
      ...
      `Defensive Sleeve Tickers`는 후자를 고른 경우 full risk-off에서 담을 방어 ETF 목록이다.
    - form 설명에는
      `portfolio-wide risk-off`가
      개별 종목 일부 제외가 아니라
      `Market Regime` 또는 guardrail 때문에
      포트폴리오 전체가 보수 모드로 이동하는 상황이라는 뜻도 같이 보강했다.
    ...
    - interpretation summary에는
      ...
      가 추가되어,
      current run이 어떤 비중 규칙과 어떤 full risk-off contract를 사용했는지
      한 번에 읽을 수 있다.
