...
8. Phase 27은 백테스트 결과 상단에서 데이터 가능 범위, 실제 결과 기간, stale / missing / malformed price 문제를 더 먼저 보이게 만드는 중이다.
9. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
