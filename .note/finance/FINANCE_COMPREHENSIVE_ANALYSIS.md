...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-06 / Phase 36 Selected Portfolio Dashboard + value / holding drift check
...
| Phase 36 | Final Selected Portfolio Monitoring And Rebalance Operations | `Operations > Selected Portfolio Dashboard`를 추가해 Final Review에서 선정된 포트폴리오와 current weight / value / holding 기반 drift를 read-only 운영 대상으로 확인하게 했으며 manual QA 대기 중 |
...
17. Phase 36은 implementation_complete / manual_qa_pending 상태다. `Operations > Selected Portfolio Dashboard`를 추가해 Final Review에서 선정된 포트폴리오를 read-only 운영 대상으로 확인하고, current weight / current value / shares x price 입력 기반 drift / rebalance-needed 신호를 확인한다. DB latest close는 입력 보조 기능이며, account holding 자동 연결이나 주문 초안은 아니다.
...
| Phase 36 Final Selected Portfolio Monitoring And Rebalance Operations | Final Review에서 선정된 포트폴리오와 current weight / value / holding 기반 drift를 `Operations > Selected Portfolio Dashboard`에서 read-only로 다시 확인하는 phase다. manual QA 대기 중이다. | `phases/phase36/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
