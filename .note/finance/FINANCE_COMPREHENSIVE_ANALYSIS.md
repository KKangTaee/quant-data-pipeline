...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-04-21 / Phase 26 implementation handoff
...
| Web Runtime / UI | Streamlit에서 single / compare / history / saved replay / Pre-Live Review를 실행한다 | `app/web/runtime/*`, `app/web/pages/backtest.py` |
...
| Backtest UI | `app/web/pages/backtest.py` | Single / Compare / History / Saved Portfolio / Pre-Live Review 화면 |
...
| Web UI | `app/web/pages/backtest.py` | Single / Compare / History / Saved Portfolio / Pre-Live Review 화면 |
...
3. 현재 제품 경로는 DB-backed runtime, Backtest UI, compare, history, saved replay, Pre-Live Review까지 확장됐다.
...
7. Phase 26은 Phase 27~30을 시작하기 전에 과거 backlog와 현재 foundation gap을 다시 정렬했고, 사용자 checklist QA가 남아 있다.
8. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| 우선순위 | 왜 필요한가 | 기준 문서 |
|---|---|---|
| Phase 26 Foundation stabilization | 과거 pending / backlog를 현재 제품 기준으로 다시 정렬했고, Phase 27~30 입력으로 넘길 항목을 분리했다. | `phase26/` |
| Phase 27 Data integrity / backtest trust | missing data, stale data, common-date truncation, excluded ticker를 사용자가 놓치지 않게 해야 한다. | `data_architecture/DATA_QUALITY_AND_PIT_NOTES.md`, `code_analysis/BACKTEST_RUNTIME_FLOW.md` |
| Phase 28 Strategy family parity | annual / quarterly / 신규 전략이 UI, payload, history, saved replay, Real-Money, Guardrail에서 의도된 수준으로 맞아야 한다. | `code_analysis/STRATEGY_IMPLEMENTATION_FLOW.md`, `phase23/`, `phase24/` |
| Phase 29 Candidate review workflow | 백테스트 결과를 current candidate, near miss, watchlist, pre-live 후보로 반복 가능하게 넘겨야 한다. | `operations/`, `backtest_reports/strategies/` |
| Phase 30 Portfolio proposal / pre-live monitoring | 최종 목표인 포트폴리오 구성 제안으로 가기 위해 후보 묶음과 paper/pre-live tracking surface가 필요하다. | `phase25/`, `backtest_reports/` |
