...
10. Phase 29는 current candidate registry를 `Backtest > Candidate Review`에서 검토 보드로 읽고, 후보를 compare 또는 Pre-Live Review로 넘기는 첫 workflow를 추가했다.
...
| Phase 29 Candidate review workflow | current candidate를 검토 보드로 읽고 compare / Pre-Live Review로 넘기는 첫 surface를 추가했으며, 다음에는 Latest / History 결과를 후보 검토 초안으로 넘기는 handoff를 다룬다. | `operations/`, `backtest_reports/strategies/`, `phase29/` |
