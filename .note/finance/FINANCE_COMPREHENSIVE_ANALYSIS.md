...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 34 Final Review tab reboundary handoff
...
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview / inline paper observation / final selection decision으로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 / Phase 33 / Phase 34 문서 |
...
| Phase 34 | Final Portfolio Selection Decision Pack | `Backtest > Final Review` 탭을 분리해 단일 후보 또는 saved proposal의 validation, robustness, paper observation 기준, operator judgment를 하나의 final review record로 저장하고 Phase35 handoff를 확인하게 했으며 manual QA 대기 중 |
...
15. Phase 34는 implementation_complete / manual_qa_pending 상태다. `Backtest > Final Review` 탭을 분리해 단일 후보 또는 saved proposal을 읽고 validation / robustness / paper observation 기준 / operator judgment를 하나의 `Final Portfolio Selection Decision` record로 저장한 뒤 Phase35 handoff를 확인한다. 이 decision도 live approval이나 주문 지시가 아니다.
...
| Phase 34 Final Portfolio Selection Decision Pack | Final Review 탭에서 검증과 paper observation 기준까지 본 후보나 proposal을 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 phase다. 현재는 implementation_complete / manual_qa_pending 상태이며, final decision 저장소 / evidence pack / Final Review UI / review / Phase35 handoff를 구현했다. | `phases/phase34/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Final Review validation / robustness / paper observation / Final Portfolio Selection Decision, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
