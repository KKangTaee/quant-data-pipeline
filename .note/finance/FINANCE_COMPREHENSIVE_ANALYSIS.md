...
9. Phase 28은 annual strict, quarterly strict, price-only ETF 전략의 지원 범위와 cadence 차이를 `Strategy Capability Snapshot`으로 보이게 만들고, history record의 재실행 / form 복원 가능성도 `History Replay / Load Parity Snapshot`으로 확인하게 만드는 중이다.
...
| Phase 28 Strategy family parity | annual / quarterly / 신규 전략이 UI, payload, history, saved replay, Real-Money, Guardrail에서 의도된 수준으로 맞아야 하며, capability snapshot과 history replay/load parity snapshot을 추가했다. | `code_analysis/STRATEGY_IMPLEMENTATION_FLOW.md`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `phase28/` |
