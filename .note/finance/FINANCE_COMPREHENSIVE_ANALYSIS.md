...
- Phase 24 first implementation에서는 같은 price-only infrastructure 위에
  ...
  다만 이 시점의 범위는 compile / import / DB-backed smoke validation까지이며,
  아직 `Backtest` UI selector, compare, history, saved replay surface에는 노출되지 않았다.
...
- `GlobalRelativeStrengthStrategy`는 Phase 24 first pass에서 추가된 price-only ETF allocation strategy다.
  ...
  `MA200` trend filter를 통과하지 못한 슬롯은 cash proxy로 두는 core/runtime smoke 단계다.
  아직 UI catalog / compare / history / saved replay까지 연결된 제품 전략 surface는 아니다.
