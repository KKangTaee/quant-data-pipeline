...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 34 Final Portfolio Selection Decision Pack kickoff
...
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview / paper tracking ledger로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 / Phase 33 문서 |
...
| Phase 34 | Final Portfolio Selection Decision Pack | Phase 33 paper ledger를 읽어 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 decision pack을 시작했으며, final decision 계약과 저장소 경계를 먼저 정의했다 |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Portfolio Risk / Live Readiness Validation Pack, Robustness / Stress Validation Pack, Paper Portfolio Tracking Ledger 저장 / review 단계까지 확장됐다.
...
14. Phase 33은 complete / manual_qa_completed 상태다. Phase 32 handoff를 받아 후보나 proposal을 실제 돈 없이 관찰할 append-only paper tracking ledger로 저장하고, 저장된 ledger를 다시 읽어 Phase34 final selection handoff 준비 상태를 확인하는 흐름을 구현하고 사용자 QA까지 완료했다. 아직 paper PnL 자동 계산, 최종 선정 decision, live approval, 주문 지시는 아니다.
15. Phase 34는 active / not_ready_for_qa 상태다. 저장된 paper ledger record를 읽어 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 `Final Portfolio Selection Decision Pack`을 여는 단계이며, 첫 작업으로 final decision row 계약과 live approval / 주문 지시 경계를 정의했다.
...
| Phase 33 Paper Portfolio Tracking Ledger | Phase 32에서 준비 가능성이 확인된 후보나 proposal을 실제 돈 없이 관찰할 append-only ledger로 저장하고 다시 읽는다. 현재는 complete / manual_qa_completed 상태이며, Phase34 final selection decision pack을 열 준비가 되었다. | `phases/phase33/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `operations/PAPER_PORTFOLIO_TRACKING_LEDGER_GUIDE.md` |
| Phase 34 Final Portfolio Selection Decision Pack | paper tracking까지 본 후보나 proposal을 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 phase다. 현재는 active / not_ready_for_qa 상태이며, final decision 계약과 저장소 경계를 먼저 정의했다. | `phases/phase34/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
