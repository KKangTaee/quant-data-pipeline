...
성격:
- long-format raw-like financial store
- 확장형 팩터 생성에 적합
...
`financial_statements.py`에는 `filing_date`, `accepted_at`가 존재하지만,
현재 전략/팩터 계층과 직접 연결된 흔적은 약하다.
...
좋은 점:
- 라벨 단위 세부 정보가 저장된다
- filing_date와 accepted_at이 들어간다
...
필요한 연결:
- `financial_statements.py`의 `filing_date`, `accepted_at`
