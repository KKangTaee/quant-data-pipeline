...
| Pre-Live persistence | Phase 25에서 후보 운영 기록소가 `.note/finance/PRE_LIVE_CANDIDATE_REGISTRY.jsonl`로 분리되었고, `draft-from-current` helper로 current candidate에서 Pre-Live 기록 초안을 만들 수 있다. Backtest UI dashboard는 아직 후속 검토 대상이다. | `phase25/`, `operations/PRE_LIVE_CANDIDATE_REGISTRY_GUIDE.md` |
