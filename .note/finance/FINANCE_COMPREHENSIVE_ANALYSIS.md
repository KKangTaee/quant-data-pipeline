...
이 섹션은 전체 시스템을 이해할 때 자주 보는 핵심 함수 목록이다.
함수별 detailed call flow나 새 구현 시 갱신해야 할 단계는
`.note/finance/code_analysis/` 문서를 우선한다.
...
- `finance.strategy.GlobalRelativeStrengthStrategy`
...
- `GlobalRelativeStrengthStrategy`는 Phase 24 first pass에서 추가된 price-only ETF allocation strategy다.
  현재는 `1M / 3M / 6M / 12M` trailing return 평균 score로 상위 ETF를 고르고,
  `MA200` trend filter를 통과하지 못한 슬롯은 cash proxy로 두는 구조다.
  Phase 24 UI replay pass 기준으로 `Backtest` single strategy, compare, history prefill,
  run-again, saved portfolio replay까지 연결된 제품 전략 surface가 되었다.
  DB-backed 실행에서는 MA / relative-strength warmup 이후 데이터가 비어버리는 risky ticker를
  결과 universe에서 제외하고 `excluded_tickers` / warning으로 남긴다.
  예를 들어 기본 preset의 특정 ETF 가격 이력이 충분하지 않으면 전체 실행을 중단하지 않고,
  실제 사용 가능한 ETF 묶음으로 실행하되 해석 전 데이터 보강 필요성을 표시한다.
- 다음 ETF later pass에서는 여기에
  `Min ETF AUM ($B)`와 `Max Bid-Ask Spread (%)` current-operability policy도 추가되어,
  shared runtime helper가 `nyse_asset_profile` snapshot을 읽어
  `etf_operability_status`
  와 AUM/spread failed symbol preview를 함께 만든다.
- strict annual family 3종도 Phase 12 first pass부터
  같은 real-money contract를 사용하며,
  `quality_snapshot_equal_weight` 경로에 `min_price` investability filter와
  `min_history_months` candidate filter가 추가되어
  annual strict ranking candidate에도 실제로 적용된다.
- 그리고 next pass에서는 여기에
  `Min Avg Dollar Volume 20D ($M)` liquidity proxy도 추가되어,
  각 리밸런싱 날짜 직전 최근 20거래일 평균 거래대금이 너무 작은 후보를
  annual strict ranking 단계에서 제외할 수 있게 되었다.
- 그리고 same runtime helper가 현재는 benchmark가 있을 때
  validation surface까지 함께 만든다.
  즉 real-money 결과는 이제
  - gross / net / cost
  - benchmark overlay
  - benchmark CAGR / coverage / net CAGR spread
  - benchmark-relative drawdown / rolling underperformance diagnostics
  를 한 번에 갖는 구조다.
- latest later pass에서는 strict annual family에
  - `Min Benchmark Coverage (%)`
  - `Min Net CAGR Spread (%)`
  기준도 추가되어,
  benchmark가 존재하더라도 coverage나 상대 CAGR이 promotion policy 아래면
  `benchmark_policy_status = normal / watch / caution / unavailable`
  로 따로 읽고 `promotion_decision`에도 반영한다.
- next later pass에서는 strict annual family에
  - `Min Liquidity Clean Coverage (%)`
  - `liquidity_clean_coverage`
  기준도 추가되어,
  단순히 liquidity filter가 켜져 있는지만이 아니라
  실제 리밸런싱 행 중 얼마나 많은 비율이 유동성 제외 없이 지나갔는지를
  `liquidity_policy_status = normal / watch / caution / unavailable`
  로 읽고 `promotion_decision`에도 함께 반영한다.
- latest broader-promotion later pass에서는 strict annual family에
  - `Max Underperformance Share (%)`
  - `Min Worst Rolling Excess (%)`
  기준도 추가되어,
  기존 validation surface가 계산하던
  - `rolling_underperformance_share`
  - `rolling_underperformance_worst_excess_return`
  을 실제 승격 정책에도 연결한다.
  - 즉 이제는 validation을 "보여주기만" 하는 것이 아니라,
    `validation_policy_status = normal / watch / caution / unavailable`
    로 요약해서 `promotion_decision`에 직접 반영한다.
- latest broader-benchmark later pass에서는 strict annual family에
  - `Benchmark Contract`
    - `Ticker Benchmark`
    - `Candidate Universe Equal-Weight`
  기준도 추가되어,
  benchmark overlay를 단일 ticker reference만이 아니라
  같은 candidate universe를 단순 균등 보유한 기준선으로도 읽을 수 있게 되었다.
  - runtime meta에는
    - `benchmark_contract`
    - `benchmark_label`
    ...
    가 남고,
  - single / compare / history / `Load Into Form`도 같은 contract를 복원한다.
  - 단, current first pass에서는 actual underperformance guardrail rule은 여전히
    `benchmark_ticker`를 기준으로 동작한다.
...
- Phase 17 first implementation slice에서는 strict annual family 3종에
  `partial cash retention` contract도 추가되어,
  trend filter가 raw selected top-N 중 일부만 탈락시켰을 때
  survivor reweighting 대신 탈락 슬롯 비중을 현금으로 유지하는 실험이 가능해졌다.
  - current slice는 `Trend Filter`의 부분 탈락에만 적용된다.
  - `market regime`와 `underperformance / drawdown guardrail`의 전체 risk-off는
    기존처럼 전부 현금 처리다.
  - result row에는
    ...
    가 추가되어,
    selection history / interpretation surface에서
    "survivors reweighted"와 "rejected slots left in cash"를 구분해 읽을 수 있다.
- same strict annual family에는 next slice로
  `defensive sleeve risk-off` contract도 추가되었다.
  - current contract:
    - `cash_only`
    - `defensive_sleeve_preference`
  - `defensive_sleeve_preference`일 때는
    포트폴리오 전체를 쉬어야 하는 구간이 오면
    `BIL / SHY / LQD` 같은 defensive sleeve로 회전한다.
  - first slice 기준 적용 범위는
    `market regime`, `underperformance guardrail`, `drawdown guardrail`
    같은 포트폴리오 전체 전환 state다.
  - `Trend Filter`의 partial rejection은 여전히
    `partial cash retention`이 담당하고,
    defensive sleeve contract와는 분리해서 읽는다.
  - 구현 보정:
    defensive sleeve ticker가 strict annual candidate universe로 섞여
    liquidity exclusion을 오염시키지 않도록
    candidate universe와 sleeve ticker를 분리했다.
  - result/meta에는
    - `Risk-Off Mode`
    - `Risk-Off Reason`
    - `Defensive Sleeve Ticker`
    - `Defensive Sleeve Count`
    - `defensive_sleeve_active_count`
    - `defensive_sleeve_active_share`
    가 남아,
    defensive sleeve actual activation을 current anchor 기준으로 다시 읽을 수 있게 됐다.
- same strict annual family에는 third slice로
  `concentration-aware weighting` contract도 추가되었다.
  - current contract:
    - `equal_weight`
    - `rank_tapered`
  - `rank_tapered`는
    selected top-N을 pure equal-weight로만 두지 않고,
    상위 ranked 종목에 조금 더 높은 비중을 주는
    bounded weighting mode다.
  - single / compare / history / `Load Into Form`까지 같은 contract를 복원한다.
  - row/meta에는
    - `Weighting Mode`
    - `Next Weight`
    - `weighting_mode`
    가 남아,
    pure equal-weight와 rank-tapered execution을 결과 surface에서도 구분해 읽을 수 있게 됐다.
  - Phase 17 representative rerun 기준으로는
    `Value`, `Quality + Value` current anchor 모두
    gate는 유지했지만 lower-MDD exact rescue는 만들지 못했다.
- Phase 18 first slice에서는 same strict annual family에
  `next-ranked eligible fill` contract도 추가되었다.
  - current UI label:
    - `Fill Rejected Slots With Next Ranked Names`
  - 이 contract는
    `Trend Filter`가 raw top-N 중 일부만 탈락시킨 경우,
    ranked 다음 후보 중에서도 trend를 통과하는 이름으로
    빈 슬롯을 먼저 채우는 selection-structure redesign이다.
  - 적용 순서는:
    - raw top-N 확정
    - trend rejection 발생
    - next-ranked eligible fill 시도
    - 그래도 빈 슬롯이 남으면
      기존 `partial cash retention` 또는 survivor reweighting이 처리
  - `market regime`, `underperformance guardrail`, `drawdown guardrail`
    같은 포트폴리오 전체 전환 상황은 이 lane의 fill 대상이 아니며,
    기존 risk-off contract와 분리해서 읽는다.
  - row/meta에는
    ...
    가 남아,
    selection history / interpretation surface에서
    “filled names로 raw rejection slot을 메꿨는지”를 따로 읽을 수 있게 됐다.
  - Phase 19 first slice에서는 이 lane과 기존 partial cash retention lane을
    operator-facing contract 하나로 다시 묶었다.
    - current UI / payload contract:
      - `reweight_survivors`
      - `retain_unfilled_as_cash`
      - `fill_then_reweight`
      - `fill_then_retain_cash`
    - 즉 사용자는 이제 두 개 boolean을 조합해 해석하는 대신,
      trend rejection 이후 빈 슬롯을 어떻게 처리할지
      하나의 명시적 contract로 고를 수 있다.
    - 구현 방식은 compatibility-first다.
      - current strategy core는 여전히 legacy boolean 기반으로 동작하고
      - UI/runtime/history는 explicit mode를 저장한 뒤
        내부 실행 직전 boolean으로 다시 변환한다.
  - Phase 19 second slice에서는
    selection history / interpretation summary도 같은 contract 언어를 직접 쓰도록 정리했다.
    - single / compare strict annual form에서는
      `Advanced Inputs > Overlay`와
      `Advanced Inputs > Portfolio Handling & Defensive Rules`를 분리했다.
      - `Overlay`
        - `Trend Filter`
        - `Market Regime`
      - `Portfolio Handling & Defensive Rules`
        ...
        - `Defensive Sleeve Tickers`
      로 읽히게 정리해,
      overlay 자체와 overlay 이후 처리 규칙을 구분해서 보게 했다.
    - `Rejected Slot Handling Contract` help는
      각 option을 줄글 하나로 이어 쓰지 않고,
      option별 bullet 설명으로 다시 읽을 수 있게 보강했다.
    - selection history row에는
      - `Rejected Slot Handling`
      - `Filled Count`
      - `Filled Tickers`
      가 같이 남는다.
    - interpretation summary에는
      - `Rejected Slot Handling`
      - `Filled Events`
      - `Cash-Retained Events`
      가 추가되어,
      current run이 trend rejection 이후 어떤 처리 계약을 실제로 사용했는지
      한 눈에 읽을 수 있다.
    - 내부 boolean 컬럼은 interpretation 계산에는 계속 쓰되,
      `History` 표 display에서는 숨겨
      operator가 boolean 조합 대신 contract 언어를 중심으로 읽도록 유지한다.
  - Phase 19 third slice에서는
    `Risk-Off`와 `Weighting`도 같은 수준으로 history / interpretation에 다시 정리했다.
    - single / compare strict annual form에서는
      `Advanced Inputs > Portfolio Handling & Defensive Rules` 안에
      - `Weighting Contract`
      - `Risk-Off Contract`
      - `Defensive Sleeve Tickers`
      가 같은 언어로 보인다.
    - `Risk-Off Contract`는
      `Cash Only`와 `Defensive Sleeve Preference` 중 무엇을 쓸지 고르는 계약이고,
      `Defensive Sleeve Tickers`는 후자를 고른 경우
      포트폴리오 전체를 쉬어야 하는 구간에서 담을 방어 ETF 목록이다.
    - form 설명에는
      `portfolio-wide risk-off`가
      개별 종목 일부 제외가 아니라
      `Market Regime` 또는 guardrail 때문에
      factor 포트폴리오 전체를 멈추고 현금 또는 방어 ETF 쪽으로 전환하는 상황이라는 뜻도 같이 보강했다.
    - `Weighting Contract`, `Rejected Slot Handling Contract`, `Risk-Off Contract`는
      enable/disable 토글형 기능이 아니라
      백테스트 실행 시 항상 저장되는 기본 처리 계약이고,
      관련 상황이 실제로 발생할 때 결과에 영향이 나타나는 구조라는 설명도 추가했다.
    - selection history / meta에는
      - `Weighting Contract`
      - `Risk-Off Contract`
      - `Risk-Off Reasons`
      - `Defensive Sleeve Tickers`
    ...
    - interpretation summary에는
      ...
      가 추가되어,
      current run이 어떤 비중 규칙과 어떤 포트폴리오 전체 전환 contract를 사용했는지
      한 번에 읽을 수 있다.
    - row-level interpretation 문구도
      market regime / guardrail 때문에
      full cash로 갔는지 defensive sleeve로 회전했는지를
      더 직접적으로 설명한다.
      - old run payload도 same form에서 계속 복원 가능하다.
    - 추가 UX 보강:
      - latest result의 nested tabs 이름을
        `Selection History Table`, `Interpretation Summary`, `Selection Frequency`
        로 더 직접적으로 바꿨다.
      - `Selection History Table` 탭 상단에는
        `Interpretation` 열이 row-level interpretation이라는 설명을 추가했다.
      - `Interpretation Summary` 탭 상단에는
        이 표가 행별 문장이 아니라 실행 전체 요약이며,
        `Rejected Slot Handling`, `Weighting Contract`, `Risk-Off Contract`,
        `Filled Events`, `Cash-Retained Events`, `Defensive Sleeve Activations`
        를 먼저 보면 된다는 안내를 추가했다.
      - `Backtest > History` 화면에는
      ...
      - 이후 추가 보정:
        - `Run Again`은 rerun 성공 후 `Single Strategy` 패널로 자동 이동해
          refreshed `Latest Backtest Run`을 바로 보여준다.
        - `Load Into Form`은 입력값만 불러오는 동작임을 더 명확히 안내하고,
          다시 실행해야 최신 결과가 갱신된다는 설명을 남겼다.
        - `Single Strategy`로 이동한 뒤 `Back To History` shortcut도 추가했다.
  - representative rerun first pass 기준:
    - `Value` trend-on probe는
      cash drag와 `MDD`, validation은 개선됐지만
      current runtime meta 기준 still `hold / blocked`였다
    - `Quality + Value` trend-on probe는
      `CAGR`, `MDD`, cash share는 개선됐지만
      still `hold / blocked`였다
    - `Value` anchor-near second pass에서도
      `base + psr`, `base + psr + pfcr`, `Top N = 12~16`
      전체가 still `hold / blocked`였다
    - 따라서 current first pass에서는
      meaningful redesign lane으로는 확인됐지만,
      current practical anchor replacement 결과로 보지는 않는다.
- ETF 전략군도 now optional
  `underperformance guardrail` / `drawdown guardrail`
  actual rule을 같이 받을 수 있고,
  runtime meta에는 ETF guardrail trigger count / share가 기록된다.
  - `GTAA`
  - `Risk Parity Trend`
  - `Dual Momentum`
  모두 같은 contract를 사용한다.
  - UI에서는 이제 ETF guardrail이 비활성화된 run에서도
    `Real-Money > 실행 부담`과 `Execution Context`에
    `ON / OFF`, trigger count, trigger share가 계속 보인다.
    즉 guardrail surface는 "켜졌을 때만 나타나는 조건부 섹션"이 아니라
    ETF 실전 계약을 확인하는 고정 surface로 읽으면 된다.
- same strict annual contract는 now optional
  `drawdown guardrail` actual rule도 받을 수 있고,
  trailing strategy max drawdown이 설정값보다 더 깊거나
  benchmark 대비 drawdown gap이 설정값보다 더 나빠지면
  그 rebalance는 cash로 물러난다.
  - result row와 real-money surface에는
