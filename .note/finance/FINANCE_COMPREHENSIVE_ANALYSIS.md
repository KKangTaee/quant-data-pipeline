...
| Web Runtime / UI | Streamlit에서 single / compare / candidate review / Pre-Live 운영 기록 / Operations history / saved replay / Portfolio Proposal을 실행한다 | `app/web/runtime/*`, `app/web/pages/backtest.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_review*.py` |
...
| Web UI | `app/web/pages/backtest.py`, `app/web/backtest_history.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py` | Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal 주 흐름과 Operations Backtest Run History / Saved Portfolio 보조 재현 화면, 공용 status/route UI component |
