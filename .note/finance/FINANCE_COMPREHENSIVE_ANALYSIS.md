...
| Web Runtime / UI | Streamlit에서 single / compare / candidate review / Operations history / saved replay / Pre-Live Review / Portfolio Proposal을 실행한다 | `app/web/runtime/*`, `app/web/pages/backtest.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_pre_live_review*.py` |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Pre-Live Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Backtest Run History는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Web UI | `app/web/pages/backtest.py`, `app/web/backtest_history.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_pre_live_review.py`, `app/web/backtest_pre_live_review_helpers.py` | Single / Compare / Candidate Review / Pre-Live Review / Portfolio Proposal 주 흐름과 Operations Backtest Run History / Saved Portfolio 보조 재현 화면, 공용 status/route UI component |
...
3. 현재 제품 경로는 DB-backed runtime, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live Review, Portfolio Proposal draft까지 확장됐다.
...
10. Phase 29는 current candidate registry와 Latest / History 결과를 `Backtest > Candidate Review`에서 Candidate Packaging 초안, Candidate Review Note, registry draft, Pre-Live route 확인으로 이어지게 하는 workflow를 추가했고 사용자 QA까지 완료했다. 현재 사용자-facing 흐름에서는 Draft 확인 / Review Note 저장 / Registry 저장 / Pre-Live 진입 평가를 `Candidate Packaging` 하나의 순서형 화면으로 묶어 본다.
...
| Phase 29 Candidate review workflow | current candidate와 Latest / History 결과를 Candidate Packaging 초안, Candidate Review Note, registry draft, Pre-Live route 확인으로 이어지게 하는 surface를 추가했고 QA까지 완료했다. 현재 운영 가이드에서는 Draft 확인 / Review Note 저장 / Registry 저장 / Pre-Live 진입 평가를 Candidate Packaging 하나의 사용자-facing 순서형 화면으로 묶어 본다. | `operations/`, `backtest_reports/strategies/`, `phase29/` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live Review / Portfolio Proposal, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
