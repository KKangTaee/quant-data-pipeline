...
| Backtest UI | `app/web/pages/backtest.py` | Single / Compare / History / Saved Portfolio 화면 |
...
| Pre-Live persistence | Phase 25에서 후보 운영 기록소가 `.note/finance/PRE_LIVE_CANDIDATE_REGISTRY.jsonl`로 분리되었고, UI entry point / review workflow는 진행 중이다. | `phase25/`, `operations/PRE_LIVE_CANDIDATE_REGISTRY_GUIDE.md` |
...
3. 현재 제품 경로는 DB-backed runtime, Backtest UI, compare, history, saved replay까지 확장됐다.
...
| pre-live registry helper | `plugins/quant-finance-workflow/scripts/manage_pre_live_candidate_registry.py` | Pre-Live 후보 운영 기록 template / list / show / append / validate |
