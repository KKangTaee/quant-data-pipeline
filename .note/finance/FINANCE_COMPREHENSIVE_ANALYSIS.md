...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-04-30 / Backtest workflow navigation cleanup
...
| Web Runtime / UI | Streamlit에서 single / compare / candidate review / history / saved replay / Pre-Live Review / Portfolio Proposal을 실행한다 | `app/web/runtime/*`, `app/web/pages/backtest.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_pre_live_review*.py` |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Pre-Live Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, History / Saved Portfolio replay는 보조 재현 도구로 제품 화면에서 열린다. |
...
| Web UI | `app/web/pages/backtest.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_pre_live_review.py`, `app/web/backtest_pre_live_review_helpers.py` | Single / Compare / Candidate Review / Pre-Live Review / Portfolio Proposal 주 흐름과 Run History / Saved Portfolio 보조 재현 화면, 공용 status/route UI component |
