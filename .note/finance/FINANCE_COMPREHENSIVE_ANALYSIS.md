...
| Web Runtime / UI | Streamlit에서 Overview dashboard, single / compare / candidate review / Pre-Live 운영 기록 / Operations history / saved replay / Portfolio Proposal을 실행한다 | `app/web/runtime/*`, `app/web/overview_dashboard*.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history*.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py` |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Backtest Run History는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Pre-Live persistence | Phase 25에서 후보 운영 기록소가 `.note/finance/PRE_LIVE_CANDIDATE_REGISTRY.jsonl`로 분리되었고, 현재 UI에서는 `Backtest > Candidate Review`의 운영 상태 저장 구간에서 current candidate를 Pre-Live 운영 기록으로 저장한다. | `phase25/`, `operations/PRE_LIVE_CANDIDATE_REGISTRY_GUIDE.md` |
...
| Web UI | `app/web/overview_dashboard.py`, `app/web/overview_dashboard_helpers.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py` | Overview 후보 / funnel / next action dashboard, Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal 주 흐름과 Operations Backtest Run History / Saved Portfolio 보조 재현 화면, 공용 result/status/route UI component |
