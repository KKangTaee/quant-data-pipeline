...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-06 / Phase 36 Selected Portfolio Dashboard + drift alert preview
...
| Web Runtime / UI | Streamlit에서 Overview dashboard, single / compare / candidate review / Pre-Live 운영 기록 / Operations history / Candidate Library / saved replay / Portfolio Proposal / Final Review / Selected Portfolio Dashboard를 실행한다 | `app/web/runtime/*`, `app/web/overview_dashboard*.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history*.py`, `app/web/backtest_candidate_library*.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py`, `app/web/backtest_final_review*.py`, `app/web/final_selected_portfolio_dashboard*.py` |
| Review / Proposal / Pre-Live / Selected Operations | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview / inline paper observation / final selection decision / selected portfolio operations dashboard로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 / Phase 33 / Phase 34 / Phase 35 / Phase 36 문서 |
...
| Phase 36 | Final Selected Portfolio Monitoring And Rebalance Operations | `Operations > Selected Portfolio Dashboard`를 추가해 Final Review에서 선정된 포트폴리오와 current weight / value / holding 기반 drift 및 alert preview를 read-only 운영 대상으로 확인하게 했으며 manual QA 대기 중 |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal, Final Review가 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal은 단일 후보 직행 평가와 다중 후보 proposal draft 작성 / feedback에 집중한다. Final Review는 단일 후보 또는 saved proposal을 같은 검증 언어로 읽어 validation, robustness / stress 질문, paper observation 기준, 최종 선정 / 보류 / 거절 / 재검토 판단을 남기며 현재 workflow를 `최종 판단 완료`로 마무리한다. Backtest Run History, Candidate Library, Selected Portfolio Dashboard는 Operations의 보조 재현 / 감사 / 선정 이후 운영 확인 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Portfolio proposal / final review / selected operations workflow | Phase 30에서 `.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 저장소와 `Backtest > Portfolio Proposal` UI가 추가됐다. 단일 current candidate는 proposal 저장을 반복하지 않고 Live Readiness 직행 가능 여부를 확인하며, 여러 후보를 묶을 때만 목적 / 역할 / 비중이 있는 proposal draft로 저장한다. saved proposal의 Monitoring / Pre-Live Feedback / Paper Tracking Feedback은 다중 후보 작성 흐름의 `4. 저장된 Portfolio Proposal 확인`에서 읽는다. Phase 31~32의 validation / robustness 계산은 Final Review에서 재사용한다. Phase 33의 Paper Ledger helper와 registry는 호환성 / 운영 artifact로 유지하지만, 현재 main flow는 별도 ledger save를 필수로 요구하지 않는다. Phase 34는 `Backtest > Final Review` 탭과 `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` 저장소를 추가해 단일 후보 또는 saved proposal의 validation, robustness, inline paper observation 기준, 최종 선정 / 보류 / 거절 / 재검토 판단을 append-only로 남기며 사용자 QA까지 완료했다. Phase 35는 별도 후속 가이드 panel을 제거하고 Final Review를 마지막 active panel로 고정했다. Phase 36은 Final Review에서 선정된 row를 `Operations > Selected Portfolio Dashboard`에서 read-only로 읽고, current weight 직접 입력, current value 입력, shares x price 입력으로 target 대비 drift와 `REBALANCE_NEEDED` 신호를 확인하며, drift 결과를 read-only alert preview와 Final Review review trigger table로 다시 읽게 했다. shares x price 입력에서는 DB latest close를 보조로 읽을 수 있지만, 아직 optimizer, correlation / turnover / capacity 정량 엔진, stress sweep 실행 엔진, paper PnL 시계열 자동 계산, account holding 자동 연결, alert persistence, live readiness approval / 주문 지시는 아니다. | `phases/phase30/`, `phases/phase31/`, `phases/phase32/`, `phases/phase33/`, `phases/phase34/`, `phases/phase35/`, `phases/phase36/`, `operations/PORTFOLIO_PROPOSAL_REGISTRY_GUIDE.md`, `operations/PAPER_PORTFOLIO_TRACKING_LEDGER_GUIDE.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
...
17. Phase 36은 implementation_complete / manual_qa_pending 상태다. `Operations > Selected Portfolio Dashboard`를 추가해 Final Review에서 선정된 포트폴리오를 read-only 운영 대상으로 확인하고, current weight / current value / shares x price 입력 기반 drift / rebalance-needed 신호와 read-only alert preview를 확인한다. DB latest close는 입력 보조 기능이며, account holding 자동 연결, alert persistence, 주문 초안은 아니다.
...
| Phase 36 Final Selected Portfolio Monitoring And Rebalance Operations | Final Review에서 선정된 포트폴리오와 current weight / value / holding 기반 drift 및 alert preview를 `Operations > Selected Portfolio Dashboard`에서 read-only로 다시 확인하는 phase다. manual QA 대기 중이다. | `phases/phase36/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
