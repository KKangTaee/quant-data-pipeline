...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 31 Portfolio Risk Validation Pack
...
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack으로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 문서 |
...
| Phase 31 | Portfolio Risk / Live Readiness validation pack | current candidate / Pre-Live / Portfolio Proposal을 새 승인 registry 없이 읽어 단일 후보와 proposal draft를 같은 validation pack으로 요약하고, component risk / overlap / concentration first pass와 Phase 32 robustness handoff를 구현하고 manual QA 대기 중 |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal 안에서는 Phase 31 Validation Pack이 단일 후보와 proposal draft를 같은 위험 검증 언어로 읽는다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Portfolio proposal persistence / monitoring / validation | Phase 30에서 `.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 저장소와 `Backtest > Portfolio Proposal` UI가 추가됐다. 단일 current candidate는 proposal 저장을 반복하지 않고 Live Readiness 직행 가능 여부를 확인하며, 여러 후보를 묶을 때만 목적 / 역할 / 비중이 있는 proposal draft로 저장한다. saved proposal의 Monitoring / Pre-Live Feedback / Paper Tracking Feedback은 보조 도구에서 읽는다. Phase 31은 같은 화면에 read-only Validation Pack을 추가해 단일 후보 / 작성 중 proposal / 저장 proposal을 component risk, overlap, concentration, blocker, Phase 32 handoff 관점으로 읽는다. 아직 optimizer, correlation / turnover / capacity 정량 엔진, live readiness approval은 아니다. | `phases/phase30/`, `phases/phase31/`, `operations/PORTFOLIO_PROPOSAL_REGISTRY_GUIDE.md` |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Portfolio Risk / Live Readiness Validation Pack까지 확장됐다.
...
12. Phase 31은 implementation_complete / manual_qa_pending 상태다. 새 승인 registry를 만들지 않고 `Backtest > Portfolio Proposal`에서 단일 후보, 작성 중 proposal, 저장 proposal을 Validation Pack으로 읽어 component risk / overlap / concentration / blocker / Phase 32 handoff를 확인한다. 아직 정량 optimizer, correlation / turnover / capacity 엔진, 최종 승인 기록은 아니다.
13. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| Phase 31 Portfolio Risk / Live Readiness Validation | Portfolio Proposal 이후 바로 승인으로 뛰지 않고, 후보 또는 proposal이 다음 robustness 검증으로 넘어갈 만큼 구조적으로 말이 되는지 먼저 본다. 현재는 `Backtest > Portfolio Proposal` 안에 read-only Validation Pack을 추가해 단일 후보 / 작성 중 proposal / 저장 proposal의 component risk, blocker, paper tracking gap, overlap first pass, Phase 32 handoff를 확인하게 만들고 manual QA 대기 중이다. | `phases/phase31/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Portfolio Risk Validation Pack, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
