...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-04 / Phase 35 Post-Selection Operating Guide implementation handoff
...
| Web Runtime / UI | Streamlit에서 Overview dashboard, single / compare / candidate review / Pre-Live 운영 기록 / Operations history / Candidate Library / saved replay / Portfolio Proposal / Final Review / Post-Selection Guide를 실행한다 | `app/web/runtime/*`, `app/web/overview_dashboard*.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history*.py`, `app/web/backtest_candidate_library*.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py`, `app/web/backtest_final_review*.py`, `app/web/backtest_post_selection_guide*.py` |
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview / inline paper observation / final selection decision / post-selection operating guide로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 / Phase 33 / Phase 34 / Phase 35 문서 |
...
| Phase 35 | Post-Selection Operating Guide | `Backtest > Post-Selection Guide`에서 selected final review record를 읽어 리밸런싱 / 축소 / 중단 / 재검토 운영 기준을 `.note/finance/registries/POST_SELECTION_OPERATING_GUIDES.jsonl`에 append-only로 기록하고 review하게 했으며 manual QA 대기 중 |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal, Final Review, Post-Selection Guide는 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal은 단일 후보 직행 평가와 다중 후보 proposal draft 작성 / feedback에 집중한다. Final Review는 단일 후보 또는 saved proposal을 같은 검증 언어로 읽어 validation, robustness / stress 질문, paper observation 기준, 최종 선정 / 보류 / 거절 / 재검토 판단과 Phase35 handoff를 남긴다. Post-Selection Guide는 selected final decision만 읽어 리밸런싱 / 축소 / 중단 / 재검토 운영 기준을 기록하고 다시 확인한다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Portfolio proposal / final review / operating guide persistence | Phase 30에서 `.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 저장소와 `Backtest > Portfolio Proposal` UI가 추가됐다. 단일 current candidate는 proposal 저장을 반복하지 않고 Live Readiness 직행 가능 여부를 확인하며, 여러 후보를 묶을 때만 목적 / 역할 / 비중이 있는 proposal draft로 저장한다. saved proposal의 Monitoring / Pre-Live Feedback / Paper Tracking Feedback은 다중 후보 작성 흐름의 `4. 저장된 Portfolio Proposal 확인`에서 읽는다. Phase 31~32의 validation / robustness 계산은 Final Review에서 재사용한다. Phase 33의 Paper Ledger helper와 registry는 호환성 / 운영 artifact로 유지하지만, 현재 main flow는 별도 ledger save를 필수로 요구하지 않는다. Phase 34는 `Backtest > Final Review` 탭과 `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` 저장소를 추가해 단일 후보 또는 saved proposal의 validation, robustness, inline paper observation 기준, 최종 선정 / 보류 / 거절 / 재검토 판단, Phase35 handoff를 append-only로 남기며 사용자 QA까지 완료했다. Phase 35는 `Backtest > Post-Selection Guide`와 `.note/finance/registries/POST_SELECTION_OPERATING_GUIDES.jsonl` 저장소를 추가해 selected final decision의 리밸런싱 / 축소 / 중단 / 재검토 운영 기준을 append-only로 남긴다. 아직 optimizer, correlation / turnover / capacity 정량 엔진, stress sweep 실행 엔진, paper PnL 시계열 자동 계산, live readiness approval / 주문 지시는 아니다. | `phases/phase30/`, `phases/phase31/`, `phases/phase32/`, `phases/phase33/`, `phases/phase34/`, `phases/phase35/`, `operations/PORTFOLIO_PROPOSAL_REGISTRY_GUIDE.md`, `operations/PAPER_PORTFOLIO_TRACKING_LEDGER_GUIDE.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md`, `operations/POST_SELECTION_OPERATING_GUIDES_GUIDE.md` |
...
| Post-selection guide runtime | `app/web/runtime/post_selection_guides.py` | post-selection operating guide JSONL I/O helper |
| Web UI | `app/web/overview_dashboard.py`, `app/web/overview_dashboard_helpers.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_library.py`, `app/web/backtest_candidate_library_helpers.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py`, `app/web/backtest_final_review.py`, `app/web/backtest_final_review_helpers.py`, `app/web/backtest_post_selection_guide.py`, `app/web/backtest_post_selection_guide_helpers.py` | Overview 후보 / funnel / next action dashboard, Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal draft / Final Review validation / paper observation / final decision / Post-Selection Guide 운영 기준 주 흐름과 Operations Backtest Run History / Candidate Library / Saved Weighted Portfolio 보조 재현 화면, 공용 result/status/route UI component |
...
16. Phase 35는 implementation_complete / manual_qa_pending 상태다. `Backtest > Post-Selection Guide`에서 Phase34의 `SELECT_FOR_PRACTICAL_PORTFOLIO` final review record만 읽고, 리밸런싱 / 축소 / 중단 / 재검토 기준을 `.note/finance/registries/POST_SELECTION_OPERATING_GUIDES.jsonl`에 append-only로 기록 / review하게 했다. 이 guide도 live approval이나 주문 지시가 아니다.
17. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| Phase 34 Final Portfolio Selection Decision Pack | Final Review 탭에서 검증과 paper observation 기준까지 본 후보나 proposal을 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 phase다. final decision 저장소 / evidence pack / Final Review UI / review / Phase35 handoff를 구현했고 사용자 QA까지 완료했다. | `phases/phase34/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
| Phase 35 Post-Selection Operating Guide | Phase34에서 최종 선정된 후보를 바로 주문으로 연결하지 않고, 실제 운영 전 리밸런싱 / 축소 / 중단 / 재검토 기준으로 정리하는 phase다. `Backtest > Post-Selection Guide`, append-only operating guide registry, saved guide review를 구현했고 manual QA 대기 중이다. | `phases/phase35/`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md`, `operations/POST_SELECTION_OPERATING_GUIDES_GUIDE.md` |
