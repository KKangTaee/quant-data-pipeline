...
10. Phase 29는 current candidate registry를 `Backtest > Candidate Review`에서 검토 보드로 읽고, 후보를 compare 또는 Pre-Live Review로 넘기며, Latest / History 결과를 Candidate Packaging 초안, Candidate Review Note, registry draft로 보내는 workflow를 추가했고 사용자 QA까지 완료했다. 현재 사용자-facing 흐름에서는 Draft / Review Note / Registry / Board route 확인을 `Candidate Packaging` 하나의 단계로 묶어 본다.
...
| Phase 29 Candidate review workflow | current candidate를 검토 보드로 읽고 compare / Pre-Live Review로 넘기는 surface와, Latest / History 결과를 Candidate Packaging 초안, Candidate Review Note, registry draft로 보내는 workflow를 추가했고 QA까지 완료했다. 현재 운영 가이드에서는 Draft / Review Note / Registry / Board route 확인을 Candidate Packaging 하나의 사용자-facing 단계로 묶어 본다. | `operations/`, `backtest_reports/strategies/`, `phase29/` |
