...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-04-30 / Backtest UI module split
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Pre-Live persistence | Phase 25에서 후보 운영 기록소가 `.note/finance/PRE_LIVE_CANDIDATE_REGISTRY.jsonl`로 분리되었고, 현재 UI에서는 `Backtest > Candidate Review`의 운영 상태 저장 구간에서 current candidate를 Pre-Live 운영 기록으로 저장한다. | `phases/phase25/`, `operations/PRE_LIVE_CANDIDATE_REGISTRY_GUIDE.md` |
| Portfolio proposal persistence / monitoring | Phase 30에서 `.note/finance/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 저장소와 `Backtest > Portfolio Proposal` UI가 추가됐다. 단일 current candidate는 proposal 저장을 반복하지 않고 Live Readiness 직행 가능 여부를 확인하며, 여러 후보를 묶을 때만 목적 / 역할 / 비중이 있는 proposal draft로 저장한다. saved proposal의 Monitoring / Pre-Live Feedback / Paper Tracking Feedback은 보조 도구에서 읽는다. 아직 optimizer, correlation / turnover / capacity 평가, live readiness approval은 아니다. | `phases/phase30/`, `operations/PORTFOLIO_PROPOSAL_REGISTRY_GUIDE.md` |
...
| Web UI | `app/web/overview_dashboard.py`, `app/web/overview_dashboard_helpers.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_library.py`, `app/web/backtest_candidate_library_helpers.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py` | Overview 후보 / funnel / next action dashboard, Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal 주 흐름과 Operations Backtest Run History / Candidate Library / Saved Weighted Portfolio 보조 재현 화면, 공용 result/status/route UI component |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft까지 확장됐다.
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
