...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 32 Robustness / Stress Validation Preview first work unit
...
| Phase 32 | Robustness / Stress validation preview | Phase 31 Validation Pack 아래에 robustness / stress 입력 preview를 추가해 period / contract / benchmark / 성과 snapshot / compare evidence가 stress 검증 실행에 충분한지 먼저 읽게 만들었으며, 아직 실제 stress sweep engine이나 최종 선정은 아님 |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal 안에서는 Phase 31 Validation Pack이 단일 후보와 proposal draft를 같은 위험 검증 언어로 읽고, Phase 32 preview가 stress 검증 실행 전 입력 gap과 suggested sweep을 보여준다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Portfolio Risk / Live Readiness Validation Pack, Robustness / Stress Validation Preview까지 확장됐다.
...
13. Phase 32는 active / not_ready_for_qa 상태다. 첫 작업으로 Validation Pack 아래에 `Robustness / Stress Validation Preview`를 추가해 stress 실행 전 입력 gap과 suggested sweep을 보여준다. 아직 실제 기간 분할 backtest, benchmark sensitivity, parameter sweep engine, 최종 투자 선정은 아니다.
...
| Phase 32 Robustness / Stress Validation Preview | 좋은 결과가 특정 기간 / benchmark / parameter 조건에만 의존한 것인지 확인하기 위한 단계다. 현재 첫 작업으로 Validation Pack 아래에 robustness input preview를 추가해 stress 실행 전 입력 gap과 suggested sweep을 보여준다. | `phases/phase32/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Portfolio Risk Validation Pack / Robustness Preview, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
