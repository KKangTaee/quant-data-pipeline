...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-04-28 / Phase 30 product-flow reorientation kickoff
...
| Web Runtime / UI | Streamlit에서 single / compare / candidate review / history / saved replay / Pre-Live Review를 실행한다 | `app/web/runtime/*`, `app/web/pages/backtest.py` |
| Review / Pre-Live | 결과를 진단하고 candidate note / registry / paper / watchlist / hold / re-review로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 문서 |
...
| Phase 30 | Product-flow / proposal contract / helper split | Portfolio Proposal 구현 전에 Phase 29 이후 사용 흐름, `backtest.py` 점진 리팩토링 경계, proposal row 계약, registry I/O helper 경계를 active하게 정리 중 |
...
| Backtest UI | `app/web/pages/backtest.py` | Single / Compare / Candidate Review / History / Saved Portfolio / Pre-Live Review 화면 |
...
| Review / Pre-Live layer | Real-Money 진단 이후 candidate note / registry / paper / watchlist / hold / re-review를 기록하는 운영 준비 계층 | `phase25/`, `phase29/`, `phase30/` |
...
| Web UI | `app/web/pages/backtest.py` | Single / Compare / Candidate Review / History / Saved Portfolio / Pre-Live Review 화면 |
...
3. 현재 제품 경로는 DB-backed runtime, Backtest UI, compare, Candidate Review, history, saved replay, Pre-Live Review까지 확장됐다.
...
11. Phase 30은 active 상태지만 아직 Portfolio Proposal UI / persistence 구현 단계가 아니다. 먼저 Phase 29 이후의 사용 흐름, `backtest.py` 리팩토링 경계, Portfolio Proposal row 계약을 정리했고 registry JSONL I/O helper를 runtime module로 첫 분리했다.
...
| Phase 30 Portfolio proposal / pre-live monitoring | 최종 목표인 포트폴리오 구성 제안으로 가기 위해 후보 묶음과 paper/pre-live tracking surface가 필요하다. 현재는 기능 구현 전 Phase 29 이후의 사용 흐름, `backtest.py` 리팩토링 경계, proposal row 계약, registry I/O helper 경계를 먼저 active하게 정리 중이다. | `phase30/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare/history/saved replay/Pre-Live Review, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
