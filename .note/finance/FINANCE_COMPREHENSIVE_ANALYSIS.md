...
- 최초 기준 시점: 2026-03-11
- 최근 동기화 기준: 2026-05-03 / Phase 34 Final Portfolio Selection Decision Pack implementation handoff
...
| Web Runtime / UI | Streamlit에서 Overview dashboard, single / compare / candidate review / Pre-Live 운영 기록 / Operations history / Candidate Library / saved replay / Portfolio Proposal을 실행한다 | `app/web/runtime/*`, `app/web/overview_dashboard*.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history*.py`, `app/web/backtest_candidate_library*.py`, `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py` |
| Review / Proposal / Pre-Live | 결과를 진단하고 candidate note / registry / proposal draft / paper / watchlist / hold / re-review / validation pack / robustness preview / paper tracking ledger / final selection decision으로 관리한다 | Real-Money surface, Phase 25 / Phase 29 / Phase 30 / Phase 31 / Phase 32 / Phase 33 / Phase 34 문서 |
...
| Phase 34 | Final Portfolio Selection Decision Pack | Phase 33 paper ledger를 읽어 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 decision pack, append-only final decision 저장소, saved decision review, Phase35 handoff를 구현했고 manual QA 대기 중 |
...
| Backtest UI | `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_strategy.py`, `app/web/backtest_single_forms.py`, `app/web/backtest_single_runner.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_library.py`, `app/web/backtest_candidate_library_helpers.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py` | Backtest page shell, Single Strategy form / runner, Compare & Portfolio Builder / saved replay, Candidate Review / Pre-Live 운영 기록, Portfolio Proposal 주 흐름과 Phase 31 Validation Pack / Phase 32 Robustness Preview / Phase 33 Paper Ledger / Phase 34 Final Decision, Operations Backtest Run History, Candidate Library 후보 replay, 공용 result/status/route UI component |
...
| Backtest UI surface | Single Strategy, Compare, Candidate Review, Portfolio Proposal은 주 후보 검토 흐름으로 이어지고, Pre-Live 운영 기록은 Candidate Review 안에서 저장한다. Portfolio Proposal 안에서는 Phase 31 Validation Pack이 단일 후보와 proposal draft를 같은 위험 검증 언어로 읽고, Phase 32 pack이 stress 검증 입력 gap, stress / sensitivity summary, Phase33 paper ledger handoff를 보여준다. Phase 33 ledger를 저장한 뒤에는 Phase 34 Final Selection Decision Pack에서 선정 / 보류 / 거절 / 재검토 판단과 Phase35 handoff를 남긴다. Backtest Run History와 Candidate Library는 Operations의 보조 재현 / 감사 도구로 열린다. Saved Weighted Portfolio replay는 Compare 영역의 보조 재현 도구로 남아 있다. |
...
| Portfolio proposal persistence / monitoring / validation | Phase 30에서 `.note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` 저장소와 `Backtest > Portfolio Proposal` UI가 추가됐다. 단일 current candidate는 proposal 저장을 반복하지 않고 Live Readiness 직행 가능 여부를 확인하며, 여러 후보를 묶을 때만 목적 / 역할 / 비중이 있는 proposal draft로 저장한다. saved proposal의 Monitoring / Pre-Live Feedback / Paper Tracking Feedback은 다중 후보 작성 흐름의 `4. 저장된 Portfolio Proposal 확인`에서 읽는다. Phase 31은 같은 화면에 read-only Validation Pack을 추가해 단일 후보 / 작성 중 proposal / 저장 proposal을 component risk, overlap, concentration, blocker, 다음 단계 안내 관점으로 읽는다. Phase 32는 이 Validation Pack 아래에 robustness preview, stress / sensitivity summary, Phase33 handoff를 붙여 period / contract / benchmark / CAGR / MDD / compare evidence gap과 paper ledger 준비 상태를 보여준다. Phase 33은 `.note/finance/registries/PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl` 저장소와 paper ledger draft / save / review surface를 추가해 시작일, target weights, benchmark, review cadence, trigger, Phase34 handoff를 append-only로 남긴다. Phase 34는 `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` 저장소와 final decision draft / save / review surface를 추가해 paper ledger 기반 최종 선정 / 보류 / 거절 / 재검토 판단과 Phase35 handoff를 append-only로 남긴다. 아직 optimizer, correlation / turnover / capacity 정량 엔진, stress sweep 실행 엔진, paper PnL 시계열 자동 계산, live readiness approval / 주문 지시는 아니다. | `phases/phase30/`, `phases/phase31/`, `phases/phase32/`, `phases/phase33/`, `phases/phase34/`, `operations/PORTFOLIO_PROPOSAL_REGISTRY_GUIDE.md`, `operations/PAPER_PORTFOLIO_TRACKING_LEDGER_GUIDE.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
...
| Backtest UI, compare, Operations history, saved replay 흐름을 어떻게 볼지 | `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
...
| Web UI | `app/web/overview_dashboard.py`, `app/web/overview_dashboard_helpers.py`, `app/web/pages/backtest.py`, `app/web/backtest_common.py`, `app/web/backtest_single_*.py`, `app/web/backtest_compare.py`, `app/web/backtest_result_display.py`, `app/web/backtest_history.py`, `app/web/backtest_candidate_library.py`, `app/web/backtest_candidate_library_helpers.py`, `app/web/backtest_ui_components.py`, `app/web/backtest_candidate_review.py`, `app/web/backtest_candidate_review_helpers.py`, `app/web/backtest_portfolio_proposal.py`, `app/web/backtest_portfolio_proposal_helpers.py` | Overview 후보 / funnel / next action dashboard, Single / Compare / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Validation Pack / Robustness Preview / Paper Ledger / Final Decision 주 흐름과 Operations Backtest Run History / Candidate Library / Saved Weighted Portfolio 보조 재현 화면, 공용 result/status/route UI component |
...
3. 현재 제품 경로는 DB-backed runtime, Overview 후보 대시보드, Backtest UI, compare, Candidate Review, Operations Backtest Run History, saved replay, Pre-Live 운영 기록, Portfolio Proposal draft, Portfolio Risk / Live Readiness Validation Pack, Robustness / Stress Validation Pack, Paper Portfolio Tracking Ledger 저장 / review, Final Portfolio Selection Decision 저장 / review 단계까지 확장됐다.
...
12. Phase 31은 complete / manual_qa_completed 상태다. 새 승인 registry를 만들지 않고 `Backtest > Portfolio Proposal`에서 단일 후보, 작성 중 proposal, 저장 proposal을 Validation Pack으로 읽어 component risk / overlap / concentration / blocker / 다음 단계 안내를 확인한다. 아직 정량 optimizer, correlation / turnover / capacity 엔진, 최종 승인 기록은 아니다.
13. Phase 32는 complete / manual_qa_completed 상태다. Validation Pack 아래에 `Robustness / Stress Validation Preview`, `Stress / Sensitivity Summary`, `Phase 33 Handoff`를 추가해 stress 실행 전 입력 gap, stress row 계약, paper ledger 준비 상태를 보여주고 사용자 QA까지 완료했다. 아직 실제 기간 분할 backtest, benchmark sensitivity, parameter sweep engine, paper ledger 저장, 최종 투자 선정은 아니다.
...
15. Phase 34는 implementation_complete / manual_qa_pending 상태다. 저장된 paper ledger record를 읽어 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 `Final Portfolio Selection Decision Pack`을 구현했고, `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 append-only로 저장한 뒤 Phase35 handoff를 확인한다. 이 decision도 live approval이나 주문 지시가 아니다.
16. 자세한 코드 흐름은 `code_analysis/`, DB와 데이터 품질은 `data_architecture/`, 결과 기록은 `backtest_reports/`에서 관리한다.
...
| Phase 27 Data integrity / backtest trust | missing data, stale data, common-date truncation, excluded ticker를 사용자가 놓치지 않게 하는 첫 surface를 구현했고 QA까지 완료했다. | `data_architecture/DATA_QUALITY_AND_PIT_NOTES.md`, `code_analysis/BACKTEST_RUNTIME_FLOW.md`, `phases/phase27/` |
| Phase 28 Strategy family parity | annual / quarterly / 신규 전략이 UI, payload, history, saved replay, Real-Money, Guardrail에서 의도된 수준으로 구분되도록 capability snapshot, history / saved portfolio replay/load parity snapshot, compare / weighted data trust, Real-Money / Guardrail scope surface를 추가했고 사용자 QA까지 완료했다. | `code_analysis/STRATEGY_IMPLEMENTATION_FLOW.md`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `phases/phase28/` |
...
| Phase 30 Portfolio proposal / pre-live monitoring | 최종 목표인 포트폴리오 구성 제안으로 가기 위해 후보 묶음과 paper/pre-live tracking surface가 필요하다. 현재는 Phase 29 이후의 사용 흐름, `backtest.py` 리팩토링 경계, proposal row 계약, registry I/O helper 경계, Proposal Draft UI / persistence, Monitoring Review, Pre-Live Feedback, Paper Tracking Feedback을 구현하고 manual QA 대기 중이다. | `phases/phase30/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
| Phase 31 Portfolio Risk / Live Readiness Validation | Portfolio Proposal 이후 바로 승인으로 뛰지 않고, 후보 또는 proposal이 다음 robustness 검증으로 넘어갈 만큼 구조적으로 말이 되는지 먼저 본다. `Backtest > Portfolio Proposal` 안에 read-only Validation Pack을 추가해 단일 후보 / 작성 중 proposal / 저장 proposal의 component risk, blocker, paper tracking gap, overlap first pass, 다음 단계 안내를 확인하게 만들었고 manual QA까지 완료했다. | `phases/phase31/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
| Phase 32 Robustness / Stress Validation Pack | 좋은 결과가 특정 기간 / benchmark / parameter 조건에만 의존한 것인지 확인하기 위한 단계다. Validation Pack 아래에 robustness input preview, stress / sensitivity summary, Phase33 paper ledger handoff를 추가했고 사용자 QA까지 완료했다. | `phases/phase32/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md` |
| Phase 33 Paper Portfolio Tracking Ledger | Phase 32에서 준비 가능성이 확인된 후보나 proposal을 실제 돈 없이 관찰할 append-only ledger로 저장하고 다시 읽는다. 현재는 complete / manual_qa_completed 상태이며, Phase34 final selection decision pack의 입력 source가 되었다. | `phases/phase33/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `operations/PAPER_PORTFOLIO_TRACKING_LEDGER_GUIDE.md` |
| Phase 34 Final Portfolio Selection Decision Pack | paper tracking까지 본 후보나 proposal을 최종 실전 후보로 선정 / 보류 / 거절 / 재검토할지 판단하는 phase다. 현재는 implementation_complete / manual_qa_pending 상태이며, final decision 저장소 / evidence pack / UI save / review / Phase35 handoff를 구현했다. | `phases/phase34/`, `code_analysis/WEB_BACKTEST_UI_FLOW.md`, `operations/FINAL_PORTFOLIO_SELECTION_DECISIONS_GUIDE.md` |
...
> NYSE / ETF 데이터 수집, MySQL 기반 loader, DB-backed strategy runtime, Backtest UI compare / Operations history / saved replay / Candidate Review / Pre-Live 운영 기록 / Portfolio Proposal / Portfolio Risk Validation Pack / Robustness-Stress Validation Pack / Paper Portfolio Tracking Ledger / Final Portfolio Selection Decision, strategy report, Pre-Live 운영 준비를 연결하는 퀀트 백테스트 워크벤치
