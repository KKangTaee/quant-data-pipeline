...
- `pending` warning 문구 한글화 / 설명 정리 확대
  - stale / missing / malformed / excluded 의미를 더 일관되게 설명한다.
- `pending` compare / history / saved replay 확장 검토
  - 단일 실행뿐 아니라 비교 / 저장 / 재실행 흐름에서도 Data Trust Summary를 유지할지 확인한다.
- `pending` strict annual / quarterly preflight 표현 정리
...
- `pending` Global Relative Strength manual UI validation
...
Phase 27은 active 상태다.
첫 작업인 `Backtest Data Trust Summary + Global Relative Strength preflight`는 구현/문서 동기화가 완료됐다.
다음 판단은 사용자가 GRS 화면과 최신 실행 결과에서 이 정보가 실제로 이해 가능한지 QA하는 것이다.
