...
1. `Value` strongest raw winner는 rare하게 `real_money_candidate`까지 올라간다.
...
- `PHASE13_QUALITY_STRICT_SPY_DOMINANCE_SEARCH`
  - `capital_discipline` case
  - `validation = caution`, `rolling = normal`, `out_of_sample = normal`
  - 즉 strict annual quality는 최근 split review보다도
...
1. practical GTAA candidate는 `Promotion = production_candidate`, `Deployment = watchlist_only`
   까지 올라간다.
2. ETF family는 strict annual과 달리,
   `validation = watch` 상태에서도 `hold`를 벗어나는 practical case가 이미 있다.
3. 대신 더 공격적인 ETF candidate는
   **`ETF operability` current-data gap** 때문에 practical candidate에서 제외되는 경우가 보였다.
...
- aggressive GTAA near-miss:
  ...
  - report 기준으로는 `ETF operability` policy를 사실상 꺼야 non-hold 유지가 가능했다.
...
2. `Value` balanced near-miss, `Quality` hold case, `GTAA` watchlist candidate를
   canonical calibration case로 고정한다.
