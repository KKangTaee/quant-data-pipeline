...
현재 첫 작업은 기능 구현이 아니라
사용 흐름 재정렬과 `backtest.py` 리팩토링 경계 검토다.
...
- `pending` Portfolio Proposal row 계약 정의
- `pending` Portfolio Proposal 저장소 위치와 append-only 정책 검토
...
다음 작업에서는 실제 모듈 분리 또는 Portfolio Proposal 계약 정의 중 하나를 선택한다.
