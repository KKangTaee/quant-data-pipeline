...
현재는 구현 완료 / manual QA 대기 상태의 진행 요약이다.
사용자 checklist QA가 완료되면 이 문서를 closeout 기준으로 다시 갱신한다.
...
## 이번 phase에서 현재까지 완료된 여섯 번째 작업

### 7. Portfolio Proposal Pre-Live Feedback

- `Backtest > Portfolio Proposal`에 `Pre-Live Feedback` tab을 추가했다.
- proposal 저장 당시의 Pre-Live snapshot과 현재 active Pre-Live record를 비교한다.
- 후보별 saved Pre-Live, current Pre-Live, status drift, review overdue, tracking cadence, current next action을 볼 수 있다.
- feedback gap은 proposal 자동 수정이나 live approval이 아니라, 운영 상태 재확인 신호다.

쉽게 말하면:

- 저장된 포트폴리오 제안이 현재 Pre-Live 관찰 상태와 어긋나지 않는지 확인하는 화면이 생겼다.

## 이번 phase에서 현재까지 완료된 일곱 번째 작업

### 8. Portfolio Proposal Paper Tracking Feedback

- `Backtest > Portfolio Proposal`에 `Paper Tracking Feedback` tab을 추가했다.
- proposal 저장 당시의 evidence snapshot과 현재 active Pre-Live record의 result snapshot을 비교한다.
- 후보별 saved/current CAGR, saved/current MDD, delta, performance signal, tracking cadence, stop condition, success condition을 볼 수 있다.
- feedback gap은 paper tracking 미진입, missing result, CAGR / MDD 악화 같은 재확인 신호다.
- 이 surface는 실제 paper PnL 자동 계산, proposal 자동 수정, live approval이 아니다.

쉽게 말하면:

- 저장된 포트폴리오 제안이 최신 paper tracking 성과 snapshot 기준으로 악화됐는지 다시 보는 화면이 생겼다.

## 아직 남아 있는 것

- Phase 30 manual QA
- Candidate Review / Pre-Live / History / Saved Portfolio의 추가 `backtest.py` 모듈 분리
  - Phase 30 QA와 섞지 않고 별도 special refactor task로 진행한다.
...
여섯 번째 작업으로 Portfolio Proposal Pre-Live Feedback surface를 구현했다.
일곱 번째 작업으로 Portfolio Proposal Paper Tracking Feedback surface를 구현했다.
이제 사용자가 Phase 30 checklist 기준으로 manual QA를 진행한다.
추가 `backtest.py` 모듈 분리는 Phase 30 이후 별도 special refactor task로 분리한다.
