...
"저장된 후보나 proposal을 실제 돈 없이 어떤 조건으로 관찰할 것인가"를 저장하는 legacy / compatibility 장부다.

`FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`은
"검증과 paper observation 기준을 본 뒤 최종 실전 후보로 선정할지, 보류할지, 거절할지, 재검토할지"를 저장한다.
...
| Paper portfolio tracking ledger | `.note/finance/registries/PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl` | legacy / compatibility paper tracking 시작 조건과 Phase34 handoff 저장 |
| Final selection decision registry | `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` | Final Review에서 최종 선정 / 보류 / 거절 / 재검토 판단 저장 |
...
| Final Review UI | `Backtest > Final Review` | 단일 후보 또는 saved proposal의 validation / paper observation / 최종 판단 기록 |
...
- `Final Review로 이동`
  - 상세 validation, robustness / stress 질문, paper observation 기준, 최종 판단은 `Backtest > Final Review`에서 진행한다.
  - Final Review의 사용자-facing 저장 액션은 `최종 검토 결과 기록` 하나다.
...
- 현재 main flow에서는 `Save Paper Tracking Ledger`를 Portfolio Proposal 탭의 필수 단계로 요구하지 않는다.
- Final Review record는 paper observation 기준과 최종 후보 판단을 함께 남기며, live approval이나 주문 지시가 아니다.
...
- 실제 paper PnL 시계열 자동 계산은 아직 없다. `Paper Tracking Feedback`은 Pre-Live record에 저장된 `result_snapshot` 기준 비교를 함께 보여주고, Final Review는 paper observation 기준을 최종 검토 기록에 포함한다.
- 현재 active workflow는 `Portfolio Proposal -> Final Review -> 최종 판단 완료`에서 끝난다. 별도 Post-Selection Guide panel은 제거했다.
