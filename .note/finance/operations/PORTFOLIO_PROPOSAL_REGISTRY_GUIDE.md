...
어떤 용도로 쓰고, Current Candidate Registry / Pre-Live Candidate Registry / Paper Portfolio Tracking Ledger와
어떻게 구분하는지 설명한다.
...
`PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl`은
"저장된 후보나 proposal을 실제 돈 없이 어떤 조건으로 관찰할 것인가"를 저장한다.
...
| Paper portfolio tracking ledger | `.note/finance/registries/PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl` | 후보 / proposal의 paper tracking 시작 조건과 Phase34 handoff 저장 |
| Final selection decision registry | `.note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` | paper ledger를 읽은 최종 선정 / 보류 / 거절 / 재검토 판단 저장 |
| Runtime helper | `app/web/runtime/portfolio_proposal.py` | proposal JSONL append / load helper |
| Paper ledger runtime helper | `app/web/runtime/paper_portfolio_ledger.py` | paper ledger JSONL append / load helper |
| Final decision runtime helper | `app/web/runtime/final_selection_decisions.py` | final decision JSONL append / load helper |
...
- `Validation Pack / Paper Tracking Ledger`
  - Phase 31 / 32 validation pack 아래에서 Phase 33 paper ledger draft를 확인한다.
  - 단일 후보 direct path나 저장된 proposal detail은 저장된 source가 있으므로 `Save Paper Tracking Ledger`를 사용할 수 있다.
  - 작성 중 proposal은 source가 아직 durable registry에 없으므로 proposal draft를 먼저 저장해야 paper ledger 저장이 열린다.
- `저장된 Portfolio Proposal 확인`
...
- `저장된 Paper Tracking Ledger 확인`
  - 저장된 ledger 목록과 component, review trigger, Phase34 handoff를 확인한다.
  - 이 확인 영역은 proposal이나 Pre-Live record를 자동 수정하지 않는다.
- `Final Selection Decision Pack`
  - 저장된 paper ledger detail 아래에서 최종 선정 / 보류 / 거절 / 재검토 판단을 명시적으로 저장한다.
- `저장된 Final Selection Decision 확인`
  - 저장된 decision과 source paper ledger, selected components, Phase35 handoff를 다시 읽는다.
...
- `Save Paper Tracking Ledger`는 live approval이 아니라 paper tracking 시작 조건을 append-only ledger에 남기는 버튼이다.
- `Save Final Selection Decision`은 최종 후보 판단을 남기는 버튼이지 live approval이나 주문 지시가 아니다.
...
- 실제 paper PnL 시계열 자동 계산은 아직 없다. 현재 ledger는 시작 조건과 review rule을 저장하고, `Paper Tracking Feedback`은 Pre-Live record에 저장된 `result_snapshot` 기준 비교를 함께 보여준다.
- Phase 35 post-selection operating guide는 아직 구현되지 않았다.
