...
- `Create Proposal Draft`
  - current candidate 여러 개를 선택한다.
  - proposal 목적, type, status, capital scope를 입력한다.
  - 후보별 proposal role, target weight, weight reason을 입력한다.
  - target weight 합계와 blocker를 확인한다.
  - `Save Portfolio Proposal Draft`로 명시 저장한다.
- `Proposal Registry`
  - 저장된 proposal draft 목록을 확인한다.
  - 선택한 proposal row의 JSON 내용을 inspect한다.
- `Monitoring Review`
  - 저장된 proposal draft를 monitoring summary table로 확인한다.
  - proposal을 선택해 component table, blocker, review gap, operator decision을 다시 본다.
  - `Monitoring State`는 review 요약이지 live approval이 아니다.
- `Pre-Live Feedback`
  - proposal 저장 당시의 Pre-Live snapshot과 현재 active Pre-Live record를 비교한다.
  - Status drift, overdue review, tracking cadence, current next action을 확인한다.
  - 이 tab은 읽기 전용이며 proposal이나 Pre-Live record를 자동 수정하지 않는다.
- `Paper Tracking Feedback`
  - proposal 저장 당시 evidence snapshot의 CAGR / MDD와 현재 active Pre-Live result snapshot의 CAGR / MDD를 비교한다.
  - CAGR / MDD delta, performance signal, tracking cadence, stop condition, success condition을 확인한다.
  - 이 tab은 실제 paper PnL 자동 계산이나 live approval이 아니라 최신 Pre-Live snapshot을 읽는 보조 surface다.
...
- `Monitoring Review`에서 `review_ready`로 보여도 실전 승인이나 주문 가능 상태라는 뜻은 아니다.
- `Pre-Live Feedback`에서 gap이 보여도 자동으로 proposal이 reject되거나 hold되는 것은 아니다.
