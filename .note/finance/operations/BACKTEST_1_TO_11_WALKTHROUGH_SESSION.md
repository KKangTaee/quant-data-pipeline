...
| 7단계 | `Portfolio Proposal` | 단일 후보는 추가 저장 없이 Live Readiness 직행 가능 여부를 확인하고, 여러 후보는 목적, 역할, 비중, capital scope가 있는 포트폴리오 초안으로 묶음 | 단일 후보는 `LIVE_READINESS_DIRECT_READY`, 다중 후보 proposal은 `LIVE_READINESS_CANDIDATE_READY`면 향후 Live Readiness 단계가 읽을 수 있는 형태 |
