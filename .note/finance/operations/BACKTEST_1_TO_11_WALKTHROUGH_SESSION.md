...
| 7단계 | `Review Notes에서 Registry 후보 여부 결정` | 저장된 Review Note를 보고 실제 후보 registry row로 남길지, 참고 기록으로만 둘지 결정 | 후보로 남길 가치가 있으면 row preview를 확인하고 8단계로 이동 |
...
- 저장 뒤 7단계 `Review Notes`에서 registry 후보로 남길지 결정한다.
