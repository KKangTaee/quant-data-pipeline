...
| 7단계 | `Review Notes에서 Registry 후보 범위 결정` | 저장된 Review Note를 보고 `Current Candidate / Near Miss / Scenario / Stop` 중 어디까지 남길지 결정 | `7단계 Registry 후보 범위 판단`이 Current / Near Miss / Scenario 중 하나이고, Record Type이 그 범위와 맞음 |
| 8단계 | `Current Candidate Registry append` | Review Note에서 만든 registry row preview를 보고 명시적으로 append | `Append To Current Candidate Registry` 클릭 |
...
이미 저장한 Review Note를 어떤 registry 범위로 남길지 정하는 단계다.
...
여기서 추천된 Record Type과 실제 선택한 Record Type이 맞아야 8단계 append가 가능하다.
...
| 위치 | 추가 내용 | 목적 |
|---|---|---|
| `Reference > Guides` | `핵심 개념 가이드`, `1~11 단계 실행 흐름`, `단계 통과 기준`, `문서와 파일` 묶음으로 재정리 | 실습 중 필요한 설명과 stop/go 기준을 한 화면에서 더 쉽게 찾게 함 |
| `Reference > Guides > GTAA Risk-Off 후보군 보는 법` | defensive ticker와 GTAA universe의 교집합 설명 | fallback 후보를 잘못 읽지 않게 함 |
| `Reference > Guides > 단계 통과 기준` | 4->5, 5->6, 6->7로 넘어가는 최소 기준 | 단계형 흐름과 stop/go 기준을 분리 |
...
| `Backtest > Candidate Review > Review Notes` | 7단계 Registry 후보 범위 판단 box와 record type gate | Review Note를 Current / Near Miss / Scenario / Stop 중 어디까지 남길지 먼저 판단 |
...
- 범위 판단과 Record Type이 맞으면 8단계 `Append To Current Candidate Registry`로 넘어간다.
