...
기술적으로는 가능하다.
`Latest Backtest Run`이나 `History`에서 single run을 바로 `Candidate Draft`로 보낼 수 있기 때문이다.
...
따라서 5단계는 "Candidate Draft 생성의 기술적 필수 조건"은 아니지만,
...
Candidate Draft로 넘길 때 Review Note에 남긴다.
...
사용자 질문에 따라 6단계와 7단계의 역할을 아래처럼 다시 정리한다.

기존처럼 `6단계 = Candidate Intake Draft 확인`, `7단계 = Save Candidate Review Note`로 나누면
같은 화면에서 이미 확인한 내용을 두 번 통과하는 것처럼 느껴진다.
그래서 사용자-facing 흐름은 다음처럼 본다.

| 단계 | 이름 | 하는 일 | 통과 기준 |
|---|---|---|---|
| 6단계 | `Candidate Intake & Review Note 저장` | Draft가 제대로 넘어왔는지 확인하고, 운영자 판단 / 이유 / 다음 행동을 Review Note로 저장 | `6단계 Intake 저장 준비`가 `READY_TO_SAVE`이고 `Save Candidate Review Note`를 클릭 |
| 7단계 | `Registry 후보 범위 결정 및 저장` | 저장된 Review Note를 보고 `Current Candidate / Near Miss / Scenario / Stop` 중 어디까지 남길지 결정하고, 통과한 row만 registry에 append | `7단계 Registry 후보 범위 판단`이 Current / Near Miss / Scenario 중 하나이고, Record Type이 그 범위와 맞으면 `Append To Current Candidate Registry` 클릭 |
| 8단계 | `Candidate Board 운영 확인` | registry에 저장된 후보를 다시 읽고 `Pre-Live`, `Compare 재검토`, `Board 보류` 중 다음 경로를 정함 | `8단계 Candidate Board 운영 판단`의 Route가 `PRE_LIVE_READY`면 9단계로, `COMPARE_REVIEW_READY`면 Compare로 진행 |

6단계에서 `Save Candidate Review Note` 버튼이 활성화되려면 아래 조건을 채워야 한다.

- 후보 이름 또는 strategy key와 source가 확인된다.
- CAGR, MDD, End Balance 같은 result snapshot이 있다.
- Data Trust snapshot이 있고, `error`가 아니라 `OK` 또는 설명 가능한 `WARNING`이다.
- Real-Money signal이 비어 있지 않다.
- universe / ticker / cadence를 재현할 수 있는 settings snapshot이 있다.
- `Operator Reason`과 `Next Action`이 작성되어 있다.

이 기준은 전략의 품질 점수가 아니라 저장 준비도다.
6단계에서 저장한 Review Note는 current candidate registry가 아니며,
7단계에서 다시 읽고 registry row로 남길지 결정한다.

### 7단계에서 범위를 정하는 법

7단계는 좋은 후보인지 다시 백테스트하는 단계가 아니라,
이미 저장한 Review Note를 어떤 registry 범위로 남길지 정하고 저장까지 마무리하는 단계다.

| 범위 | 의미 | 7단계 저장 조건 |
|---|---|---|
| `Current Candidate` | 현재 기준 후보로 남길 수 있는 주 후보 | `consider_registry_candidate`, Compare 근거, Data Trust 해석 가능, Real-Money gate가 `hold/blocked`가 아님, 설정 snapshot 있음 |
| `Near Miss` | 지금 주 후보는 아니지만 다시 볼 대안 | `keep_as_near_miss_review` 또는 `needs_more_evidence`, result / data trust / settings / next action이 남아 있음 |
| `Scenario` | 실전 후보보다 설정 비교용 후보 | `keep_as_scenario_review`, 설정 snapshot과 비교용 이유가 남아 있음 |
| `Stop` | registry append 보류 | reject, Data Trust error, result/settings 부족, operator reason 부족, current 후보인데 Compare 근거가 없음 |

UI에서는 `Backtest > Candidate Review > Review Notes`에서 저장한 Note를 고른 뒤
`7단계 Registry 후보 범위 판단`을 먼저 본다.
여기서 추천된 Record Type과 실제 선택한 Record Type이 맞아야 같은 단계 안에서 append가 가능하다.
예를 들어 Current Candidate 범위가 아니면 `current_candidate`로 append하지 않고,
Near Miss 또는 Scenario로 남기거나 7단계에서 멈춘다.
...
Candidate Board는 같은 `registry_id`의 latest row만 보여주므로 화면 변화가 작게 보일 수 있다.
의도적으로 같은 Review Note를 새 revision으로 남겨야 할 때만
`같은 Review Note를 새 registry revision으로 다시 저장` 체크박스를 켠다.

### 8단계에서 다음 경로를 정하는 법

8단계는 다시 백테스트를 하는 단계가 아니다.
7단계에서 registry에 저장한 후보가 운영 화면에서 읽힐 만큼 충분한지,
그리고 다음에 어느 화면으로 가야 하는지를 정하는 단계다.

| Route | 의미 | 다음 행동 |
|---|---|---|
| `PRE_LIVE_READY` | `current_candidate`이고 Real-Money gate가 `hold/blocked`가 아니며, result / contract / review context가 남아 있음 | 9단계 `Pre-Live Review`에서 paper tracking, watchlist, hold 같은 운영 상태를 저장 |
...
| `BOARD_HOLD` | 후보 row의 역할, 성과 snapshot, 설정 snapshot, Real-Money signal, 판단 메모 중 일부가 부족함 | registry row 또는 Review Note를 보강하고 8단계에서 다시 확인 |

따라서 8단계를 통과했다는 말은 항상 9단계로 간다는 뜻이 아니다.
`PRE_LIVE_READY`만 9단계로 넘어가고,
`COMPARE_REVIEW_READY`는 정상적인 통과이지만 Compare 재검토 경로로 돌아간다.
...
| `Reference > Guides > 단계 통과 기준` | 4->5, 5->6, 6->7 기준과 7단계 registry 저장 기준 | 단계형 흐름과 stop/go 기준을 분리 |
| `Backtest 결과 > Real-Money > 현재 판단 > 5단계 Compare 진입 평가` | 10점 만점 readiness box | Checklist 상세를 보기 전에 Compare 진입 가능성을 빠르게 확인 |
| `Backtest > Compare & Portfolio Builder > Strategy Comparison` | 6단계 Candidate Draft 진입 평가 | Compare 이후 Candidate Draft로 넘길 수 있는지 점수로 확인 |
| `Backtest > Candidate Review > Candidate Intake Draft` | 6단계 Intake 저장 준비 box와 disabled save button | Draft가 저장 가능한 상태일 때만 Review Note를 저장하도록 함 |
| `Backtest > Candidate Review > Review Notes` | 7단계 Registry 후보 범위 판단 box와 record type gate | Review Note를 Current / Near Miss / Scenario / Stop 중 어디까지 남길지 정하고 통과 시 같은 단계에서 append |
| `Backtest > Candidate Review > Review Notes` | 같은 Review Note의 중복 registry append 기본 차단 | Candidate Board에는 latest row만 보여 반복 클릭이 무의미하게 보이는 문제를 방지 |
| `Backtest > Candidate Review > Candidate Board` | 8단계 Candidate Board 운영 판단 box | 저장된 후보가 Pre-Live로 갈지, Compare로 돌아갈지, Board에 보류될지 확인 |

## 다음 실습 시작점

현재 후보가 5단계 Compare를 통과했다면 다음 실습은 6단계에서 시작한다.

확인할 것:

- `Send Selected Strategy To Candidate Draft`를 누른 뒤 `Candidate Review > Candidate Intake Draft`로 이동한다.
...
- `6단계 Intake 저장 준비`가 `READY_TO_SAVE`인지 확인한다.
- `Save Candidate Review Note`를 눌러 Review Note를 저장한다.
- 저장 뒤 7단계 `Review Notes`에서 Current / Near Miss / Scenario / Stop 범위를 확인한다.
- 범위 판단과 Record Type이 맞으면 같은 7단계 안에서 `Append To Current Candidate Registry`를 누른다.
- 8단계 `Candidate Board`에서 방금 저장한 후보의 Route를 확인한다.
- Route가 `PRE_LIVE_READY`면 `Open Selected Candidate In Pre-Live Review`로 9단계를 연다.
