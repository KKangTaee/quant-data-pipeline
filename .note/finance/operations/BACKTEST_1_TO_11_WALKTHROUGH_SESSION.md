...
`1단계 데이터 최신화`부터 `Portfolio Proposal -> Final Review -> 최종 판단 완료`까지
...
사용자 질문에 따라 기존 6 / 7 / 8단계로 쪼개져 보이던 Candidate Review 구간은 하나의 사용자-facing 단계로 다시 묶는다.

이 구간은 전략을 다시 검증하는 단계가 아니다.
4단계와 5단계에서 확인한 후보를 Portfolio Proposal이 읽을 수 있는 형태로 포장하는 단계다.
즉, 좋은 백테스트 결과를 `Review Note`, `Current Candidate Registry row`, `Pre-Live 운영 기록`, `Portfolio Proposal route`까지 이어지는 하나의 후보 패키지로 만드는 과정이다.

| 단계 | 이름 | 하는 일 | 통과 기준 |
|---|---|---|---|
| 6단계 | `Candidate Packaging` | Draft 확인, Review Note 저장, registry 저장, Pre-Live 운영 기록 저장, Portfolio Proposal 이동 판단을 한 화면에서 순서대로 처리 | `운영 기록 저장 및 다음 단계 판단` Route가 `PORTFOLIO_PROPOSAL_READY`면 7단계 Portfolio Proposal로 진행 |
| 7단계 | `Portfolio Proposal` | 단일 후보는 추가 저장 없이 Final Review 입력 후보로 읽고, 여러 후보는 목적, 역할, 비중, capital scope가 있는 포트폴리오 초안으로 묶음 | 단일 후보는 `LIVE_READINESS_DIRECT_READY`, 다중 후보 proposal은 `LIVE_READINESS_CANDIDATE_READY`면 현재 흐름에서 Final Review가 읽을 수 있는 형태 |
| 8단계 | `Final Review 검증` | Validation, Robustness, Stress / Sensitivity, Paper Observation 기준을 한 화면에서 확인 | hard blocker가 없고 최종 판단을 기록할 근거가 설명 가능하면 9단계로 진행 |
| 9단계 | `최종 판단 및 테스트 검증 기록` | 선정 / 보류 / 거절 / 재검토 판단과 이유를 `최종 검토 결과 기록`으로 저장 | `FINAL_REVIEW_RECORD_READY`이고 이유 / 제약 / 다음 행동이 남아 있으면 10단계로 진행 |
| 10단계 | `기록된 최종 검토 결과 확인` | 저장된 final decision을 다시 열어 실전 후보 선정 여부를 확인 | `SELECT_FOR_PRACTICAL_PORTFOLIO`이면 실전 후보 선정으로 읽고, live approval / order와는 분리 |
...
`Append To Current Candidate Registry`를 누른 직후에는
`3. Pre-Live 진입 평가`의 `Packaging 확인 후보`가 방금 저장한 registry row로 자동 선택된다.
후보 선택 label은 `Strategy Family | Role | Title | id=<registry_id>` 형식으로 표시되므로,
같은 GTAA 후보가 여러 개 있어도 `registry_id`와 Title로 구분한다.
선택 영역 바로 아래의 `방금 저장한 후보` 카드에서 Registry ID, Revision ID, Source Review Note,
Recorded At을 확인한 뒤 route를 읽는다.
...
| `Reference > Guides > 단계 통과 기준` | 4->5, 5->6, 6->7, 7->8, 8->9, 9->10, 최종 판단 해석 기준 | 단계형 흐름과 stop/go 기준을 분리 |
...
- `3. Pre-Live 진입 평가`에서 자동 선택된 방금 저장한 후보의 Registry ID / Revision ID를 확인하고 Route를 확인한다.
