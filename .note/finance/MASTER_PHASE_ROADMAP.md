...
- 따라서 현재 운영 기준은:
  ...
  - `Phase 13`: deployment-readiness / probation active phase
- 이번 phase에서는
...
- 현재 active phase는
  ...
  - 승격된 전략 후보를 실제 운용 후보 shortlist로 다시 묶는
    `Phase 13 deployment-readiness / probation`
    방향이다

즉 지금은
“실전형 validation contract, workflow first pass, strategy promotion first pass는 확보했고,
이제 shortlist / probation / monitoring 계약으로 실제 운용 후보를 더 엄격하게 검증하는 단계”
라고 보는 것이 가장 정확하다.
