...
- `Value` current practical anchor `Top N = 14 + psr` bounded follow-up
- `Quality + Value` current strongest practical anchor follow-up
...
- `Value`에서는 lower-MDD near-miss로 `+ pfcr`가 있었지만
  `production_candidate / watchlist`로 gate가 내려갔다
...
- `Quality + Value`에서도 lower-MDD 대안은 있었지만
  practical gate를 그대로 유지한 채 `MDD`만 더 낮추는 exact hit는 아직 없었다
