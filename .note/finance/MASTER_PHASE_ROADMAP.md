...
- first implementation slice로
  `strict annual partial cash retention` contract가 실제 코드에 연결됐다
- 다음 active question은
  이 contract가 strongest/current anchor에서 same-gate lower-MDD rescue를 만드는지 representative rerun으로 다시 확인하는 것이다
