...
- portfolio 후보를 후보라고 부를 기준과 최소 기록 항목을 먼저 고정한다

### 쉽게 말하면
- 전략 하나가 아니라
  전략 묶음이 더 좋은 후보가 되는지 보는 단계다
- 단, 좋은 숫자가 나온 조합을 바로 후보라고 부르지 않고,
  source / weight / date alignment / saved replay까지 남기는 재현 가능한 후보인지 먼저 확인한다
...
- `Phase 21`에서 `33 / 33 / 34` portfolio bridge가 재현 가능하다는 점은 확인했지만,
  아직 최종 portfolio winner나 promotion 기준은 정하지 않았기 때문이다
...
- 다음 작업은 portfolio-level benchmark / guardrail interpretation과
  weight alternative 비교 범위를 정하는 것이다
...
- `Phase 22`
  ...
  - weighted portfolio를 재현 가능한 후보로 부를 기준을 먼저 고정한다
- `Phase 23`
  - quarterly / alternate cadence productionization 검토
- `Phase 24`
  - new strategy expansion
- `Phase 25`
  - pre-live operating system / deployment readiness
