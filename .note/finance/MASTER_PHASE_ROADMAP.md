...
- 다음 단계는
  `factor subset / controlled addition downside search`
  그리고 `Quality`, `Quality + Value` family candidate improvement다
...
- 현재는 `Phase 14`를 practical closeout 상태로 두고,
  `Phase 15`에서 candidate quality improvement를 먼저 진행 중이다
- 지금 가장 자연스러운 immediate next step은
  `Value` downside-improvement를 factor subset / addition 쪽으로 한 번 더 밀고,
  이후 `Quality`, `Quality + Value` family에 같은 방법을 적용하는 것이다
