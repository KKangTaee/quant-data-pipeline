...
현재 권고 reading:
- `Phase 18`
  - 아직 fully closed 되지 않은 structural redesign phase이며,
    remaining backlog가 무엇인지 먼저 확인하는 용도
- `Phase 19`
  - 이미 manual validation까지 끝난 contract cleanup / interpretation stabilization phase
- `Phase 20`
  - 이미 manual validation까지 끝난 operator workflow hardening phase
- `Support Track`
  - repo-local automation / registry / plugin / skill은 main phase가 아니라 별도 보조 트랙으로 본다
- `Phase 21`
  - 이제 current annual strict family와 portfolio bridge를 같은 frame에서 다시 검증하는 deep validation phase
- `Phase 22 ~ 25`
  - portfolio-level candidate -> cadence expansion -> new strategy -> pre-live operating system 순서로 넓힌다
