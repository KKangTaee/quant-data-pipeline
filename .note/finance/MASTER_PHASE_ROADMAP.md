...
- current annual strict candidate와 portfolio bridge를 같은 frame에서 다시 본 `Phase 21`은 manual validation까지 완료되었고,
  ...
  `Phase 33`은 complete / manual_qa_completed 상태다.
  ...
  Phase35는 별도 후속 가이드 탭이 현재 단계에서는 과하다는 판단을 반영해,
  Backtest workflow를 `Portfolio Proposal -> Final Review -> 최종 판단 완료`로 단순화했고 사용자 QA가 남아 있다.
...
| Phase 36 | Final Selected Portfolio Monitoring And Rebalance Operations | `implementation_complete` | `manual_qa_pending` | Final Review에서 선정된 포트폴리오와 current weight / value / holding 기반 drift 및 alert preview를 Operations dashboard에서 read-only로 확인하게 했으며 QA를 기다린다 |

### Phase 31. Portfolio Risk And Live Readiness Validation

### 목적
- Candidate Review / Pre-Live / Portfolio Proposal 결과를 읽어 포트폴리오 위험과 Live Readiness 검증 결과를 보여준다.
...
### 목적
- Phase 32에서 paper ledger 준비 가능성이 확인된 후보나 proposal을 실제 돈 없이 관찰할 append-only ledger로 남긴다.
...
### 목적
- Backtest workflow를 `Portfolio Proposal -> Final Review -> 최종 판단 완료`로 단순화한다.

### 왜 필요한가
- 최종 후보 판단은 이미 Final Review가 validation, robustness, paper observation, operator judgment를 묶어서 기록한다.
- 별도 후속 탭을 더 만들면 같은 판단을 다시 확인하는 중복 단계가 된다.
- 사용자는 Final Review에서 투자 가능 / 내용 부족 / 투자하면 안 됨 / 재검토 필요를 바로 읽으면 된다.

### 현재 메모
- Phase 35는 implementation_complete / manual_qa_pending 상태다.
- `.note/finance/phases/phase35/` 아래 phase plan, TODO, 네 개 작업 단위, completion summary, next-phase preparation, checklist를 새 흐름으로 정리했다.
- `Backtest` workflow navigation은 `Final Review`에서 끝난다.
- `Backtest > Final Review`의 saved decision review에서 투자 가능 후보 / 내용 부족 / 투자하면 안 됨 / 재검토 필요를 확인한다.
- 별도 후속 가이드 panel과 별도 후속 registry를 만들지 않는다.
...
### 목적
- Final Review에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 선정된 포트폴리오를 Operations 화면에서 운영 대상으로 확인한다.

### 왜 필요한가
- Phase35에서 Final Review가 마지막 판단 단계로 정리됐으므로, 선정 이후 운영 확인은 Backtest workflow가 아니라 Operations surface로 분리하는 것이 자연스럽다.
- 최종 선정 포트폴리오를 나중에 다시 찾고, target allocation / evidence / next action을 읽는 home이 필요하다.
...
- drift 결과를 운영 경고와 Final Review review trigger 관점으로 읽는 read-only preview를 추가했다.
