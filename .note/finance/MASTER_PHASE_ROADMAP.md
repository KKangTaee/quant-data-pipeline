...
- current annual strict candidate와 portfolio bridge를 같은 frame에서 다시 본 `Phase 21`은 manual validation까지 완료되었고,
  ...
  그리고 초안을 별도 Candidate Review Note로 저장하는 흐름을 구현한 상태다.
...
| Phase 29 | Candidate Review And Recommendation Workflow | `active` | `manual_qa_pending` | current candidate를 검토 보드로 읽고, Latest / History 결과를 후보 검토 초안과 review note로 넘기는 workflow를 구현했다 |
...
- 다음 작업 후보는 review note 중 어떤 것을 실제 current candidate registry row로 남길지 기준을 정리하는 것이다.
