...
이 프로젝트는 투자 결론을 먼저 내리는 프로젝트가 아니라,
**사용자가 전략을 만들고 백테스트하고 검증할 수 있는 퀀트 프로그램을 개발하는 프로젝트**다.
...
- phase 문서에서 별도로 명시하지 않는 한,
  백테스트 결과는 투자 추천이나 실전 운용 승인으로 해석하지 않는다.
...
| Paper / Pre-Live Layer | paper run, watchlist, small-capital trial 같은 운영 루틴을 준비한다 | Phase 25 이후 본격화 |
