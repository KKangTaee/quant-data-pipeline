...
### 2026-05-06
- Compare 단계 표현 정리:
  - user report에 따라 `5단계 Compare 결과` 안에 `6단계 Candidate Packaging 진입 평가`가 같이 노출되어 단계가 꼬여 보이는 문제를 확인했다.
  - 실제 기능은 6단계 실행이 아니라 5단계 Compare 결과를 바탕으로 다음 Candidate Review로 넘겨도 되는지 판단하는 stop/go gate였다.
  - UI title / verdict / button / caption을 `Compare 통과 판단`, `Candidate Review로 이동 가능`, `Send Selected Strategy To Candidate Review` 중심으로 정리했다.
  - `WEB_BACKTEST_UI_FLOW.md`의 Compare flow 설명도 `6단계 Candidate Packaging 진입 평가`가 아니라 `Compare 통과 판단`으로 갱신했다.
