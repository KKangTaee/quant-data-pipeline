...
### 예시 / 필요 상황
- Phase 32에서는 `Backtest > Portfolio Proposal`의 Validation Pack 아래에서 robustness preview, stress summary, Phase33 handoff를 보여준다.
...
- Phase 32의 `Phase 33 Handoff`가 `READY_FOR_PAPER_LEDGER_PREP`이면,
  Phase 33에서 paper ledger row를 만들 준비가 된 후보로 볼 수 있다.
- Phase 33에서는 `Backtest > Portfolio Proposal`의 Validation Pack 아래에서
  `Save Paper Tracking Ledger`를 눌러 `.note/finance/registries/PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl`에 append-only row를 저장한다.
...
### 기본 설명
Phase 31~33의 portfolio risk, robustness / stress, paper tracking 근거를 모아
후보나 proposal을 최종 실전 후보로 `선정`, `보류`, `거절`, `재검토` 중 어디에 둘지 판단하는 근거 묶음이다.

### 왜 사용되는지
paper tracking ledger가 `READY_FOR_FINAL_SELECTION_REVIEW`로 읽히더라도
그 자체가 자동 선정이나 주문 지시는 아니다.
최종 후보 판단은 백테스트 성과, validation blocker, stress gap, paper tracking 상태,
