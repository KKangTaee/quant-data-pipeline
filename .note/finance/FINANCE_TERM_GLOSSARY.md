...
- Phase 30에서는 `Backtest > Portfolio Proposal`에서 proposal draft를 만들고 `.note/finance/PORTFOLIO_PROPOSAL_REGISTRY.jsonl`에 append-only로 저장한다. `Monitoring Review`에서는 저장된 proposal의 blocker와 review gap을 다시 확인한다. 이 저장과 review는 live trading 승인이나 주문 지시와는 분리한다.
