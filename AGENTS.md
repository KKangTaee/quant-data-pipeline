...
- For Post-Selection operating guides after final selection review, use `.note/finance/registries/POST_SELECTION_OPERATING_GUIDES.jsonl` as the append-only operating guide registry. Keep it separate from final decisions: final decisions record whether a portfolio candidate was selected, while post-selection guides record rebalancing, reduce, stop, and re-review operating rules. They still do not create live approval, broker orders, or auto-trading instructions.
...
- For user-facing post-selection operating guide work, use `Backtest > Post-Selection Guide` to read only selected final review records and turn them into rebalancing / reduce / stop / re-review operating guides. Do not route this work back into Portfolio Proposal, and do not treat it as live approval or order execution.
