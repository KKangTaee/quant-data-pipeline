...
- For user-facing final review work, use `Backtest > Final Review` to inspect the selected current candidate or saved proposal, read Validation / Robustness / Paper Observation criteria together, and record the final select / hold / reject / re-review judgment. Final Review is the current user-facing end of the portfolio selection workflow.
- Do not add a separate Post-Selection Guide step to the active Backtest workflow unless the user explicitly reopens that design. The current product flow is `Portfolio Proposal -> Final Review -> 최종 판단 완료`.
