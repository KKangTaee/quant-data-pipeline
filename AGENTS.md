...
- `갱신 실행 결과/진단 패널`, `수집 실행 결과 패널`, `refresh job result display`, `운영 진단 UI`처럼 run / job / row / status 중심 패널을 추가하려면, 구현 전에 반드시 사용자에게 명시적으로 확인을 받아야 한다.
- 개선과 개편은 `갱신 실행 결과/진단 패널`을 만드는 행위가 아니다. 이런 패널은 필요할 수 있는 운영 보조 도구일 뿐이며, 실사용 workflow 개선으로 간주하지 않는다.
