...
- focused Python 28개를 포함한 combined Python regression 102개, React 13개, typecheck/build/compile/diff check와 actual responsive Browser QA를 통과했다.
- PC 상세 패널 follow-up에서 고정 760px frame 때문에 낮은 화면 하단이 밀리는 회귀를 수정했다. frame은 parent viewport에 맞춰 520~760px로 계산하고 navigation 아래로 정렬하며, 본문 scroll과 항상 보이는 destination footer를 분리했다.
...
- 초기 QA와 PC follow-up screenshot은 generated artifact `reference-center-react-v1-qa.png`, `reference-center-pc-bottom-fix-qa.png`로 남겼고 commit 대상에서 제외했다.
