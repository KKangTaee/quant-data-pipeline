...
- Detail open now aligns the component 80px below the Streamlit navigation, reserves a 16px bottom gap, and fills the remaining parent viewport without an arbitrary upper cap. It recalculates on browser resize; the detail header/footer stay fixed while only the body scrolls, so the destination action remains visible.
