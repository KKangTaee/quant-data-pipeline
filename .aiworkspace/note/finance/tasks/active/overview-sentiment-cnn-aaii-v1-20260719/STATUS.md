...
- 완료 범위: 합성점수 없는 CNN 시장 행동 / AAII 투자자 인식 두 축, Hero 교차 판정, 균형 current evidence, CNN 고정 + AAII 응답/Spread 전환의 동시 2 graph, 1W·1M 실제 관측 변화 card, 접힌 상세 근거. 실제 monitoring 기능이 아닌 confirm/reverse/persist Watch guide는 화면에서 제거했다.
...
- 후속 UI polish: 1W·1M의 `end_value`가 같은 것은 각각 동일한 최신 CNN·AAII 관측을 종점으로 쓰는 정상 계약이다. 혼동을 줄이기 위해 기간 변화량을 주값으로, 공통 최신값을 `현재` 보조값으로 재배치했고 source별 colored top border는 작은 원형 marker로 대체했다.
- Watch guide 정리: `WATCH / 다음 확인 조건`은 alert·condition persistence가 없는 정적 가이드라서 React component와 전용 CSS를 삭제했다. 화면은 기간 변화 다음에 상세 근거가 바로 이어지며 backend `watch_conditions` payload는 호환성을 위해 유지한다.
