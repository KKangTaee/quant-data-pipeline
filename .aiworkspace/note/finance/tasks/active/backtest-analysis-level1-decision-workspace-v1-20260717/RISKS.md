...
## 15차 Portfolio Mix Current Risk Assessment

- Task 46은 pure draft/read model만 구현했다. 실제 runner, weighted build, save와 Level2 handoff는
  Task 48 전까지 callable capability를 제공하지 않으므로 current primary UI는 아직 legacy route다.
- new-schema saved shelf는 legacy prototype row를 의도적으로 숨긴다. migration은 구현하지 않으며
  실제 new Mix 저장/복원은 Task 48 Browser QA에서 새 row로 검증한다.
