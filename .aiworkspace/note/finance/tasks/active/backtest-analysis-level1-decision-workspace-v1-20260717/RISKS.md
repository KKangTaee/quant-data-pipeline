...
## 12차 Current Selection And Factor Presentation Closeout Assessment

- fresh desktop/760px Browser QA는 current context와 factor presentation을 확인했다. 실제 이전 결과를
  만든 뒤 strategy/price를 바꾸고 단일 lifecycle strip으로 전환되는 end-to-end click은 in-app
  Browser의 긴 iframe coordinate 제한으로 재현하지 못했다. pure lifecycle, state adapter, source
  boundary regression이 설정 변경·가격 갱신·재실행 실패와 handoff 차단을 보완한다.
- long-running 8505 server는 `runOnSave=false` / `fileWatcherType none`이어서 restart 전 old Python과
  old build를 계속 보여준다. current UI 확인은 server restart가 필요하다.
- repository-wide service contract의 12 baseline failures와 frontend dependency audit moderate 1 /
...
## 15차 Portfolio Mix Initial Risk Assessment

- Task 46 시점에는 pure draft/read model만 있었고 실제 runner, weighted build, save와 Level2 handoff가
  없었다. Task 48에서 callable capability와 primary route cutover를 완료했다.
- new-schema saved shelf는 legacy prototype row를 의도적으로 숨긴다. migration은 구현하지 않으며
  actual new Mix 저장/복원은 Task 49 Browser QA의 새 row로 검증했다.
- Single settings runtime catalog가 변경되면 effective fingerprint도 함께 바뀌는 것이 의도다. 장시간
  열린 Streamlit process에서 old runtime/new React build가 섞이지 않도록 final QA는 fresh process를 쓴다.
- Task 47 fallback은 같은 read model의 step/result/action을 보존하지만 모든 component field 편집을
  native Streamlit로 복제하지 않는다. React unavailable 상태의 full edit parity는 final source/visual
  acceptance gap으로 남기고 primary React와 Python validation을 우선한다.
- frontend audit moderate 1 / high 1은 Vite breaking upgrade를 요구한다. production build는 통과했고
  dependency major migration은 Portfolio Mix feature 범위 밖이다.

## 15차 Task 48 Runtime / Cutover Assessment

- Browser QA에서는 protected Run History, saved JSONL, Practical Validation registry append를 피하기 위해
  실제 `Mix 실행`, `저장`, `Level2 등록` side effect 버튼은 누르지 않았다. runner/weighted/store/handoff 경계는
  injected callable TDD로 검증했고 final manual smoke에서 실제 side effect는 별도 확인 대상이다.
- 기존 saved prototype row는 의도적으로 shelf에 나타나지 않는다. 새 저장을 한 번 만들기 전 empty state가
  보이는 것이 정상이며 legacy migration/rewrite는 하지 않는다.
- synchronous Streamlit handler는 최종 completed/error state를 보존하지만 long-running runner 중 React card의
  중간 `running` repaint를 보장하지 않는다. 실행 중 진행률 streaming은 별도 async/job 범위다.
- `runOnSave=false` / `fileWatcherType none`인 장기 실행 서버는 source/build 변경 뒤 반드시 restart해야 한다.
  Task 49 closeout QA도 current source/build pair로 fresh restart한다.

## 15차 Portfolio Mix Closeout Assessment

- full service의 12 failures는 기존 Sentiment 1, Final Review 4, liquidity copy 1, Practical Validation 6
  source-contract debt다. focused 79 tests와 Portfolio Mix service selector 5 tests는 통과했고 신규 Mix
  failure는 없다.
- actual Browser QA가 run/save/restore/edit/rerun과 fresh/stale action lifecycle을 확인했다. Level2 CTA는
  callable 노출까지만 확인했으며 protected registry append 자체는 TDD handler test로 검증했다.
- new-schema-only shelf 때문에 prototype legacy saved row는 보이지 않는다. 사용자가 prototype 데이터를
  보존할 필요가 없다고 승인했으므로 migration/rewrite를 추가하지 않는다.
- synchronous Streamlit handler는 장시간 실행 중 component card의 중간 progress repaint를 보장하지 않는다.
  final completed/error state는 보존하며 streaming progress는 별도 async/job 범위다.
- Python fallback은 같은 read model/validation/result/action을 제공하지만 모든 React field의 full edit parity는
  없다. primary React unavailable 상태의 end-to-end visual QA는 후속 compatibility gap이다.
- iframe 안 전체 keyboard Tab 순회/focus ring과 screen-reader 실제 동작은 자동화하지 않았다. semantic control,
  aria source contract와 760px layout은 테스트/Browser QA로 확인했다.
- frontend audit moderate 1 / high 1은 Vite breaking upgrade를 요구한다. current production build는 통과했으며
  dependency migration은 별도 compatibility task다.

## 16차 Portfolio Mix Result Evidence Closeout Assessment

- CUA는 한 SVG에서 다른 SVG로 좌표를 옮길 때 브라우저의 실제 `pointerleave`/`mouseleave`를 항상
  합성하지 않아 tooltip 제거를 시각 자동화로 완전히 재현하지 못했다. pointer 값 갱신은 actual QA로,
  `pointerleave / mouseleave / pointercancel / blur` clear는 RED→GREEN source contract로 검증했다.
- 긴 history는 dependency 없는 SVG path를 사용하므로 point가 매우 많을 때 DOM 수는 선형 증가한다.
  current 약 119개월 actual run은 문제없지만 수십 년 daily Mix로 확장하면 downsampling을 별도 검토한다.
- 첫 월 또는 non-finite 월 수익률은 차트에서 제외하고 표에 `계산값 없음`으로 보존한다. 이는 결측을
  0%로 위조하지 않기 위한 계약이다.
- full service 12 failures와 frontend dependency audit moderate 1 / high 1은 기존 baseline이다.
  Portfolio Mix focused 83 tests와 current production build에는 신규 failure가 없다.
- registry, Run History, saved JSONL, `.superpowers/`, generated screenshots는 closeout commit에서 제외한다.

## 17차 Portfolio Mix Chart Geometry Closeout Assessment

- actual pointer first/middle/last와 monthly middle은 검증했다. 모든 119개월 좌표를 브라우저에서 순회하지
  않았지만 pure geometry source contract와 boundary tests가 plot clamp/rounding을 고정한다.
- 760px outer/document와 component grid/card horizontal overflow는 0이다. 더 좁은 별도 breakpoint는 이번
  사용자 요청 범위가 아니며 기존 760px product contract를 유지한다.
- full service baseline은 이번 React/CSS geometry 범위에서 재실행하지 않았다. focused UI/boundary 84 tests와
  production build가 신규 변경 경계를 검증하며, 기존 full-service 12 failures는 16차 기록을 유지한다.
- registry, Run History, saved JSONL, `.superpowers/`, generated screenshots는 stage/commit하지 않는다.

## 18차 Portfolio Mix Long Multi-Select Closeout Assessment

- desktop과 760px에서 검색·선택 변경·내부 scroll을 실제 확인했다. 520px 이하 1열 breakpoint는 source/CSS
  contract로 남고 이번 product standard Browser QA에는 포함하지 않았다.
- option 수가 수백 개여도 DOM에는 filtered 전 전체 button이 남고 scroll로 높이만 제한한다. 현재 GTAA 목록은
  동작하지만 수천 option으로 확장하면 virtualization을 별도 검토한다.
- frontend dependency audit moderate 1 / high 1과 full service 기존 12 failures는 이번 selector 변경 범위 밖이다.
- registry, Run History, saved JSONL, `.superpowers/`, Playwright scratch와 generated screenshots는
  stage/commit하지 않는다.

## 19차 Portfolio Mix Monthly Return Y-Axis Closeout Assessment

- actual QA는 `±10%` range를 사용한 현재 119개월 결과와 760px breakpoint를 확인했다. `±1%` 최소
  range와 sub-1% 소수 label은 source contract/build로 고정했지만 별도 actual portfolio로 만들지는 않았다.
- 신규 scale은 actual maximum을 nice upper bound로 올려 최고 막대가 항상 axis 끝까지 차지하지 않는다.
  이는 월별 절댓값 비교를 위한 의도된 여백이며 manual zoom/range control은 이번 범위 밖이다.
- frontend dependency audit moderate 1 / high 1과 full service 기존 12 failures는 이번 chart presentation 범위 밖이다.
- registry, Run History, saved JSONL, `.superpowers/`, generated screenshots는 stage/commit하지 않는다.
