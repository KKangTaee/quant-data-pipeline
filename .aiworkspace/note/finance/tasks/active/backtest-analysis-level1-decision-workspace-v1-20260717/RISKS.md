...
- native multi-select는 일반 select보다 많은 option을 가진 factor/ticker field에서 키보드
  부담이 남는다. current first-read에서는 technical/default field가 disclosure에 있고
  overflow는 없지만, 검색형 combobox 전환은 별도 UX 범위다.
- repository-wide service contract 11 failures는 pre-7차와 동일하다: Sentiment React 1,
  Final Review 4, Practical Validation 6. Level1 focused 113 tests와 actual 3-strategy run은
  모두 통과했다.
