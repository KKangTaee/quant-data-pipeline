...
- 2026-06-03 bugfix: yfinance can return empty `1d / 1m` futures frames for some symbols even when `2d / 1m` has current candles. The collector now performs one bounded 2d retry for symbols that produced no normalized rows in the initial 1d request. This is a coverage recovery path, not a freshness guarantee; stale warnings still remain when the provider latest candle is delayed.
