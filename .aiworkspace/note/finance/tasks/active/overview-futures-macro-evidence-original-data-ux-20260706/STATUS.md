...
- Current evidence / raw appendix follow-up complete: `CurrentEvidencePanel.tsx` now renders inside `MacroContextSection`, so `현재 근거` belongs to `매크로 컨텍스트` instead of reading as a separate fourth block. The lower Streamlit disclosure is renamed to `원본 데이터 / 계산 추적` and maps raw tables back to `매크로 컨텍스트`, `최근 흐름`, and `과거 점검`.
- Current evidence / raw appendix QA complete: RED/GREEN focused contracts, 26-test Futures Macro contract suite, `py_compile`, React `npm run build`, `git diff --check`, and Browser DOM QA passed. Screenshots saved as `browser-qa-futures-macro-current-evidence-raw-appendix.png` / `browser-qa-futures-macro-after-pagedown.png` and left uncommitted as generated QA artifacts.
