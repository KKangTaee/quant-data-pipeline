...
Status: User-approved design; detailed implementation plan complete
Date: 2026-07-22
...
### A. Current Level1 result surfaces 정식 통합 — 채택

공통 Python freshness model을 만들고 현재 활성 결과 화면 두 곳에 투영한다. Single Strategy는 `BacktestAnalysisResultWorkspace`, Portfolio Mix는 별도 `PortfolioMixWorkspace`가 소유한다. 두 화면이 동일한 상태·행동 계약을 사용하며 legacy UI를 되살리지 않는다.
