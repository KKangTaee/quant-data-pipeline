...
4. module label만으로 `accepted_limit`나 `resolve_now`를 추정하지 않는다.
...
- coverage가 충분하고 liquidity/capacity source만 proxy이면 자동으로 제거할 수 없는 source 한계는 `accepted_limit`, explicit comparator가 있는 관측 주의는 `measured_caution`으로 설명한다.
...
- 기본 profile에서는 누락된 method evidence를 accepted limit 또는 review로 읽고, profile이 명시적으로 요구한 경우만 blocker다.
