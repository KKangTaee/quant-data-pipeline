...
- Streamlit 1.57에서는 context/decision mount boundary와 fragment rerun을 Browser
  QA로 확인했다. Streamlit custom-component 또는 fragment lifecycle을 업그레이드할
  때는 replay pending 중 context DOM 유지와 두 iframe height/overflow를 다시
  검증해야 한다.
- Backtest의 결과 curve가 비어 있는 초기 화면에서 Altair `Infinite extent` warning이
  남는다. Practical Validation two-surface/component 오류는 아니며 이번 replay
  경계 범위에서는 console error와 component-ready warning이 0이다.
