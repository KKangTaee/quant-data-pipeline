...
- `Effective EOD Date` is the latest usable return date, not necessarily the latest raw price row date. If the newest EOD date has too few usable rows, the service falls back to the prior eligible trading date and the UI now exposes that fallback in the status detail.
- Sector / Industry first render can still take several seconds on cold cache because weekly/monthly trend and ticker-leader queries are DB-backed and computed synchronously in Streamlit. A 120-second `st.cache_data` layer is used for same-control rerenders, while keeping freshness reasonably close to manual refresh cycles.
