...
Status: Implemented — original 1차~5차, correctness follow-up 1차~3차, partial-history follow-up 1차~3차 Complete
...
**Tech Stack:** Python 3.12, pandas, unittest, React 19, TypeScript, Vite, Streamlit Browser QA.
...
- [x] Add an AMD-like failing test with 95 monthly rows and non-positive TTM EPS in the first three visible 3-year slots. Assert `status == "PARTIAL"`, `observation_count == 33`, `missing_point_count == 3`, timeline length `36`, and three `NON_POSITIVE_EPS` gaps.
- [x] Add a failing warmup test where 70 rows produce 11/12 available points. Assert the first timeline slot is `MISSING/INSUFFICIENT_ROLLING_PER_WARMUP` and the remaining slot indices are not compressed.
...
- [x] Implement one slot per visible month, deterministic gap reason priority, `READY/PARTIAL/INSUFFICIENT_HISTORY`, and valid-only backward-compatible `series`.
- [x] Re-run the two tests and the full `tests.test_us_stock_valuation` module.
...
- [x] Add a failing service test that changes the first three months of the latest 36-month window to non-positive EPS and asserts `history_options["3y"]` is `PARTIAL` with `33/36` and timeline gap metadata.
- [x] Run the exact test; expect failure on current `INSUFFICIENT_HISTORY` status.
- [x] Make no service change if the pure result already passes through; otherwise add only the minimal JSON-safe handoff needed.
- [x] Re-run the exact test plus combined Market Context service tests.
...
- [x] Add a failing static contract test requiring `PARTIAL`, `timeline`, `contiguousHistorySegments`, `계산 가능`, and `결측 월은 연결·보간하지 않습니다` in the React source.
- [x] Run the exact Python static test; expect failure because React currently rejects every non-READY history.
- [x] Extend TypeScript history types and derive a full timeline fallback from legacy series for S&P.
- [x] Render READY/PARTIAL charts, a PARTIAL coverage banner, missing-slot inspector copy, and separate actual/baseline/band paths per contiguous AVAILABLE segment.
- [x] Add minimal responsive styles for the coverage banner and gap inspector.
- [x] Re-run the static test and `npm run build --prefix app/web/streamlit_components/market_context_valuation`.
...
- [x] Run read-only AAPL/AMD actual service smoke and assert AAPL `3y READY 36/36`, AAPL `5y PARTIAL 42/60`, AMD `3y PARTIAL 33/36`, AMD `5y PARTIAL 39/60` on current DB evidence.
- [x] Run focused U.S. stock/Market Context/S&P/Nasdaq backend tests and the repository's isolated-module full regression command.
- [x] Run fresh React build, Python compile, `git diff --check`, and staged-path audit.
- [x] Use `browser:control-in-app-browser` for actual desktop and 420px QA. Verify partial badge, left/interior gap, selector interaction, S&P regression, zero console errors, and no horizontal overflow.
- [x] Capture one representative screenshot outside the repository and include its absolute path in the final report.
- [x] Apply `finance-doc-sync` and `finance-integration-review`; record durable contract only, keep run details in `RUNS.md`, and leave the unrelated research folder untouched.
- [x] Commit the implementation/docs as coherent Korean units without amending prior commits.
