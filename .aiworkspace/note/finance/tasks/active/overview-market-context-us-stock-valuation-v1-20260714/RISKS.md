...
- Some older 3/5-year points remain unavailable even when raw positive-P/E months exceed the rolling warmup because complete historical filing+SEP evidence is not present at every visible month. The UI now renders valid months as `PARTIAL`, reports exact gap reasons, and does not synthesize or connect the missing months. Expanding coverage still requires a separate historical SEP/PIT backfill.
- Companies with non-positive current TTM EPS remain outside the P/E surface. A separate loss-company analysis needs metric selection and research before implementation; the current screen must not turn negative earnings into a valid-looking P/E.
...
## Closed In 2026-07-15 Partial-History Follow-up

- A single unavailable month no longer hides every calculable 3/5-year point; `PARTIAL` preserves the exact 12/36/60-month calendar axis and valid observations.
- Missing calendar months, non-positive EPS periods, rolling warmup, and PIT-evidence gaps keep distinct reason codes and cannot be compressed into neighboring slots.
- React splits chart paths at every missing slot, preventing a line or scenario band from visually implying an interpolated valuation through unavailable evidence.
- Legacy S&P history fallback, retained Nasdaq backend tests, desktop/420px overflow checks, and zero browser-console errors close the main regression risk for this UI change.
