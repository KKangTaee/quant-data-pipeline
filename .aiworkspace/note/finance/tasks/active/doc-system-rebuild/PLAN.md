...
Status: Draft
Created: 2026-05-12
Task Path: `.aiworkspace/note/finance/tasks/active/doc-system-rebuild/`

## 1. Goal

`finance` 프로젝트의 문서 체계를 Notion의 Codex 문서 운영 가이드에 맞춰 다시 구성한다.

현재 `.aiworkspace/note/finance/`는 장기 지식, phase 기록, task 기록, 코드 분석, 운영 메모, 연구 메모, runtime 산출물이 한 공간에 섞여 있다. 이 때문에 Codex가 작업 시작 시 무엇을 먼저 읽어야 하는지, 진행 중 발견한 내용을 어디에 기록해야 하는지, 완료 후 어떤 내용을 장기 문서로 승격해야 하는지 판단하기 어렵다.
...
- `.aiworkspace/note/finance/docs/`가 장기 지식의 기준 위치가 된다.
- `.aiworkspace/note/finance/phases/`가 phase 단위 계획과 통합 기록의 기준 위치가 된다.
- `.aiworkspace/note/finance/tasks/`가 개별 실행 task 기록의 기준 위치가 된다.
- `.aiworkspace/note/finance/agent/`가 Codex 반복 실수, 교훈, 운영 팁의 기준 위치가 된다.
- `.aiworkspace/note/finance/reports/backtests/`가 durable backtest report의 기준 위치가 된다.
- `.aiworkspace/note/finance/registries/`와 `.aiworkspace/note/finance/saved/`는 삭제하지 않고 유지한다.
- `AGENTS.md`는 새 문서 구조 기준으로 짧고 강한 작업 규칙 문서로 정리된다.
- 기존 문서 중 장기적으로 필요한 내용만 새 `docs/` 문서에 최소 승격된다.
- 삭제 또는 재배치 후 `find .aiworkspace/note/finance -maxdepth 3`로 새 구조가 확인된다.
...
- `.aiworkspace/note/finance/` 문서 구조
- `AGENTS.md`
- finance Codex 운영 문서
- 기존 `.aiworkspace/note/finance` 문서의 삭제, 축약, 재배치
...
- [x] 현재 `.aiworkspace/note/finance/` 폴더와 파일 목록을 저장한다.
...
- [x] `.aiworkspace/note/finance/docs/` 생성
- [x] `.aiworkspace/note/finance/docs/INDEX.md` 생성
- [x] `.aiworkspace/note/finance/docs/PRODUCT_DIRECTION.md` 생성
- [x] `.aiworkspace/note/finance/docs/ROADMAP.md` 생성
- [x] `.aiworkspace/note/finance/docs/PROJECT_MAP.md` 생성
- [x] `.aiworkspace/note/finance/docs/GLOSSARY.md` 생성
- [x] `.aiworkspace/note/finance/docs/architecture/README.md` 생성
- [x] `.aiworkspace/note/finance/docs/flows/README.md` 생성
- [x] `.aiworkspace/note/finance/docs/data/README.md` 생성
- [x] `.aiworkspace/note/finance/docs/runbooks/README.md` 생성
- [x] `.aiworkspace/note/finance/phases/active/`, `.aiworkspace/note/finance/phases/done/` 생성
- [x] `.aiworkspace/note/finance/tasks/active/`, `.aiworkspace/note/finance/tasks/done/` 생성
- [x] `.aiworkspace/note/finance/agent/GOTCHAS.md`, `.aiworkspace/note/finance/agent/LESSONS.md` 생성
...
- 상세 시스템 설명은 `.aiworkspace/note/finance/docs/`로 이동한다.

### Step 5 - Migrate / Remove Old Documentation Tree

재배치 대상:

- `.aiworkspace/note/finance/backtest_reports/` -> `.aiworkspace/note/finance/reports/backtests/`
- `.aiworkspace/note/finance/backtest_reports/strategies/` -> `.aiworkspace/note/finance/reports/backtests/strategies/`
- `.aiworkspace/note/finance/backtest_reports/phase*/` -> 1차 임시 archive -> 최종적으로 `runs/`, `candidates/`, `validation/`으로 분류
...
- `.aiworkspace/note/finance/.DS_Store`

보존:

- `.aiworkspace/note/finance/registries/`
- `.aiworkspace/note/finance/saved/`
- 새 `.aiworkspace/note/finance/docs/`
- 새 `.aiworkspace/note/finance/reports/backtests/`
- 새 `.aiworkspace/note/finance/phases/`
- 새 `.aiworkspace/note/finance/tasks/`
- 새 `.aiworkspace/note/finance/agent/`
...
- 시스템 설명은 `.aiworkspace/note/finance/docs/`로 이동한다.

### Risk 3 - phase와 task가 다시 섞임

대응:

- phase는 `.aiworkspace/note/finance/phases/active/<phase>/`
- task는 `.aiworkspace/note/finance/tasks/active/<task>/`
