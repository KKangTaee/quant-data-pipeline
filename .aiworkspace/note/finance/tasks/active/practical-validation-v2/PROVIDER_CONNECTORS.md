...
시장 환경 데이터를 별도로 읽어야 한다. 이 작업은 12개 진단 중 proxy / `NOT_RUN`으로 남아 있던
항목을 실제 provider snapshot 기반으로 바꾸기 위한 것이다.

특히 새 ETF가 후보에 들어올 때마다 코드에 ticker를 직접 추가하는 방식은 오래 유지하기 어렵다.
그래서 `nyse_etf`와 `nyse_asset_profile`을 기준으로 운용사와 공식 source 후보를 발견하고,
검증된 endpoint를 DB에 저장한 뒤, 그 source map을 사용해 holdings / exposure / operability 수집을
반복 실행할 수 있게 한다.
...
2026-05-28 기준 P2 provider connector는 closeout 완료 상태다.
이 문서는 이후 provider coverage 확대를 위한 기준으로 유지한다.
Coverage 확대는 P2 blocker가 아니라 별도 provider support task로 분리한다.
...
|---|---|---|---|
| ETF provider source map | `symbol`, `provider`, `data_kind`, `parser`, `source_url`, `source_status`, `product_id`, `fund_family` | 2, 3, 9, 10 수집 가능 여부 판정 | `finance_meta.etf_provider_source_map` |
...
|---|---|---|---|
| `finance_meta.etf_provider_source_map` | ETF별 공식 provider endpoint / parser map | normalized connector metadata | `nyse_etf`, `nyse_asset_profile`, 공식 issuer product list / URL 검증 |
...
- `etf_holdings_snapshot`과 `etf_exposure_snapshot`은 P2-3에서 official holdings / exposure foundation을 저장한다.
- `macro_series_observation`은 P2-4에서 FRED market-context observation을 저장한다.

## P2-3 구현 상태: ETF holdings / exposure foundation

상태: `initial_implementation_complete`

P2-3은 ETF 내부 구성 row와 exposure aggregate를 DB에 저장한다.
UI나 Practical Validation 진단 연결은 아직 하지 않고, P2-5에서 loader 결과를 12개 진단에 연결한다.

구현 파일:

| 파일 | 구현 내용 |
|---|---|
| `finance/data/db/schema.py` | `etf_holdings_snapshot`, `etf_exposure_snapshot` 추가 |
| `finance/data/etf_provider.py` | official holdings download/API fetch, normalize, canonical refresh, exposure aggregation |
| `finance/loaders/provider.py` | `load_etf_holdings_snapshot()`, `load_etf_exposure_snapshot()` read path |
| `finance/loaders/__init__.py` | holdings / exposure loader public export |

초기 source map:

| provider | ticker | 수집 방식 | 현재 coverage |
|---|---|---|---|
| iShares / BlackRock | `AOR`, `IEF`, `TLT` | official holdings CSV | `actual` |
| SSGA / SPDR | `SPY`, `BIL` | official daily holdings XLSX | `actual` |
| SSGA / SPDR | `SPY` | fund page sector breakdown | sector exposure `actual` |
| Invesco | `QQQ` | official holdings JSON API | holdings `actual` |
| Invesco | `QQQ` | official weighted sector API | sector exposure `actual` |
| SSGA / SPDR | `GLD` | row-level holdings source pending | `missing` / 후속 필요 |

현재 저장 기준:

- holdings business key: `(fund_symbol, as_of_date, source, holding_id)`
- exposure business key: `(fund_symbol, as_of_date, source, exposure_type, exposure_name)`
- `weight_pct`는 provider가 표시하는 percent point 단위로 저장한다. 예: `34.13`은 34.13%다.
- holdings refresh 기본값은 `canonical_refresh`다. 같은 fund / as_of_date / source 범위는 삭제 후 다시 저장해 빠진 holdings row가 남지 않게 한다.
- `AOR`은 현재 1차 ETF holdings만 저장한다. iShares CSV의 Aggregate Underlying 구간은 2차 look-through expansion으로 분리한다.
- `GLD`는 금 bar list가 PDF 성격이므로 synthetic 100% gold row를 만들지 않고 missing으로 둔다.
...
source map은 현재 Practical Validation과 Selected Portfolio Dashboard에서 자주 쓰는 ETF부터 검증하고,
동일한 discovery 함수를 `nyse_etf` 전체로 확장할 수 있게 둔다. DB에 저장되는 map은 ticker hardcoding이 아니라
검증된 provider endpoint cache다.
...
현재 구현된 source map discovery:

- `finance.data.etf_provider.discover_and_store_etf_provider_source_map()`
  - `nyse_etf`와 `nyse_asset_profile`을 조인해 fund family / name으로 provider 후보를 추론한다.
  - iShares는 공식 product list에서 product URL과 holdings CSV endpoint를 찾는다.
  - SSGA / SPDR은 daily holdings XLSX endpoint pattern을 검증한다.
  - Invesco는 holdings / sector API endpoint pattern을 검증한다.
  - `GLD`, `IAU`처럼 금 현물 ETF는 row-level holdings 대신 `commodity_gold` parser로 100% gold exposure를 저장한다.
- `finance_meta.etf_provider_source_map`
  - verified row만 Practical Validation 수집 가능 대상으로 읽는다.
  - failed / unsupported row는 자동 수집 불가 사유를 남기는 관리용 metadata다.
...
1. provider source map과 ticker-to-provider adapter mapping을 정의하고 DB에 저장한다.
2. `finance/data/db/schema.py`에 source map / provider snapshot / holdings / macro table을 추가한다.
...
## P2-4 구현 상태: Macro / Sentiment market-context foundation
...
Practical Validation의 macro / sentiment 진단이 benchmark price-action proxy에만 의존하면,
전략이 통과한 시점의 시장 환경을 별도 근거로 설명하기 어렵다.
P2-4는 VIX, 장단기 금리차, 신용 스프레드 같은 재현 가능한 market-context series를 DB에 저장해,
P2-5에서 `Regime / Macro Suitability`와 `Sentiment / Risk-On-Off Overlay`가 실제 관측값을 읽을 수 있게 만든다.

구현 파일:

| 파일 | 변경 |
|---|---|
| `finance/data/db/schema.py` | `macro_series_observation` 추가 |
| `finance/data/macro.py` | FRED API / CSV 수집, normalization, UPSERT 저장 |
| `finance/loaders/macro.py` | observation range loader, 기준일 snapshot / staleness loader |
| `finance/loaders/__init__.py` | macro loader export |

초기 source map:

| series_id | source | category | 용도 |
|---|---|---|---|
| `VIXCLS` | FRED API 또는 official CSV | `volatility` | volatility / risk-off context |
| `T10Y3M` | FRED API 또는 official CSV | `yield_curve` | yield curve inversion / recession risk context |
| `BAA10Y` | FRED API 또는 official CSV | `credit_spread` | credit stress context |

저장 규칙:

- `series_id`, `observation_date`, `source`를 business key로 UPSERT한다.
- `FRED_API_KEY` 또는 `api_key`가 있으면 FRED `series/observations` API를 사용한다.
- API key가 없으면 FRED official `fredgraph.csv` download를 사용한다.
- API key는 DB의 `source_ref`에 저장하지 않는다. `source_ref`는 공개 series page만 저장한다.
- `load_macro_snapshot()`은 validation 기준일 이전 최신 관측값과 `staleness_days`, `snapshot_status`를 반환한다.

현재 한계:

- sentiment는 아직 별도 composite index가 아니라 VIX / credit spread / yield curve 기반 market-context proxy다.
- FRED observation date 기준 저장이며, vintage / revision point-in-time 계층은 아직 구현하지 않았다.
- Practical Validation 12개 진단 연결은 P2-5에서 진행한다.
...
2. `etf_provider_source_map`, `etf_operability_snapshot`, `etf_holdings_snapshot`, `etf_exposure_snapshot`, `macro_series_observation` schema 추가
3. iShares / SSGA / Invesco / FRED 수집 adapter 골격 추가
4. iShares AOR / IEF / TLT holdings CSV 수집 / UPSERT
5. SSGA SPY / BIL daily holdings XLSX, Invesco QQQ holdings / sector API 수집 / UPSERT
...
5. 완료: Practical Validation provider coverage UI 연결
6. 완료: Practical Validation macro / sentiment / operability domain 연결
...
- `app/web/streamlit_app.py`
  ...
  - `Provider Source Map`, `ETF Operability`, `ETF Holdings / Exposure`, `Macro Context` tabs 추가
...
- `GLD` / `IAU`는 금 현물 ETF 특성상 row-level stock holdings가 아니라 `commodity_gold` rule로 100% gold exposure를 저장한다.

## P2-5B 구현 상태: Practical Validation provider context 연결

P2-5B는 DB에 저장된 provider snapshot을 Practical Validation 12개 진단의 compact evidence로 연결한다.

구현 내용:

- `app/web/backtest_practical_validation_connectors.py`
  - ETF operability, ETF holdings, ETF exposure, FRED macro loader 결과를 compact provider context로 변환
  - full holdings row와 full macro series 대신 coverage, missing symbol, top holding / exposure, macro latest snapshot만 반환
- `app/web/backtest_practical_validation_helpers.py`
  - provider snapshot 조회 기준일은 저장된 backtest 종료일이 아니라 Practical Validation 실행일로 둔다. backtest 종료일이 과거인 saved mix도 현재 수집된 ETF 운용성 / holdings / macro snapshot으로 검증해야 하기 때문이다.
  - 2번 Asset Allocation Fit: ETF exposure snapshot 우선
  - 3번 Concentration / Overlap / Exposure: ETF holdings snapshot 기반 top holding / overlap 우선
  - 5번 Regime / Macro Suitability: FRED VIX / yield curve / credit spread snapshot 우선
  - 6번 Sentiment / Risk-On-Off Overlay: FRED 기반 risk-on/off context 우선
  - 9번 Leveraged / Inverse ETF Suitability: provider product metadata 우선
  - 10번 Operability / Cost / Liquidity: expense, AUM, ADV, spread, premium/discount snapshot 우선
- `app/web/backtest_practical_validation.py`, `app/web/backtest_final_review.py`
  - Provider Coverage 요약 table 표시
- `app/web/backtest_practical_validation.py`
  - Provider Coverage 아래에 ETF별 Provider Data Gaps 표를 표시
  - 부족한 ETF operability는 official 또는 DB bridge collector로 보강하고, holdings / exposure는 지원되는 official source map이 있을 때 같은 화면에서 수집
  - source map이 없는 holdings / exposure는 먼저 자동 source discovery 대상으로 분리하고, 자동 탐색 후에도 verified map이 없으면 수동 connector 보강 대상으로 남긴다.

## P2-5C 구현 상태: provider source map discovery 연결

P2-5C는 "새 ETF가 들어올 때마다 코드에 mapping을 직접 추가해야 하는가?" 문제를 줄인다.
Practical Validation 화면과 Ingestion 콘솔은 이제 `finance_meta.etf_provider_source_map`을 먼저 읽고,
verified source map이 있으면 holdings / exposure / operability 수집 가능 대상으로 판단한다.

구현 내용:

- `finance/data/db/schema.py`
  - `PROVIDER_SCHEMAS["etf_provider_source_map"]` 추가
- `finance/data/etf_provider.py`
  - `discover_and_store_etf_provider_source_map()` 추가
  - `nyse_etf` + `nyse_asset_profile` 기반 provider 후보 추론
  - iShares product list, SSGA holdings XLSX pattern, Invesco holdings / sector API pattern 검증
  - `GLD`, `IAU` 같은 금 현물 ETF는 `commodity_gold` parser로 holdings / exposure snapshot 생성
  - 기존 official snapshot 수집은 verified source map을 static map보다 먼저 사용
- `app/jobs/ingestion_jobs.py`
  - `run_discover_etf_provider_source_map()` job wrapper 추가
- `app/web/streamlit_app.py`
  - Ingestion의 Practical Validation Provider Snapshots에 `Provider Source Map` tab 추가
- `app/web/backtest_practical_validation.py`
  - Provider Data Gaps가 verified source map을 읽어 collectable / discovery-needed / manual-needed를 구분
  - 부족 데이터 일괄 보강 버튼은 먼저 source map discovery를 실행하고, 그 뒤 다시 collection plan을 계산한다.

확인된 smoke 결과:

- `GLD`, `IAU`, `MTUM`, `QUAL`, `SOXX`, `USMV`, `XLE`, `XLU` source map discovery는 16개 verified row를 저장했다.
- 같은 8개 ETF의 holdings / exposure 수집은 `etf_holdings_snapshot` 522 rows, `etf_exposure_snapshot` 81 rows를 저장했다.
- 2026-05-11 saved portfolio mix Practical Validation source 기준으로 holdings / exposure coverage는 100% actual로 올라갔고, connector mapping needed 목록은 비었다.

저장 경계:

- `PRACTICAL_VALIDATION_RESULT_SCHEMA_VERSION=5`
- JSONL에는 `provider_coverage`, `provider_coverage_display_rows`, provider-based diagnostic compact evidence만 저장한다.
- full holdings row, full macro observation, raw provider response는 저장하지 않는다.
- Provider Data Gaps의 일괄 수집 버튼은 DB provider snapshot과 `WEB_APP_RUN_HISTORY.jsonl` 실행 이력만 갱신한다. Practical Validation result 저장은 사용자가 결과를 다시 확인한 뒤 `검증 결과 저장`으로 명시 실행한다.

아직 하지 않은 것:

- Stress Interpretation에서 macro / exposure context를 stress window 원인 해석에 직접 결합하는 작업은 다음 단계다.
- Strategy-specific Sensitivity Runtime은 다음 작업 단위에서 계속 진행한다.

## 검증 기준

- [x] provider snapshot 수집 job wrapper가 표준 `JobResult`를 반환한다.
- [x] Ingestion 화면에서 P2 provider snapshot 실행 지점이 제공된다.
- [x] provider table이 없어도 loader와 UI가 실패하지 않는다.
- [x] 수집기는 같은 symbol / as_of_date / source 기준으로 반복 실행 가능하다.
- [x] 수집 결과에 `source`, `source_ref`, `as_of_date`, `collected_at`, `coverage_status`가 남는다.
- [x] coverage가 낮은 ETF는 `REVIEW` 또는 `NOT_RUN`으로 보인다.
- [x] proxy data는 `provider_snapshot`처럼 표시되지 않는다.
- [x] `nyse_asset_profile` 기존 reader와 runtime ETF operability path가 깨지지 않는다.
- [x] Practical Validation result row는 compact evidence만 저장한다.
- [x] Final Review에서 provider coverage gap을 판단 근거로 볼 수 있다.
- [x] 새 table이 추가되면 `DATA_DB_PIPELINE_FLOW.md`와 data architecture 문서를 갱신한다.
