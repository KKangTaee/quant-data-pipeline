...
| Alternative portfolio challenge | Practical Validation | SPY, QQQ, 60:40, cash-aware baseline 대비 복잡성 보상 여부 표시. All Weather-like proxy는 후속 | 후보를 자동 탈락시키는 최적화 engine으로 만들지 않는다 |
...
사용자 화면에서는 질문 / 선택지를 한글로 표시하고, 코드 / JSON에는 영어 id를 저장한다.

권장 질문:

| 화면 질문 | 화면 선택지 | 저장 id 예시 | 반영 domain |
|---|---|---|---|
| 이 포트폴리오를 어떤 목적으로 검증할까요? | 손실 방어 중심 / 수익과 위험의 균형 / 성장 중심 / 공격적 수익 추구 / 헤지 또는 전술적 대응 | `defensive`, `balanced`, `growth`, `aggressive`, `hedged_tactical` | asset allocation, benchmark challenge, stress |
| 어느 정도의 손실까지 감내할 수 있나요? | -10% 내외 / -20% 내외 / -35% 내외 / 그 이상도 가능 | `dd_10`, `dd_20`, `dd_35`, `dd_above_35` | drawdown, tail, rolling |
| 이 포트폴리오를 어느 기간 동안 운용할 생각인가요? | 3개월 미만 / 6~12개월 / 1~3년 / 3년 이상 | `lt_3m`, `6_to_12m`, `1_to_3y`, `gt_3y` | leveraged / inverse, cost, rolling window |
| 어떤 상품과 운용 복잡도까지 허용하나요? | 광범위 ETF만 / 섹터·테마 ETF까지 허용 / 인버스·레버리지 ETF를 제한적으로 허용 / 높은 회전율·전술 리밸런싱도 허용 | `broad_etf_only`, `sector_theme_allowed`, `inverse_leverage_limited`, `tactical_high_turnover_allowed` | ETF operability, cost, suitability |
| 단순 대안보다 무엇이 더 좋아야 하나요? | 손실이 더 작아야 함 / 수익률이 더 높아야 함 / Sharpe·안정성이 좋아야 함 / 하락장에서 더 잘 버텨야 함 / 특정 자산·섹터·테마 노출이 목적임 | `lower_mdd`, `higher_return`, `better_risk_adjusted`, `better_downside_defense`, `target_exposure` | alternative challenge |
...
| 균형형 | `balanced_core` | 수익과 위험 균형. CAGR / MDD / diversification을 균형 있게 봄 |
| 성장형 | `growth_aggressive` | 성장과 upside 우선. 높은 equity exposure는 허용하되 overfit / tail risk는 계속 확인 |
| 전술 / 헤지형 | `hedged_tactical` | hedge 또는 tactical exposure. inverse / cash / bond role과 cadence mismatch를 더 강하게 봄 |
| 사용자 지정 | `custom` | 사용자가 답변으로 threshold / weight를 직접 구성 |

profile로 무력화하면 안 되는 invariant hard blocker는
사용자가 공격형 / 성장형 profile을 골랐더라도 자동 통과로 바꾸면 안 되는 치명적 문제다.
쉽게 말하면 "위험을 감수한다"와 "검증 자체가 깨졌다"는 서로 다르다.
...
검증용 성과 측정 구간이다.
예를 들어 36개월 rolling window는 3년 단위 성과 구간을 한 달씩 밀면서
CAGR, MDD, Sharpe, benchmark 대비 성과가 여러 시점에서도 유지되는지 확인한다.

profile별 기본 rolling window는 아래처럼 시작한다.

| profile | 기본 rolling window | 해석 |
|---|---:|---|
| 방어형 | 24개월 | 짧은 구간에서도 하락 방어가 깨지는지 빠르게 확인한다 |
| 균형형 | 36개월 | 중기 운용 안정성과 benchmark 대비 균형을 본다 |
| 성장형 | 60개월 | 성장형은 짧은 부진이 잦으므로 긴 cycle에서 우위가 유지되는지 본다 |
| 전술 / 헤지형 | 24개월 | 국면 대응형은 짧은 window에서도 역할이 확인되어야 한다 |
| 사용자 지정 | 36개월 기본 | 사용자가 별도 지정하지 않으면 균형형 기준을 쓴다 |

rolling step은 MVP에서 1개월을 기본으로 둔다.
12개월 rolling return은 보조 단기 지표로 계산할 수 있지만,
route 판정의 기본 window는 profile별 값을 우선한다.

권장 MVP:

- monthly return으로 profile별 rolling return과 12개월 보조 rolling return 계산
...
```text
one_way_cost_bps = user_input or default_one_way_cost_bps
...
기본 `cost assumption`은 거래 수수료 / 거래비용 가정이다.
여기에는 증권사 수수료뿐 아니라 bid-ask spread, slippage, 세금성 비용처럼
실제 체결에서 백테스트보다 성과를 깎을 수 있는 비용을 보수적으로 포함한다.

MVP 기본값은 균형형 기준 `one-way 10 bps`로 둔다.
`10 bps`는 0.10%이며, `one-way`는 매수 또는 매도 한쪽 거래 1회 기준이다.
따라서 같은 금액을 매수한 뒤 나중에 매도하면 단순 왕복 기준으로 약 20 bps 비용을 가정한다.

초기 구현에서는 profile별로 비용 숫자를 과도하게 다르게 두기보다
데이터가 부족한 경우 기본 `one-way 10 bps`를 사용하고,
방어형 / 저비용 장기 운용 profile에서는 cost drag 해석을 더 엄격하게 둔다.
ETF별 expense ratio, turnover, average dollar volume, spread coverage가 붙으면
이 기본값을 component별 / 전략별 비용 모델로 보정한다.
...
`sensitivity perturbation grid`는 최적 조합을 다시 찾는 grid search가 아니라,
현재 후보의 설정을 조금 흔들어도 결과가 크게 무너지지 않는지 확인하는
검증 시나리오 목록이다.
...
| 대상 | perturbation | 해석 |
|---|---|---|
| 모든 후보 | start / end를 일부 이동하거나 최근 3년 / 5년 window를 비교 | 특정 시작일 / 종료일에만 맞는 결과인지 확인 |
| Mix | component weight를 +/- 5%p 조정하고 나머지 weight는 비례 보정 | 60:40 같은 한 점에서만 좋은지 확인 |
| Mix | drop-one component test 후 남은 component weight를 100%로 재정규화 | 특정 component 하나에 과도하게 의존하는지 확인 |
| GTAA | interval, moving average window, top/bottom selection, rebalance day perturbation | cadence / signal window에 과도하게 맞춘 결과인지 확인 |
| Equal Weight | rebalance frequency, ticker subset / sector grouping perturbation | 구성 ticker나 sector 묶음에만 의존하는지 확인 |
| GRS / Relative Strength | lookback window, top_n, skip period perturbation | momentum window와 selection count에 과도하게 의존하는지 확인 |

MVP 판정은 `PASS / REVIEW / NOT_RUN` 중심으로 시작한다.
perturbation runner가 없으면 `NOT_RUN`이고, 주변 설정이 대부분 무너지면 `REVIEW`다.
명백한 대규모 best-only search인데 OOS / sensitivity evidence가 전혀 없을 때만
`BLOCKED` 후보로 검토한다.
...
- 원본 run_history 파일 자체는 validation row에 복사하지 않는다.
- Practical Validation 실행 시 local run_history를 읽을 수 있으면 요약값만 선택적으로 저장한다.
- 같은 source title / strategy family의 recent local run count를 표시한다.
- compare universe에서 selected rank를 표시한다.
- selected source가 best-only인지, mix construction인지 표시한다.

권장 `overfit_audit` shape:
...
```json
{
  ...
  "run_history_window": "recent_local_session_or_user_selected_range",
  "interpretation": "관련 실험 횟수가 많아 sensitivity 검증을 강화해야 합니다."
}
```

`run_history`는 worktree별 local generated artifact라서 commit 대상이 아니다.
따라서 `trial_count`는 공식 성과 지표가 아니라
선택 과정이 얼마나 많이 탐색됐는지를 보여주는 audit disclosure로 다룬다.
...
| 판정 기준 | SPY, QQQ, 60/40 proxy, cash-aware baseline 대비 CAGR / MDD / Sharpe / recovery / cost drag. All Weather-like proxy는 후속 |
...
후보가 방어형 / 저손실 / 현금 대기 성격을 주장할 때
단순히 현금을 섞은 대안보다 복잡성이 보상되는지 확인한다.

`All Weather-like proxy`는 Bridgewater 원본 전략을 복제한다는 뜻이 아니라,
주식 / 채권 / 금 / 원자재류를 섞은 regime 분산형 대리 비교군이다.
ETF 구성과 비중 가정에 따라 결과가 달라질 수 있으므로 MVP에서는 후속으로 둔다.
...
- SPY, QQQ, 60:40 proxy, cash-aware baseline 대비 복잡성이 보상되는지 challenge table을 만든다.
- All Weather-like proxy는 ETF / weight assumption을 별도 확정한 뒤 후속으로 붙인다.
...
- 모든 후보는 start / end 이동, 최근 3년 / 5년 등 주요 window perturbation부터 시작한다.
- mix는 weight +/- 5%p, component drop-one부터 시작하고 남은 weight를 100%로 재정규화한다.
- strategy-specific perturbation은 GTAA / Equal Weight / GRS처럼 기존 runtime이 이미 지원하는 설정 범위 안에서만 실행한다.
- run_history 기반 trial count는 원본 파일을 저장하지 않고 validation row 안의 local audit summary로만 선택적으로 남긴다.

주의:

- 대규모 parameter search engine은 별도 설계가 필요하다.
- DSR / PBO는 후속 고급 검증으로 둔다.
- run_history 자동 매칭이 불확실하면 과감하게 `NOT_RUN` 또는 `REVIEW`로 둔다.
...
| `.aiworkspace/note/finance/docs/flows/BACKTEST_UI_FLOW.md` | flow 문서 갱신 |
...
```text
Validation Profile
  - 방어형 (conservative_defensive)
  - 균형형 (balanced_core)
  - 성장형 (growth_aggressive)
  - 전술 / 헤지형 (hedged_tactical)
  - 사용자 지정 (custom)
```

초기 default 예시:

| 항목 | 방어형 | 균형형 | 성장형 | 전술 / 헤지형 | 사용자 지정 |
|---|---:|---:|---:|---:|---:|
| minimum history | 60 months | 60 months | 72 months | 36 months | profile 답변 기준 |
| rolling window | 24 months | 36 months | 60 months | 24 months | 36 months 기본 |
| rolling step | 1 month | 1 month | 1 month | 1 month | 1 month |
| benchmark win rate review line | 55% | 50% | 45% | 목적별 비교 | 사용자 지정 |
| one-way cost assumption | 10 bps | 10 bps | 10 bps | 10 bps | 사용자 입력 또는 10 bps |
| cost interpretation | 엄격 | 보통 | 보통 | turnover / slippage 엄격 | 사용자 지정 |
| concentration review | max weight > 60% | max weight > 75% | max weight > 90% | hedge 목적 기준 | 사용자 지정 |
| equity exposure review | > 70% | > 85% | > 95% | hedge 목적 기준 | 사용자 지정 |
| MDD review line | < -15% | < -25% | < -35% | 목적별 stress 기준 | 사용자 지정 |
...
사용자 화면에서는 "검증 기준으로도 완화할 수 없는 차단 사유"처럼 설명한다.
...
성장형 / 공격적 profile에서도 자동 pass로 바꾸지 않는다.
...
| simple baseline curves | price loader / replay helper | SPY, QQQ, 60/40, cash-aware challenge. All Weather-like proxy는 후속 |
...
| O | rolling window 최소 길이는 profile별로 얼마로 둘 것인가? | 기본값은 방어형 24개월, 균형형 36개월, 성장형 60개월, 전술 / 헤지형 24개월, 사용자 지정 36개월로 둔다. rolling step은 1개월이다. |
