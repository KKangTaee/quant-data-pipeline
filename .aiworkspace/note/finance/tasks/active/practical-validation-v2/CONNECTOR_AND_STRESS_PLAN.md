...
2026-05-28 closeout 기준으로 위 조건은 충족됐다.
남은 provider coverage 확대와 strategy-specific sensitivity runtime sweep은 P2 blocker가 아니라 P3 또는 별도 task 후보로 둔다.
...
- [x] provider data가 없는 상태에서도 Practical Validation 화면이 깨지지 않는다.
- [x] `NOT_RUN`이 `PASS`처럼 보이지 않는다.
- [x] provider snapshot과 DB price proxy의 출처가 UI와 result row에 남는다.
- [x] full holdings row, full macro series, full dataframe은 JSONL에 저장하지 않는다.
- [x] Final Review에서 provider gap / stress interpretation을 판단 근거로 읽을 수 있다.
- [x] Selected Portfolio Dashboard가 기존 selected decision row를 계속 읽는다.
- [x] `py_compile`이 통과한다.
- [x] run_history / temp CSV / local generated artifact는 commit하지 않는다.
...
P2 closeout 기준으로 stress interpretation과 sensitivity 해석은 provider / macro context와 연결됐다.
기존 `nyse_asset_profile`과 price history는 coverage gap을 설명하는 bridge / fallback으로 유지한다.
다음 작업은 P3 범위를 정해 Final Review handoff와 Selected Portfolio Dashboard monitoring 연결을 정리하는 것이다.
