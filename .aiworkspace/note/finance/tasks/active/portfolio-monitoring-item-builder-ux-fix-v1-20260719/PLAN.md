...
- drawer footer를 브라우저 가시 영역 안에 유지하되 전체 workbench 높이는 보존한다.
...
- [x] component frame은 자연 높이를 유지하고 drawer panel만 560px로 제한해 body만 scroll되게 한다.
...
## Regression Follow-up — 2026-07-19

- [x] iframe 자체의 560px 강제 축소가 전체 탭을 자르는 회귀를 실패 테스트로 고정했다.
- [x] 동일한 catalog recovery snapshot이 X 닫기 뒤 재적용되는 경로를 stable key로 차단했다.
- [x] 자연 높이 `1,803px` / drawer `560px`, X 닫기·동일 recovery 재전송·재오픈 Browser QA를 완료했다.
