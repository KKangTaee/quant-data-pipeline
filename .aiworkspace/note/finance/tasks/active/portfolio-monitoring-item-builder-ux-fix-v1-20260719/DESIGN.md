...
1. drawer open 동안 Streamlit iframe 높이를 `560px`로 제한한다. drawer는 그 프레임을
   채우고 header/stepper/footer는 고정 row, body만 `overflow-y: auto`로 스크롤한다.
...
4. Python은 drawer step, query, draft의 허용 필드만 정규화해 session에 저장하고 다음
   workspace에 한 번만 포함한다. React는 이 recovery projection으로 초기 state를 만든다.
...
- drawer open frame height는 560px, closed state는 auto measurement를 사용한다.
