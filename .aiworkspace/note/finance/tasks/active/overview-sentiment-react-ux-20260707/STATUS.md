...
- Divergence copy follow-up status: complete. Removed the confusing `지표 합의 상태` framing from React and renamed the section to `엇갈리는 지점`. Moved the three axis cards from metric-definition copy to service-owned current interpretation: CNN headline explains whether headline fear/greed is pushing the read, CNN components explain the internal split, and AAII explains bearish / spread pessimism or neutrality from stored values.
