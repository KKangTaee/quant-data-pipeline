...
- UI는 exact-date DB close를 참고값으로 표시하지만 execution price를 자동 적용하지 않는다.
...
`매수·매도 기록` action은 유형, 거래일, 정수 수량, actual execution price, optional fee, note를 받는다. 거래일 DB close를 참고값으로 표시하고 sell은 거래 전·후 수량을 preview한다.
...
- React eligibility, 입력 validation, close reference, before/after preview, ledger edit/void interaction
