...
- `최신 판단 재확인`은 서버가 최신 source를 다시 확인한 뒤 정확한 Final Review 후보로 이동하며, 계속 추적은 새 selected 판단으로만 해제한다.
- lifecycle lock은 새 replay를 막지만 기존 항목의 `추적 종료`는 원래 selected 판단의 마지막 유효 값으로 종결할 수 있다.
- 오래된 기존 항목도 append-only decision registry 전체 이력에서 requested 판단을 찾는다.
...
- Main-dev integration recheck: Portfolio Monitoring Python `214 passed, 8 subtests passed`, 병합 영향 focused Python `295 passed, 16 subtests passed`, React `37 passed`, TypeScript typecheck와 4개 production build 통과.
- Broad `tests/test_service_contracts.py`는 main-dev 병합 상태에서 `18 failed, 851 passed, 41 subtests passed`이며 master 병합 전·후 실패 목록이 동일하다. 이 task가 추가한 회귀가 아니라 기존 Practical Validation/Sentiment/Final Review/Futures 계약 drift로 분리한다.
