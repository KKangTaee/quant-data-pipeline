...
- 좌표는 5D / 20D가 공유하는 chart bound를 사용한다. 두 horizon의 visible median path와 말일 q25/q75 range로 bound를 한 번 계산해 관측 anchor를 고정하며, 제거된 중간 step q25/q75는 scale 입력에서도 제외한다. 상세 계약은 `CONDITIONAL_PATH_READABILITY_DESIGN.md`의 Stable Coordinate Follow-up을 따른다.
