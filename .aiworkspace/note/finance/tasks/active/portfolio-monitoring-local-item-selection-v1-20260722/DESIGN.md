...
### A. 활성 그룹 항목 상세 projection 선적재 — 채택
...
- 완화: active 항목은 그룹당 최대 10개이고 차트는 항목당 최대 120행이라는 기존 제품
  경계를 그대로 사용한다. 종료 이력은 누적될 수 있으므로 장기 이력이 큰 그룹은 payload와
  초기 조회 비용을 후속 관찰한다.
...
- Browser QA: 서로 다른 두 종목을 선택했을 때 Streamlit rerun 없이 그룹 영역은 유지되고
  개별 추적 제목·차트만 즉시 바뀌는지 확인한다. 항목별 상세 높이가 달라지면 iframe 높이
  재계산으로 바깥 scroll 위치가 조정될 수 있으나 전체 page rerun과 구분한다.
