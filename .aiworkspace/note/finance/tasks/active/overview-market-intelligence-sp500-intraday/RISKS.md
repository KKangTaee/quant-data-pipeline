...
| Yahoo quote endpoint may require cookie / crumb | Use `yfinance.data.YfData` session for quote fast path; direct urllib quote request returned 401 in local probe. |
| Quote fast path could fail or be partial | Keep existing yfinance 5m OHLCV download as fallback and report source / method in job details. |
