...
| 대상 | 기준 위치 | 이유 |
|---|---|---|
| 현재 후보 / 최종 판단 JSONL | `.aiworkspace/note/finance/registries/` | 제품 workflow가 읽는 append-only source-of-truth |
| 사용자가 저장한 portfolio setup | `.aiworkspace/note/finance/saved/` | 재사용 가능한 사용자 입력 데이터 |
| task 진행 로그 | `.aiworkspace/note/finance/tasks/active/<task>/` | 현재 작업 실행 기록 |
| phase 계획 / QA / 완료 문서 | `.aiworkspace/note/finance/phases/active/` 또는 `done/` | phase 단위 관리 기록 |
| local run history / temp artifact | `.aiworkspace/note/finance/run_history/`, `.aiworkspace/note/finance/run_artifacts/` | 보통 커밋하지 않는 runtime 산출물 |
