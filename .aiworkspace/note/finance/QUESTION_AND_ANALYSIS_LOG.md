...
- Analysis result: 사용자가 exact portfolio-basis-date 방식을 선택했다. Today item return은 공통 `basis_date`의 정확한 `flow_adjusted_index - 1`만 사용하고 그 날짜가 null/missing이면 `None`이다. selected-position은 자체 `latest_usable_date` exact row를 유지하며 단순 current/initial 비율과 이전·이후 관측 fallback은 금지한다.
- Follow-up: 최종 리뷰 보정과 signed contribution `+$…` / `-$…`를 포함해 전체 `4/4차` fresh regression·1280/760/420 actual QA를 다시 완료했다. 전체 종목 목록, Portfolio Monitoring UI, DB/ingestion/provider/write 경계는 변경하지 않았다.
...
- Interpreted goal: master의 Market Movers·Portfolio Monitoring·Reference Center와 현재 브랜치의 Economic Cycle·Futures Macro V2·Sentiment PIT를 잃지 않고 검증 가능한 merge 상태로 만든다.
- Analysis result: 충돌은 finance 문서 지도 5개뿐이며, canonical current slot은 `STATUS_MANIFEST`와 task status 기준 Sentiment `2/4차`, latest completed slot은 Portfolio Monitoring 최초 설정 정정이 소유한다. 양쪽 나머지 기록은 recent/parallel/completed로 보존했다.
- Follow-up: focused Python·React·Browser QA와 conflict/staged artifact 검사를 통과했다. broad suite의 비범위 contract drift와 미추적 research/QA 산출물은 merge 범위에서 분리한다.
...
- Follow-up: `2026-07-practical-validation-validation-audit` research에 현행 module / board / gate audit과 구현 가이드를 남겼고, `practical-validation-category-results-v1-20260706`에서 Flow 4를 `카테고리별 검증 결과` 중심으로 재구성했다. selected-route preflight는 `Final Review 이동 요약`으로 분리했고, stress / construction / sentiment gate severity를 후보 특성에 맞게 낮췄다.
...
### 2026-08-17 - confirmed current state와 미래 전환 driver는 별도 gate로 판단한다

- User request: 과거 데이터로 현재 국면을 안정적으로 확정하고, 현재 정보에서 가능한
