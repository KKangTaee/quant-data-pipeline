...
### 2026-07-19 - worktree별 Finance app은 frontend 준비와 process ownership을 한 명령으로 관리한다

- User request: worktree 이름과 port만 바꿔 frontend build, Streamlit 시작과 종료를 한 번에 실행하는 alias 성격의 명령을 요청함.
- Interpreted goal: 새 worktree/merge에서 ignored React build가 빠져 fallback으로 내려가는 일을 막고 여러 port를 안전하게 운영해야 함.
- Analysis result: 개인 executable `qweb`가 전체 `app/web` build package의 source fingerprint와 output을 확인하고, qweb-owned PID/command/cwd/listener가 일치할 때만 종료하는 구조가 적합하다.
- Follow-up: Backtest `build/`와 Overview `component_static/`을 포함한 설치·23개 TDD 검증·runbook을 완료했다. 기존 수동 8521은 해당 terminal에서 한 번 종료한 뒤 `qweb start main-dev 8521`로 전환한다.
