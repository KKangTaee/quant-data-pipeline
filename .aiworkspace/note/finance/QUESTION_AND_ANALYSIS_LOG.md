...
- Analysis result: S&P 500 / Nasdaq-100 / portfolio-watchlist / major-cap earnings는 first-class coverage가 맞다. P0는 official macro/Fed, S&P/major earnings, holidays/OPEX/index rebalance, Treasury auctions를 포함하고, earnings는 official/issuer-confirmed/provider-estimate/cross-checked/not-confirmed 상태를 표시해야 한다. 2차~8차 구현으로 taxonomy contract, expanded official macro, earnings universe authority, market-structure events, service-owned workbench payload, React scaffold, and refresh command boundary를 추가했다.
- Follow-up: 상세 리서치는 `.aiworkspace/note/finance/researches/active/2026-07-events-calendar-scope/`에 남겼다. 다음 구현 차수는 9차 이벤트 레일 / 자료 신뢰 / calendar hover-density 개선이다.
