...
- post-merge 문서 정리는 역할별로 판단한다. workflow 상태는 명시적 사용자 결정, 정규화된 task/phase `STATUS.md`, compact manifest, Roadmap 요약 순으로 확인하고 root log는 handoff로만 읽는다. INDEX에는 current/latest 이력을 쌓지 않고 문서 탐색 구조가 바뀐 경우에만 수정한다.
