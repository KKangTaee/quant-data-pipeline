...
| 13-2 | `phase13-gate-validation-qa-matrix-v1` | `finance-integration-review` | Practical Validation / Final Review / Selected Dashboard gate and severity QA matrix | Next |
...
- Phase 13 inventory를 기준으로 Practical Validation, Final Review, Selected Portfolio Dashboard의 gate / route / severity consistency를 확인한다.
- `NOT_RUN`, stale, partial, missing, `NEEDS_INPUT`, `BLOCKED`, `REVIEW` 상태가 pass처럼 보이지 않는지 QA matrix로 정리한다.
- code defect가 확인되면 별도 implementation task로 분리한다.
