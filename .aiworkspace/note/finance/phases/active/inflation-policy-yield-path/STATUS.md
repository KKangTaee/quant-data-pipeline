...
- 전체 5차 중 1차 데이터 기반·2차 Core PCE/정책/금리 backend·3차 workbench·4차
  조건부 S&P 500 스트레스 완료
- 현재 단위: 5차 독립 침체 위험 모델 대기
...
- 4차 actual Browser QA: desktop hard gate와 390px mobile overflow 0 확인

## Next

5차 침체 모델은 별도 episode/OOS gate로 구현하며 기존 경제 사이클 확률을 재사용하지
않는다. 4차 equity는 official EPS 빈티지와 joint-path validation이 준비될 때까지
