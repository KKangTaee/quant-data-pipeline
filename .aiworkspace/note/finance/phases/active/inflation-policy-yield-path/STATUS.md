...
- actual DB/Browser 재감사 기준 전체 5차 중 4차 equity까지 완료,
  5차 독립 침체 위험 모델 active
...
- 실제 2026-08-03 replay: Core PCE 1개월/Q4/Q4, policy, joint rate와 equity artifact `READY`;
  inflation/policy/rates/reverse/equity component `READY`, 통합 snapshot은 recession만 남아 `LIMITED`
...
- 연말 Q4/Q4 5상태·3개 threshold·5개 다음 발표 민감도, 정책·저항 event probability,
  reverse와 equity는 actual validation을 통과해 공개; 침체만 독립 input/validation gate
  전이라 `NOT_AVAILABLE`
...
- FactSet Earnings Insight의 두 CY 라벨 검증 annual bottom-up EPS 80 release/160 rows를
  적재했다. 77 completed origin의 MAE 6.9258%가 best baseline 7.6929%보다 낮고 OOS
  80% interval coverage 0.8125로 equity가 `READY`다.
...
5차 침체 모델을 별도 episode/OOS gate로 구현하며 기존 경제 사이클 확률을 재사용하지
않는다. 완료 후 동일 snapshot에서 다섯 component와 침체를 최종 통합 검증한다.
