...
| `app/runtime/backtest.py` | UI payload를 DB-backed runtime 실행으로 변환하는 public compatibility facade. ETF / strict runtime family와 shared freshness / real-money helpers를 아직 소유한다 |
...
8A부터 Risk-On Momentum 5D 구현은 `app/runtime/backtest_risk_on_momentum.py`로 이동했지만,
기존 caller compatibility를 위해 `app.runtime.backtest`에서도 같은 runner를 계속 export한다.
...
- `app/runtime/backtest.py`는 public compatibility facade와 shared runtime helper를 유지한다. Strategy family가 커지면 전용 `app/runtime/backtest_*` module로 구현을 옮기고 기존 export를 유지한다.
