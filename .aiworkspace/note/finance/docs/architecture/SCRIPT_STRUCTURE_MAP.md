...
| `app/web/backtest_practical_validation/page.py` | `Backtest > Practical Validation` 5-flow 화면 orchestration. 후보 Source 확인, 검증 기준 설정 / 실전 재검증 실행, 근거 Workbench, 저장 / Final Review 이동, source / validation profile / replay / provider action session state wiring을 소유한다 |
