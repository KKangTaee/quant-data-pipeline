...
| `app/web/ingestion_console.py` | `Workspace > Ingestion` render / session-state boundary. Korean purpose-first job guide, common recent-run summary, symbol preset/source controls, explicit ingestion job scheduling, runtime metadata handoff, and selected collection workbench dispatch for `일상 운영 / 검증 데이터`, `수동 복구 / 진단`, `실행 기록 / 결과`. Run history/detail/log/failure artifact display lives in the records section; price / statement / PIT diagnostic panel rendering remains in the manual / diagnostics section. Read-only diagnostic orchestration is delegated to `app/services/ingestion_diagnostics.py` |
