...
| `app/web/reference_guides.py` | `Reference > Guides`의 task-first Reference Center, Portfolio Selection Journey, flowchart, decision gate, reference drawer render |
| `app/services/reference_guides_catalog.py` | `Reference > Guides`용 Streamlit-free guide catalog. task cards, journeys, status concepts, records map, troubleshooting playbooks를 제공 |
