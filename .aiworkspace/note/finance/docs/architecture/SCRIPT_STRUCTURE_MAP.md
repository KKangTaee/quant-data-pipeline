...
| `app/web/operations_overview.py` | `Operations > Operations Overview` / `Operations Console` render와 Streamlit-free Operations read model. Today action queue, Portfolio Monitoring / System Data Health primary lane, no-live approval / order / auto rebalance boundary를 표시 |
