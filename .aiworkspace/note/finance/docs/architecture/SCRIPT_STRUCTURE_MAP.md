...
| `app/web/components/*/frontend/component_static/` | Backtest Analysis, Practical Validation, Final Review 계열 12개 Streamlit React component의 canonical Vite 배포 산출물. `index.html`과 relative JS/CSS assets를 Git에 포함하므로 clean checkout은 별도 npm build 없이 로드한다. 각 `component.py`는 `component_static/index.html` 존재를 availability 기준으로 사용하며 과거 `frontend/build/`은 사용하지 않는다 |
