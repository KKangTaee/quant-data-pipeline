...
| `app/services/overview_market_intelligence.py` | Compatibility facade for older imports. It re-exports the split `app/services/overview/*` domain services and keeps only a thin `fetch_market_mover_compact_metadata` wrapper for legacy monkeypatchable provider hooks |
