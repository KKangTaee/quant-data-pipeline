...
| `app/web/backtest_analysis_result_workspace.py` | `backtest_analysis_result_workspace_v1`의 current session adapter, validated intent dispatch, React / Python fallback mount를 소유한다. 실행 전에는 result surface를 만들지 않고 stale result는 참고용으로 보존한다 |
| `app/web/components/backtest_analysis_result_workspace/` | Level1 결과용 React/Vite presentation bundle. KPI, strategy/benchmark SVG, current/target holdings, Level1 technical handoff, Level2 questions, evidence, user tables, technical appendix와 760px/ResizeObserver contract만 소유한다 |
...
| `app/services/backtest_analysis_result_workspace.py` | Streamlit-free Level1 result evidence read model. `run_result_id`, lifecycle, technical handoff gate, deduplicated Level2 questions, strategy/benchmark chart, current/target holdings, evidence/user table/technical appendix projection을 계산한다 |
