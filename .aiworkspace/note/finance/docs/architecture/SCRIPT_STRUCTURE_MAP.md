...
| `app/web/overview/market_context_helpers.py` / `market_movers_helpers.py` / `futures_macro_helpers.py` / `sentiment_helpers.py` / `events_helpers.py` | Overview primary tab helper bridge modules. Header/control/refresh branch/snapshot detail/tabpanel glue를 tab별로 소유하며, 아직 남은 low-level compatibility helper body는 필요한 범위에서 `legacy_dashboard.py`를 호출한다 |
...
| `app/web/overview/legacy_dashboard.py` | 구조 분리 후 남은 Overview lower-level helper compatibility surface. Active page / tab entrypoint와 tab-local helper bridge ownership은 `app/web/overview/*`와 `*_helpers.py`로 이동했고, 확인된 unused wrapper는 V9에서 제거했다. 아직 Market Context refresh bar, Market Movers refresh / browser-session auto refresh internals, manual investigation panel, Futures Macro panel internals, Sentiment detail internals, Events row / calendar internals를 보존한다. 수집 action은 `app/jobs/overview_actions.py` facade를 호출한다 |
