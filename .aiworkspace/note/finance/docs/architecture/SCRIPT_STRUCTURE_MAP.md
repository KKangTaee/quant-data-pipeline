...
| `app/web/overview/legacy_dashboard.py` | V1 구조 분리 후 남은 Overview helper compatibility surface. Market Context direct refresh bar, Market Movers refresh / browser-session auto refresh, manual investigation panel, Futures Macro panel helpers, Sentiment detail helpers, Events row / calendar helpers를 보존한다. Active page ownership과 standalone tab wrapper는 `app/web/overview/*`로 이동했고, 확인된 unused wrapper는 V9에서 제거했다. 수집 action은 `app/jobs/overview_actions.py` facade를 호출한다 |
