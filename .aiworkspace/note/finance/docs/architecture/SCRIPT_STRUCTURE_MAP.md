...
| `app/web/components/backtest_analysis_decision_workspace/` | Level1 React/Vite presentation bundle. 고정 질문, Single / Mix entry, 목적별 strategy catalog, new / saved Mix inner mode, decision-first KPI / 이유 / 저장·인계 intent, 760px one-column layout와 ResizeObserver height sync만 소유한다 |
| `app/web/backtest_single_settings_workspace.py` | Single Strategy 공통 Python settings shell. 현재 설정 대상 summary, compact ticker evidence, 밝은 section container와 760px responsive contract를 소유하며 strategy 분류 / Gate / 실행은 소유하지 않는다 |
| `app/web/backtest_single_strategy.py` | `Backtest > Single Strategy` 화면 orchestration, React catalog가 고른 strategy의 prefill / family variant segmented control / summary / form dispatch / latest result 연결. 별도 Strategy dropdown과 Strategy Detail panel은 active flow가 아니다 |
| `app/web/backtest_single_forms/` | Single Strategy의 Equal Weight, GTAA, GRS, Risk Parity, Dual Momentum, Risk-On, Quality / Value 계열 strategy-specific form render. 모든 current form은 `핵심 실행 설정 -> 투자 대상 Universe -> 선택·보유 규칙 -> 비용·위험 기준` 순서와 한국어 실행 CTA를 공유한다. Strict Quality / Value의 전체 ticker, PIT / statement / coverage 원문은 `Universe 근거` disclosure에 둔다 |
