...
| `app/web/today_page.py` | Today B안 presentation과 기존 loader adapter. Overview visual token을 재사용해 시장 결론, 근거·일정, 대표 포트폴리오, 세 owner link를 responsive shell로 렌더링한다. Portfolio는 기존 default group만 읽는 전용 read-only loader를 사용하며 provider fetch나 write action은 소유하지 않는다 |
...
| `app/services/overview/*` | Overview UI-facing domain service implementation modules. `economic_cycle.py` owns v3 current observed state, recent 1/3/6M change, deterministic actual 12M path (duplicate replay는 `updated_at -> id -> model_version`, current row 우선), transition monitor, exact-baseline provisional intramonth comparison and stable LIMITED/ERROR contract, `market_context.py` owns retained cockpit / source confidence composition, `market_movers.py` owns movers / group leadership / breadth / date windows, `events.py` owns market event calendar / macro week lane, `sentiment.py` owns CNN / AAII sentiment, `data_health.py` owns collection ops / ingestion handoff, `why_it_moved.py` owns catalyst links / compact metadata, and `ia.py` owns the closeout guide |
...
| `app/jobs/economic_cycle_refresh.py` | 경제사이클 fail-closed refresh. Overview 수동 액션은 RTDSM 4개 공식 원천만 수집한 뒤 최신 종료 월 confirmed snapshot을 강제 재발행하며, 실패 시 last-good를 유지한다. 기존 17-series overlap·`intramonth_nowcast` 함수는 자동화 호환 경계로 분리해 보존한다 |
