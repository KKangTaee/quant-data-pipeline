...
| `app/web/backtest_final_review.py` | `Backtest > Final Review`의 Practical Validation Gate 통과 후보 Candidate Board priority / review queue, 선택 후보 Decision Cockpit, 최종 선정 / 보류 / 거절 / 재검토 결과 기록 checklist / route guide, Evidence Appendix의 Practical Diagnostics / Look-through / Robustness Lab / Paper Observation / Investability Evidence Packet read-only 확인, 저장된 최종 판단 review ledger / Selected Dashboard handoff / Decision Dossier 화면 render |
