...
| `app/web/overview/market_context.py` / `market_movers.py` / `futures_macro.py` / `sentiment.py` / `events.py` | Overview primary tab entrypoint modules. V1에서는 `legacy_dashboard.py` helper를 호출해 동작을 보존하며, V2에서 탭별 controls / session-state / render orchestration을 이 파일들로 옮긴다 |
