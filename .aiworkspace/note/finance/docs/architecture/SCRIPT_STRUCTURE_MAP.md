...
| `app/web/backtest_final_review/page.py` | `Backtest > Final Review`의 Decision Desk, Practical Validation Gate 통과 후보 stable-key 검토 대상 selector / 후보 비교 상세, 선택 후보 투자 검토서와 통합 `판단 기록`, selected-route gate 통과 후보의 Monitoring handoff, 보류 / 거절 / 재검토 저장, Evidence Appendix의 Practical Diagnostics / Look-through / Robustness Lab / Paper Observation / Investability Evidence Packet read-only 확인, 저장된 최종 선정 review ledger / Selected Dashboard handoff / Decision Dossier 화면 render. Final Review first-read에서는 별도 `Step 1 / Candidate Board`, visible Review Queue, standalone Decision Cockpit과 CNN / AAII 시장심리 패널을 렌더링하지 않는다 |
