...
| `app/web/overview_dashboard.py` | `Workspace > Overview`의 Market Movers, Sector / Industry, Events, Candidate Ops tab render. Events 탭은 저장된 FOMC / earnings event row, event type filter, refresh 버튼을 제공 |
...
| 스크립트 | 관리하는 기능 |
|---|---|
| `app/jobs/ingestion_jobs.py` | `Workspace > Ingestion` 또는 Overview 수동 refresh에서 실행하는 수집 / refresh job wrapper. OHLCV, fundamentals, statement refresh, asset profile, Practical Validation provider snapshot, S&P 500 universe / intraday snapshot, FOMC / earnings calendar job을 표준 `JobResult`로 감싼다 |
...
| `finance/data/market_intelligence.py` | S&P 500 current constituent parsing / 저장, S&P 500 / Top1000 / Top2000 intraday previous-close snapshot 수집 / 저장, Fed 공식 FOMC calendar parsing / 저장, yfinance earnings calendar prototype, Overview market event calendar persistence helper |
