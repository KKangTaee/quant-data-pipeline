...
| `app/web/backtest_practical_validation/workspace_panel.py` | Practical Validation Flow 3 first-read workspace render. Final Review 이동 판단, 먼저 해결할 blocker, compact evidence summary, 조건부 / 후속 근거 expander, React Fix Queue availability, Streamlit fallback을 소유한다 |
...
| `app/web/components/practical_validation_fix_queue/` | Practical Validation Fix Queue React component. Final Review 이동 판단, top fix items, compact review / evidence summary를 read-only card로 렌더링하며 validation execution, gate calculation, provider action, registry persistence, handoff는 Python path가 소유한다 |
