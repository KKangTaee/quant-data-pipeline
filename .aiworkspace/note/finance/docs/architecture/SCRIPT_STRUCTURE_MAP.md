...
| `app/web/backtest_single_strategy.py` | `Backtest > Single Strategy` 화면 orchestration, Strategy dropdown / prefill notice / form dispatch / latest result 연결. 별도 Strategy Detail panel은 active flow가 아니다 |
| `app/web/backtest_single_forms/` | Single Strategy의 Equal Weight, GTAA, GRS, Risk Parity, Dual Momentum, Quality / Value 계열 strategy-specific form render. Strict Quality / Value는 data readiness note, compact preset basis, form-level Price Freshness Preflight와 collapsed advanced contracts를 소유한다 |
...
| `app/web/backtest_practical_validation/page.py` | `Backtest > Practical Validation` current one-shell orchestration. 후보/기준, 최신 replay, 결과/해결 구분, 저장/Final Review 이동의 4단계를 `@st.fragment` interaction boundary 안에서 렌더링하고 Python intent validation / execution / persistence를 소유한다. Final Review route 이동만 app rerun을 사용한다 |
...
| `app/web/components/practical_validation_decision_workspace/` | current Level2 React one-shell. Final Review visual token, 분리된 후보/검증 관점, four-step flow, Step 2 pending, non-empty lane, 5개 category 중 하나를 여는 Step 3 disclosure, 760px single-column, ResizeObserver height sync를 담당하고 intent만 반환한다 |
...
| `app/services/backtest_final_review_decision_brief.py` | Streamlit-free current Final Review projection owner. stored curve → exact-common behavior, measured-only observation / finding / trait, structured Monitoring condition, canonical route capability를 `decision_brief_v1`으로 만들고 chart bulk를 제외한 `decision_brief_snapshot_v1`을 제공한다 |
