...
| `app/web/streamlit_app.py` | Finance Console 목적형 top navigation과 page-level routing. `Research / Portfolio / Data / Help`를 등록하고 Today를 `default=True` browser root `/`로 둔다. 기존 `/overview`, `/institutional-portfolios`, `/backtest`, `/selected-portfolio-dashboard`, `/ingestion`, `/reference` path는 유지한다 |
| `app/web/today_page.py` | Today B안 presentation과 기존 loader adapter. Market Research visual token을 재사용해 시장 결론, 근거·일정, 대표 포트폴리오, 세 owner link를 responsive shell로 렌더링한다. Portfolio는 기존 default group만 읽는 전용 read-only loader를 사용하며 provider fetch나 write action은 소유하지 않는다 |
| `app/services/today.py` | Streamlit-free Today compact read model. Economic Cycle, S&P 500, Futures Macro, Sentiment, Events, default Portfolio Monitoring 결과를 `READY / PARTIAL / UNAVAILABLE / EMPTY` 상태와 명시적 as-of로 투영한다 |
| `app/web/ingestion_console.py` + `app/web/ingestion/*` | `Data > Data Operations` render path. `ingestion_console.py` is a compatibility facade. `app/web/ingestion/page.py` owns the shell / session-state boundary, `registry.py` owns active vs legacy compatibility action metadata, `guides.py` owns Korean purpose-first job guide metadata, `styles.py` owns responsive CSS, `results.py` owns pure result-summary helpers, `dispatcher.py` owns UI action dispatch and read-only diagnostic job wrapping, and `sections.py` owns selected collection workbench renderers. Run history/detail/log/failure artifact display remains in `실행 이력`; price / statement / PIT diagnostics still run through the shared scheduled job path and Streamlit-free diagnostic service |
...
| `app/web/reference_contextual_help.py` | Market Research, Institutional Holdings, Data Operations, Backtest Analysis, Practical Validation, Final Review에 붙는 read-only Reference help expander와 exact item deep link render |
| `app/services/reference_contextual_help.py` | Market Research부터 Final Review까지 주요 workflow 화면의 contextual help catalog, surface lookup helper, Reference item / target / duplicate surface drift report |
| `app/web/overview_dashboard.py` | `Research > Market Research`의 explicit compatibility wrapper. 현재는 기존 `/overview` import path 호환을 위해 `render_overview_dashboard`만 re-export하고 active body는 `app/web/overview/page.py`로 위임한다 |
| `app/web/overview/page.py` | `Research > Market Research` active page shell. title, market session banner, selected-view lazy dispatch를 관리하고 view entry modules로 위임한다 |
| `app/web/overview/navigation.py` | Market Research view navigation constants, query-param slug mapping, `st.pills` fallback selector render, selected-view dispatch helper |
| `app/web/overview/market_context.py` / `market_movers.py` / `futures_macro.py` / `sentiment.py` / `events.py` | Retained Overview entrypoint modules used by Market Research views. Active path는 선택된 view의 user flow order만 소유하고, view-local Streamlit glue는 각 `*_helpers.py`로 위임한다 |
| `app/web/overview/market_context_helpers.py` / `market_movers_helpers.py` / `futures_macro_helpers.py` / `sentiment_helpers.py` / `events_helpers.py` | Market Research view helper modules. Header/control/refresh branch/snapshot detail/viewpanel glue를 view별로 소유하며, action 실행은 `app/jobs/overview_actions.py`, read-model loading은 service/helper boundary를 통해 수행한다 |
| `app/web/overview/components/*` | Market Research active page / view가 쓰는 domain visual component implementation. `common.py`는 visual token / CSS / shared strip, `layout.py`는 session banner, `market_context.py`는 retained cockpit / analog / source confidence / IA closeout, `market_movers.py`는 breadth / refresh status, `events.py`는 macro week / event agenda, `data_health.py`는 Data Operations handoff renderer를 소유한다 |
...
| `app/web/overview_dashboard_helpers.py` | Market Research compatibility helper용 cached market intelligence service wrapper. Market Context, Market Movers, Events, Sentiment, Data Health, IA read model service imports를 제공한다. Candidate Ops overview snapshot helpers는 V9에서 제거했고 Candidate Ops는 current Market Research view가 아니다 |
...
| `app/web/backtest_single_strategy.py` | `Backtest > Single Strategy` orchestration. React catalog가 고른 strategy를 schema-driven settings surface에 연결하고 validated intent를 shared runner로 전달하며 latest result를 보존한다. pending 실행 완료 뒤에는 fragment 전용 rerun이 아니라 app-scope rerun으로 결과를 안정적으로 교체한다. strategy-specific native form dispatch, 별도 Strategy dropdown과 Strategy Detail panel은 active flow가 아니다 |
...
| `app/web/backtest_portfolio_mix_workspace.py` | current Portfolio Mix Python adapter/runtime. intent allow-list와 nonce dedup, same Single settings payload projection, compare runner 순차 실행, weighted result atomic 교체, new-schema save/restore/rerun, distinct Level2 handoff와 fallback을 소유한다. weighted/component bundle의 가격 종목 합집합을 중복 제거해 수동 refresh를 실행하며 refresh 직후 Mix를 자동 실행하지 않는다 |
...
| `app/web/final_selected_portfolio_dashboard.py` | `Portfolio > Portfolio Monitoring` 화면 render. Legacy file name은 Selected Portfolio Dashboard를 유지한다. CNN / AAII market sentiment context overlay, 사용자 monitoring portfolio 생성 / 선택 / soft delete, Final Review selected strategy 추가 / 제거, 활성 direct stock·ETF 가격 최신성 projection과 명시적 refresh event/run-history bridge, strategy별 Snapshot / Monitoring Scenario / recheck readiness / symbol freshness / provider evidence / continuity check / source contract / Monitoring Timeline / Review Signal Policy / Open Issues / optional preflight / recheck comparison / optional Actual Allocation / allocation evidence boundary / Decision Dossier / Audit / 전환 비교 표시. 자격 변경 item의 `최신 판단 재확인` event는 client source를 신뢰하지 않고 서버에서 최신 lifecycle을 다시 읽어 Final Review active source hint와 page route를 설정한다 |
...
| `app/services/backtest_level1_price_freshness.py` | Streamlit-free Level1 공통 가격 최신성 계약. Single/Mix result bundle과 configuration에서 재현에 필요한 종목·날짜 evidence를 합치고 `current / refresh_required / provider_gap / rerun_required` 사용자 행동 상태를 만든다. DB 조회와 ingestion 실행은 소유하지 않는다 |
| `app/services/portfolio_monitoring/decision_lifecycle.py` | Streamlit-free Final Review 판단 lifecycle owner. Append-only row를 `selection_source_id -> source_type/source_id -> decision_id` subject로 묶고 `updated_at -> created_at -> decision_id` 순서의 최신 판단, item-local 실행 잠금, requested/effective decision provenance를 catalog와 replay/read model에 공통 투영한다 |
| `app/services/portfolio_monitoring/price_refresh.py` | Streamlit-free Portfolio Monitoring direct-security 가격 최신화 owner. 선택 그룹의 활성 stock·ETF를 deduplicate하고 최근 완료 NYSE session 대비 stale/missing을 판정한다. 명시 클릭에서 기존 daily OHLCV job을 bounded overlap으로 실행하고 DB freshness를 다시 읽어 success/partial/failed를 구분한다. Selected strategy와 종료 항목은 제외한다 |
...
| `app/services/overview/*` | Market Research UI-facing domain service implementation modules. Internal Overview package name is retained. `economic_cycle.py` owns v3 current observed state, recent 1/3/6M change, deterministic actual 12M path (duplicate replay는 `updated_at -> id -> model_version`, current row 우선), transition monitor, exact-baseline provisional intramonth comparison and stable LIMITED/ERROR contract, `market_context.py` owns retained cockpit / source confidence composition, `market_movers.py` owns movers / group leadership / breadth / date windows, `events.py` owns market event calendar / macro week lane, `sentiment.py` owns CNN / AAII sentiment, `data_health.py` owns collection ops / ingestion handoff, `why_it_moved.py` owns catalyst links / compact metadata, and `ia.py` owns the closeout guide |
...
| 스크립트 | 관리하는 기능 |
|---|---|
| `app/jobs/ingestion_jobs.py` + `app/jobs/ingestion/common.py` | `Data > Data Operations`과 승인된 action facade에서 사용하는 수집 / refresh job wrapper. `ingestion_jobs.py` wraps OHLCV, legacy fundamentals compatibility, EDGAR statement refresh, asset profile, Practical Validation provider snapshot, SEC Form 25 delisting evidence, S&P 500 universe / intraday snapshot, S&P 500 valuation context, quote gap diagnostics, FOMC / macro / earnings calendar jobs as standard `JobResult`; 경제 사이클은 explicit vintage collection과 DB-only current materialization entry를 제공하되 scheduler/UI 진단 panel에는 등록하지 않는다. `app/jobs/ingestion/common.py` owns symbol parsing, normalized result creation, progress event helpers, execution profile resolution, and pipeline status helpers |
| `app/jobs/overview_actions.py` | `Research > Market Research`의 bounded refresh action facade. Internal Overview prefix is retained. Market Research UI 대신 market intraday snapshot, futures OHLCV, events, sentiment, quote-gap diagnostics, browser-session auto refresh, run-history append 호출을 모은다. Market Context refresh bundle은 S&P 500 movers, sentiment, event calendars만 소유하며 Top1000 / Top2000 / Futures refresh는 전용 Market Movers / Futures Macro / Data Operations 흐름에 둔다 |
| `app/jobs/economic_cycle_refresh.py` | 경제사이클 fail-closed refresh. Market Research 수동 액션은 RTDSM 4개 공식 원천만 수집한 뒤 최신 종료 월 confirmed snapshot을 강제 재발행하며, 실패 시 last-good를 유지한다. 기존 17-series overlap·`intramonth_nowcast` 함수는 자동화 호환 경계로 분리해 보존한다 |
| `app/jobs/overview_automation.py` | Market Research market intelligence run-once automation orchestrator. Internal Overview prefix is retained. `standard`, `safe`, `events`, `browser_safe` profile의 cadence, weekday / US market-hours guard, lock, run history metadata를 처리한다. 경제사이클 월중 job은 `safe/standard/broad`에만 등록하고 `browser_safe`에서는 실행하지 않는다 |
...
| `finance/economic_cycle_catalog.py` / `economic_cycle_features.py` / `economic_cycle_labels.py` / `economic_cycle_observed_state.py` | legacy 17-series 역할/변환, leakage-safe 월별 factor, activity/labor+eligible USREC retrospective label과 실물 8개 상태 계약. 현재 공식 국면 authority가 아니라 intramonth·자산 해석 호환 입력으로 유지한다 |
| `finance/economic_cycle_model.py` / `economic_cycle_validation.py` / `economic_cycle_pipeline.py` / `economic_cycle_interpretation.py` | shadow horizon 확률 모델·rolling-origin 호환, snapshot orchestration과 시장 맥락 해석. Pipeline은 RTDSM confirmed monthly publisher를 closed-month rollover 기본값으로 사용하고, legacy exact-artifact 월중 provisional materialization은 별도 run kind로 유지한다 |
| `finance/economic_cycle_realtime_history.py` / `economic_cycle_core_state.py` / `economic_cycle_confirmed_state.py` / `economic_cycle_transition_{dataset,drivers,model,validation,production}.py` | RTDSM `IPT/H/EMPLOY/RUC`의 PIT 수준·3개월 모멘텀, 동일 후보 2회 확인, 1/3/6개월 변화, unrestricted next destination과 다음 3 usable release 전환압력의 production authority. 품질 gate를 통과한 artifact/current snapshot만 fail-closed로 발행한다 |
...
| `finance/data/economic_cycle_vintages.py` / `finance/data/economic_cycle_results.py` | 경제 사이클 raw vintage full/overlap 증분 수집·revision UPSERT와 model artifact/current·replay·intramonth compact snapshot persistence. Snapshot은 observed state, recent changes, transition monitor JSON을 additive field로 보존한다 |
