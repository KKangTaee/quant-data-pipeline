...
`backtest-dev`에서 나온 전략 결과를 main 제품 workflow에 승격하려면 [Strategy Promotion Contract](../../reports/backtests/STRATEGY_PROMOTION_CONTRACT.md)를 먼저 채운다. 이 contract는 새 전략 개발이나 투자 승인이 아니라, Practical Validation source로 넘겨도 되는 evidence와 blocker를 확인하는 handoff 기준이다.
...
0. `backtest-dev -> main-dev` handoff가 필요한 전략이면 Strategy Promotion Contract를 채워 universe, PIT / survivorship, parameter / optimization, OOS / walk-forward, cost / liquidity, replay, generated artifact, known failure, `NOT_RUN` evidence, monitoring trigger를 먼저 확인한다.
1. 전략 성격을 먼저 분류한다.
