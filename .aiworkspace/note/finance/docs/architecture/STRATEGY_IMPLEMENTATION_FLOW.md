...
`backtest-dev`에서 나온 전략 결과를 main 제품 workflow에 승격하려면 [Strategy Promotion Contract](../../reports/backtests/STRATEGY_PROMOTION_CONTRACT.md)를 먼저 채운다. 이 contract는 새 전략 개발이나 투자 승인이 아니라, Practical Validation source로 넘겨도 되는 evidence와 blocker를 확인하는 handoff 기준이다. main-dev Practical Validation은 이후 기존 compact evidence를 Robustness Experiment run-set으로 묶어 OOS / walk-forward / regime / stress / cost / parameter perturbation 근거가 어떤 `robustness_run_set_id`에 속하는지 추적한다.
...
0. `backtest-dev -> main-dev` handoff가 필요한 전략이면 Strategy Promotion Contract를 채워 universe, PIT / survivorship, parameter / optimization, OOS / walk-forward, cost / liquidity, replay, generated artifact, known failure, `NOT_RUN` evidence, monitoring trigger를 먼저 확인한다. 이후 Practical Validation의 Robustness Experiment run-set이 해당 source / promotion contract reference와 frozen parameter set을 compact하게 인용한다.
1. 전략 성격을 먼저 분류한다.
...
- Strategy Promotion Contract가 채워졌고 `BLOCKED` / critical `NOT_RUN`이 해소됐는지 확인
- Practical Validation / Final Review에서 `robustness_run_set_id`와 OOS / walk-forward / regime / stress / cost / parameter perturbation non-PASS evidence를 추적할 수 있는지 확인
...
- Strategy Promotion Contract guide / template when `backtest-dev` handoff is involved
- `WORK_PROGRESS.md`
