...
| `app/services/overview/economic_cycle.py` | `economic_cycle_snapshot`의 최신 월말과 최근 최대 12개월 replay를 `economic_cycle_v3` current observed state, recent change, actual level/momentum path와 transition monitor로 변환한다. Exact model/date intramonth row는 baseline-relative provisional 변화만 제공하고 월말 headline/history를 변경하지 않는다. Legacy probability-only row는 현재 국면으로 복원하지 않고 LIMITED로 남기며 provider fetch, fit, materialization, DB write를 하지 않는다 |
