...
| `finance/data/economic_cycle_results.py` | validation artifact와 current/historical replay compact snapshot의 serialize/UPSERT 경계. 미승인 horizon의 numeric probability를 저장하지 않는다 |
| `finance/economic_cycle_features.py` / `economic_cycle_labels.py` / `economic_cycle_model.py` / `economic_cycle_validation.py` / `economic_cycle_pipeline.py` | strict origin input을 월별 factor/real-economy label로 변환하고 h0/h1/h2 direct model, rolling-origin calibration/publication gate, artifact/current/replay materialization을 수행한다 |
...
| `app/services/overview/economic_cycle.py` | approved `economic_cycle_snapshot`과 최대 121개월 replay history를 JSON-safe compact read model로 변환한다. provider fetch, fit, materialization, DB write를 하지 않으며 `NOT_MATERIALIZED`/`LIMITED`/`ERROR`를 안정적으로 표현한다 |
...
- 경제 사이클은 `collector -> raw vintage DB -> strict as-of loader -> feature/label/model/validation -> approved artifact/snapshot -> Overview service -> React` 순서다. Overview render는 마지막 두 read-only 단계만 사용한다.
