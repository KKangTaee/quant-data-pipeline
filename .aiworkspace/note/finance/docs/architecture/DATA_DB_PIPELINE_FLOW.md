...
| `finance/data/economic_cycle_vintages.py` | 경제 사이클 catalog를 FRED/ALFRED vintage mode로 수집·정규화하고 `(series_id, observation_date, realtime_start, source)` key로 `macro_series_vintage_observation`에 UPSERT한다. 초기 full bootstrap과 마지막 stored `realtime_start`를 overlap하는 series별 증분 수집을 분리하며 API key 부재와 missing value를 명시적으로 보존한다 |
| `finance/data/fred_vintages.py` / `finance/inflation_policy_catalog.py` | generic FRED/ALFRED pagination·release-clock·vintage interval primitive와 독립 inflation-policy 26-series catalog. 경제 사이클 catalog/orchestration을 import하지 않고 `released_at`이 검증된 raw row만 공용 vintage 원장에 UPSERT한다 |
...
| `finance/data/economic_cycle_results.py` | validation artifact와 `current/historical_replay/intramonth_nowcast` compact snapshot의 serialize/UPSERT 경계. 월중 row에는 직전 월말 `baseline_as_of_date`, source coverage, source collection timestamp를 별도 보존하며 기존 월말 row를 backfill하거나 재작성하지 않는다 |
| `finance/economic_cycle_features.py` / `economic_cycle_labels.py` / `economic_cycle_model.py` / `economic_cycle_validation.py` / `economic_cycle_pipeline.py` | strict origin input을 월별 factor/real-economy label로 변환하고 h0/h1/h2 direct model, rolling-origin calibration/publication gate, artifact/current/replay materialization을 수행한다. Pipeline의 explicit provisional scoring만 LIMITED artifact를 계산하며 artifact validation status 자체는 바꾸지 않는다. 월중 materializer는 exact approved artifact와 partial current-month origin을 사용하고 h0 계산 불가 시 write 전에 실패하며, closed-month rollover는 누락된 직전 월말만 append한다 |
...
| `app/services/overview/economic_cycle.py` | `economic_cycle_snapshot`과 최근 최대 60개월 replay history를 JSON-safe compact read model로 변환한다. DB의 더 긴 replay는 보존한다. exact model/date로 직전 월말과 최신 `intramonth_nowcast`를 짝지어 확률·factor delta와 source coverage를 별도 `intramonth` 계약으로 제공하되 monthly history는 변경하지 않는다. 유효 확률과 publication status를 조합해 `VERIFIED/PROVISIONAL/UNAVAILABLE`를 만들며 provider fetch, fit, materialization, DB write를 하지 않는다 |
...
- 경제 사이클 월말은 `collector -> raw vintage DB -> strict as-of loader -> feature/label/model/validation -> approved artifact/current·replay snapshot -> Overview service -> React` 순서다. 월중은 `weekday overlap collector -> closed-month append-only rollover -> exact artifact partial-origin materializer -> intramonth_nowcast row -> exact baseline comparison -> React 별도 점/점선` 순서이며 monthly ribbon/history를 수정하지 않는다. 자산 경로는 별도의 `FRED/EIA/futures/S&P price·actual EPS ingestion -> DB -> economic_cycle_assets/sp500 loader -> pathway engine -> interpretation -> Overview service -> React` read-only 경로이며 모델 확률이나 publication gate를 바꾸지 않는다.
