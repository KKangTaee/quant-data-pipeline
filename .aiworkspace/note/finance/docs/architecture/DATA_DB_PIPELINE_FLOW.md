...
| `finance/data/inflation_policy_results.py` | 검증된 inflation-policy model artifact, 순방향·역산 snapshot, AUTO/USER 저항 정의와 저항 상태 snapshot의 finite JSON 검증·business-key UPSERT 경계 |
| `finance/loaders/inflation_policy.py` | current/replay cutoff의 최신 eligible row와 Core PCE rolling-origin을 재구성할 전체 eligible vintage를 분리해 읽는 DB-only 경계 |
| `finance/inflation_policy_model.py` / `finance/inflation_policy_pipeline.py` | 당시 공개된 vintage로 bridge·ridge·momentum을 학습하고 component별 publication status를 보존한 compact artifact/snapshot을 조립·선택 저장 |
