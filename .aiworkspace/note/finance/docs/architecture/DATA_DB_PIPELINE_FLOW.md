...
- 경제 사이클 공식 월말은 `RTDSM 4-series collector -> source-isolated vintage DB -> PIT loader -> canonical state + two-release confirmation -> unrestricted transition pressure/destination gate -> current·replay snapshot -> Overview service -> React` 순서다. 수동 액션은 직전 완결 월말만 확인·재발행하고 실패 시 last-good를 유지한다. Legacy 17-series 월중은 `overlap collector -> exact-artifact intramonth_nowcast -> exact baseline provisional comparison` 호환 경계로 남으며 공식 headline/ribbon/history를 수정하지 않는다. 자산 경로는 별도의 `FRED/EIA/futures/S&P price·actual EPS ingestion -> DB -> economic_cycle_assets/sp500 loader -> pathway engine -> interpretation -> Overview service -> React` 경로이며 observed-state 계산이나 transition condition을 바꾸지 않는다.
