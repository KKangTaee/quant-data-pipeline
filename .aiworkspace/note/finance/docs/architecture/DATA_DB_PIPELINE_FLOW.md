...
| `finance/economic_cycle_realtime_history.py` | RTDSM 4지표의 expanding robust-scale 장기 shadow 국면을 만들고 independent transition sample gate와 현행 8지표 공통기간 parity를 함께 판정한다. 두 gate가 모두 통과할 때만 후속 model experiment를 허용하며 DB snapshot이나 UI payload를 쓰지 않는다 |
...
- 경제 사이클 월말은 `collector -> raw vintage DB -> strict as-of loader -> feature -> observed-state/current·replay snapshot -> Overview service -> React` 순서다. Shadow label/model/validation artifact는 저장 호환과 별도 연구에 남지만 UI는 사용하지 않는다. RTDSM 연구는 `official XLSX -> shared vintage DB(source-isolated) -> 12M-window loader -> long-history shadow state -> sample+parity audit`에서 끝나며 2026-08-12 `NO_GO_PARITY` 결과 때문에 product snapshot으로 연결하지 않는다. 월중은 `weekday overlap collector -> closed-month append-only rollover -> exact artifact partial-origin materializer -> intramonth_nowcast row -> exact baseline provisional comparison -> React` 순서이며 monthly headline/ribbon/history를 수정하지 않는다. 자산 경로는 별도의 `FRED/EIA/futures/S&P price·actual EPS ingestion -> DB -> economic_cycle_assets/sp500 loader -> pathway engine -> interpretation -> Overview service -> React` read-only 경로이며 observed-state 계산이나 transition condition을 바꾸지 않는다.
