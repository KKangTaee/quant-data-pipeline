...
- Inflation / Policy는 `official source -> raw refresh -> released_at PIT DB -> strict loader -> 독립 model artifact/snapshot -> service -> workbench` 순서다. 현재 1차는 strict loader까지 구현됐으며 기존 경제 사이클 확률·artifact·snapshot은 어느 단계에서도 입력이나 fallback이 아니다.
