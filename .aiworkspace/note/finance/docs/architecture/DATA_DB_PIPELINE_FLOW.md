...
- 경제 사이클 월말은 `collector -> raw vintage DB -> strict as-of loader -> feature -> observed-state/current·replay snapshot -> Overview service -> React` 순서다. Shadow label/model/validation artifact는 저장 호환과 별도 연구에 남지만 UI는 사용하지 않는다. 월중은 `weekday overlap collector -> closed-month append-only rollover -> exact artifact partial-origin materializer -> intramonth_nowcast row -> exact baseline provisional comparison -> React` 순서이며 monthly headline/ribbon/history를 수정하지 않는다. 자산 경로는 별도의 `FRED/EIA/futures/S&P price·actual EPS ingestion -> DB -> economic_cycle_assets/sp500 loader -> pathway engine -> interpretation -> Overview service -> React` read-only 경로이며 observed-state 계산이나 transition condition을 바꾸지 않는다.
