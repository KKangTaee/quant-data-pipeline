...
| Philadelphia Fed RTDSM official XLSX | 경제 사이클 next-transition 연구의 장기 provider-native `IPT/H/EMPLOY/RUC` vintage source. 현행 8지표 제품 국면과 분리해 `source=philadelphia_fed_rtdsm`으로 저장하며 sample+parity gate 전에는 model/UI가 소비하지 않는다 |
...
| Nasdaq Trader / NYSE official holiday calendars | Overview Events 미국 휴장·조기폐장 source. Nasdaq Trader에 요청 연도 표가 없으면 NYSE의 multi-year official schedule로 해당 연도만 보완하고, 연도별 coverage checkpoint를 별도로 저장한다 |
...
| Yahoo / yfinance ticker calendar | Overview Events earnings primary free provider estimate source. 매일 priority universe와 persisted S&P 500 shard를 조회해 `market_event_calendar`에 `EARNINGS` row로 저장하고, row가 없는 ticker는 job result의 `symbol_diagnostics` 및 collection checkpoint에 missing / failure reason을 남긴다 |
| yfinance futures OHLCV | Overview futures data source. 주요 선물 1m / 5m / daily OHLCV를 `futures_ohlcv`에 저장하되 exchange-grade realtime feed로 보지 않는다. 1d / 1m 요청에서 일부 futures symbol이 빈 응답이거나 지나치게 희소한 응답이면 해당 symbol만 2d / 1m으로 한 번 보강 수집하고 run diagnostics에 남긴다. 1m rows are retained for stored-candle chart / diagnostic context; successful 1d ingestion requests ten years and excludes incomplete daily sessions. 수동 daily refresh의 same-date pending session은 `17:15 ET` 이후 stored 2d/5m의 exact `18:00 ET <= bar < 17:00 ET` 구간으로 재구성하며, 17/17일 때만 기존 1d row의 `final_*`에 원자 저장한다. completed resolver는 이 explicit final을 raw daily보다 우선하고 latest-good current plus immutable 5D / 20D forecast history를 `finance_meta.futures_macro_snapshot` / `futures_macro_forecast_history`에 materialize한다. Economic Cycle also reads stored `GC=F` / `DX-Y.NYB`. Freshness is based on stored latest candle versus current time, not hidden as missing data |
...
| `finance/data/futures_market.py`, `finance/data/futures_session_finalization.py` | Overview futures raw OHLCV 수집과 완료 세션 파생 저장 경계. 전자는 Economic Cycle 달러 가격 확인용 `DX-Y.NYB`를 포함한 yfinance symbol preset, 1m/5m/daily UPSERT, empty/sparse fallback, `futures_instrument`·`futures_ohlcv`·`futures_market_monitor_run` diagnostics를 저장한다. `app/services/futures_market_monitoring.py`는 stored 1m candle chart / diagnostic context를 최신 저장 candle 기준으로 읽고 stale 여부를 별도 계산한다. 후자는 최신 daily compact probe, stored 5m exact-window reader, DST-safe aggregate, 17/17 transactional `final_*` writer를 소유하고 `app/jobs/futures_macro_daily_finalization.py`가 수동 refresh에서만 pending-to-final 흐름을 조정한다. successful 1d ingestion 뒤 `app/services/futures_macro_snapshot.py`가 completed-session input으로 10년 current macro 및 5D/20D outlook compact snapshot을 만들고 `finance/data/futures_macro_snapshot.py`가 UPSERT한다. compatible stored input 재실행은 5m 수집과 nested replay를 생략하며 `finance/loaders/futures_macro_snapshot.py`, Overview render, `다시 읽기`는 provider fetch / outlook replay를 실행하지 않는다 |
...
| `finance/economic_cycle_features.py` / `economic_cycle_observed_state.py` / `economic_cycle_labels.py` / `economic_cycle_model.py` / `economic_cycle_validation.py` / `economic_cycle_pipeline.py` | strict origin input을 월별 factor로 변환하고 실물 8개 기반 현재 국면·최근 변화·인접 전환 조건을 계산해 current/replay snapshot에 저장한다. Horizon model과 rolling-origin gate는 shadow artifact 호환으로 유지한다. 월중 materializer는 exact approved artifact와 partial current-month origin을 사용하되 월말 headline을 바꾸지 않으며, closed-month rollover는 누락된 직전 월말만 append한다 |
| `finance/economic_cycle_realtime_history.py`, `finance/economic_cycle_core_state.py`, `finance/economic_cycle_transition_{dataset,model,validation,experiment}.py` | RTDSM 4지표를 과거·현재 공통 canonical state 후보로 만들고 raw 국면 의미·개정·episode 안정성을 먼저 감사한다. 통과한 경우에만 자유 목적지와 다음 3-release 전환압력을 episode 가중치·expanding OOS·baseline/calibration gate로 검증한다. experiment는 writer를 받지 않으며 actual 2026-08-12 `NO_GO_CORE_STATE` 때문에 DB snapshot이나 UI payload를 쓰지 않는다 |
...
| `app/services/overview/economic_cycle.py` / `economic_cycle_asset_freshness.py` | 최신 월말 replay를 observed state, recent change, actual level/momentum path와 transition monitor로 변환하고 `cycle_snapshot`/`asset_pathways` freshness를 분리한다. 자산 일간 series는 최대 5 영업일, EIA 주간 series는 최대 14 달력일 기준으로 판정한다. 지연된 last-good 측정값은 날짜·변화량을 보존하지만 `supports_current_signal=false`로 현재 집계에서 제외한다. UI read path는 provider fetch, fit, materialization, DB write를 하지 않는다 |
...
- 경제 사이클 월말은 `collector -> raw vintage DB -> strict as-of loader -> feature -> observed-state/current·replay snapshot -> Overview service -> React` 순서다. Shadow label/model/validation artifact는 저장 호환과 별도 연구에 남지만 UI는 사용하지 않는다. RTDSM 연구는 `official XLSX -> shared vintage DB(source-isolated) -> 12M-window loader -> canonical core audit -> unrestricted episode dataset/model -> chronological publication gate`의 read-only 경계다. 2026-08-12 actual raw episode 안정성 `NO_GO_CORE_STATE`로 model fit 전에 중단되어 product snapshot으로 연결하지 않았다. 월중은 `weekday overlap collector -> closed-month append-only rollover -> exact artifact partial-origin materializer -> intramonth_nowcast row -> exact baseline provisional comparison -> React` 순서이며 monthly headline/ribbon/history를 수정하지 않는다. 자산 경로는 별도의 `FRED/EIA/futures/S&P price·actual EPS ingestion -> DB -> economic_cycle_assets/sp500 loader -> pathway engine -> interpretation -> Overview service -> React` read-only 경로이며 observed-state 계산이나 transition condition을 바꾸지 않는다.
