...
| `finance/loaders/inflation_policy.py` | current/replay cutoff의 최신 eligible macro row, Core PCE rolling-origin 전체 vintage, official S&P 500 EPS release vintage와 저장 `^GSPC` 가격을 분리해 읽는 DB-only 경계 |
| `finance/inflation_policy_model.py` / `finance/inflation_policy_equity_stress.py` / `finance/inflation_policy_pipeline.py` | 당시 공개된 vintage로 Core PCE와 year-end EPS×multiple response를 시간순 학습하고 component별 publication status를 보존한 compact artifact/snapshot을 조립·선택 저장 |
...
| `app/jobs/inflation_policy_refresh.py` | 독립 macro vintage·SEP·FOMC decision 필수 수집과 BEA/ACM 선택 수집을 bounded 순서로 실행한다. 필수 source와 `PCEPILFE`, `DGS2`, `DGS10`, `DFII10`, `T10YIE` coverage가 없으면 향후 materialization을 fail-closed한다. raw context만 갱신하고 확률 snapshot은 만들지 않는다 |
...
- Inflation / Policy는 `official source -> raw refresh -> released_at PIT DB -> strict/current + all-vintage loader -> 독립 model artifact/snapshot -> service -> workbench` 순서다. 2차 backend는 혼합형 Core PCE, 익명 SEP·실제 표결 policy marginal, 63/252/504일 동적 저항대와 publication gate까지 구현됐다. 기존 경제 사이클 확률·artifact·snapshot은 어느 단계에서도 입력이나 fallback이 아니다.
