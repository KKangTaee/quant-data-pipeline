...
| `finance/data/futures_market.py`, `finance/data/futures_session_finalization.py` | Overview futures raw OHLCV 수집과 완료 세션 파생 저장 경계. 전자는 yfinance symbol preset, 1m/5m/daily UPSERT, empty/sparse fallback, run diagnostics를 저장한다. 후자는 최신 daily compact probe, stored 5m exact-window reader, DST-safe aggregate, 17/17 transactional `final_*` writer를 소유한다. `app/jobs/futures_macro_daily_finalization.py`가 수동 refresh에서만 pending-to-final 흐름을 조정한다. successful 1d ingestion 뒤 `app/services/futures_macro_snapshot.py`가 completed-session input으로 current macro 및 5D/20D outlook compact snapshot을 만들고 `finance/data/futures_macro_snapshot.py`가 UPSERT한다. compatible stored input 재실행은 5m 수집과 nested replay를 생략하며 Overview render/`다시 읽기`는 provider fetch를 실행하지 않는다 |
