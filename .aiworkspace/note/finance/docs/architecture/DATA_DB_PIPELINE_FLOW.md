...
| `app/services/overview/economic_cycle.py` | `economic_cycle_snapshot`과 최대 121개월 replay history를 JSON-safe compact read model로 변환한다. 유효 확률과 publication status를 조합해 `VERIFIED/PROVISIONAL/UNAVAILABLE`를 만들며 provider fetch, fit, materialization, DB write를 하지 않는다 |
