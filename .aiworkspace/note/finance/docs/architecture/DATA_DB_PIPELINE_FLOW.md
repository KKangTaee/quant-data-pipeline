...
| `app/services/overview/us_stock_valuation.py` / `market_context_valuation.py` | 개별주 60m/36m log(PER), FOMC+기업 초과 EPS 성장 시나리오, READY/COLLECTABLE/NOT_APPLICABLE/ERROR read model과 S&P/개별주 failure isolation 경계. Graph 1 P/E readiness와 Graph 2 growth-evidence readiness는 독립적이며 화면 진입은 read-only다 |
