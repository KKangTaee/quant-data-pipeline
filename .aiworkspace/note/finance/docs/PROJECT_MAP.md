...
| Overview Macro Context Cockpit read model | `app/services/overview_market_intelligence.py` via `build_overview_macro_context_cockpit`; rendered by `app/web/overview_ui_components.py` and loaded through `app/web/overview_dashboard_helpers.py`. `context_findings` is the user-facing Market Context findings rail with conclusion / interpretation / evidence / freshness fields; `next_checks` remains a compatibility payload and should not be treated as a user-facing action checklist |
