...
| Practical Validation Flow 3 React component | `app/web/components/practical_validation_fix_queue/`; compatibility path name keeps Fix Queue, but the visible surface is a read-only validation conclusion summary. It shows Final Review movement state plus category-level pass / fail / review and sends detailed cause / module evidence to Flow 4 without owning validation execution or persistence |
...
| `app/web/backtest_practical_validation/workspace_panel.py` | Flow 3 first-read workspace renderer. Final Review move judgment, category-level pass / fail / review conclusion summary, React component availability, and Streamlit fallback을 소유한다 |
