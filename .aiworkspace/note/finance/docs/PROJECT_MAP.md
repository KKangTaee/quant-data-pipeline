...
| Overview dashboard render | `app/web/overview_dashboard.py`; `Workspace > Overview` top-level deep navigation uses a selected-tab lazy renderer, not native eager `st.tabs`. The primary selector uses Streamlit internal `st.pills` state, styled as Korean-first text tabs with an active red underline rather than anchor navigation. Query-param slugs such as `?overview_tab=market-movers` remain compatibility input, but the page does not render tab `<a href>` links. `Market Context` is the default selected surface; fresh Overview entry defers its heavy cockpit body behind `시장 맥락 불러오기`. Primary Overview tabs are Market Context, Market Movers, Sentiment, and Events. `Futures Monitor` and `Sector / Industry` standalone tabs are soft-removed from primary navigation; futures / sector evidence remains available inside Market Context read models and hidden helper code until a later cleanup decision. Data Health is handled through Market Context source / refresh evidence plus Operations / Ingestion ownership; Candidate Ops is not rendered in Overview |
