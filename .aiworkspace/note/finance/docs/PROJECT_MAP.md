...
| `app/web/final_selected_portfolio_dashboard.py` | Selected Portfolio Dashboard screen render, portfolio card shelf / 생성 / 선택 / soft delete, Final Review selected strategy slot 추가 / 설정 적용 / 제거, portfolio-level Monitoring Scenario summary / value curve / strategy performance / rebalance target table, strategy별 Monitoring Scenario, continuity / Monitoring Signals / Open Issues / optional preflight / allocation monitoring / Decision Dossier / 하단 evidence detail |
...
- Selected Portfolio Dashboard는 사용자가 만든 monitoring portfolio card shelf를 먼저 보여주고, 선택한 portfolio 안에 Final Review selected strategy slot을 담아 start / latest-end mode / balance / memo를 저장한다. Monitoring Scenario 실행은 slot 저장과 분리되며, portfolio-level 현재 가치 / 손익 / 수익률 / CAGR / MDD / benchmark spread / rebalance target을 먼저 보여준 뒤 read-only recheck readiness / symbol freshness / provider evidence / continuity / timeline / review signal / open issue / optional deployment preflight / allocation boundary를 하단 상세로 확인한다. Deployment Readiness preflight도 승인 / 주문 / broker-account 연동 / 자동 리밸런싱을 만들지 않는다.
