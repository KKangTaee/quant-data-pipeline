...
| Overview dashboard render | `app/web/overview_dashboard.py`; `Workspace > Overview` top-level deep navigation uses a selected-tab lazy renderer, not native eager `st.tabs`. `Market Context` is the default selected surface. Primary Overview tabs are Market Context, Market Movers, Futures Monitor, Sentiment, Sector / Industry, and Events. Data Health is handled through Market Context source / refresh evidence plus Operations / Ingestion ownership; Candidate Ops is not rendered in Overview |
