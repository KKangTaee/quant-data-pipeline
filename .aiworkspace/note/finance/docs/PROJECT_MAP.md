...
| Practical Validation | `app/web/backtest_practical_validation/page.py`; includes contextual Reference help entry point and orchestrates the 5-flow Practical Validation screen |
| Practical Validation UI components | `app/web/backtest_practical_validation/components.py` |
| Practical Validation workspace panel | `app/web/backtest_practical_validation/workspace_panel.py`; owns Flow 3 first-read workspace, Fix Queue, core evidence group render, and React fallback boundary |
...
| Practical Validation Fix Queue React component | `app/web/components/practical_validation_fix_queue/`; read-only Fix Queue / review count / core evidence card surface |
...
| `app/web/backtest_practical_validation/workspace_panel.py` | Flow 3 first-read workspace renderer. Gate fix guidance, Fix Queue display rows, core / conditional / downstream evidence groups, React Fix Queue component availability, and Streamlit fallback을 소유한다 |
...
| `app/web/components/practical_validation_fix_queue/` | Practical Validation Fix Queue React component. Read-only component surface만 담당하며 validation execution, gate calculation, persistence, provider action, handoff는 Python path가 소유한다 |
