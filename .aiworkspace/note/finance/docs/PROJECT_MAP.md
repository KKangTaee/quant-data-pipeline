...
| Practical Validation workspace read model service | `app/services/backtest_practical_validation_workspace.py` |
...
| Practical Validation workspace panel | `app/web/backtest_practical_validation/workspace_panel.py`; owns Flow 3 single first-read workspace, Final Review move judgment, first fix work, compact evidence summary, and React fallback boundary |
...
| Practical Validation Fix Queue React component | `app/web/components/practical_validation_fix_queue/`; read-only Final Review move judgment / top fix items / compact evidence summary surface |
...
| `app/services/backtest_practical_validation_workspace.py` | Streamlit-free screen-oriented workspace read model. Practical Validation result에서 gate summary, Fix Queue, core / conditional / downstream evidence groups, technical details를 묶어 Flow 3 / Flow 4 화면이 같은 contract를 읽게 한다 |
...
| `app/web/backtest_practical_validation/workspace_panel.py` | Flow 3 first-read workspace renderer. Gate fix guidance, Final Review move judgment, first fix work, compact core evidence summary, conditional / downstream evidence groups, React Fix Queue component availability, and Streamlit fallback을 소유한다 |
...
| `app/web/components/practical_validation_fix_queue/` | Practical Validation Fix Queue React component. Read-only Final Review move judgment / top blocker / compact evidence summary surface만 담당하며 validation execution, gate calculation, persistence, provider action, handoff는 Python path가 소유한다 |
...
- Practical Validation은 source를 실전 투입 전 조건으로 검증하고 source traits 기반 module planner로 필수 / 조건부 / 후속 참고 검증과 Final Review 이동 gate를 만든다. 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 3은 workspace read model을 통해 Final Review 이동 가능 여부, 먼저 해결할 blocker, compact evidence summary를 단일 first-read surface로 먼저 보여주고, Flow 4의 audit board, provider board, Robustness Lab, Applied Validation Map은 board registry를 통해 어떤 module의 evidence인지 표시한다. `검증 결과 저장(기록용)`은 audit trail만 남기고, Gate 미통과 result는 Final Review 후보가 아니다.
