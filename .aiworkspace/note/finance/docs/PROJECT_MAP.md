...
| Overview dashboard render | `app/web/overview_dashboard.py`; `Workspace > Overview` top-level deep navigation uses a selected-tab lazy renderer, not native eager `st.tabs`. `Market Context` is the default selected surface. Market Movers, Futures Monitor, Sentiment, Sector / Industry, Events, Data Health, and Candidate Ops render only after selection; Candidate Ops loads the shared dashboard snapshot inside its selected branch |
