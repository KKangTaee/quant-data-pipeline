...
- Practical Validation은 source를 실전 투입 전 조건으로 검증하고 source traits 기반 module planner로 필수 / 조건부 / 후속 참고 검증과 Final Review 이동 gate를 만든다. 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 3은 workspace read model을 통해 Final Review 이동 가능 여부와 `Final Review 이동을 막는 이슈`를 단일 first-read surface로 먼저 보여주며, 각 이슈는 `현재 문제 / 완료 기준 / 보강 위치 / 영향`으로 읽는다. Flow 4는 같은 read model의 criteria detail groups를 `Final Review로 넘기기 전 확인 기준` board로 보여주고, 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 먼저 요약한 뒤 audit board, provider board, Robustness Lab, Applied Validation Map을 board registry 기준으로 이어서 표시한다. `NEEDS_INPUT` / `NOT_RUN` 같은 raw status는 기술 tag로 남기며, `검증 결과 저장(기록용)`은 audit trail만 남기고 Gate 미통과 result는 Final Review 후보가 아니다.
