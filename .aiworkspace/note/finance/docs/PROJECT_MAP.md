...
| Overview historical analog service | `app/services/overview_market_context_analog.py`; sector leadership -> sector ETF proxy -> SPY-relative historical distribution read model. Supports selected as-of bounded replay and 5D / 20D / monthly pattern windows using existing DB prices plus current universe / sector metadata. The read model exposes requested vs effective as-of alignment, limiting symbols, and basis warnings when common daily price coverage is older than the selected date. `app/web/overview_dashboard.py` places historical analog controls immediately before the analog analysis flow, and `app/web/overview_ui_components.py` renders the result as a basis bar, method grid, summary / caution split, and separate Macro comparison section. 3차-A adds an additive `macro_conditioned_analog` pilot using GLD price proxy context; 3차-B adds one stored futures daily OHLCV Rate Pressure proxy condition using `ZN=F` / `ZB=F`; 3차-C adds `macro_dimension_audit` for stored FRED `T10Y3M` / `VIXCLS` / `BAA10Y` availability / bucket preview and event / sentiment annotation or deferred status. FRED rates / events / sentiment hard conditioning remain disabled or excluded |
