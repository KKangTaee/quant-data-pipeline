...
| Overview Macro Context Cockpit read model | `app/services/overview_market_intelligence.py` via `build_overview_macro_context_cockpit`; rendered by `app/web/overview_ui_components.py` and loaded through `app/web/overview_dashboard_helpers.py`. `brief_rows` is the user-facing Market Context brief sequence for movement, breadth, Futures/Macro backdrop, and optional Events background. Events / data-source limitations are absorbed into these brief rows only when they change the market-context interpretation; detailed source / freshness state remains in source confidence. `context_findings` / `next_checks` remain compatibility payloads and should not be rendered as a default user-facing action checklist |
