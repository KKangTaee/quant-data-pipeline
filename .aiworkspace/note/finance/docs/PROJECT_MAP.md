...
| Practical Validation workspace read model service | `app/services/backtest_practical_validation_workspace.py`; includes Flow 3 summary and Flow 4 criteria detail groups |
...
| Practical Validation workspace panel | `app/web/backtest_practical_validation/workspace_panel.py`; owns Flow 3 single first-read workspace, Final Review move judgment, first fix work, compact criteria summary, user-facing blocker explanation fields, and React fallback boundary |
...
| Practical Validation Fix Queue React component | `app/web/components/practical_validation_fix_queue/`; read-only Final Review move judgment / top fix items / next step / compact criteria summary surface. It shows what was checked, what is missing, what to do next, and technical status tags without owning validation execution or persistence |
...
| `app/services/backtest_practical_validation_workspace.py` | Streamlit-free screen-oriented workspace read model. Practical Validation result에서 gate summary, Fix Queue, criteria detail groups, user-facing module explanations, core / conditional / downstream evidence groups, technical details를 묶어 Flow 3 / Flow 4 화면이 같은 contract를 읽게 한다 |
...
| `app/web/backtest_practical_validation/page.py` | Practical Validation UI orchestration for 5-flow screen: candidate/profile check, practical validation execution, workspace panel, evidence workbench, save / Final Review move. It keeps source selection, validation profile, replay execution, provider action, session state, save-only audit copy, and Final Review handoff wiring |
...
| `app/web/backtest_practical_validation/workspace_panel.py` | Flow 3 first-read workspace renderer. Gate fix guidance, Final Review move judgment, first fix work, compact criteria summary, React Fix Queue component availability, and Streamlit fallback을 소유한다 |
...
| `app/web/components/practical_validation_fix_queue/` | Practical Validation Fix Queue React component. Read-only Final Review move judgment / top blocker / next step / compact criteria summary surface만 담당하며 validation execution, gate calculation, persistence, provider action, handoff는 Python path가 소유한다 |
...
- Practical Validation은 source를 실전 투입 전 조건으로 검증하고 source traits 기반 module planner로 필수 / 조건부 / 후속 참고 검증과 Final Review 이동 gate를 만든다. 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 3은 workspace read model을 통해 Final Review 이동 가능 여부, 먼저 해결할 blocker, next step, compact criteria summary를 단일 first-read surface로 먼저 보여주고, blocker는 `무엇을 검증했나 / 부족한 점 / 해야 할 일 / 왜 중요한가`로 설명한다. Flow 4는 같은 read model의 criteria detail groups를 `Final Review로 넘기기 전 확인 기준` board로 보여준 뒤 audit board, provider board, Robustness Lab, Applied Validation Map을 board registry 기준으로 이어서 표시한다. `NEEDS_INPUT` / `NOT_RUN` 같은 raw status는 기술 tag로 남기며, `검증 결과 저장(기록용)`은 audit trail만 남기고 Gate 미통과 result는 Final Review 후보가 아니다.
