...
- Workspace > Institutional Portfolios는 Overview Market Movers와 분리된 read-only research surface로, 투자 대가 / 기관별 delayed SEC Form 13F portfolio와 분기별 reported change를 탐색한다.
...
- Institutional Portfolios는 SEC Form 13F 공식 data sets를 DB로 적재한 뒤 기관별 holdings, 신규 / 증가 / 감소 / 전량 매도 후보, 섹터 노출, 종목별 보유 기관 reverse lookup을 읽는 Workspace research surface다. 13F 지연, long holdings 한계, CUSIP-symbol mapping caveat를 항상 표시하며 추천 / 매수매도 신호 / live approval로 해석하지 않는다.
