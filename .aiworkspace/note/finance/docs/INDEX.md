...
- Current product state: recent merged work is grouped as Overview / Market Context, Backtest Analysis, Practical Validation / Final Review, Operations / Portfolio Monitoring, and UI / Engine Boundary. Overview primary tabs are `Market Context`, `Market Movers`, `Futures Macro`, `Sentiment`, and `Events`. `Market Context` now renders two React valuation surfaces: Shiller-only 60-month log(PER) relative zones with 36-month sensitivity, and FOMC SEP GDP+PCE-based EPS plus SPX scenario bands. Graph 2 prefers four official S&P actual As-Reported quarters and transparently falls back to the latest Robert Shiller TTM EPS when official rows are unavailable; source quality and basis dates remain visible. `Futures Monitor` / `Sector / Industry` are not primary navigation surfaces. See [Roadmap](./ROADMAP.md).
