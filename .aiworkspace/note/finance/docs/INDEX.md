...
- Latest completed task: [Economic Cycle Asset Context V1 2026-07-16](../tasks/active/overview-economic-cycle-asset-context-v1-20260716/STATUS.md) — 정적 `시장의 다음 질문`을 네 canonical factor 기반 `자산별 확인 포인트`로 교체했다. 채권·금리/주식/금·달러/원자재를 `우호·부담·혼재·자료 부족`으로 구분하고 상위 근거 두 개와 바뀌는 조건을 2×2 카드로 표시한다. 자산 수익률 예측이나 매매 신호는 만들지 않는다.
