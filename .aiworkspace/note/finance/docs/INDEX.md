...
- Current active task: [US Stock Freshness Refresh V1 2026-07-15](../tasks/active/overview-market-context-us-stock-freshness-refresh-v1-20260715/STATUS.md) — written spec review 단계. 선택 종목 상단의 단일 `최신 데이터로 다시 계산` action으로 마지막 완료 NYSE 거래일, 시장가치/가격 정렬, 실제 재무 raw gap을 판정하고 exact selected-symbol scope만 수집한다. profile/price는 CIK 없이 실행하고 SEC statement만 CIK identity를 요구한다.
