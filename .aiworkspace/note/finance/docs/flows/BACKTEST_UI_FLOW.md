...
- `검증 신호 · Policy Signals`: result tab 안에서 compact summary, entry criteria board, grouped signal board, hard blocker 상세만 먼저 보여준다. Practical Validation review focus rows는 1차 처리 항목이 아니라 source evidence로 저장하고, `1차 처리 결과`에는 2차에서 확인할 count / handoff notice만 표시한다. Benchmark / execution / raw technical payload는 접힌 상세 근거로 낮추며 nested `현재 판단 / 검토 근거 / 실행 부담 / 기술 상세` 탭은 active render path에서 사용하지 않는다.
