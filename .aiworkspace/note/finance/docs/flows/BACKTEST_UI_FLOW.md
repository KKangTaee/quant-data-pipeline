...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, 표시 전용 `데이터 보강 대상` React board와 기존 Python 수집 버튼을 묶은 `데이터 보강 / 수집 실행` action center, 접힌 `상세 근거 / 원자료`, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. Flow 2에서 현재 세션 replay가 없으면 Flow 3 / Flow 4 / Flow 5와 결과 JSON을 렌더링하지 않는다. Flow 3 / Flow 4 visible UI는 적용된 REVIEW-only Practical Validation category를 숨기지 않고 `데이터 주의`, `2단계 실용성 주의`, `최종 판단 참고`, `Monitoring 추적` role로 구분한다. Final Review / Monitoring 전용 항목은 first-read action issue가 아니라 보조 참고로 낮추며, 단계별 소유권 inventory는 내부 read model에만 남긴다. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
