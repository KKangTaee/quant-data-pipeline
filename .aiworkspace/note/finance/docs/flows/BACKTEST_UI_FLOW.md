...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 사용자는 profile과 5개 답변을 고르고, 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Final Review 이동 판단, 먼저 해결할 blocker, 다음 단계, compact 기준 요약을 단일 first-read surface로 보여준다. 먼저 해결할 blocker는 `무엇을 검증했나 / 부족한 점 / 해야 할 일 / 왜 중요한가`를 보여주고 raw status는 `기술 기준` tag로 낮춘다. Flow 4는 `Final Review로 넘기기 전 확인 기준` board를 먼저 보여준 뒤 Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center를 담고, Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 `Final Review 이동 전 검증 상태`를 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
