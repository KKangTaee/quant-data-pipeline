...
| `app/services/overview/*` | Streamlit-free Overview service modules. `sp500_valuation.py`는 60m/36m log(PER), 대칭 2σ anchor, FOMC EPS/current SPX scenario, 과거 SEP vintage 기반 1년 rolling reconstruction read model을 소유한다. React는 hover와 표시만 담당한다. `market_movers.py`, `why_it_moved.py`, `events.py`, `sentiment.py`가 다른 current tab read model을 소유하며 `market_context.py` legacy cockpit service는 retained compatibility code다 |
