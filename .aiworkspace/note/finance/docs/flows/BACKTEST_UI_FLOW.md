...
| `app/web/backtest_practical_validation.py` | `Practical Validation` stage render. Clean V2 source 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, `Final Review Gate / 검증 모듈`, `검증 근거 보드`, `보강 액션`, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당 |
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 `4. Final Review Gate / 검증 모듈`에서 module list와 Applied Validation Map을 보여주고, 각 evidence / action board title 아래에 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
