...
| `app/web/final_selected_portfolio_dashboard.py` | `Operations > Selected Portfolio Dashboard` 화면 render. Final Review selected 후보 pool과 사용자-created monitoring portfolio setup을 읽고, 나의 포트폴리오 생성 / 선택 / soft delete, selected 전략 추가 / 제거, strategy별 Monitoring Scenario, Monitoring Signals, 전환 비교, optional preflight, Actual Allocation / allocation evidence boundary, Decision Dossier / Audit을 보여준다 |
| `app/web/final_selected_portfolio_dashboard_helpers.py` | Selected Portfolio Dashboard의 dashboard portfolio / selected strategy pool / strategy comparison table, handoff table, component / continuity / source contract / recheck preflight / recheck readiness / symbol freshness / provider evidence / open issue follow-up / deployment readiness / recheck comparison / value / holding input / drift / alert preview / allocation boundary / filter helper. Evidence table은 service read model을 표시한다 |
...
- `Selected Portfolio Dashboard`: `SELECT_FOR_PRACTICAL_PORTFOLIO` selected row만 운영 후보로 읽는다. Dashboard는 Final Review의 `open_review_items`와 review trigger를 `Open Issues` tab에서 보여주고, `deployment_readiness_policy_snapshot`과 recheck / provider / continuity / review signal / allocation boundary evidence를 `Deployment Readiness` tab에서 read-only preflight로 묶는다. 이 preflight는 실제 돈 투입 승인, 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. 통과 후보가 없으면 dashboard row를 만들지 않고 empty state로 남긴다.
...
| `app/web/final_selected_portfolio_dashboard.py` | Operations dashboard 화면 render, 나의 포트폴리오 생성 / 선택 / soft delete, Final Review selected strategy pool에서 추가 / 제거, strategy별 Snapshot / Monitoring Scenario / readiness + symbol freshness + provider evidence + setup + result tabs, Continuity check, Monitoring Timeline / Review Signals / Open Issues / Why Selected / optional Actual Allocation / allocation evidence boundary / optional preflight / Decision Dossier / Audit / 전환 비교 표시 |
