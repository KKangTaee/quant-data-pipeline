...
| `app/web/overview_dashboard.py` | `Workspace > Overview`에서 Market Context, Market Movers, Sentiment, Events primary tab render와 selected-tab lazy dispatch를 조정한다. Primary selector는 내부 `st.pills` 상태를 쓰되 Korean-first text tab + active red underline으로 표시하며, query-param slug는 직접 진입 호환 입력으로만 읽고 tab `<a href>` navigation은 렌더링하지 않는다. Fresh Overview entry는 기본 `Market Context` cockpit body를 `시장 맥락 불러오기` 뒤로 지연한다. Market session banner, daily snapshot refresh action bar, browser-session auto refresh heartbeat, Market Movers manual investigation panel, Sentiment context, Events agenda/calendar/quality/raw views, Market Context historical analog 부족 ETF 가격 이력 보강 버튼을 조정한다. Futures Monitor와 Sector / Industry standalone tabs는 primary navigation에서 soft-removed 상태다 |
| `app/web/overview_dashboard_helpers.py` | Overview dashboard용 cached market intelligence service wrapper. Legacy candidate snapshot helpers are retained for audit but Candidate Ops is no longer rendered as an Overview tab |
