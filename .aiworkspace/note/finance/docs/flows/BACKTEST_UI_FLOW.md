...
2026-07-08 기준으로, Backtest Analysis의 `Single Strategy` 전략 선택과 `Portfolio Mix Builder` 전략 선택 / variant 선택은 Streamlit-owned 흐름을 유지한다. 별도 `Strategy Detail` React panel은 active flow가 아니며, 정리 대상은 각 전략 form 내부의 preset 기준, preflight, advanced input grouping이다. React는 `Price Freshness Preflight`, Factor Readiness action panel, Handoff action 같은 좁은 interaction / visual component에만 사용한다. Quality / Value strict form의 visible `Universe Contract`는 `PIT Monthly Snapshot Universe`만 노출한다. `Static Managed Research Universe`와 `Historical Dynamic PIT Universe`는 기존 saved payload / old run replay 호환용 legacy internal path로만 유지한다. PIT Monthly는 사전 생성된 DB monthly snapshot을 읽는 근사 PIT 경로이고 공식 index membership이 아니다.
...
| `app/web/overview_dashboard.py` / `app/web/overview/*` | `Workspace > Overview`에서 Market Context, Market Movers, Futures Macro, Sentiment, Events primary tab render와 selected-tab lazy dispatch를 조정한다. Compatibility wrapper는 `overview_dashboard.py`, page shell은 `overview/page.py`, navigation은 `overview/navigation.py`, tab entrypoint는 `overview/{tab}.py`, tab-local Streamlit glue는 `overview/{tab}_helpers.py`가 소유한다. Primary selector는 내부 `st.pills` 상태를 쓰되 Korean-first text tab + active red underline으로 표시하며, query-param slug는 직접 진입 호환 입력으로만 읽고 tab `<a href>` navigation은 렌더링하지 않는다. `Market Context`는 선택되면 즉시 cockpit body를 렌더링하며 별도 `시장 맥락 불러오기` gate를 두지 않는다. Market session banner, Market Context direct refresh, Market Movers snapshot / auto-refresh, Futures Macro refresh / context panel, Sentiment React workbench / fallback details, Events agenda/calendar/quality/raw views는 각 tab helper를 통해 렌더링한다. Sentiment React는 `app/web/streamlit_components/sentiment_workbench`가 표시 / 상호작용을 맡고, DB read / refresh action / analysis text는 `app/services/overview/sentiment.py`와 Python helper가 소유한다. Futures Monitor와 Sector / Industry standalone tabs는 primary navigation에서 soft-removed 상태다 |
...
| `app/services/overview/*` | Streamlit-free Overview market intelligence service modules. `market_movers.py`는 S&P 500 / Top1000 / Top2000 movers, yearly period, group leadership, breadth heatmap, date windows를 만들고, `why_it_moved.py`는 manual investigation / session-only compact metadata와 selected-symbol basic indicator snapshot / chart payload를 만들며, `events.py`, `sentiment.py`, `market_context.py`, `data_health.py`가 각 domain read model을 소유한다 |
...
| `app/services/backtest_portfolio_mix_readiness.py` | Streamlit-free Portfolio Mix Builder readiness helper. mix role flag 같은 순수 판정을 UI 밖에서 담당 |
...
| `app/services/backtest_validation_status_policy.py` | Streamlit-free Practical Validation / audit status normalization policy |
...
| `app/services/backtest_validation_efficacy.py` | Streamlit-free validation method-strength audit read model. Practical Validation의 compact method evidence 중 walk-forward temporal validation, OOS holdout validation, regime split validation만 읽는다. Source, latest replay, benchmark / comparator, provider freshness, PIT / survivorship, stress / robustness, realism은 각 owner audit이 소유한다 |
...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel과 저장 / Final Review 이동 CTA intent 소비, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, 표시 전용 `데이터 보강 대상` React board와 기존 Python 수집 버튼을 묶은 `데이터 보강 / 수집 실행` action center, 접힌 `상세 근거 / 원자료`, service handoff 결과의 session state 반영을 담당. Flow 2에서 현재 세션 replay가 없으면 Flow 3 / Flow 4와 결과 JSON을 렌더링하지 않는다. Flow 3 / Flow 4 visible UI는 적용된 REVIEW-only Practical Validation category를 숨기지 않고 `데이터 주의`, `2단계 실용성 주의`, `최종 판단 참고`, `Monitoring 추적` role로 구분한다. Final Review / Monitoring 전용 항목은 first-read action issue가 아니라 보조 참고로 낮추며, 단계별 소유권 inventory는 내부 read model에만 남긴다. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
| `app/services/backtest_evidence_read_model.py` | Streamlit-free final decision evidence read model. Final Review Candidate Board priority / 내부 selection-readiness / Decision Record guide / Saved Decision Review ledger / investability evidence packet / deployment-readiness policy snapshot / open review item / detailed scorecard / Level2 REVIEW module별 stored audit trace / score impact / selection rationale / required decision note / saved decision status / table row, Selected Dashboard evidence check row, Decision Dossier markdown read model과 selected decision source contract를 공통으로 만든다 |
...
| `app/web/backtest_final_review/page.py` | Final Review 화면 render. Decision Desk와 stable-key 검토 대상 selector / 후보 비교 상세, 선택 후보 investment report React section, 상태 / 권장 판단 / route / 사유 / 저장 CTA를 합친 `판단 기록`, Monitoring 후보 handoff 상태 안내, Evidence Appendix의 Validation / Robustness / Paper Observation / Investability Evidence Packet read-only 확인, saved final selection review ledger, Selected Dashboard handoff summary, Decision Dossier download. Final Review first-read에서는 별도 `Step 1 / Candidate Board`, visible Review Queue / standalone Decision Cockpit과 CNN / AAII 시장심리 패널을 렌더링하지 않는다 |
...
| `app/runtime/backtest/result_bundle.py` | Backtest runtime 결과를 UI가 읽는 summary / chart / metadata bundle로 변환 |
| `app/runtime/backtest/stores/candidate_registry.py` | current candidate / review note / pre-live registry JSONL read / append helper |
| `app/runtime/backtest/stores/portfolio_proposal.py` | portfolio proposal draft JSONL read / append helper |
| `app/runtime/backtest/stores/paper_portfolio_ledger.py` | paper portfolio tracking ledger JSONL read / append helper |
| `app/runtime/backtest/stores/final_selection_decisions.py` | final portfolio selection decision JSONL read / append helper |
| `app/runtime/backtest/stores/portfolio_selection.py` | current selection source, practical validation result, Final Decision, selected monitoring log, saved mix JSONL helper |
| `app/runtime/backtest/read_models/final_selected_portfolios.py` | final selection decision registry를 read-only dashboard row, Selected Dashboard handoff review, 사용자 monitoring portfolio saved state / strategy slot, continuity check, selected component replay recheck, component contribution, optional Allocation Check / drift preview / allocation drift evidence boundary로 변환하는 helper |
...
| `.aiworkspace/note/finance/registries/PORTFOLIO_SELECTION_SOURCES.jsonl` | Current Backtest Analysis source persistence. 첫 후보 source 선택 시 생성 |
| `.aiworkspace/note/finance/registries/PRACTICAL_VALIDATION_RESULTS.jsonl` | Current Practical Validation result persistence. 첫 검증 결과 저장 시 생성 |
| `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` | current final selection decision persistence. 첫 final decision 저장 시 생성 |
| `.aiworkspace/note/finance/registries/SELECTED_PORTFOLIO_MONITORING_LOG.jsonl` | 모니터링 후보 선정 후 optional monitoring check record. 자동 생성하지 않고 명시 저장이 필요할 때만 사용 |
...
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. Decision Desk는 후보 수와 우선순위를 보여주되 visible Review Queue table은 렌더링하지 않는다. stable key selector에서 후보를 고른 뒤 `최종 검토서 확인`으로 저장 evidence를 읽을 후보를 확정하고, 선택이 바뀌면 report / 판단 기록을 숨겨 stale 판단을 막는다. 투자 검토서는 단일 상태와 세 점수 축 아래 총평을 보여주고, 바로 이어서 `성과 해석 / 위험 해석 / 근거 신뢰도 / Monitoring 적합성` 4개 세로 행을 표시한다. 10개 조건부 Monitoring 패턴은 named evidence adapter로 적용 여부를 판정하며 첫 화면에는 중요도 상위 최대 6개만 `현재 진단 / 의미 / 변화 조건 / 다음 행동`으로 보여준다. source / 기준일 / technical path와 나머지 패턴은 접힌 상세에 둔다. REVIEW는 `최종 판단에서 결정할 것 / 2단계에서 인수한 제한사항 / Monitoring으로 넘길 조건 / 선정 전 해소할 차단 항목`으로 stage ownership을 나누고, 하단 `남은 판단 근거`에서 module별 stored audit row를 measured / derived / qualitative / missing contract로 구분한다. open 개수만으로 투자 매력도를 감점하거나 cap하지 않는다. 하단은 연결된 `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어` shell이며, 실험은 적용 가능한 상위 3개만 `바꿀 것 / 같게 둘 것 / 확인할 것`으로 표시하고 counterfactual backtest 전에는 점수 개선으로 예측하지 않는다. 사용자는 통합 `판단 기록`에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토 중 하나를 append-only record로 저장할 수 있다. selected-route gate를 통과한 selected row만 Portfolio Monitoring 후보 handoff 대상이며, Python selection-readiness model / Decision Record guide / Evidence Appendix / Saved Decision Review가 기존 gate와 persistence 경계를 계속 소유한다. Final Review는 새 검증, provider fetch, DB 수집, live approval, broker order, auto rebalance를 수행하지 않는다.
...
### Backtest Analysis Handoff / Policy Signals 책임

Handoff owns entry judgment and source registration action. `2차 실전성 검증 Handoff`는 1차 source 등록 기준 통과 / 보류, 먼저 해결할 blocker 수, source 등록 버튼, 저장 boundary를 한 곳에서 보여준다. 보이는 Handoff card는 readiness score나 2차 review focus count를 진입 조건처럼 표시하지 않는다. Policy Signals owns evidence detail. `검증 신호 · Policy Signals`는 같은 진입 가능 판단을 다시 반복하지 않고, `Data Trust`, `Execution Source`, `Validation Source` 중심의 1차 기준을 `검증 기준 상세` React board로 설명한다. 각 기준은 `?` help로 사람이 읽는 설명과 실제 체크 항목을 보여주며, 2차 review focus 상세 목록은 source contract로만 Practical Validation의 `Backtest에서 넘어온 2차 확인 항목`으로 넘긴다.

The production path uses targeted React custom components only where HTML design and real interaction must live together. `app/web/components/backtest_handoff_action/` owns the visible Handoff card and source registration button inside the Handoff panel, while Python receives the submit event and performs source registration / registry write / rerun. `app/web/components/backtest_price_refresh_action/` owns the visible Data Trust price-refresh card and update button, while Python receives the submit event and runs the bounded OHLCV refresh. `app/web/components/backtest_policy_signal_board/` owns the first-stage evidence board UI only; Python service code still owns policy classification, gate math, persistence, and source handoff.

Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 / 다음 행동 -> 검증 기준 상세`의 4개 user-facing flow로 읽는다. Flow 1은 source snapshot 확인만 소유하고, Flow 2가 validation profile과 latest runtime replay 실행을 함께 소유한다. 현재 브라우저 세션에서 Flow 2 replay를 실행하기 전에는 Flow 3 / Flow 4와 결과 JSON을 보여주지 않는다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 compatibility path를 유지하되 visible UI는 `검증 결론`, `카테고리별 검증 요약`, `검증 결과 저장(기록용)`, `저장하고 Final Review로 이동` CTA를 렌더링한다. React는 CTA intent만 전달하고, Python path가 save-only audit append, gate 확인, Final Review handoff, session state, rerun을 계속 소유한다. raw status, row evidence, module table, resolution guide는 Flow 4로 낮춘다. Flow 4는 같은 workspace read model의 `visible_criteria_detail_groups`를 `카테고리별 검증 결과` board로 풀어 각 기준의 사용자-facing 판정 근거와 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide를 보여준다. REVIEW-only라도 Practical Validation이 소유한 applied category는 숨기지 않고 `review_role` / `review_role_label`로 `데이터 주의` 또는 `2단계 실용성 주의`를 표시한다. Final Review / Monitoring 전용 reference는 visible action issue가 아니라 appendix / handoff 참고로 낮춘다. 그 다음 `data_action_board`를 `app/web/components/practical_validation_data_action_board/` React board로 렌더링해 지금 수집 가능한 항목, source map 탐색 항목, connector mapping 필요 항목, 현재 수집으로 해결되지 않는 항목을 표시한다. 단계별 소유권 inventory는 read model 내부에는 남지만 visible expander로 렌더링하지 않는다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
`practical-validation-source-context-v1`부터 Step 1 source snapshot은 단일 / mix 여부, component 전략, target weight, universe, score horizon, rebalance / trend / risk-off 설정을 strategy brief로 보여준다. Result Table에는 성과 row와 별도로 compact monthly selection / holdings history를 표시한다. 이 history는 Backtest Analysis handoff 또는 saved / weighted mix replay payload에서 온 compact snapshot이며, full holdings 원장은 workflow JSONL이 아니라 기존 runtime / DB 경계에 남긴다.
`practical-validation-module-gate-v1`부터 Practical Validation result에는 `source_traits`, `validation_modules`, `validation_module_summary`, `final_review_gate`가 붙는다. module planner는 source kind, component count, target weights, strategy key, universe symbols, replay contract, profile, input checks, diagnostic rows, audit rows를 읽어 필수 검증 / 조건부 검증 / 후속 참고를 분리한다. 필수 module의 `BLOCKED` / `NEEDS_INPUT` / `NOT_RUN`은 `저장하고 Final Review로 이동`을 막고, `REVIEW`는 원칙적으로 `READY_WITH_REVIEW`를 만들지만 `review_role`로 stage ownership을 분리한다. `pv_data_caution`과 `pv_practical_caution`은 Flow 4에서 사용자가 볼 주의 항목이고, `final_decision_input`은 Final Review의 최종 선택 참고, `monitoring_followup`은 Portfolio Monitoring 추적 참고, `final_readiness_blocker`는 저장 전 보강 경계다. 단, `practical-validation-selected-route-preflight-v1`부터 같은 Final Review selection policy를 Practical Validation에서 먼저 실행하며, selected-route 저장을 막는 `BLOCK`, `REVIEW_REQUIRED`, gross-only / net-cost gap, 핵심 data coverage / validation method strength / backtest realism gap은 `Selected-route Preflight` 필수 module의 `NEEDS_INPUT`으로 승격해 이동을 차단한다. module row는 `Gate Effect`, `Gate Reason`, `review_role`, `stage_decision_surface`로 이동 차단 / PV 주의 / Final Review 참고 / Monitoring 추적 여부를 명시한다. 조건부 module은 source traits에 맞을 때만 적용하며, 단일 ETF-like 후보에는 weighted-mix 전용 risk contribution / component role module을 `NOT_APPLICABLE`로 낮춘다.

`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 Flow 3 `검증 결론`에서 Practical Validation outcome과 카테고리별 통과 / 실패만 먼저 보여주고, Flow 4 첫 board에서 gate-relevant 기준별 판정 근거를 자세히 보여준다. Applied Validation Map은 Flow 4의 보조 `검증-근거 연결 지도`로 접어 둔다. 각 evidence / action board title 아래에는 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
Flow 4의 첫 criteria card는 module table 값을 그대로 반복하지 않고 `resolution_guide`를 읽어 `검증한 것`, `해결해야 할 항목`, `해결 방법`, `통과 기준`, `위치`로 보여준다. `해결 방법`은 slash-joined 문장이 아니라 `action_steps`를 번호형 목록으로 렌더링하며, non-PASS audit row의 `Next Action`을 가장 구체적인 단계로 우선 사용한다. 예를 들어 `latest_replay` blocker는 Flow 2 `검증 기준 설정 / 실전 재검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하고 어떤 상태가 되어야 통과인지 함께 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 읽되, `REVIEW`는 단일 의미가 아니다. `pv_data_caution`, `pv_practical_caution`, `final_decision_input`, `monitoring_followup`, `final_readiness_blocker` role을 붙여 현재 Flow 4에서 볼 주의와 Final Review / Monitoring 참고를 분리한다. raw route id는 JSON context에 남긴다.
...
provider snapshot 조회 기준일은 저장된 backtest 종료일이 아니라 Practical Validation 실행일이다. 저장된 mix의 backtest 기간이 과거에 끝나도, Final Review 전 검증은 현재 수집된 ETF 운용성 / holdings / macro context로 확인한다.
...
`practical-validation-required-taxonomy-refactor-v1`부터 `validation_efficacy_audit`의 user-facing 의미는 `Validation Method Strength`다. 이 audit은 walk-forward temporal validation, OOS holdout validation, regime split validation만 읽고, runtime replay / benchmark / provider freshness / PIT / survivorship / robustness는 각 owner module로 분리한다. 새 DB write, 새 JSONL registry, user memo, preset, approval, order, auto rebalance는 추가하지 않는다.
`final-review-selection-readiness-gate-v1`부터 Final Review gate policy는 selection-readiness와 deployment-readiness를 분리한다. `selection_gate_policy_snapshot`은 최종 관찰 후보 저장 여부를 판단하고, `deployment_readiness_policy_snapshot`은 향후 실제 자금 투입 감사에 쓸 엄격한 판정을 보존한다. 기본 `REVIEW`는 `open_review_items`로 남기며, live approval / broker order / account sync / auto rebalance는 만들지 않는다.
`validation-efficacy-gate-policy-link-v1` 이후 Final Review selection gate는 `Validation Method Strength` route를 selected-route 조건으로 읽는다. 핵심 `VALIDATION_EFFICACY_NEEDS_INPUT`과 `VALIDATION_EFFICACY_BLOCKED`는 selected-route blocker이며, `VALIDATION_EFFICACY_REVIEW`는 기본적으로 `open_review_items`로 남긴다.
`validation-efficacy-gate-policy-refinement-v2`부터 Final Review gate policy는 `Validation Method Strength`의 non-PASS row도 evidence로 병합한다. Walk-forward / OOS / regime row가 `REVIEW`이면 hold / re-review 요구로, `NEEDS_INPUT` 또는 `BLOCKED`이면 selected-route blocker로 표시된다.
...
`data-coverage-gate-policy-link-v1` 이후 Final Review selection gate는 Data Coverage Audit route를 selected-route 조건으로 읽는다. 핵심 `DATA_COVERAGE_NEEDS_INPUT`과 `DATA_COVERAGE_BLOCKED`는 selected-route blocker이며, `DATA_COVERAGE_REVIEW`는 기본적으로 `open_review_items`로 남긴다.
...
`construction-risk-gate-policy-v1` 이후 Final Review selection gate는 Construction Risk / Risk Contribution / Component Role / Weight audit route와 non-PASS row criteria를 selected-route 조건으로 읽는다. `BLOCKED`와 weighted mix component role / weight rationale의 핵심 `NEEDS_INPUT`은 selected-route blocker이며, 일반 `REVIEW`는 `open_review_items`로 남긴다. 단일 component 후보의 mix 전용 audit은 selection gate에서 비적용으로 낮춘다.
...
`backtest-realism-gate-policy-link-v1` 이후 Final Review selection gate는 Backtest Realism Audit route를 selected-route 조건으로 읽는다. 핵심 `BACKTEST_REALISM_NEEDS_INPUT`과 `BACKTEST_REALISM_BLOCKED`, 명시적 transaction cost / net cost curve / net performance / gross-only selection gap은 selected-route blocker이며, 일반 `BACKTEST_REALISM_REVIEW`는 `open_review_items`로 남긴다.
...
Flow 4 `검증 기준 상세`에는 `데이터 보강 / 수집 실행` action center를 표시한다. 사용자는 원자료 표를 먼저 펼치기 전에 `immediate_collect`, `source_map_discovery`, `connector_needed`, `no_action`으로 분류된 데이터 보강 대상을 카드로 본다. 여기서 provider는 ETF 운용사 / 공식 외부 데이터 원천을 뜻한다. 이전 `수집 대상 근거` / `수집 대상 상세` expander는 별도 first-read 영역이 아니라 이 board의 reason / next action / availability로 흡수한다. 수집 가능한 ETF 운용사 / holdings / exposure / macro 데이터는 같은 action center의 기존 Python 수집 버튼에서 일괄 수집 / 보강할 수 있다. 버튼 주변에는 `수집하는 것`, `하지 않는 것`, `실행 후 다음 단계`를 표시해 수집이 백테스트 재실행, 검증 판정 변경, Final Review 판단, registry / saved rewrite가 아님을 설명한다. React board는 props 표시만 맡고, 수집 계획과 ingestion job orchestration은 Practical Validation service가 맡는다. 공식 source mapping이 없는 holdings / exposure는 먼저 `etf_provider_source_map` discovery를 실행해 verified endpoint를 찾고, 자동 탐색 후에도 없을 때만 수동 connector mapping 필요로 남긴다. 원자료와 검산용 evidence tabs, `보강 작업 상세 / 수집 원자료`는 기본 접힌 `상세 근거 / 원자료`에 둔다.
...
- `Operations > Operations Overview`: Operations Console landing page다. Portfolio Monitoring Status summary와 Evidence Health mini strip을 먼저 보여준 뒤 priority / evidence / metric 기반 review queue, Portfolio Monitoring, System / Data Health lane을 보여준다. Summary는 selected dashboard와 monitoring portfolio setup에서 active portfolio / assigned strategy / stale 또는 pending scenario metadata / blocked / missing / incomplete slot / open review / target snapshot / next review를 읽기 전용으로 요약한다. Evidence Health는 scenario freshness, selected evidence readiness, open review, system run health를 이미 로드된 payload에서만 요약하고 provider DB 세부 조회나 새 수집을 실행하지 않는다. Review queue는 setup blocker, system run failure, scenario freshness, open review, routine monitoring, no-selected-row guidance를 정렬하지만, job 실행 / registry write / scenario replay는 하지 않는다. selected monitoring과 system run health만 top-level Operations surface로 남기며, archive / development-history decision table은 운영 화면에 노출하지 않는다. Backtest Run History와 Candidate Library 데이터 / helper code 삭제는 별도 audit 전까지 하지 않는다.
...
- `Operations > System / Data Health`: 웹앱 ingestion / refresh / factor job의 run health를 점검한다. triage flow, 최근 실행 상태, action inbox, failure CSV, run artifact, related logs, runtime snapshot을 보여주며, job 실행은 `Ingestion`에서 처리한다. Backtest replay / Candidate Library archive 화면은 현재 Operations 상단 탭에 노출하지 않는다.
...
| `app/web/streamlit_app.py` | Operations navigation에 `Operations Overview`, `Portfolio Monitoring`, `System / Data Health` 등록 |

데이터 기준:

- source-of-truth: `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`
- dashboard portfolio setup: `.aiworkspace/note/finance/saved/SELECTED_DASHBOARD_PORTFOLIOS.jsonl`
  - user portfolio name / description
  - backward-compatible `selected_decision_ids`
  - `strategy_slots` with selected decision id, start, latest-end mode, optional end, virtual capital, memo
- selected filter:
  - v3 row는 `monitoring_candidate == true`
  - legacy row는 `decision_route == SELECT_FOR_PRACTICAL_PORTFOLIO` 또는 `selected_practical_portfolio == true`
...
- performance recheck:
  - strategy slot 저장과 scenario update는 분리
  - 실행 후 portfolio-wide cockpit에서 value / P&L / return / CAGR / MDD / benchmark spread / rebalance target을 먼저 표시
  - 실행 전후 Recheck Operations Preflight가 replay contract readiness와 DB symbol freshness를 하나의 route로 확인하되 UI에서는 하단 상세 근거로 표시
  ...
  - 사용자가 지정한 virtual start / end / capital로 DB-backed strategy replay 실행
...
append / load helper는 `app/runtime/backtest/stores/portfolio_proposal.py`가 담당한다.
...
2026-04-30 리팩토링 이후 Backtest shell은 약 100 lines의 page shell이다. 2026-06-30 startup navigation 보정 이후 shell 파일은 `app/web/backtest_page.py`에 둔다.
...
- module split 후에는 `python3 -m py_compile app/web/backtest_page.py app/web/backtest_*.py app/web/streamlit_app.py`를 기본 확인한다.
...
- `app/runtime/backtest/stores/candidate_registry.py`로 registry JSONL read / append helper를 먼저 분리했다.
...
- `Candidate Readiness`: promotion policy signal, execution source checks, validation source checks
- `Practical Evidence`: provider / data coverage / realism / robustness / construction evidence
- `Final Decision Gate`: selection hard blocker와 open review item 분리
- `Monitoring Check`: 모니터링 이후 recheck readiness, freshness, provider evidence, review signal
...
- `실전성 검증 Handoff`: 1차 source 등록 기준 통과 / 보류, 먼저 해결할 blocker, source registration button inside the Handoff panel, source 등록 boundary를 한 panel에서 보여준다. hard blocker가 없으면 `promotion_decision=hold` 같은 review focus가 있어도 버튼은 활성화되고, 상세 사유와 count는 Backtest Analysis에서 펼치지 않는다. Handoff owns entry judgment and source registration action. React custom component는 보이는 Handoff card / button / submit event만 맡고, source registration write / rerun은 Python path가 계속 소유한다.
- `검증 신호 · Policy Signals`: result tab 안에서 1차에서 확인 가능한 source 등록 기준만 `검증 기준 상세` React board로 보여준다. 각 1차 row는 무엇을 확인했는지, 왜 통과 / 차단인지, 미통과 시 무엇을 봐야 하는지를 표시한다. Practical Validation review focus rows는 1차 처리 항목이 아니라 source evidence로 저장하고, Backtest Analysis에서는 count / group handoff를 표시하지 않는다. 상세 row는 Practical Validation `1. 선택 후보 확인`의 `Backtest에서 넘어온 2차 확인 항목`에서 이어 본다. Benchmark / execution / raw technical payload는 접힌 상세 근거로 낮추며 nested `현재 판단 / 검토 근거 / 실행 부담 / 기술 상세` 탭은 active render path에서 사용하지 않는다.
...
- `Practical Validation review focus`: hold 사유, caution source, benchmark / drawdown / liquidity / price freshness / regime / split-period 확인 항목이다. Backtest Analysis에서는 count와 전달 상태를 표시하지 않고, 상세 row는 Practical Validation `1. 선택 후보 확인`의 `Backtest에서 넘어온 2차 확인 항목`에서 처음 보여준다.
...
- 상단은 한 줄 결론, 상태 pill, `계산 기준일 / 가격 기준 / 사용 데이터 / 1차 데이터 확인` 4개 요약으로 구성한다.
- raw date / row count / common latest price / latest-date spread는 별도 guide/expander가 아니라 4개 요약의 보조 detail로 압축한다.
- excluded ticker와 malformed price row처럼 결과 해석에 직접 영향을 주는 항목만 `1차 데이터 확인`으로 상세 표시한다.
- `meta["warnings"]`에 들어온 실전성 review focus는 Data Trust의 1차 데이터 기준 요약에 표시하지 않는다. 상세 판단은 source contract로 전달된 뒤 Practical Validation `Backtest에서 넘어온 2차 확인 항목`에서 이어 본다.
- 2026-07-05 V1부터 DB 공통 최신 가격일이 현재 기준 최신 완료 거래일보다 오래된 경우, `데이터 기준 요약` 아래에 `가격 데이터 업데이트` action을 표시한다. 이 action은 주말 / NYSE 휴장일을 제외한 목표 기준일까지 현재 백테스트 ticker만 기존 OHLCV ingestion job으로 보강한다. 2026-07-05 후속 보정부터 보이는 action card와 버튼은 `app/web/components/backtest_price_refresh_action/` React custom component가 함께 렌더링하고, Python은 submit event를 받아 refresh job만 실행한다. 실행 후 자동 재백테스트, 후보 등록, 2차 검증 전송은 하지 않으며 사용자가 `Run Backtest`를 다시 눌러 최신 DB 기준 결과를 계산한다.

첫 적용 대상:

- `Global Relative Strength` single strategy 실행 전 `Price Freshness Preflight`
- Strict Quality / Value / Quality+Value family form-level `Price Freshness Preflight`
- `Latest Backtest Run`의 공통 `데이터 기준 요약`

Strict Quality / Value / Quality+Value family form-level `Factor Readiness`는 가격 최신성, provider/source gap, statement shadow gap을 내부 metric table이 아니라 `문제 / 영향받는 티커 / 해결 방법` 카드로 보여준다. 가격 보강이 가능한 문제는 Backtest OHLCV refresh service를 실행하고, statement 보강은 targeted `Extended Statement Refresh`를 실행한다. provider/source gap은 반복 update 버튼이 아니라 Data Trust / universe 조정 대상 수동 확인으로 표시한다.
2026-07-08 Symbol Resolver V1부터 Factor Readiness는 `nyse_symbol_lifecycle`의 `event_type=ticker_change` evidence를 읽어 `티커 변경 후보`를 가격 refresh보다 우선 표시할 수 있다.
후보는 same CIK, lifecycle coverage, source reference, resolved ticker price freshness를 `evidence_factors`로 설명한다. LOW confidence 후보는 화면에 남기되 자동 반영 action에서 제외하고 수동 확인으로 표시한다.
사용자가 `티커 변경 반영 후 데이터 보강` action을 누를 때만 lifecycle row를 `resolution_status=active`로 저장하고, price refresh는 원래 source ticker를 바꾸지 않은 채 resolved ticker로 collection을 실행한다.
Factor Readiness는 ticker-change 후보쌍 / 신뢰도 / 기간 경계 / 다음 행동을 `근거 보기`에 나눠 보여주며, repair action 결과는 Factor Readiness 재확인과 백테스트 재실행을 안내한다.
이 action은 current price freshness repair다. Price refresh plan/details의 `source_range` / `resolved_range` / `split_status`는 future PIT stitching을 위한 metadata-only contract이며, 실제 old/new ticker price series 합성이나 official corporate-action ingestion을 자동 수행하지 않는다.
...
## Latest Backtest Run 결과 표시 경계

2026-07-01 cleanup 이후 Run Backtest 직후에는 별도 `Execution Summary` / developer payload preview를 먼저 보여주지 않는다.
2026-07-01 result-flow reorder 이후에는 `Latest Backtest Run` 제목도 제거하고, 전략명 중심 결과 헤더를 첫 결과 surface로 둔다.

현재 기준:

- 실행 직후 화면은 `전략명 백테스트 결과` 헤더 안의 통합 KPI band, 내부 title을 가진 `데이터 기준 요약` panel, `실전성 검증 Handoff`, 상세 결과 탭 순서로 읽는다.
- 결과 헤더는 기간 / 계산 기준 / Universe / Data / Execution을 보조 기준선으로 낮추고, End Balance / CAGR / Sharpe Ratio / Maximum Drawdown을 같은 결과 surface 안에서 먼저 보여준다.
- `데이터 기준 요약`은 standalone Streamlit heading이 아니라 Data Trust custom panel 내부 title로 표시하고, `먼저 볼 결론`은 같은 panel 안의 읽기 cue로 둔다.
- `데이터 기준 요약`은 결과보다 먼저 나오는 사전 gate가 아니라, 핵심 성과를 읽은 직후 그 성과가 어떤 데이터 기준으로 계산됐는지 설명하는 해석 기준이다.
- `실전성 검증 Handoff`는 데이터 기준까지 확인한 뒤 바로 다음 단계 action을 판단하는 영역이므로 상세 결과 탭보다 먼저 둔다. 2026-07-02 V1부터 이 영역은 중복 Streamlit container / 중복 heading 없이 하나의 custom handoff panel에서 진입 상태, source 등록 boundary를 함께 보여준다. 2026-07-05 entry-gate correction 이후 보이는 Handoff card는 `1차 진입 기준 / 먼저 해결 / 다음 단계`로 정리한다. `promotion_decision=hold` 같은 review focus는 버튼을 막는 1차 blocker가 아니라 Practical Validation으로 전달되는 2차 확인 항목이며, Backtest Analysis에서는 count / 상세를 표시하지 않는다. 보이는 Handoff card와 버튼은 React custom component가 함께 렌더링하고, Python은 submit event를 받아 current selection source 등록만 수행한다.
...
## Promotion Policy / Guardrail Scope 흐름

Phase 28 이후 compare, history, saved portfolio에는
전략별 Promotion Policy / Guardrail 지원 범위를 같은 언어로 보여주는 표를 둔다.
...
- 모든 전략에 같은 검증 근거 UI를 강제로 붙였다는 뜻이 아니다.
...
- GRS score, cash ticker, trend window, benchmark contract, top-N concentration, ETF operability inputs
...
Practical Validation Gate 통과 후보
  ...
  -> 2. Final Review 투자 검토서
     -> 단일 판단 상태, 세 점수 축, 총평, 강점 / 약점, 저장 전 질문, 10개 조건부 Monitoring 패턴, Final Review 확인 필요 확인
  -> 3. 판단 기록
     -> 내부 selection-readiness에서 상태 / 차단 수 / 권장 판단 확인
     -> route / 판단 사유 선택과 Final Review 판단 저장
...
- `검토 대상` selector는 표시 label이 아니라 validation / source stable key를 option identity로 사용한다. 후보를 바꾸는 것만으로 투자 검토서 / 판단 기록을 열지 않으며, `최종 검토서 확인`으로 현재 후보를 확정한 뒤 저장된 evidence를 읽는다. 확정 뒤 후보가 바뀌면 이전 report와 판단 기록을 숨기고 stale 안내를 표시한다.
- `Final Review 투자 검토서`는 Python `backtest_evidence_read_model`과 `backtest_final_review_guidance`가 만든 report payload를 React component가 렌더링하는 first-read surface다. 외부 Investment Report card 없이 단일 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평과 해석 4행, 강점 / 약점, 저장 전 확인 질문, 행동형 조건부 패턴을 표시한다. 하단은 연결된 `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어` shell이며, REVIEW는 stored audit trace를, 실험은 자동 실행 없는 상위 3개 가설을 표시한다. React는 표시만 하며 gate 계산, DB / provider fetch, registry write, strategy variant 생성은 하지 않는다.
- Level2 REVIEW disposition은 `최종 판단에서 결정할 것`, `2단계에서 인수한 제한사항`, `Monitoring으로 넘길 조건`, `선정 전 해소할 차단 항목`으로 분리한다. 데이터 주의 / 2단계 실용성 주의는 이미 점수·근거 신뢰도에 반영된 인수 제한이며 Final Review에서 Flow4 보강 작업을 다시 지시하지 않는다. final decision input만 사용자가 선택 / 보류 사유에 반영하고, monitoring follow-up은 추적 조건으로 확정하며, blocker는 2단계로 돌아가 해소한다.
- 조건부 패턴은 집중도, 주식-채권 분산, 금리 / 듀레이션, 인플레이션, tail risk, 추세, 구성요소 의존, 유동성 / 비용, benchmark, parameter sensitivity 10종이다. 저장 evidence의 명시된 source adapter만 사용해 `판단 가능 / 조건부 추적 / 추가 검증 필요 / 적용 제외`를 판정한다. 첫 화면은 최대 6개를 보여주고 기술 trace는 상세에서만 노출한다. 다음 실험 아이디어는 적용 가능한 salience 상위 3개만 현재 후보와 동일 기간·benchmark·비용 조건의 별도 counterfactual backtest 가설로 제시하며 자동 실행하거나 현재 점수를 바꾸지 않는다.
...
- Candidate Review와 Portfolio Proposal의 판단 field는 준비 기록이고, Final Review의 `Final Review 판단 저장`이 최종 판단 record를 남기는 주 decision surface다. 실제 자금 투입 가능 여부는 향후 Live / Deployment Readiness에서 별도로 판단한다.
- `Final Review 판단 저장`은 `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 `SELECT_FOR_PRACTICAL_PORTFOLIO`, `HOLD_FOR_MORE_PAPER_TRACKING`, `REJECT_FOR_PRACTICAL_USE`, `RE_REVIEW_REQUIRED` row를 append-only로 저장할 수 있다. Portfolio Monitoring 후보 handoff는 selected-route gate를 통과한 `monitoring_candidate=True` row만 대상이다.
...
Backtest > Final Review
  ...
  -> Evidence Appendix에서 이전 검증 결과 확인
     -> 저장된 Final Review 판단 기록 확인
     -> Dashboard handoff 확인
...
- final decision registry가 Final Review 판단 record 원본이며, Monitoring 후보 여부는 v3 row의 `monitoring_candidate` / `monitoring_handoff_state`로 구분한다.
- 새 UI가 저장하는 `decision_route`는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, `내용 부족 / 관찰 필요`, `투자하면 안 됨`, `재검토 필요` 중 하나다.
