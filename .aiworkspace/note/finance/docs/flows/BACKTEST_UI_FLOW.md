...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, 표시 전용 `데이터 보강 대상` React board, 기존 Python Provider / Data 보강 액션, 접힌 `상세 근거 / 원자료`, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. Flow 2에서 현재 세션 replay가 없으면 Flow 3 / Flow 4 / Flow 5와 결과 JSON을 렌더링하지 않는다. Flow 3 / Flow 4 visible UI는 Final Review 전용 REVIEW 판단과 REVIEW-only / empty category를 actionable issue로 노출하지 않으며, 단계별 소유권 inventory는 내부 read model에만 남긴다. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 validation profile과 5개 답변을 고른 뒤 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 2에서 현재 세션 replay 결과가 없으면 첫 진입 화면에는 Flow 1 / Flow 2만 보이고, Flow 3 / Flow 4 / Flow 5와 Practical Validation Result JSON은 렌더링하지 않는다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Practical Validation outcome summary, 실패 category count, category-level `통과 / 실패`를 단일 first-read conclusion surface로 보여준다. Flow 3은 Final Review 이동 가능 / 보류, REVIEW count, Final Review 참고 항목, 조건부 근거 / 후속 참고 expander를 보여주지 않는다. Flow 4는 `visible_criteria_detail_groups` 기반 `카테고리별 검증 결과` board에서 먼저 `통과`, `보강 후 재검증 필요`, `실전 사용 어려움` main outcome summary를 보여주고, 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약한다. `READY`는 사용자-facing 통과로 읽고, `REVIEW`는 보강 실패가 아니라 Final Review에서 해석할 metadata로만 유지하며 Flow 3 / Flow 4 visible actionable issue에는 노출하지 않는다. REVIEW-only / empty category는 내부 read model에는 남지만 PV visible category result에는 표시하지 않는다. `NEEDS_INPUT` / `NOT_RUN`은 보강 후 재검증 대상이고, `BLOCKED`는 현재 상태로 실전 후보 사용이 어렵다는 차단 결론이다. 각 기준 상세는 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치`로 보여준다. Flow 4 guide는 module-level 문구만 쓰지 않고 가능하면 audit row의 non-PASS `Criteria`와 `Next Action`을 끌어와 실제 부족 항목과 실행 방법을 표시한다. `통과 기준`은 보강 뒤 무엇이 되어야 해결된 것인지 판단하는 기준이며, 위치는 `Flow4 > 데이터 > 데이터 품질 / 편향 통제 상세`, `Flow4 > 데이터 보강 대상 / 액션`처럼 사용자가 찾을 수 있는 화면 경로로 표시한다. Flow 4는 `카테고리별 검증 결과 -> 데이터 보강 대상 / 액션 -> 상세 근거 / 원자료` 순서로 읽으며, `데이터 보강 대상` board가 `immediate_collect`, `source_map_discovery`, `connector_needed`, `no_action`을 표시 전용으로 구분한다. 수집 가능한 provider / holdings / exposure / macro gap만 criteria card에서 Flow 4 보강 액션으로 이동하는 `수집하기` CTA를 갖고, 실제 수집 실행은 기존 Python Provider / Data 보강 액션이 담당한다. 단계별 소유권 inventory는 내부 read model에는 남지만 visible expander로 렌더링하지 않는다. 핵심근거, 데이터품질, 구성리스크, 검증방법론, 강건성, Raw Evidence는 기본 접힌 `상세 근거 / 원자료`에서만 확인한다. legacy Final Review gate technical expander는 visible Flow 4 path에서 렌더링하지 않는다. Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 검증 상태와 보강 action을 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. 통과 기준은 `final_review_gate.can_save_and_move=True`이며, 저장만 된 blocked / needs input / not run validation row는 기록으로 남더라도 source picker에서 숨긴다. Final Review는 전용 Decision Desk shell에서 command center와 flow rail로 현재 판단 상태를 먼저 보여준다. 그다음 Candidate Board로 Gate 통과 후보의 decision state / suggested decision / blocker / open review / 주요 audit route를 비교하고, select-ready / hold / blocked counts, first-review candidate, review queue, primary reason, next action을 lane card로 보여준다. 선택 후보는 Decision Cockpit에서 Must Fix / open review items / monitoring seed를 먼저 확인한다. 이후 `selection_gate_policy_snapshot`이 통과한 후보만 `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 정식 저장하며, 이 저장은 Portfolio Monitoring 후보 기록이다. Decision Record Checklist는 suggested decision, selection gate, operator reason requirement, live approval / order disabled boundary를 저장 전 안내한다. 보류 / 거절 / 재검토는 저장 action이 아니라 상태 안내와 보강 방향으로 표시한다. 저장된 선정 기록은 Saved Decision Review ledger에서 selected count, latest selection, detail tabs, Portfolio Monitoring handoff, Decision Dossier export로 다시 읽는다. 기존 hold / reject / re-review row는 과거 호환용으로 읽을 수 있지만 새 UI의 정식 저장 action은 아니다. Portfolio Monitoring handoff는 기존 selected row를 다시 검증하지 않고 selected rows / dashboard rows / monitorable / blocked counts와 checklist만 read-only로 보여준다. 선택된 Practical Validation result와 diagnostics 요약, Construction Risk, Risk Contribution, Component Role / Weight, Validation Method Strength, Data Coverage, Backtest Realism, Robustness / Paper Observation / Investability Evidence Packet 상세는 `Evidence Appendix`에서 read-only로 확인한다. profile-aware selection gate policy는 hard blocker와 open review item을 분리한다. critical gap, benchmark / comparator parity blocker, Validation Method Strength / Data Coverage / Backtest Realism의 핵심 `NEEDS_INPUT` / `BLOCKED`, explicit gross-only 또는 net/cost 적용 부재, weighted mix component role / weight rationale 부재, selected-route policy blocker가 남아 있으면 `SELECT_FOR_PRACTICAL_PORTFOLIO` 저장은 차단된다. 기본 `REVIEW`는 저장을 막지 않고 `open_review_items`로 Portfolio Monitoring과 향후 Live / Deployment Readiness에서 이어서 본다. 기존처럼 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 함께 보존한다. Structured waiver는 현재 UI / persistence가 없고, future policy도 `BLOCK` waiver를 허용하지 않는다. 저장된 모니터링 후보 선정 기록은 read-only Decision Dossier markdown으로 다운로드할 수 있지만 report 파일을 자동 생성하지 않는다.
- `Final Review`의 `시장 심리 Context Overlay`는 Decision Desk 아래에서 저장된 CNN / AAII snapshot을 market backdrop으로만 보여준다. 이 overlay는 Candidate Board priority, selected-route gate, Final Decision save readiness, registry write, live approval / order / auto rebalance에 영향을 주지 않는다.
- `Operations > Portfolio Monitoring`: `SELECT_FOR_PRACTICAL_PORTFOLIO` selected row만 strategy pool로 읽는다. Legacy implementation file / helper name은 Selected Portfolio Dashboard를 유지한다. 화면은 먼저 CNN / AAII `시장 심리 Context Overlay`와 Active Portfolio Monitoring Scenario를 보여주고, 그 아래 `나의 포트폴리오` fixed-height card shelf와 선택한 portfolio command band, selected decision strategy slot compact board로 start / latest-end mode / balance / memo를 관리한다. Scenario update는 slot 저장과 분리되며, portfolio-wide cockpit에서 실행 전 / 부분 집계 / 전체 집계 상태와 value / P&L / return / CAGR / MDD / benchmark spread / target snapshot을 먼저 보여준다. `포트폴리오 시나리오 업데이트`는 현재 slot signature 기준 pending / stale strategy만 기본 실행하고, `전체 재실행`을 켠 경우에만 기존 최신 결과까지 다시 replay한다. Target snapshot table의 `Target Snapshot Date`는 마지막 monitoring scenario가 산출한 목표 비중 기준일이고 `Next Review Date`는 다음 수동 재계산 예정일이다. 이 표는 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. Final Review의 `open_review_items`, review trigger, `deployment_readiness_policy_snapshot`, recheck / provider / continuity / review signal / allocation boundary evidence는 사용자가 선택한 1개 strategy 상세 / optional preflight에서 read-only로 확인한다. Portfolio Monitoring의 sentiment overlay도 Monitoring Scenario, Review Signals, saved dashboard setup, monitoring log, broker order, auto rebalance에 연결하지 않는다. 이 preflight도 실제 돈 투입 승인, 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. 통과 후보가 없으면 strategy pool은 비어 있고 Final Review 선정 안내를 보여준다.
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 source snapshot 확인만 소유하고, Flow 2가 validation profile과 latest runtime replay 실행을 함께 소유한다. 현재 브라우저 세션에서 Flow 2 replay를 실행하기 전에는 Flow 3 / Flow 4 / Flow 5와 결과 JSON을 보여주지 않는다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 compatibility path를 유지하되 visible UI는 `검증 결론`과 `카테고리별 검증 요약`을 read-only로 보여준다. Flow 3은 Practical Validation outcome과 카테고리별 통과 / 실패만 보여주고, Final Review 이동 상태, REVIEW count, 조건부 / 후속 참고 expander를 보여주지 않는다. raw status, row evidence, module table, resolution guide는 Flow 4로 낮춘다. Flow 4는 같은 workspace read model의 `visible_criteria_detail_groups`를 `카테고리별 검증 결과` board로 풀어 각 기준의 사용자-facing 판정 근거와 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide를 보여준다. 그 다음 `data_action_board`를 `app/web/components/practical_validation_data_action_board/` React board로 렌더링해 지금 수집 가능한 항목, source map 탐색 항목, connector mapping 필요 항목, 현재 수집으로 해결되지 않는 항목을 표시한다. 단계별 소유권 inventory는 read model 내부에는 남지만 visible expander로 렌더링하지 않는다. Full `criteria_detail_groups`에는 Final Review reference metadata가 남지만, REVIEW-only / empty category는 Flow 3 / Flow 4 visible category result에서 숨긴다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
...
Flow 4 `검증 기준 상세`에는 `데이터 보강 대상 / 액션`을 표시한다. 사용자는 Provider Data Gaps 표나 원자료를 먼저 펼치기 전에 `immediate_collect`, `source_map_discovery`, `connector_needed`, `no_action`으로 분류된 데이터 보강 대상을 카드로 본다. 이전 `수집 대상 근거` / `수집 대상 상세` expander는 별도 first-read 영역이 아니라 이 board의 reason / next action / availability로 흡수한다. 수집 가능한 provider / holdings / exposure / macro 데이터는 같은 화면의 기존 Python Provider / Data 보강 액션에서 일괄 수집 / 보강할 수 있다. React board는 props 표시만 맡고, 수집 계획과 ingestion job orchestration은 Practical Validation service가 맡는다. 공식 source mapping이 없는 holdings / exposure는 먼저 `etf_provider_source_map` discovery를 실행해 verified endpoint를 찾고, 자동 탐색 후에도 없을 때만 수동 connector mapping 필요로 남긴다. 원자료와 검산용 evidence tabs는 기본 접힌 `상세 근거 / 원자료`에 둔다.
...
- `Final Review`: Portfolio Monitoring 후보 선정 저장 / 미통과 상태 안내
...
Practical Validation Gate 통과 후보
  ...
  -> 3. 모니터링 후보 선정 저장
     -> selection gate 통과 시 Selected Dashboard 추적 후보로 선정
...
- Candidate Review와 Portfolio Proposal의 판단 field는 준비 기록이고, Final Review의 `모니터링 후보 선정 저장`만 Selected Portfolio Dashboard에서 관찰할 후보를 명시하는 주 decision surface다. 실제 자금 투입 가능 여부는 향후 Live / Deployment Readiness에서 별도로 판단한다.
- `모니터링 후보로 선정`은 `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 append-only로 저장한다. `HOLD_FOR_MORE_PAPER_TRACKING`, `REJECT_FOR_PRACTICAL_USE`, `RE_REVIEW_REQUIRED`는 과거 row 호환 / 상태 안내 route로만 읽고 새 정식 저장 action으로 만들지 않는다.
...
- final decision registry가 모니터링 후보 선정 기록 원본이다.
