...
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. Decision Desk는 후보 수와 우선순위를 보여주되 visible Review Queue table은 렌더링하지 않는다. stable key selector에서 후보를 고른 뒤 `최종 검토서 확인`으로 저장 evidence를 읽을 후보를 확정하고, 선택이 바뀌면 report / Decision Cockpit / Final Decision Action을 숨겨 stale 판단을 막는다. 투자 검토서는 단일 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평, 강점 / 약점, 저장 전 질문, 10개 조건부 Monitoring 패턴을 first-read로 보여준다. REVIEW는 관측값 / 판단 기준 / source / 기준일 / score policy를 노출하며 open 개수만으로 투자 매력도를 감점하거나 cap하지 않는다. 하단은 `점수 근거 / REVIEW 근거 / 대안 실험 후보` 세 탭이고, 대안은 counterfactual backtest 전에는 점수 개선으로 예측하지 않는다. 이후 사용자는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토 중 하나를 append-only 판단 record로 저장할 수 있다. selected-route gate를 통과한 selected row만 Portfolio Monitoring 후보 handoff 대상이며, Decision Cockpit / Final Decision Action / Evidence Appendix / Saved Decision Review가 기존 gate와 persistence 경계를 계속 소유한다. Final Review는 새 검증, provider fetch, DB 수집, live approval, broker order, auto rebalance를 수행하지 않는다.
...
- `Final Review 투자 검토서`는 Python `backtest_evidence_read_model`이 만든 report payload를 React component가 렌더링하는 first-read surface다. 외부 Investment Report card 없이 단일 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평, 강점 / 약점, 저장 전 확인 질문, 10개 조건부 패턴을 표시한다. React는 표시만 하며 gate 계산, DB / provider fetch, registry write, strategy variant 생성은 하지 않는다.
- Level2 REVIEW disposition은 `데이터 주의`, `2단계 실용성 주의`, `최종 판단 참고`, `Monitoring 추적`, `저장 전 보강`을 first-read `Final Review 확인 필요`에 분리한다. 관측값 / 판단 기준 / source / 기준일 / score policy를 함께 보여주며, 미측정 또는 open REVIEW 개수는 투자 매력도를 자동 감점하지 않는다.
- 조건부 패턴은 집중도, 주식-채권 분산, 금리 / 듀레이션, 인플레이션, tail risk, 추세, 구성요소 의존, 유동성 / 비용, benchmark, parameter sensitivity 10종이다. 저장 evidence만으로 `supported / indicative / insufficient`를 판정하고, 대안은 예상 점수가 아니라 별도 counterfactual backtest 후보로만 제시한다.
