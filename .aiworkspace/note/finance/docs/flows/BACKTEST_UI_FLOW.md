...
- `Final Review`: primary question은 `이 포트폴리오를 실제 투자 검토 대상으로 계속 추적할 가치가 있는가?`다. source별 latest eligible Practical Validation result를 후보로 만들고 React one-shell 안에서 후보 선택 → 결론 → 누적 성과/Benchmark·Underwater·실행 관측 → 실제 강점/약점 → measured-only trait map → structured Monitoring 변화 조건 → canonical 최종 판단/사유 → evidence disclosure 순으로 읽는다. overall investment score와 기존 headline score는 current 화면에서 제거하고 evidence confidence만 disclosure의 보조 metadata로 둔다. React는 candidate / route / reason intent와 SVG 표현만 맡고 Python이 eligibility, exact-common 계산, observation normalization, dedup, save evaluation, 자동 Decision ID, row append를 소유한다. selected route만 기존 Gate와 closed evidence를 통과할 때 Monitoring 후보가 되며 보류/거절/재검토는 judgment row만 남긴다. Final Review는 새 검증, provider fetch, DB 수집, live approval, broker order, auto rebalance를 수행하지 않는다.
...
Practical Validation Gate 통과 후보
  ...
  -> 2. Decision Workspace
     ...
     -> 실제 강점 / 약점, trait map, Monitoring 변화 조건
  -> 3. 최종 판단과 사유
...
Backtest > Final Review
  ...
  -> Decision Brief 결론 / 행동 근거 / 실제 강점·약점 / trait 확인
