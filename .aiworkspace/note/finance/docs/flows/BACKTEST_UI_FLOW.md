...
이 문서는 Streamlit Backtest 화면의 single strategy, Portfolio Mix Builder, Practical Validation, Final Review, Portfolio Monitoring 흐름을 설명한다.

현재 Portfolio Monitoring의 React one-shell/서비스 소유권은 `../architecture/PORTFOLIO_MONITORING_REACT_COMMAND_CENTER.md`를 기준으로 한다. Final Review의 `monitoring_candidate=True` 후보는 direct 미국 주식·ETF와 함께 DB-backed group에 등록할 수 있다.
...
외부 자료 보강을 실행하면 Python은 source별 collection result와 `recheck_required` lifecycle을 session에 남기고 stale replay / decision result를 지운다. Decision Workspace는 이를 Step 2의 `자료 보강 -> 재검증 -> 새 결과 저장 -> Final Review 확인` 진행 상태와 `보강된 데이터로 재검증` CTA로 투영한다. partial / unknown / failed collector 결과는 성공으로 합치지 않으며, collection 완료 안내는 validation 완료가 아니므로 warning으로 표시한다. replay 뒤에는 같은 surface가 Gate에 따라 `save_ready` 또는 blocker 경로로 바뀐다. collection, replay, registry append, Final Review route는 서로 다른 Python-owned action 경계를 유지한다.
...
다음 6개 화면은 접힌 contextual `Reference help`에서 현재 문맥에 맞는 exact item deep link를 제공한다. Portfolio Monitoring은 실제 업무를 바로 시작하도록 별도 contextual panel을 두지 않고, 모니터링 흐름·scenario·stale 안내는 canonical Reference catalog가 소유한다.
...
catalog drift guard는 required surface coverage, item id·관련 item 참조, destination allowlist, forbidden legacy label, 검색 문자열을 점검한다. contextual-help drift guard는 6개 surface key 중복, exact item 참조와 `/reference` target 무결성을 Streamlit-free test로 검증한다.
