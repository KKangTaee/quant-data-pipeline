...
UI form, payload 복원, archive replay, candidate archive replay, saved weighted portfolio replay를 수정할 때 먼저 확인한다.
...
| `app/web/backtest_analysis.py` | `Backtest Analysis` stage wrapper. Single Strategy와 Portfolio Mix Builder를 먼저 렌더링하고, Reference help / Strategy Evidence / Bridge / Governance / ETF evidence / current-anchor / rerun matrix 패널은 `전략 개발 참고` advanced control 뒤에서 필요할 때만 보여준다 |
...
| `app/runtime/portfolio_selection_v2.py` | current selection source, practical validation result, Final Decision, selected monitoring log, saved mix JSONL helper |
...
Legacy Portfolio Proposal은 단순 weighted portfolio 저장값이 아니라,
후보 묶음의 목적과 검토 근거를 함께 담던 과거 제안 초안 기록이다.
...
기존 proposal draft record는 삭제하지 않고 archive / recovery compatibility로 보존한다.
...
Legacy Portfolio Proposal은 `SAVED_PORTFOLIOS.jsonl`의 replay 계약을 대체하지 않는다.
Saved Portfolio는 재현 가능한 weight setup이고,
legacy Portfolio Proposal은 그 setup이나 candidate set을 왜 제안 후보로 보았는지 설명하던 과거 검토 단위다.
...
| 우선순위 | 분리 후보 | 대표 책임 | 이유 |
|---|---|---|---|
| 1 | Legacy Candidate / Proposal cleanup | Candidate Review / Portfolio Proposal UI helper 삭제, route fallback, archive compatibility 보존 | 완료: 5C에서 legacy UI / helper module을 삭제했다. registry / saved JSONL은 보존하고 archive / recovery 경로만 유지한다 |
| 2 | Registry runtime helpers | current candidate / pre-live / proposal registry I/O compatibility | 신규 current workflow source-of-truth가 아니며 archive / recovery compatibility로만 읽는다 |
| 3 | History module | run history display, selected record, run again, load into form, Practical Validation source handoff | 완료: render flow는 `app/web/backtest_history.py`, replay / parity helper는 `app/web/backtest_history_helpers.py`로 분리. `Operations > Archive: Backtest Runs`에서 사용한다 |
...
- Backtest shell은 `Backtest Analysis -> Practical Validation -> Final Review`를 주 workflow navigation으로 보여준다. Backtest Analysis 안에서 `Single Strategy`와 `Portfolio Mix Builder`를 선택한다. `History`는 메인 흐름에서 제외하고 `Operations > Archive: Backtest Runs` page로 연다.
...
- Portfolio Mix Builder는 `app/web/backtest_compare.py`로 분리되어, component 실행 / weighted portfolio service 호출, saved portfolio replay / load를 page shell에서 제거했다. 6A 이후 saved Mix replay workspace의 render/helper 책임은 `app/web/backtest_compare_saved_replay.py`가 소유한다.
...
- Global Relative Strength는 재무제표 selection history 대상이 아니라 price-only ETF relative strength strategy로 설명한다. 5A 이후에는 기존 Latest Run selection history를 통해 top-N 선택, trend rejected slot, cash proxy retention을 함께 보여준다.
...
저장된 후보 자체의 그래프 재검토는 `Operations > Archive: Candidates`에서 처리한다.
...
5C physical cleanup 이후 legacy Candidate Review / Portfolio Proposal UI와 helper module은 삭제됐고,
primary Backtest navigation은 `Backtest Analysis -> Practical Validation -> Final Review`만 직접 dispatch한다.

보존되는 것은 기록과 archive / recovery compatibility다.

- `CURRENT_CANDIDATE_REGISTRY.jsonl`, `PRE_LIVE_CANDIDATE_REGISTRY.jsonl`, `PORTFOLIO_PROPOSAL_REGISTRY.jsonl`, paper ledger JSONL은 삭제하지 않는다.
- legacy route request는 primary panel로 열지 않고 Backtest Analysis fallback으로 정규화한다.
- `Operations > Archive: Candidates`는 legacy current / Pre-Live candidate record를 inspect / replay하는 archive recovery 화면이다.
- 새 후보 생성과 검증의 정식 경로는 Backtest Analysis에서 current selection source를 만들고 Practical Validation / Final Review로 이어지는 source chain이다.
- legacy record의 준비 field는 historical compatibility 정보이며, Final Review의 selected-route decision만 Portfolio Monitoring에서 관찰할 후보를 명시하는 주 decision surface다.
...
현재 UI는 이 record를 새 workflow의 필수 단계로 요구하지 않는다.
