...
### Current Practical Validation Level2 Contract
...
`상세 검증 근거`는 Step 3 disclosure이며 별도 Flow 5가 아니다. Step 1은 `1A. 검증할 후보`와 `1B. 어떤 관점으로 검증할까요?`로 나눠 후보 목록과 검증 정책이 같은 포트폴리오 설계안처럼 읽히지 않게 한다. `app/services/backtest_practical_validation_decision_workspace.py`가 verified / measured caution / resolve-now / engineering-required / Final Review handoff를 root issue 기준으로 한 번만 투영한다. measured caution은 계산된 `validated_caution`에 observation과 comparator가 모두 있을 때만 만들며, `accepted_limit / final_decision / monitoring_transfer`는 측정값이 있어도 Final Review handoff class를 유지한다. `app/services/backtest_practical_validation_explanation.py`는 audit row를 검증 항목, 확인 결과, 의미, 다음 행동의 사용자 설명으로 바꾸고 raw 함수 경로와 내부 status는 `technical_trace`로 낮춘다. 상세 근거는 데이터/편향, 검증 방법, 포트폴리오 구성, 실전 현실성, 스트레스/강건성 5개 category 중 하나만 펼친다. `app/web/components/practical_validation_decision_workspace/`는 이 read model을 `context / decision` surface로 나눠 표시하고 intent만 반환한다. 후보/profile surface는 replay fragment 밖에서 유지하며, replay/Level2 해결/save surface만 fragment 안에서 pending과 새 결과로 갱신한다. Python page/service는 applicability, Gate, registered handler 확인, replay, audit save, Final Review handoff와 surface별 action allow-list를 계속 소유한다. 후보/profile 변경과 Final Review route 이동만 app rerun을 사용한다. action 0건이면 빈 action board를 렌더링하지 않는다. custom profile / advanced replay / raw evidence는 `고급 설정과 원본 근거` secondary disclosure에 둔다.

one-shell hero는 Level2의 고정 질문만 표시하고 후보별 타이틀로 바뀌지 않는다. 선택된 후보와 판정 기준은 Step 1 첫머리의 compact summary에 함께 표시한다. `1A` 후보 목록은 처음에는 닫혀 있고 `후보 변경`을 눌렀을 때만 세로 목록으로 열리며, 목록이 길면 해당 목록 내부에서만 스크롤한다. `1B`의 5개 검증 관점은 데스크톱에서 한 행 5열, 760px에서 2열 줄바꿈으로 표시하고 홀수 번째 마지막 `사용자 지정`은 두 열 전체를 사용한다.

Level2 action은 실행 가능 여부와 실행 이력을 함께 본다. provider 보강이 필요한 경우에는 provider 실행 뒤 replay cache와 decision-result cache를 무효화하고 `최신 데이터 기준 재검증`을 이어서 실행한다. 공식 source 탐색을 이미 시도했지만 계약이 만들어지지 않았거나 등록 parser가 현재 수집기에서 처리 불가능한 경우에는 같은 `지금 해결` CTA를 다시 만들지 않고 `engineering_required`로 전환한다. 아직 `candidate` 상태인 source는 최초 discovery action을 유지하고, discovery job 예외도 요청 symbol을 run metadata에 보존한다. holdings / exposure / operability 계약은 각 target data-kind별로 종결 여부를 판단하며, 같은 symbol/data-kind에 candidate와 failed row가 함께 있으면 terminal 상태가 우선한다. 완료된 월간 replay는 요청 월의 최신 평가, 바로 이전 월의 완결 리밸런싱과 같은 실제 결과일, 월말 7일 이내 공통 가격일, 31일 이내 gap이 함께 확인될 때만 동일 replay를 다시 요구하지 않고 Monitoring handoff로 종결한다. 설명되지 않은 장기 또는 모순된 날짜 gap은 engineering blocker다. 따라서 사용자 클릭 뒤 같은 두 작업이 독립적으로 반복 노출되는 상태는 정상 스펙이 아니다.

기존 `practical_validation_fix_queue` / `practical_validation_data_action_board`와 아래 Flow 3 / Flow 4 상세 설명은 migration compatibility와 구현 이력이다. current active first-read 계약이 충돌할 때는 이 섹션을 우선한다.
...
Practical Validation current는 legacy `practical_validation_workspace`를 compatibility evidence source로 유지하면서, active first-read projection은 `practical_validation_decision_workspace_v1`이 소유한다. current eligible 후보는 unresolved actionable / critical engineering / missing contract가 모두 0이어야 한다. Level2-owned REVIEW는 producer가 `evidence_state=computed`를 제공할 때만 `validated_caution / resolved`로 종결한다. 계산되지 않은 필수 방법론은 `NEEDS_INPUT -> evidence_state=missing -> engineering_required / deferred`로 연결하고, 후보 기간 밖 stress window는 validator 누락과 구별해 `NOT_APPLICABLE`로 집계한다. accepted limit / final decision / monitoring transfer는 Level2 repair queue가 아니라 Final Review handoff다. React는 raw status를 finding kind로 바꾸거나 Gate를 계산하지 않는다.
blocked Level2에서는 handoff를 prospective 항목으로만 설명하고 실제 승격으로
표시하지 않는다. eligible validation을 저장한 뒤 Final Review Decision Brief는
같은 root issue를 `최종 판단 입력 / 인수한 검증 한계 / Monitoring 이관 조건`
세 lane 중 하나에서만 소비한다. 구조화 조건이 완결되지 않은 Monitoring REVIEW는
Final Review에 넘기지 않고 Level2 engineering blocker로 유지한다.
...
Practical Validation Gate 통과 후보
  -> Backtest > Final Review
  -> 1. primary question과 latest eligible 후보 선택
  -> 2. Decision Workspace
     -> 결론
     -> 누적 성과 / Benchmark, Underwater, 실행 관측
     -> 실제 강점 / 약점
...
```text
Backtest > Final Review
  -> latest eligible 검토 대상 선택
  -> Decision Brief 결론 / 행동 근거 / 실제 강점·약점 확인
  -> 실제 성격 / 관리 기준 대비 압력 확인
  -> structured Monitoring 변화 조건 확인
