...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 workspace panel, Flow 4 `Final Review로 넘기기 전 확인 기준` board, summary-first `검증 근거 보드`, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 사용자는 profile과 5개 답변을 고르고, 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Final Review 이동 판단과 `Final Review 이동을 막는 이슈` 큐를 단일 first-read surface로 보여준다. 각 blocker는 `현재 문제 / 완료 기준 / 보강 위치 / 영향`으로 읽고 raw status는 `기술 기준` tag로 낮춘다. Flow 4는 `Final Review로 넘기기 전 확인 기준` board에서 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 먼저 요약한 뒤 Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center를 담고, Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 `Final Review 이동 전 검증 상태`를 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 2026-07-05 V1-V7 이후 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 Final Review 이동 판단, top fix items, next step, compact criteria summary를 read-only로 보여준다. Flow 3 / Flow 4 blocker card는 `무엇을 검증했나 / 부족한 점 / 해야 할 일 / 왜 중요한가`를 먼저 보여주고, raw status는 `기술 기준` tag로 낮춘다. Flow 4는 같은 workspace read model의 `criteria_detail_groups`를 `Final Review로 넘기기 전 확인 기준` board로 먼저 풀어 각 기준의 사용자-facing 판정 근거와 보강 위치를 보여준다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 Flow 3 `2차 검증 결론 / Fix Queue`에서 Final Review 이동 판단과 먼저 해결할 blocker를 먼저 보여주고, Flow 4 첫 board에서 gate-relevant 기준별 판정 근거를 자세히 보여준다. Applied Validation Map은 Flow 4의 보조 `검증-근거 연결 지도`로 접어 둔다. 각 evidence / action board title 아래에는 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
Fix Queue의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 만든다. Flow 3 React surface는 그중 top blocker 최대 3개, next step, compact criteria summary를 먼저 보여주고, 나머지 상세는 기술 상세 / Flow 4로 낮춘다. 예를 들어 `latest_replay` blocker는 Flow 2 `실전 검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하라고 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 technical detail / JSON context에 남긴다.
...
- Single Strategy는 `app/web/backtest_single_strategy.py`, `app/web/backtest_single_forms/`, `app/web/backtest_single_runner.py`로 분리되어, form render와 runtime dispatch를 page shell에서 제거했다.
...
- annual strict, quarterly strict, price-only ETF 전략의 차이를 설명하기 위한 별도 보조 expander는 기본 실행 흐름에서 제거됐다.
