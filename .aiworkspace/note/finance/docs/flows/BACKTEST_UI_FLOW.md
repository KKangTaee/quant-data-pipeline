...
| `app/web/overview_ui_components.py` | Overview 전용 visual token, Market Context top cockpit / reading-flow section renderer, hybrid tape / sector pressure map / event timeline / evidence-row renderer, Market Movers refresh surface / metadata strip, Events summary/source/agenda/calendar/quality components, market session banner render |
