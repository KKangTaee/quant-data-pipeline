...
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. Decision Desk는 후보 수와 우선순위를 보여주되 visible Review Queue table은 렌더링하지 않는다. stable key selector에서 후보를 고른 뒤 `최종 검토서 확인`으로 저장 evidence를 읽을 후보를 확정하고, 선택이 바뀌면 report / Decision Cockpit / Final Decision Action을 숨겨 stale 판단을 막는다. 투자 검토서는 단일 상태와 세 점수 축 아래 총평을 보여주고, 바로 이어서 `성과 해석 / 위험 해석 / 근거 신뢰도 / Monitoring 적합성` 4개 세로 행을 표시한다. 10개 조건부 Monitoring 패턴은 named evidence adapter로 적용 여부를 판정하며 첫 화면에는 중요도 상위 최대 6개만 `현재 진단 / 의미 / 변화 조건 / 다음 행동`으로 보여준다. source / 기준일 / technical path와 나머지 패턴은 접힌 상세에 둔다. REVIEW는 `최종 판단에서 결정할 것 / 2단계에서 인수한 제한사항 / Monitoring으로 넘길 조건 / 선정 전 해소할 차단 항목`으로 stage ownership을 나누며 open 개수만으로 투자 매력도를 감점하거나 cap하지 않는다. 하단은 `점수 근거 / REVIEW 근거 / 대안 실험 후보` 세 탭이고, 대안은 counterfactual backtest 전에는 점수 개선으로 예측하지 않는다. 이후 사용자는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토 중 하나를 append-only 판단 record로 저장할 수 있다. selected-route gate를 통과한 selected row만 Portfolio Monitoring 후보 handoff 대상이며, Decision Cockpit / Final Decision Action / Evidence Appendix / Saved Decision Review가 기존 gate와 persistence 경계를 계속 소유한다. Final Review는 새 검증, provider fetch, DB 수집, live approval, broker order, auto rebalance를 수행하지 않는다.
...
- 조건부 패턴은 집중도, 주식-채권 분산, 금리 / 듀레이션, 인플레이션, tail risk, 추세, 구성요소 의존, 유동성 / 비용, benchmark, parameter sensitivity 10종이다. 저장 evidence의 명시된 source adapter만 사용해 `판단 가능 / 조건부 추적 / 추가 검증 필요 / 적용 제외`를 판정한다. 첫 화면은 최대 6개를 보여주고 기술 trace는 상세에서만 노출하며, 대안은 예상 점수가 아니라 별도 counterfactual backtest 후보로만 제시한다.
