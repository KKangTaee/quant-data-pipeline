...
Practical Validation current는 legacy `practical_validation_workspace`를 compatibility evidence source로 유지하면서, active first-read projection은 `practical_validation_decision_workspace_v1`이 소유한다. current eligible 후보는 unresolved actionable / critical engineering / missing contract가 모두 0이어야 한다. accepted limit / final decision / monitoring transfer는 Level2 repair queue가 아니라 Final Review handoff다. React는 raw status를 finding kind로 바꾸거나 Gate를 계산하지 않는다.
