...
Flow 4의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 유지한다. 다만 첫 criteria card는 table 값을 그대로 반복하지 않고 `resolution_guide`를 읽어 `검증한 것`, `해결해야 할 항목`, `해결 방법`, `통과 기준`, `위치`로 보여준다. 예를 들어 `latest_replay` blocker는 Flow 2 `검증 기준 설정 / 실전 재검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하고 어떤 상태가 되어야 통과인지 함께 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 Flow 4 technical detail / JSON context에 남긴다.
