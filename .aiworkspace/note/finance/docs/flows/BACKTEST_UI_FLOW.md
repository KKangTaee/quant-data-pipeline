...
| `app/web/backtest_final_review/page.py` | Final Review 화면 render. Decision Desk command center와 통합된 Review Queue / 검토 대상 selector / 후보 비교 상세, 선택 후보 investment report React section과 Decision Cockpit, Final Review 판단 저장 checklist / route guide, Monitoring 후보 handoff 상태 안내, Evidence Appendix의 Validation / Robustness / Paper Observation / Investability Evidence Packet read-only 확인, saved final selection review ledger, Selected Dashboard handoff summary, Decision Dossier download. Final Review first-read에서는 별도 `Step 1 / Candidate Board`와 CNN / AAII 시장심리 패널을 렌더링하지 않는다 |
...
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. 통과 기준은 `final_review_gate.can_save_and_move=True`이며, 저장만 된 blocked / needs input / not run validation row는 기록으로 남더라도 source picker에서 숨긴다. Final Review는 전용 Decision Desk shell에서 후보 수, 선택 가능, 보류 / 재검토, 숨김, 저장된 판단, Monitoring 연결, 오늘 먼저 볼 후보를 먼저 보여준다. 같은 first-read 영역 아래에서 Candidate Board read model의 Review Queue, `검토 대상` selector, 접힌 `후보 비교 상세`를 제공하지만, 별도 `Step 1 / Candidate Board`와 중복 select-ready / hold / blocked lane card는 렌더링하지 않는다. 선택 후보는 `Final Review 투자 검토서`에서 최종 선택 사유, 세부 점수, score cap, Level2 REVIEW 점수 영향, 강점 / 약점, 약점 개선안, 판단 저장 전 메모를 먼저 읽고, Decision Cockpit에서 Must Fix / open review items / monitoring seed를 확인한다. Level2 REVIEW는 Final Review에서 다시 실행하지 않고 role별로 투자 매력도 / 위험 / 준비도 / 근거 품질 / Monitoring 적합성 점수에 반영한다. hard blocker, selected-route not-ready, gate review-required, 과도한 open review는 높은 원점수라도 추천권으로 과대평가되지 않도록 score cap을 만든다. 이후 사용자는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토 중 하나를 Final Review 판단 route로 선택하고, operator reason과 함께 `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 append-only record로 저장할 수 있다. Decision Record Checklist는 suggested decision, selected-route gate, operator reason requirement, Monitoring handoff 여부, live approval / order disabled boundary를 저장 전 안내한다. Portfolio Monitoring 후보 handoff는 `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row의 `monitoring_candidate=True`일 때만 만들어지고, 보류 / 거절 / 재검토는 판단 기록으로만 남는다. 저장된 판단 기록은 Saved Decision Review ledger에서 selected count, latest selection, detail tabs, Portfolio Monitoring handoff, Decision Dossier export로 다시 읽는다. Portfolio Monitoring handoff는 기존 selected row를 다시 검증하지 않고 `monitoring_candidate` row, dashboard rows, monitorable / blocked counts와 checklist만 read-only로 보여준다. 선택된 Practical Validation result와 diagnostics 요약, Construction Risk, Risk Contribution, Component Role / Weight, Validation Method Strength, Data Coverage, Backtest Realism, Robustness / Paper Observation / Investability Evidence Packet 상세는 `Evidence Appendix`에서 read-only로 확인한다. profile-aware selection gate policy는 hard blocker와 open review item을 분리한다. critical gap, benchmark / comparator parity blocker, Validation Method Strength / Data Coverage / Backtest Realism의 핵심 `NEEDS_INPUT` / `BLOCKED`, explicit gross-only 또는 net/cost 적용 부재, weighted mix component role / weight rationale 부재, selected-route policy blocker가 남아 있으면 Monitoring 후보 handoff는 차단된다. 기본 `REVIEW`는 판단 record 저장을 막지 않고 `open_review_items`로 Portfolio Monitoring과 향후 Live / Deployment Readiness에서 이어서 본다. 기존처럼 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 함께 보존한다. Structured waiver는 현재 UI / persistence가 없고, future policy도 `BLOCK` waiver를 허용하지 않는다. 저장된 Final Review 판단 기록은 read-only Decision Dossier markdown으로 다운로드할 수 있지만 report 파일을 자동 생성하지 않는다.
