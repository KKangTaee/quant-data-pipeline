...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `검증한 것 / 부족한 것 / 해야 할 일 / 확인 위치` guide, 상세 evidence tabs, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
Flow 4의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 유지한다. 다만 첫 criteria card는 table 값을 그대로 반복하지 않고 `resolution_guide`를 읽어 `검증한 것`, `부족한 것` 또는 `확인할 것`, `해야 할 일`, `확인 위치`로 보여준다. 예를 들어 `latest_replay` blocker는 Flow 2 `검증 기준 설정 / 실전 재검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하라고 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 Flow 4 technical detail / JSON context에 남긴다.
