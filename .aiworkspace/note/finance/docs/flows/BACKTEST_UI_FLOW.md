...
| `app/services/backtest_evidence_read_model.py` | Streamlit-free final decision evidence read model. Final Review Candidate Board / Decision Cockpit / investability evidence packet / selected-route gate / saved decision status / table row, Selected Dashboard evidence check row, Decision Dossier markdown read model과 selected decision source contract를 공통으로 만든다 |
| `app/web/backtest_final_review.py` | Final Review 화면 render. Practical Validation Gate 통과 후보 Candidate Board, 선택 후보 Decision Cockpit, Validation / Robustness / Paper Observation / Investability Evidence Packet 기준 확인, 최종 판단 기록, saved final decision review, Decision Dossier download |
