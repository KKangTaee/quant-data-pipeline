...
| `app/web/overview_ui_components.py` | Overview 전용 visual token, Market Context 2~3문장 narrative cockpit / reading-flow section renderer, `다음 맥락 체크` / historical reference / source evidence footer renderer, hybrid tape / sector pressure map / event timeline / evidence-row renderer, Market Movers refresh surface / metadata strip, Events summary/source/agenda/calendar/quality components, market session banner render |
