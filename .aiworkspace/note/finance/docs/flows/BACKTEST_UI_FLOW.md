...
`concentration-overlap-exposure-contract-v1`부터 Practical Validation result에는 compact `construction_risk_audit`가 붙는다. 이 audit은 component max weight, provider look-through coverage, top holding, holdings overlap, dominant asset bucket, unknown exposure를 분리해 표시한다. provider holdings / exposure가 없거나 partial이면 ready로 올리지 않고, selected-route gate enforcement는 후속 construction risk gate policy task에서 처리한다. 새 JSONL registry, user memo, preset, approval, order, auto rebalance는 추가하지 않는다.
