...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, `단계별 검증 소유권`, Provider / Data 보강 액션, 접힌 `근거 부록`, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. Flow 2에서 현재 세션 replay가 없으면 Flow 3 / Flow 4 / Flow 5와 결과 JSON을 렌더링하지 않는다. Flow 3 / Flow 4 visible UI는 Final Review 전용 REVIEW 판단과 REVIEW-only / empty category를 actionable issue로 노출하지 않으며, 조건부 / 후속 참고 그룹도 Flow 3 하단 expander로 반복 노출하지 않는다. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 source snapshot 확인만 소유하고, Flow 2가 validation profile과 latest runtime replay 실행을 함께 소유한다. 현재 브라우저 세션에서 Flow 2 replay를 실행하기 전에는 Flow 3 / Flow 4 / Flow 5와 결과 JSON을 보여주지 않는다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 compatibility path를 유지하되 visible UI는 `검증 결론`과 `카테고리별 검증 요약`을 read-only로 보여준다. Flow 3은 Practical Validation outcome과 카테고리별 통과 / 실패만 보여주고, Final Review 이동 상태, REVIEW count, 조건부 / 후속 참고 expander를 보여주지 않는다. raw status, row evidence, module table, resolution guide는 Flow 4로 낮춘다. Flow 4는 같은 workspace read model의 `visible_criteria_detail_groups`를 `카테고리별 검증 결과` board로 풀어 각 기준의 사용자-facing 판정 근거와 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide를 보여준다. Flow 4에는 접힌 `단계별 검증 소유권` read-only inventory가 있어 Backtest Analysis / Practical Validation / Final Review / Portfolio Monitoring 중 어느 단계가 해당 기준을 소유하는지 확인할 수 있다. Full `criteria_detail_groups`에는 Final Review reference metadata가 남지만, REVIEW-only / empty category는 Flow 3 / Flow 4 visible category result에서 숨긴다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
...
Flow 4 `검증 기준 상세`에는 Provider Action Center를 표시한다. 사용자는 ETF별 Provider Data Gaps 표를 바로 보기 전에 actionable gap 수, 현재 수집 / 보강 가능한 항목, connector mapping 필요 항목을 카드로 먼저 본다. 수집 대상 table은 `수집 대상 근거` 아래의 접힌 `수집 대상 상세`에 두고, 수집 가능한 provider / holdings / exposure / macro 데이터는 같은 화면에서 일괄 수집 / 보강할 수 있다. 이때 화면은 버튼과 결과 표시만 맡고, 수집 계획과 ingestion job orchestration은 Practical Validation service가 맡는다. 공식 source mapping이 없는 holdings / exposure는 먼저 `etf_provider_source_map` discovery를 실행해 verified endpoint를 찾고, 자동 탐색 후에도 없을 때만 수동 connector mapping 필요로 남긴다.
