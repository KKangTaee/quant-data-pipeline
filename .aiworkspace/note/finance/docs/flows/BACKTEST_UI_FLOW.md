...
| `app/web/overview_dashboard.py` | `Workspace > Overview`에서 Market Context, Market Movers, Why It Moved, Sector / Industry, Sentiment, Events render. Market session banner, daily snapshot refresh action bar, browser-session auto refresh heartbeat, Market Movers manual investigation panel, Sector / Industry ranking/trend, Sentiment context, Events agenda/calendar/quality/raw views, Market Context historical analog 부족 ETF 가격 이력 보강 버튼을 조정한다 |
