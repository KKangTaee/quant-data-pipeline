...
| `app/web/final_selected_portfolio_dashboard.py` | `Operations > Selected Portfolio Dashboard` 화면 render. Final Review selected 후보 pool과 사용자-created monitoring portfolio setup을 읽고, fixed-height portfolio card shelf / 생성 / 선택 / collapsed management soft delete, selected portfolio command band, compact selected strategy slot board / 설정 적용 / 제거, portfolio-wide Monitoring Scenario cockpit / value curve / strategy performance / rebalance target table, strategy별 Monitoring Scenario, Monitoring Signals, 전환 비교, 하단 evidence detail, optional preflight, Actual Allocation / allocation evidence boundary, Decision Dossier / Audit을 보여준다 |
...
- `Operations > Selected Portfolio Dashboard`: `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 선정된 row만 selected strategy pool로 읽고, 사용자가 만든 dashboard portfolio에 strategy slot으로 추가해 모니터링한다. 각 slot은 start / latest-end mode / balance / memo를 저장하고 `전략 적용` 이후 별도 `모니터 시나리오 실행` 또는 `포트폴리오 시나리오 실행`으로 selected component contract를 replay한다. Dashboard는 fixed-height portfolio shelf, selected portfolio command band, compact strategy board, portfolio-wide scenario cockpit 순서로 읽힌다. Portfolio-wide cockpit은 value / P&L / return / CAGR / MDD / benchmark spread / value curve / 전략별 성과 / 리밸런싱 target을 먼저 보여주고, Snapshot, Final Review -> dashboard continuity check, source contract, Monitoring Signals의 Timeline / Review Signals / Open Issues / Why Selected / optional Actual Allocation / allocation evidence boundary / Decision Dossier / Audit을 뒤에서 보여준다. Recheck Operations Preflight / Recheck Readiness / Symbol Freshness / Provider Evidence는 하단 상세 점검으로 낮춘다. 같은 dashboard portfolio 안 전략이 2개 이상이면 최신 scenario 결과로 전환 비교를 표시한다. Preflight / Readiness / Symbol Freshness / Provider Evidence / Continuity / Timeline / Recheck Comparison / Allocation Boundary / Dossier는 read-only이며, live approval / broker order / account sync / auto rebalance는 disabled로 둔다.
...
| `app/web/final_selected_portfolio_dashboard.py` | Operations dashboard 화면 render, 나의 포트폴리오 fixed-height card shelf 생성 / 선택 / collapsed management soft delete, selected portfolio command band, Final Review selected strategy pool에서 slot 추가 / compact strategy board / 설정 적용 / 제거, portfolio-wide Monitoring Scenario cockpit / value curve / strategy performance / rebalance target, strategy별 Snapshot / Monitoring Scenario / 하단 readiness + symbol freshness + provider evidence detail, Continuity check, Monitoring Timeline / Review Signals / Open Issues / Why Selected / optional Actual Allocation / allocation evidence boundary / optional preflight / Decision Dossier / Audit / 전환 비교 표시 |
...
- performance recheck:
  - strategy slot 저장과 scenario 실행은 분리
