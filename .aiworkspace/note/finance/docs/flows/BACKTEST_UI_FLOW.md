...
- `실전성 검증 Handoff`는 데이터 기준까지 확인한 뒤 바로 다음 단계 action을 판단하는 영역이므로 상세 결과 탭보다 먼저 둔다. 2026-07-02 V1부터 이 영역은 중복 Streamlit container / 중복 heading 없이 하나의 custom handoff panel에서 진입 상태, blocker / review reason, source 등록 boundary를 함께 보여준다.
- 기존 결과 해석 순서용 checkpoint / availability badge 가이드는 기본 실행 결과 화면에서 제거했다. 반복 설명이 필요하면 `Reference` help나 guide 문서에서 다룬다.
- `Selection History`, `Dynamic Universe`, `Policy Signal Meta`는 안내 카드가 아니라 result bundle에 해당 근거가 있을 때만 열리는 조건부 결과 탭이다.
