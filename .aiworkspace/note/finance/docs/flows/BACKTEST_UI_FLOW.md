...
The production path uses targeted React custom components only where HTML design and real interaction must live together. `app/web/components/backtest_handoff_action/` owns the visible Handoff card and source registration button inside the Handoff panel, while Python receives the submit event and performs source registration / registry write / rerun. `app/web/components/backtest_policy_signal_board/` owns the first-stage evidence board UI only; Python service code still owns policy classification, gate math, persistence, and source handoff.
...
- 2026-07-05 V1부터 DB 공통 최신 가격일이 현재 기준 최신 완료 거래일보다 오래된 경우, `데이터 기준 요약` 아래에 `가격 데이터 업데이트` action을 표시한다. 이 action은 주말 / NYSE 휴장일을 제외한 목표 기준일까지 현재 백테스트 ticker만 기존 OHLCV ingestion job으로 보강한다. 실행 후 자동 재백테스트, 후보 등록, 2차 검증 전송은 하지 않으며 사용자가 `Run Backtest`를 다시 눌러 최신 DB 기준 결과를 계산한다.
