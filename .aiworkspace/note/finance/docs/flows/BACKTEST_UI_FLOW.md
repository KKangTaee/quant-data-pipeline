...
| `app/web/backtest_final_review/page.py` | Final Review 화면 render. Decision Desk와 stable-key 검토 대상 selector / 후보 비교 상세, confirmed candidate stale guard, 투자 검토서에 전달할 Python-owned decision action model, React route / reason intent 소비, 자동 Decision ID / constraints / next action 적용, authoritative save evaluation / append를 소유한다. Evidence Appendix, Saved Decisions ledger, Dossier / Evidence Packet / Raw JSON과 별도 `Step 1 / Candidate Board`, visible Review Queue / standalone Decision Cockpit, CNN / AAII 시장심리 패널은 렌더링하지 않는다 |
...
- `Final Review`: Practical Validation Gate를 통과한 result만 검토 대상 목록에 표시한다. Decision Desk는 후보 수와 우선순위를 보여주되 visible Review Queue table은 렌더링하지 않는다. stable key selector에서 후보를 고른 뒤 `최종 검토서 확인`으로 저장 evidence를 읽을 후보를 확정하고, 선택이 바뀌면 report / decision intent를 숨겨 stale 판단을 막는다. 투자 검토서는 단일 상태와 세 점수 축, 총평, `성과 해석 / 위험 해석 / 근거 신뢰도 / Monitoring 적합성` 4행 직후에 최종 판단 route와 사용자가 직접 작성하는 판단 사유, gate 기반 CTA를 배치한다. React는 표시와 intent만 맡고 Python이 save evaluation, 자동 Decision ID, route template, row 작성과 append를 소유한다. 이후 강점 / 약점, 저장 전 확인 질문, 최대 6개 행동형 Monitoring 패턴, `Final Review에서 결정할 것`, `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어`가 이어진다. open REVIEW 개수만으로 투자 매력도를 감점하거나 cap하지 않으며 counterfactual 실험은 자동 실행하지 않는다. Evidence Appendix와 Saved Decisions ledger는 렌더링하지 않고 selected row 운영 확인은 Operations > Portfolio Monitoring이 맡는다. Final Review는 새 검증, provider fetch, DB 수집, live approval, broker order, auto rebalance를 수행하지 않는다.
...
Practical Validation Gate 통과 후보
  ...
  -> 2. Final Review 투자 검토서
     -> 단일 판단 상태, 세 점수 축, 총평과 4행 해석 확인
  -> 3. 총평 직후 최종 판단
     -> route 선택, 사용자 판단 사유 입력, gate 기반 CTA 활성 / 비활성
  ...
  -> 4. 강점 / 약점, Monitoring 방향, Final Review 확인 필요와 근거 상세 확인
...
- `Final Review 투자 검토서`는 Python `backtest_evidence_read_model`과 `backtest_final_review_guidance`가 만든 report payload를 React component가 렌더링하는 first-read surface다. 단일 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평과 해석 4행 직후에 route / 판단 사유 / CTA를 표시하고 이후 강점 / 약점, 저장 전 확인 질문, 행동형 조건부 패턴, 연결된 근거 상세 shell을 이어서 보여준다. React는 decision intent만 전달하며 gate 계산, save evaluation, 자동 Decision ID, row 작성 / registry append, DB / provider fetch, strategy variant 생성은 Python이 소유한다.
...
- Candidate Review와 Portfolio Proposal의 판단 field는 준비 기록이고, Final Review 총평 직후의 route / 판단 사유 / CTA가 최종 판단 record를 남기는 주 decision surface다. 실제 자금 투입 가능 여부는 향후 Live / Deployment Readiness에서 별도로 판단한다.
...
Backtest > Final Review
  ...
  -> 강점 / 약점, Monitoring 방향, Final Review 확인 필요, 근거 상세 확인
