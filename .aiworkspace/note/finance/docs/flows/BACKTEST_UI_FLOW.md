...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 사용자는 profile과 5개 답변을 고르고, 화면은 `후보 / 검증 프로필 확인 -> 실전 검증 실행 -> 2차 검증 결론 / Fix Queue -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 이동 차단 / review / core evidence를 먼저 보여준다. Flow 4는 Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center를 담고, Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 별도 `시장 심리 Context Overlay`는 `context_only`, `gate_effect=none`, `registry_write=false`이며 gate, registry, saved setup, live approval / broker order / auto rebalance에 영향을 주지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 Flow 3 `2차 검증 결론 / Fix Queue`에서 이동 차단 / review / 핵심 근거를 먼저 보여주고, Applied Validation Map은 Flow 4의 보조 `검증-근거 연결 지도`로 접어 둔다. 각 evidence / action board title 아래에는 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
Fix Queue의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 먼저 보여준다. 예를 들어 `latest_replay` blocker는 Flow 2 `실전 검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하라고 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 technical detail / JSON context에 남긴다.
