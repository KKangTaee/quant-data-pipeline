...
| `app/web/overview_dashboard_helpers.py` | Overview dashboard용 cached market intelligence service wrapper와 market / sentiment / events / data-health snapshot loader. legacy candidate / Pre-Live / proposal snapshot helper는 primary Overview에서 사용하지 않는다 |
...
| `app/web/backtest_candidate_review.py` | legacy Candidate Review / Candidate Packaging / Pre-Live 운영 기록 화면 render logic. Primary Backtest navigation에서는 직접 dispatch하지 않는다 |
| `app/web/backtest_candidate_review_helpers.py` | legacy Candidate Review 판단, Review Note / registry 변환, Pre-Live status 추천 / draft 변환 / Portfolio Proposal 진입 readiness score helper |
| `app/web/backtest_portfolio_proposal.py` | legacy 단일 후보 직행 평가, 다중 후보 Portfolio Proposal 후보 선택 / 목적 / 역할 / 비중 설계, proposal draft 저장, 저장된 proposal monitoring / feedback section render logic. Primary Backtest navigation에서는 직접 dispatch하지 않는다 |
| `app/web/backtest_portfolio_proposal_helpers.py` | legacy Portfolio Proposal row 생성, 단일 후보 direct readiness / proposal save readiness 평가, 공유 validation / robustness 계산 helper, monitoring / Pre-Live / paper feedback table helper |
...
- `Candidate Review`, `Portfolio Proposal`, 기존 Pre-Live registry, 기존 proposal registry는 바로 삭제하지 않는다. 다만 새 주 흐름의 필수 join 조건이 아니라 archive / recovery compatibility로 낮춘다.
...
| `app/web/backtest_compare.py`, `app/web/backtest_compare_components.py` | Portfolio Mix Builder orchestration과 visual shell |
...
| `app/web/backtest_candidate_review*.py`, `app/web/backtest_portfolio_proposal*.py` | legacy candidate / proposal compatibility module. Primary page shell에서 직접 dispatch하지 않는다 |
...
- Candidate Review / Portfolio Proposal module은 남아 있지만 primary panel wrapper와 page-shell dispatch는 제거된 compatibility surface다.
...
- legacy helper module은 당장 물리 삭제하지 않고, 후속 cleanup에서 import 사용처와 registry migration 필요성을 확인한 뒤 제거 여부를 결정한다.
...
- legacy records를 읽어야 할 때는 `Operations > Archive: Candidates`와 관련 helper를 archive recovery 용도로 사용한다.
