...
| `app/web/final_selected_portfolio_dashboard.py` | `Operations > Selected Portfolio Dashboard` 화면 render. Final Review selected 후보 pool과 사용자-created monitoring portfolio setup을 읽고, portfolio card shelf / 생성 / 선택 / soft delete, selected strategy slot 추가 / 설정 적용 / 제거, portfolio-level Monitoring Scenario summary / value curve / strategy performance / rebalance target table, strategy별 Monitoring Scenario, Monitoring Signals, 전환 비교, 하단 evidence detail, optional preflight, Actual Allocation / allocation evidence boundary, Decision Dossier / Audit을 보여준다 |
...
- `Selected Portfolio Dashboard`: `SELECT_FOR_PRACTICAL_PORTFOLIO` selected row만 strategy pool로 읽는다. Dashboard는 먼저 `나의 포트폴리오` card shelf를 보여주고, 선택한 portfolio 안에 selected decision strategy slot을 담아 start / latest-end mode / balance / memo를 저장한다. Scenario 실행은 slot 저장과 분리되며, portfolio-level value / P&L / return / CAGR / MDD / benchmark spread / rebalance target을 먼저 보여준다. Final Review의 `open_review_items`, review trigger, `deployment_readiness_policy_snapshot`, recheck / provider / continuity / review signal / allocation boundary evidence는 하단 상세 / optional preflight에서 read-only로 확인한다. 이 preflight는 실제 돈 투입 승인, 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. 통과 후보가 없으면 strategy pool은 비어 있고 Final Review 선정 안내를 보여준다.
...
| `app/web/final_selected_portfolio_dashboard.py` | Operations dashboard 화면 render, 나의 포트폴리오 card shelf 생성 / 선택 / soft delete, Final Review selected strategy pool에서 slot 추가 / 설정 적용 / 제거, portfolio-level Monitoring Scenario summary / value curve / strategy performance / rebalance target, strategy별 Snapshot / Monitoring Scenario / 하단 readiness + symbol freshness + provider evidence detail, Continuity check, Monitoring Timeline / Review Signals / Open Issues / Why Selected / optional Actual Allocation / allocation evidence boundary / optional preflight / Decision Dossier / Audit / 전환 비교 표시 |
...
- performance recheck:
  ...
  - 실행 후 portfolio-level value / P&L / return / CAGR / MDD / benchmark spread / rebalance target을 먼저 표시
