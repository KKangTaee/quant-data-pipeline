...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, Final Review 참고 handoff count, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, 상세 evidence tabs, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 validation profile과 5개 답변을 고른 뒤 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Final Review 이동 가능 / 보류와 카테고리별 `통과 / 실패 / 확인 필요`만 단일 first-read conclusion surface로 보여준다. Flow 4는 `카테고리별 검증 결과` board에서 먼저 `통과`, `보강 후 재검증 필요`, `실전 사용 어려움` main outcome summary를 보여주고, 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약한다. `READY`는 사용자-facing 통과로 읽고, `REVIEW`는 보강 실패가 아니라 Final Review에서 해석할 참고 항목으로 넘기며 Flow 4 상세 문제 목록에는 노출하지 않는다. `NEEDS_INPUT` / `NOT_RUN`은 보강 후 재검증 대상이고, `BLOCKED`는 현재 상태로 실전 후보 사용이 어렵다는 차단 결론이다. 각 기준 상세는 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치`로 보여준다. Flow 4 guide는 module-level 문구만 쓰지 않고 가능하면 audit row의 non-PASS `Criteria`와 `Next Action`을 끌어와 실제 부족 항목과 실행 방법을 표시한다. `통과 기준`은 보강 뒤 무엇이 되어야 해결된 것인지 판단하는 기준이며, 위치는 `Flow4 > 데이터 > 데이터 품질 / 편향 통제 상세`, `Flow4 > Provider / Data 보강 액션`처럼 사용자가 찾을 수 있는 화면 경로로 표시한다. Flow 4는 검증 모듈 table, Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center도 담는다. Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 `Final Review 이동 전 검증 상태`를 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 source snapshot 확인만 소유하고, Flow 2가 validation profile과 latest runtime replay 실행을 함께 소유한다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 compatibility path를 유지하되 visible UI는 `검증 결론`과 `카테고리별 검증 요약`을 read-only로 보여준다. Flow 3은 카테고리별 통과 / 실패 / 확인 필요만 보여주고, raw status, row evidence, module table, resolution guide는 Flow 4로 낮춘다. Flow 4는 같은 workspace read model의 `criteria_detail_groups`를 `카테고리별 검증 결과` board로 풀어 각 기준의 사용자-facing 판정 근거와 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide를 보여준다. 다만 `REVIEW` 항목은 Flow 4 상세 문제로 반복하지 않고 `Final Review 참고` handoff count로만 표시한다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 Flow 3 `검증 결론`에서 Final Review 이동 판단과 카테고리별 통과 / 실패 / 확인 필요만 먼저 보여주고, Flow 4 첫 board에서 gate-relevant 기준별 판정 근거를 자세히 보여준다. Applied Validation Map은 Flow 4의 보조 `검증-근거 연결 지도`로 접어 둔다. 각 evidence / action board title 아래에는 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
Flow 4의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 유지한다. 다만 첫 criteria card는 table 값을 그대로 반복하지 않고 `resolution_guide`를 읽어 `검증한 것`, `해결해야 할 항목`, `해결 방법`, `통과 기준`, `위치`로 보여준다. `해결 방법`은 slash-joined 문장이 아니라 `action_steps`를 번호형 목록으로 렌더링하며, non-PASS audit row의 `Next Action`을 가장 구체적인 단계로 우선 사용한다. 예를 들어 `latest_replay` blocker는 Flow 2 `검증 기준 설정 / 실전 재검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하고 어떤 상태가 되어야 통과인지 함께 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, `REVIEW`는 Flow 4 main board에서 해결 대상이 아니라 Final Review 참고 count로 낮춘다. raw route id는 Flow 4 technical detail / JSON context에 남긴다.
