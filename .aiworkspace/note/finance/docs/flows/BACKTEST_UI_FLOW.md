...
Handoff owns entry judgment and source registration action. `2차 실전성 검증 Handoff`는 Practical Validation 진입 가능 여부, review / blocker 요약, source 등록 버튼, 저장 boundary를 한 곳에서 보여준다. Policy Signals owns evidence detail. `검증 신호 · Policy Signals`는 같은 진입 가능 판단을 다시 반복하지 않고, Promotion / Data Trust / Execution / Validation 근거를 `검증 기준 상세`와 접힌 기술 근거로 설명한다.

The production path remains Streamlit-only. `app/web/backtest_result_display.py`의 Handoff action shell은 `st.button`을 유지하고, source 등록 / registry write / rerun은 기존 Python path가 소유한다. `app/web/components/backtest_handoff_action/`에는 React custom component POC가 있지만, 현재 source registration action에는 연결하지 않는다. React custom component POC는 향후 같은 형태의 고급 action card가 반복될 때만 production wiring 후보로 본다.
