...
| `app/services/backtest_practical_validation_diagnostics.py` | Streamlit-free Practical Validation diagnostics service. component context assembly, 12개 Practical Diagnostics result, validation module plan, final review gate, compatibility export를 만든다 |
| `app/services/backtest_practical_validation_modules.py` | Streamlit-free Practical Validation module planner. source traits와 profile / input checks / diagnostic / audit rows를 읽어 필수 / 조건부 / 후속 참고 module과 Final Review 이동 gate를 만든다 |
...
| `app/web/backtest_practical_validation.py` | `Practical Validation` stage render. Clean V2 source 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Final Review Gate / validation module board, V2 practical diagnostics board, Provider Data Gaps 표시, service handoff 결과의 session state 반영을 담당 |
