...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 validation profile과 5개 답변을 고른 뒤 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Final Review 이동 가능 / 보류와 카테고리별 `통과 / 실패 / 확인 필요`만 단일 first-read conclusion surface로 보여준다. Flow 4는 `카테고리별 검증 결과` board에서 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약하고, `현재 문제 / 완료 기준 / 보강 위치`, 검증 모듈 table, Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center를 담는다. 보강 위치는 `Flow 4 · Audit name` 같은 내부 이름이 아니라 `검증 기준 상세 · 데이터 품질 / Provider 보강`처럼 사용자가 찾을 수 있는 화면 기준 위치로 표시한다. Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 `Final Review 이동 전 검증 상태`를 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
...
Flow 4의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 만든다. 예를 들어 `latest_replay` blocker는 Flow 2 `검증 기준 설정 / 실전 재검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하라고 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 Flow 4 technical detail / JSON context에 남긴다.
