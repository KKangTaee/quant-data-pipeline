...
| `app/web/backtest_practical_validation.py` | `Practical Validation` stage render. Clean V2 source 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Final Review Gate / validation module board / Applied Validation Map, V2 practical diagnostics board, Provider Data Gaps 표시, service handoff 결과의 session state 반영을 담당 |
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 각 board title 아래에 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시하고, 상단 `Applied Validation Map`에서 적용 보드 / 비적용 보드 / 모듈 연결을 확인하게 한다.
