...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board, summary-first `검증 근거 보드`, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 근거 Workbench -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 validation profile과 5개 답변을 고른 뒤 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Final Review 이동 가능 / 보류와 카테고리별 `통과 / 실패 / 확인 필요`만 단일 first-read conclusion surface로 보여준다. Flow 4는 `카테고리별 검증 결과` board에서 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약하고, `현재 문제 / 완료 기준 / 보강 위치`, 검증 모듈 table, Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Applied Validation Map, Provider Action Center를 담는다. Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 `Final Review 이동 전 검증 상태`를 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
