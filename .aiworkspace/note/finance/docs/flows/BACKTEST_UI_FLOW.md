...
- 실행 직후 화면은 `전략명 백테스트 결과` 헤더 안의 통합 KPI band, 내부 title을 가진 `데이터 기준 요약` panel, 상세 결과 탭, `실전성 검증 Handoff` 순서로 읽는다.
...
- `실전성 검증 Handoff`는 결과 확인 이후 다음 단계 action이므로 상세 결과 탭 아래로 둔다.
