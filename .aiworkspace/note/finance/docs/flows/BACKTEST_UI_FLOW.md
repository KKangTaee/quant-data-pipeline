...
- 상단은 한 줄 결론, 상태 pill, `계산 기준일 / 가격 기준 / 사용 데이터 / 검토 큐` 4개 요약으로 구성한다.
- raw date / row count / common latest price / latest-date spread는 별도 guide/expander가 아니라 4개 요약의 보조 detail로 압축한다.
- warning, excluded ticker, malformed price row는 별도 노란 경고 박스가 아니라 같은 패널 안의 `이번 실행 검토 큐` row로 묶어, 성과를 보기 전에 확인할 항목만 남긴다.
