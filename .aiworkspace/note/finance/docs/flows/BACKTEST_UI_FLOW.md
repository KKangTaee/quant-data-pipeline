...
- `실전성 검증 Handoff`: 진입 상태, 10점 만점 readiness score, Promotion / 실행 원천 / 검증 원천 chip, source registration button inside the Handoff panel, source 등록 boundary를 한 panel에서 보여준다. hard blocker가 없고 review focus만 있으면 상세 사유를 1차에서 펼치지 않고 `2차 확인 항목 N개`로 요약해 Practical Validation으로 넘긴다. Handoff owns entry judgment and source registration action. React custom component는 보이는 Handoff card / button / submit event만 맡고, source registration write / rerun은 Python path가 계속 소유한다.
...
- `실전성 검증 Handoff`는 데이터 기준까지 확인한 뒤 바로 다음 단계 action을 판단하는 영역이므로 상세 결과 탭보다 먼저 둔다. 2026-07-02 V1부터 이 영역은 중복 Streamlit container / 중복 heading 없이 하나의 custom handoff panel에서 진입 상태, blocker / review reason, source 등록 boundary를 함께 보여준다. V2-V6 이후 blocker / review display는 service의 grouped `Promotion / 실행 원천 / 검증 원천` summary를 사용한다. 2026-07-05 correction부터 보이는 Handoff card와 버튼은 React custom component가 함께 렌더링하고, Python은 submit event를 받아 current selection source 등록만 수행한다.
