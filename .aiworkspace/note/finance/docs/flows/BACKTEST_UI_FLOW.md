...
Handoff owns entry judgment and source registration action. `2차 실전성 검증 Handoff`는 1차 source 등록 기준 통과 / 보류, 먼저 해결할 blocker 수, Practical Validation으로 넘길 2차 확인 큐 수, source 등록 버튼, 저장 boundary를 한 곳에서 보여준다. 보이는 Handoff card는 readiness score를 진입 조건처럼 표시하지 않는다. Policy Signals owns evidence detail. `검증 신호 · Policy Signals`는 같은 진입 가능 판단을 다시 반복하지 않고, `Data Trust`, `Execution Source`, `Validation Source` 중심의 1차 기준을 `검증 기준 상세` React board로 설명한다. 각 기준은 `?` help로 사람이 읽는 설명과 실제 체크 항목을 보여주며, 2차 review focus 상세 목록은 Practical Validation의 `Backtest에서 넘어온 2차 확인 항목`으로 넘긴다.
...
- `실전성 검증 Handoff`: 1차 source 등록 기준 통과 / 보류, 먼저 해결할 blocker, 2차 확인 큐, source registration button inside the Handoff panel, source 등록 boundary를 한 panel에서 보여준다. hard blocker가 없고 review focus만 있으면 상세 사유를 1차에서 펼치지 않고 `2차 확인 큐 N개`로 요약해 Practical Validation으로 넘긴다. Handoff owns entry judgment and source registration action. React custom component는 보이는 Handoff card / button / submit event만 맡고, source registration write / rerun은 Python path가 계속 소유한다.
- `검증 신호 · Policy Signals`: result tab 안에서 1차에서 확인 가능한 source 등록 기준만 `검증 기준 상세` React board로 보여준다. 각 1차 row는 무엇을 확인했는지, 왜 통과 / 차단인지, 미통과 시 무엇을 봐야 하는지를 표시한다. Practical Validation review focus rows는 1차 처리 항목이 아니라 source evidence로 저장하고, Backtest Analysis에서는 2차 전달 count / group handoff만 표시한다. 상세 row는 Practical Validation `1. 선택 후보 확인`의 `Backtest에서 넘어온 2차 확인 항목`에서 이어 본다. Benchmark / execution / raw technical payload는 접힌 상세 근거로 낮추며 nested `현재 판단 / 검토 근거 / 실행 부담 / 기술 상세` 탭은 active render path에서 사용하지 않는다.
...
- `Practical Validation review focus`: hold 사유, caution source, benchmark / drawdown / liquidity / price freshness / regime / split-period 확인 항목이다. Backtest Analysis에서는 count와 전달 상태만 보여주고, 상세 row는 Practical Validation `1. 선택 후보 확인`의 `Backtest에서 넘어온 2차 확인 항목`에서 이어 본다.
...
- 상단은 한 줄 결론, 상태 pill, `계산 기준일 / 가격 기준 / 사용 데이터 / 1차 데이터 확인 또는 2차 전달` 4개 요약으로 구성한다.
...
- `meta["warnings"]`에 들어온 실전성 review focus는 Backtest Analysis에서 상세 row로 펼치지 않고 `2차 확인 항목` count / 위치 안내만 남긴다. 상세 판단은 Practical Validation `Backtest에서 넘어온 2차 확인 항목`에서 이어 본다.
...
- `실전성 검증 Handoff`는 데이터 기준까지 확인한 뒤 바로 다음 단계 action을 판단하는 영역이므로 상세 결과 탭보다 먼저 둔다. 2026-07-02 V1부터 이 영역은 중복 Streamlit container / 중복 heading 없이 하나의 custom handoff panel에서 진입 상태, blocker / review reason, source 등록 boundary를 함께 보여준다. V2-V6 이후 blocker / review display는 service의 grouped summary에서 출발했지만, 2026-07-05 entry-gate correction부터 보이는 Handoff card는 `1차 진입 기준 / 먼저 해결 / 2차 확인 큐`로 정리한다. `promotion_decision=hold` 같은 review focus는 버튼을 막는 1차 blocker가 아니라 Practical Validation으로 전달되는 2차 확인 큐다. 보이는 Handoff card와 버튼은 React custom component가 함께 렌더링하고, Python은 submit event를 받아 current selection source 등록만 수행한다.
