...
이 문서는 Streamlit Backtest 화면의 Backtest Analysis, Practical Validation, Final Review, Operations Console, Portfolio Monitoring, Operations-owned archive / recovery 흐름을 설명한다.
UI form, payload 복원, history replay, candidate archive replay, saved weighted portfolio replay를 수정할 때 먼저 확인한다.
...
| `app/web/overview_dashboard.py` | `Workspace > Overview`에서 Market Movers, Futures Monitor, Sentiment, Sector / Industry, Events, Data Health dashboard render. Market session banner, daily snapshot refresh action bar, browser-session auto refresh heartbeat, Market Movers manual investigation panel, Sector / Industry ranking/trend, Sentiment context, Events agenda/calendar/quality/raw views를 조정한다 |
| `app/web/overview_dashboard_helpers.py` | Overview dashboard용 cached market intelligence service wrapper와 market / sentiment / events / data-health snapshot loader. legacy candidate / Pre-Live / proposal snapshot helper는 삭제됐다 |
...
| `app/web/backtest_workflow_routes.py` | `Backtest Analysis`, `Practical Validation`, `Final Review` visible stage와 compatible submode route mapping. legacy Candidate Review / Portfolio Proposal request는 primary panel로 열지 않고 Backtest Analysis fallback으로 정규화한다 |
...
| `app/web/backtest_compare.py` | `Portfolio Mix Builder` 화면 orchestration, component portfolio 실행, weighted portfolio result / save / Practical Validation handoff, saved replay module context wiring, mix candidate handoff |
...
| `app/web/backtest_compare_saved_replay.py` | Portfolio Mix Builder `저장된 Mix` 화면 render. saved Mix table, replay parity snapshot, saved replay service 호출, replay result card, mix 검증 보드, Practical Validation source handoff adapter를 담당한다 |
...
| `app/web/backtest_history.py` | `Operations > Archive: Backtest Runs` 화면 render, selected record inspect, run again / load into form / Practical Validation source handoff |
| `app/web/backtest_history_helpers.py` | History table row, replay payload, replay parity, Real-Money / Guardrail scope helper |
| `app/web/backtest_candidate_library.py` | `Operations > Archive: Candidates` 화면 render. 저장된 legacy current / Pre-Live 후보 inspect와 저장 contract 기반 result curve rebuild |
...
| `app/services/backtest_practical_validation_source.py` | Streamlit-free validation profile / selection source builder / source component table / compact selection history helper. result bundle, run history row, saved mix, weighted mix를 Clean current source로 변환하며 weighted mix의 component role / weight reason / cost / turnover / net-cost compact evidence를 보존한다 |
| `app/web/backtest_practical_validation_handoff.py` | Backtest Analysis / Archive에서 만든 current selection source를 Practical Validation session state와 route request로 큐잉하는 얇은 Streamlit helper. legacy Candidate Review draft state를 만들지 않는다 |
...
| `app/services/backtest_robustness_run_set.py` | Streamlit-free Robustness Experiment run-set read model. 기존 Robustness Lab / temporal / realism compact evidence를 `robustness_run_set_id`가 있는 provenance summary로 묶고, Practical Validation과 Final Review가 같은 run-set id / non-PASS evidence를 읽게 한다 |
...
| `app/web/final_selected_portfolio_dashboard.py` | `Operations > Portfolio Monitoring` 화면 render. Legacy file name은 Selected Portfolio Dashboard를 유지한다. Final Review selected 후보 pool과 사용자-created monitoring portfolio setup을 읽고, CNN / AAII market sentiment context overlay와 Active Portfolio Monitoring Scenario hero를 먼저 보여준다. Hero는 no portfolio / no strategy / configured-not-run / executed 상태를 구분하고 portfolio-wide value / P&L / return / CAGR / MDD / value curve / strategy performance / rebalance target을 표시한다. Scenario 결과가 있으면 latest / previous saved Monitoring Snapshot과 current scenario comparison을 보여주고, `Save Monitoring Snapshot` / `Record Review` 명시 action으로만 compact monitoring log를 append한다. 그 아래 fixed-height portfolio card shelf / 생성 / 선택 / collapsed management soft delete, portfolio name / description edit, compact selected strategy slot board / 설정 적용 / 제거, pending-stale scenario update, 선택한 1개 strategy의 lazy Monitoring Scenario / Monitoring Signals / evidence detail, 전환 비교, optional preflight, Actual Allocation / allocation evidence boundary, Decision Dossier / Audit을 보여준다 |
| `app/web/final_selected_portfolio_dashboard_helpers.py` | Portfolio Monitoring dashboard portfolio / selected strategy pool / strategy slot / strategy comparison table, handoff table, component / continuity / source contract / recheck preflight / recheck readiness / symbol freshness / provider evidence / open issue follow-up / deployment readiness / recheck comparison / value / holding input / drift / alert preview / allocation boundary / filter helper. Evidence table은 service read model을 표시한다 |
...
| `app/runtime/portfolio_selection_v2.py` | current selection source, practical validation result, Final Decision, selected monitoring log, saved mix JSONL helper. selected monitoring log append/load는 explicit snapshot 저장과 recorded-at recent read를 지원한다 |
| `app/runtime/final_selected_portfolios.py` | final selection decision registry를 read-only dashboard row, Portfolio Monitoring handoff review, 사용자 monitoring portfolio saved state / strategy slot, continuity check, selected component replay recheck, component contribution, optional Allocation Check / drift preview / allocation drift evidence boundary, compact Monitoring Snapshot record, latest / previous / current scenario comparison으로 변환하는 helper |
...
| `.aiworkspace/note/finance/registries/SELECTED_PORTFOLIO_MONITORING_LOG.jsonl` | 모니터링 후보 선정 후 optional monitoring snapshot / review record. 자동 생성하지 않고 `Save Monitoring Snapshot` / `Record Review` 명시 저장이 필요할 때만 사용 |
...
- `Operations > Portfolio Monitoring`: `SELECT_FOR_PRACTICAL_PORTFOLIO` selected row만 strategy pool로 읽는다. Legacy implementation file / helper name은 Selected Portfolio Dashboard를 유지한다. 화면은 먼저 CNN / AAII `시장 심리 Context Overlay`와 Active Portfolio Monitoring Scenario를 보여주고, 그 아래 `나의 포트폴리오` fixed-height card shelf와 선택한 portfolio command band, selected decision strategy slot compact board로 start / latest-end mode / balance / memo를 관리한다. Scenario update는 slot 저장과 분리되며, portfolio-wide cockpit에서 실행 전 / 부분 집계 / 전체 집계 상태와 value / P&L / return / CAGR / MDD / benchmark spread / target snapshot을 먼저 보여준다. Scenario 결과가 있으면 saved latest / previous Monitoring Snapshot과 current scenario를 비교하고, 사용자가 명시적으로 `Save Monitoring Snapshot` 또는 `Record Review`를 누른 경우에만 compact monitoring log를 append한다. `포트폴리오 시나리오 업데이트`는 현재 slot signature 기준 pending / stale strategy만 기본 실행하고, `전체 재실행`을 켠 경우에만 기존 최신 결과까지 다시 replay한다. Target snapshot table의 `Target Snapshot Date`는 마지막 monitoring scenario가 산출한 목표 비중 기준일이고 `Next Review Date`는 다음 수동 재계산 예정일이다. 이 표와 monitoring snapshot은 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. Final Review의 `open_review_items`, review trigger, `deployment_readiness_policy_snapshot`, recheck / provider / continuity / review signal / allocation boundary evidence는 사용자가 선택한 1개 strategy 상세 / optional preflight에서 read-only로 확인한다. Portfolio Monitoring의 sentiment overlay도 Monitoring Scenario, Review Signals, saved dashboard setup, monitoring log, broker order, auto rebalance에 연결하지 않는다. 이 preflight도 실제 돈 투입 승인, 주문 지시, broker/account 연결, 자동 리밸런싱이 아니다. 통과 후보가 없으면 strategy pool은 비어 있고 Final Review 선정 안내를 보여준다.
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board를 만든다.
...
`data-provenance-pit-contract-v1`부터 Practical Validation Data 탭과 Final Review Investability Packet은 같은 `data_provenance_summary`를 읽는다. 이 contract는 provider / macro / price window / lifecycle / robustness compact evidence를 `source_date`, `collected_at`, `snapshot_kind`, `freshness_status`, `proxy_status`, PIT / look-ahead / survivorship risk, `decision_effect.treat_as_pass`로 보여준다. current snapshot, stale, proxy, non-PIT-safe evidence는 pass처럼 숨기지 않는다.
...
`robustness-lab-v1`부터 Stress / Sensitivity Interpretation은 Robustness Lab board로 표시한다. 이 board는 stress coverage, worst stress, sensitivity coverage, worst sensitivity delta, rolling validation, local overfit audit를 compact summary로 묶고, Final Review와 final decision evidence read model이 같은 summary row를 읽는다. `robustness-experiment-registry-v1`부터 같은 evidence는 `robustness_run_set_id`가 있는 Robustness Experiment run-set summary로도 묶인다. run-set은 board를 대체하지 않는 provenance / grouping layer이며, source id 또는 promotion contract reference, frozen parameter set, experiment types, walk-forward / OOS / regime / cost / parameter perturbation summary, generated artifact reference, non-PASS evidence, decision effect를 compact하게 들고 간다. Raw run history와 strategy-specific perturbation artifact는 workflow JSONL에 저장하지 않는다.
...
`validation-efficacy-gate-policy-link-v1` 이후 Final Review selection gate는 Validation Efficacy Audit route를 selected-route 조건으로 읽는다. 핵심 `VALIDATION_EFFICACY_NEEDS_INPUT`과 `VALIDATION_EFFICACY_BLOCKED`는 selected-route blocker이며, `VALIDATION_EFFICACY_REVIEW`는 기본적으로 `open_review_items`로 남긴다.
...
- 기존 Pre-Live registry, proposal registry, paper ledger JSONL은 삭제하지 않는다. 다만 새 주 흐름의 필수 join 조건이 아니라 archive / recovery compatibility로 낮춘다.
- Candidate Review / Portfolio Proposal UI와 helper module은 5C에서 삭제됐으며, primary Backtest navigation에서 직접 dispatch하지 않는다.
- Primary Backtest navigation은 `Backtest Analysis`, `Practical Validation`, `Final Review`만 직접 dispatch한다.
- 기존 `Single Strategy`, `Compare & Portfolio Builder`, `Portfolio Mix Builder` route request는 Backtest Analysis submode로 호환한다.
- 기존 `Candidate Review`, `Portfolio Proposal` route request는 primary panel로 열지 않고 `Backtest Analysis / Single Strategy` fallback으로 정규화한다.

Operations 화면:

- `Operations > Operations Overview`: Operations Console landing page다. Today action queue, Portfolio Monitoring, System / Data Health, Archive / Recovery, Reference / Reports lane을 보여주고, surface audit과 1차~5차 roadmap을 접힘 상세로 남긴다. selected monitoring과 system run health를 primary로, Backtest Runs와 Candidates archive를 secondary archive / recovery 도구로 해석하게 한다.
- `Operations > Portfolio Monitoring`: legacy file/helper name은 Selected Portfolio Dashboard를 유지하지만 사용자-facing route는 Portfolio Monitoring이다. `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에서 `SELECT_FOR_PRACTICAL_PORTFOLIO`로 선정된 row만 selected strategy pool로 읽고, 사용자가 만든 dashboard portfolio에 strategy slot으로 추가해 모니터링한다. Dashboard는 daily-monitoring-first로 읽힌다. 상단 Active Portfolio Monitoring Scenario가 active portfolio, 실행 상태, 설정 투자금, 평가 금액, 손익, 총 수익률, CAGR / MDD, 기준일, session update timestamp, daily badges, value curve, 전략별 성과, target snapshot을 먼저 보여준다. Portfolio가 없거나 strategy가 없거나 scenario가 아직 실행되지 않은 상태는 각각 생성 / 전략 추가 / 업데이트 실행 안내로 구분한다. Portfolio card shelf는 hero 아래 active selector이고, portfolio name / description edit, compact strategy board, `포트폴리오 시나리오 업데이트`는 그 아래 관리 영역이다. 각 slot은 start / latest-end mode / balance / memo를 저장하고, update action은 pending / stale strategy만 기본 replay하며 `전체 재실행`을 켠 경우 full refresh한다. Target snapshot은 마지막 monitoring scenario 기준 산출 목표 비중이고 Next Review Date는 수동 재계산 예정일이다. Snapshot, Final Review -> Portfolio Monitoring continuity check, source contract, Monitoring Signals의 Timeline / Review Signals / Open Issues / Why Selected / optional Actual Allocation / allocation evidence boundary / Decision Dossier / Audit은 사용자가 선택한 1개 strategy 상세를 열 때만 보여준다. Recheck Operations Preflight / Recheck Readiness / Symbol Freshness / Provider Evidence도 하단 상세 점검으로 낮춘다. 같은 dashboard portfolio 안 전략이 2개 이상이면 최신 scenario 결과로 전환 비교를 표시한다. Preflight / Readiness / Symbol Freshness / Provider Evidence / Continuity / Timeline / Recheck Comparison / Allocation Boundary / Dossier는 read-only이며, live approval / broker order / account sync / auto rebalance는 disabled로 둔다.
...
- `Operations > Archive: Backtest Runs`: 저장된 실행 기록을 inspect하고, 가능한 경우 run again, load into form, Practical Validation source handoff를 수행한다. 후보 검토 흐름의 주 단계가 아니라 과거 실행을 다시 열기 위한 archive / recovery 도구로 둔다.
...
- dashboard write policy:
  ...
  - `Save Monitoring Snapshot` / `Record Review` 명시 action만 compact monitoring log를 append함
- performance recheck:
...
## Legacy Portfolio Proposal 계약

`PORTFOLIO_PROPOSAL_REGISTRY.jsonl`과 `app/runtime/portfolio_proposal.py`는 이전 proposal draft workflow의 compatibility surface다.
현재 primary flow는 proposal draft를 필수로 만들지 않고, Backtest Analysis에서 만든 source를 Practical Validation과 Final Review가 직접 읽는다.
legacy Portfolio Proposal UI / helper module은 5C physical cleanup에서 삭제됐다.

현재 기준:

- 새 후보 묶음은 `Backtest Analysis > Portfolio Mix Builder`에서 weighted mix source로 만들고 Practical Validation으로 보낸다.
- `Portfolio Proposal`은 primary Backtest navigation에서 직접 dispatch하지 않는다.
- 기존 proposal draft row는 삭제하지 않고 archive / recovery record로 보존한다.
- saved portfolio setup은 여전히 `.aiworkspace/note/finance/saved/SAVED_PORTFOLIOS.jsonl`의 reusable weight setup이고, proposal registry와 source-of-truth가 아니다.

## `backtest.py` 리팩토링 경계

현재 `app/web/pages/backtest.py`는 Backtest page shell과 세 stage dispatch만 맡는다.
Backtest workflow의 실제 body는 `app/web/backtest_*.py` module로 분리한다.

| 영역 | 현재 책임 |
|---|---|
| `app/web/pages/backtest.py` | `Backtest Analysis -> Practical Validation -> Final Review` workflow navigation과 stage dispatch |
| `app/web/backtest_analysis.py` | Backtest Analysis stage wrapper. Single Strategy와 Portfolio Mix Builder submode 선택 |
| `app/web/backtest_single_strategy.py`, `app/web/backtest_single_forms.py`, `app/web/backtest_single_runner.py` | Single Strategy form / runtime dispatch / latest result 연결 |
| `app/web/backtest_compare.py`, `app/web/backtest_compare_components.py`, `app/web/backtest_compare_saved_replay.py` | Portfolio Mix Builder orchestration, visual shell, saved Mix replay UI |
| `app/web/backtest_practical_validation*.py` | Practical Validation render와 전용 visual shell |
| `app/web/backtest_final_review*.py` | Final Review render, helper, visual shell |
| `app/web/backtest_history.py`, `app/web/backtest_history_helpers.py` | Operations archive run history render, replay / parity helper, Practical Validation source handoff |
| `app/web/backtest_candidate_library.py` | Operations archive candidate inspect / replay |
...
- 긴 route/status 문자열은 공용 화면에서는 `app/web/backtest_ui_components.py`의 wrapping card / route panel을 사용하고, Practical Validation은 `app/web/backtest_practical_validation_components.py`의 전용 workbench shell을 사용한다.
- Backtest shell은 `Backtest Analysis -> Practical Validation -> Final Review`를 주 workflow navigation으로 보여준다. Backtest Analysis 안에서 `Single Strategy`와 `Portfolio Mix Builder`를 선택한다.
- `History`는 메인 흐름에서 제외하고 `Operations > Archive: Backtest Runs` page로 연다.
- Candidate Review / Portfolio Proposal UI / helper module은 삭제됐다. 남은 legacy는 registry JSONL, runtime reader, Candidate Library archive / recovery, selected monitoring fallback compatibility뿐이다.
- Final Review가 현재 Backtest workflow의 마지막 active panel이며, 이후 확인은 `Operations > Portfolio Monitoring`이 맡는다.
...
`Promotion Policy Signal > 현재 판단`에는 `Candidate Readiness Checkpoint` 박스를 둔다.
...
- 2026-06-12 6A 이후 saved Mix table, replay parity snapshot, saved replay service 호출, replay result card, mix 검증 보드, Practical Validation handoff adapter는 `app/web/backtest_compare_saved_replay.py`가 맡는다. `app/web/backtest_compare.py`는 public entrypoint와 orchestration callback context를 유지한다.
...
Archive: Candidates는 workflow 단계가 아니다.
legacy Candidate Review에서 저장한 후보를 나중에 다시 열어보고,
...
2026-06-12 6A 이후 `저장된 Mix` workspace의 render/helper 책임은 `app/web/backtest_compare_saved_replay.py`가 소유한다. 이 module은 기존 session-state key, saved portfolio service, Practical Validation handoff helper, run history append behavior를 그대로 사용하며 새 registry / saved source-of-truth를 만들지 않는다.
...
저장된 legacy 후보 자체의 그래프 재검토는 `Operations > Archive: Candidates`에서 처리한다.

## Legacy Candidate / Proposal Compatibility

Candidate Review, Pre-Live 운영 기록, Portfolio Proposal은 현재 primary Backtest workflow 단계가 아니다.
이전 phase에서 만든 registry와 helper는 history / archive recovery를 위해 보존하지만,
새 후보 생성과 검증의 정식 경로는 아래 source chain이다.
...
- `Backtest > Candidate Review`와 `Backtest > Portfolio Proposal`은 primary navigation에서 직접 dispatch하지 않는다.
- `Operations > Archive: Backtest Runs`의 selected record action은 legacy candidate draft queue가 아니라 current Practical Validation source handoff로 보낸다.
- `Operations > Archive: Candidates`는 `CURRENT_CANDIDATE_REGISTRY.jsonl`과 `PRE_LIVE_CANDIDATE_REGISTRY.jsonl`에 남은 compact 후보 기록을 inspect / replay하는 archive recovery 화면이다.
- `CANDIDATE_REVIEW_NOTES.jsonl`, `PRE_LIVE_CANDIDATE_REGISTRY.jsonl`, `PORTFOLIO_PROPOSAL_REGISTRY.jsonl`, `PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl`은 삭제하지 않는다. 다만 current source / validation / final decision chain의 필수 단계로 확장하지 않는다.
- legacy Candidate Review / Portfolio Proposal UI helper module은 import graph 확인 후 삭제됐다. registry JSONL과 runtime helpers는 archive / recovery compatibility로만 남긴다.
...
- `Operations > Archive: Backtest Runs > Selected History Run > Saved Input & Context`
...
- `Operations > Archive: Backtest Runs > Selected History Run > History Promotion Policy / Guardrail Scope`
...
## Archive: Backtest Runs 흐름

`Operations > Archive: Backtest Runs`는 compact summary 중심이다.
...
Phase 28 이후 Archive: Backtest Runs의 selected record 영역에는
...
## Legacy Pre-Live / Portfolio Proposal Records

Pre-Live와 Portfolio Proposal registry는 이전 workflow의 운영 기록이다.
현재 문서에서는 아래처럼 archive record로 읽는다.

- `PRE_LIVE_CANDIDATE_REGISTRY.jsonl`: legacy 후보 운영 상태와 다음 행동 기록. 새 Practical Validation / Final Review gate를 대체하지 않는다.
- `PORTFOLIO_PROPOSAL_REGISTRY.jsonl`: legacy proposal draft 기록. 새 mix 후보는 Backtest Analysis의 Portfolio Mix Builder source로 Practical Validation에 넘긴다.
- `PAPER_PORTFOLIO_TRACKING_LEDGER.jsonl`: legacy paper tracking record. Portfolio Monitoring의 current explicit Monitoring Snapshot / Review log를 대체하지 않는다.
- legacy records를 읽어야 할 때는 `Operations > Archive: Candidates`와 runtime archive helpers를 recovery 용도로 사용한다.
...
Practical Validation Gate 통과 후보
  ...
  -> 3. 모니터링 후보 선정 저장
     -> selection gate 통과 시 Portfolio Monitoring 후보로 선정
...
- legacy Candidate Review와 Portfolio Proposal의 판단 field는 준비 기록이고, Final Review의 `모니터링 후보 선정 저장`만 Portfolio Monitoring에서 관찰할 후보를 명시하는 주 decision surface다. 실제 자금 투입 가능 여부는 향후 Live / Deployment Readiness에서 별도로 판단한다.
...
Backtest > Final Review
  ...
  -> Evidence Appendix에서 이전 검증 결과 확인
     ...
     -> Portfolio Monitoring handoff 확인
