...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 `검증 결론` workspace panel, Flow 4 `카테고리별 검증 결과` board와 `통과 / 보강 후 재검증 / 실전 사용 어려움` main outcome summary, 기준별 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide, 상세 evidence tabs, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. Flow 3 / Flow 4 visible UI는 Final Review 전용 REVIEW 판단과 REVIEW-only / empty category를 actionable issue로 노출하지 않으며, 조건부 / 후속 참고 그룹도 Flow 3 하단 expander로 반복 노출하지 않는다. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
- `Practical Validation`: 선택된 단일 전략 후보 / Portfolio Mix 후보 source를 Final Review로 넘기기 전 검증 근거로 구조화한다. 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 Backtest Analysis가 넘긴 2차 review focus와 source summary / selection history를 확인하고, Flow 2는 validation profile과 5개 답변을 고른 뒤 latest runtime replay를 사용자가 실행했을 때만 현재 세션에 표시한다. Flow 3은 `workspace_panel.py`와 read-only `practical_validation_fix_queue` React component가 Practical Validation outcome summary, 실패 category count, category-level `통과 / 실패`를 단일 first-read conclusion surface로 보여준다. Flow 3은 Final Review 이동 가능 / 보류, REVIEW count, Final Review 참고 항목, 조건부 근거 / 후속 참고 expander를 보여주지 않는다. Flow 4는 `visible_criteria_detail_groups` 기반 `카테고리별 검증 결과` board에서 먼저 `통과`, `보강 후 재검증 필요`, `실전 사용 어려움` main outcome summary를 보여주고, 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약한다. `READY`는 사용자-facing 통과로 읽고, `REVIEW`는 보강 실패가 아니라 Final Review에서 해석할 metadata로만 유지하며 Flow 3 / Flow 4 visible actionable issue에는 노출하지 않는다. REVIEW-only / empty category는 내부 read model에는 남지만 PV visible category result에는 표시하지 않는다. `NEEDS_INPUT` / `NOT_RUN`은 보강 후 재검증 대상이고, `BLOCKED`는 현재 상태로 실전 후보 사용이 어렵다는 차단 결론이다. 각 기준 상세는 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치`로 보여준다. Flow 4 guide는 module-level 문구만 쓰지 않고 가능하면 audit row의 non-PASS `Criteria`와 `Next Action`을 끌어와 실제 부족 항목과 실행 방법을 표시한다. `통과 기준`은 보강 뒤 무엇이 되어야 해결된 것인지 판단하는 기준이며, 위치는 `Flow4 > 데이터 > 데이터 품질 / 편향 통제 상세`, `Flow4 > Provider / Data 보강 액션`처럼 사용자가 찾을 수 있는 화면 경로로 표시한다. Flow 4는 Input Evidence, audit board, Practical Diagnostics, Provider Coverage, Look-through Exposure Board, Robustness Lab, Provider Action Center를 담고, legacy Final Review gate technical expander는 visible Flow 4 path에서 렌더링하지 않는다. Flow 5는 save-only audit copy와 Final Review 이동을 다룬다. 기본 진입 화면은 검증 상태와 보강 action을 먼저 보여주며, contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다. `검증 결과 저장(기록용)`은 audit trail만 남기며 Gate 미통과 row를 Final Review 후보로 노출하지 않는다.
...
Practical Validation current의 현재 구현은 최소 contract를 Input Evidence로 읽고, profile-aware practical diagnostics board와 screen-oriented workspace read model을 함께 만든다. 보이는 화면은 `후보 Source 확인 -> 검증 기준 설정 / 실전 재검증 실행 -> 검증 결론 -> 검증 기준 상세 -> 저장 / Final Review 이동`의 5-flow로 읽는다. Flow 1은 source snapshot 확인만 소유하고, Flow 2가 validation profile과 latest runtime replay 실행을 함께 소유한다. Flow 3의 first-read surface는 `app/web/backtest_practical_validation/workspace_panel.py`가 소유하며, `app/web/components/practical_validation_fix_queue/` React component는 compatibility path를 유지하되 visible UI는 `검증 결론`과 `카테고리별 검증 요약`을 read-only로 보여준다. Flow 3은 Practical Validation outcome과 카테고리별 통과 / 실패만 보여주고, Final Review 이동 상태, REVIEW count, 조건부 / 후속 참고 expander를 보여주지 않는다. raw status, row evidence, module table, resolution guide는 Flow 4로 낮춘다. Flow 4는 같은 workspace read model의 `visible_criteria_detail_groups`를 `카테고리별 검증 결과` board로 풀어 각 기준의 사용자-facing 판정 근거와 `검증한 것 / 해결해야 할 항목 / 해결 방법 / 통과 기준 / 위치` guide를 보여준다. Full `criteria_detail_groups`에는 Final Review reference metadata가 남지만, REVIEW-only / empty category는 Flow 3 / Flow 4 visible category result에서 숨긴다. Python path는 validation execution, gate calculation, provider action, registry persistence, save / Final Review handoff를 계속 소유한다.
