...
| `app/web/backtest_practical_validation/page.py` | `Practical Validation` stage render. Clean current source와 저장된 Backtest summary / equity curve / result table / strategy construction / selection history snapshot 확인, 검증 프로필 입력, 최신 DB 데이터 기준 runtime 재검증 실행 버튼, Flow 3 workspace panel, summary-first `검증 근거 보드`, Provider Action Center, 저장 / Final Review 이동, service handoff 결과의 session state 반영을 담당. 기본 진입 path에는 contextual Reference help와 context-only 시장 심리 overlay를 렌더링하지 않는다 |
...
`practical-validation-board-map-v1`부터 Practical Validation result에는 `validation_board_map`, `validation_board_display_rows`, `applied_validation_board_display_rows`, `not_applicable_validation_board_display_rows`가 붙는다. 이 map은 검증 module taxonomy와 화면 board taxonomy를 분리한다. module은 검증 의미와 gate owner이고, board는 사용자에게 보이는 evidence 묶음이다. UI는 Flow 3 `2차 검증 결론 / Fix Queue`에서 Final Review 이동 판단과 먼저 해결할 blocker를 먼저 보여주고, Applied Validation Map은 Flow 4의 보조 `검증-근거 연결 지도`로 접어 둔다. 각 evidence / action board title 아래에는 `Board Type`, `Applies`, `Feeds`, `Gate` badge를 표시한다.
Fix Queue의 blocker / review module table은 내부 module id 대신 `Module`, `Status`, `Fix Location`, `Fix Action`, `Gate Effect`, `Gate Reason`을 기준으로 만든다. Flow 3 React surface는 그중 top blocker 최대 3개와 compact evidence summary를 먼저 보여주고, 나머지 상세는 기술 상세 / Flow 4로 낮춘다. 예를 들어 `latest_replay` blocker는 Flow 2 `실전 검증 실행`에서 `전략 재검증 실행` 후 Recheck / Coverage를 확인하라고 안내한다. First-read status는 `PASS / REVIEW / NEEDS_INPUT / BLOCKED / NOT_RUN / NOT_APPLICABLE`로 정규화해 보여주며, raw route id는 technical detail / JSON context에 남긴다.
