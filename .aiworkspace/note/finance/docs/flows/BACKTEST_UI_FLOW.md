...
- `실전성 검증 Handoff`: 진입 상태, 10점 만점 readiness score, Promotion / 실행 원천 / 검증 원천 chip, source 등록 boundary를 한 panel에서 보여준다. hard blocker가 없고 review focus만 있으면 상세 사유를 1차에서 펼치지 않고 `2차 확인 항목 N개`로 요약해 Practical Validation으로 넘긴다.
