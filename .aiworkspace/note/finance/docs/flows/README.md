...
- Pattern evidence: 최근 60개 일봉을 한 선으로 잇지 않고 `20D 전 → 5D 전 → 현재` 핵심 관측 시점만 실선으로 표시한다. `관측만 / 다음 5D / 다음 20D`를 전환하면 실제 horizon 확률이 네 위험 체제의 점선 분기로 표시되며, 분기 끝은 예측 좌표가 아니라 체제 범주다. 확률 카드가 숫자의 primary surface이고 지도는 가능한 분포를 공간적으로 읽는 보조 근거다.
