...
경제 사이클 view 안의 `경기 국면 | 물가·정책 경로` 선택기는 기존 경기 국면을 기본값으로 유지한다. 신규 경로는 `연말 Core PCE 다섯 상태 -> 다음 발표 준비 -> FOMC 순이동 -> 10년물 동적 전고점 군집 -> 목표 금리 조건부 역산` 순서로 읽는다. 자동 저항 기준은 읽기 전용이며 별도 USER 기준으로만 복사·저장한다. `READY`가 아니면 저장 확률을 현재 판단으로 표시하지 않고 제한 사유·관측/발표/수집 시각·버전을 보여준다. 기존 경기 사이클 확률은 이 경로 또는 침체 fallback으로 재사용하지 않는다.
