...
```text
Today initial app run
  -> market context + local 1초 session clock + action shell 렌더
  -> portfolio-only DB projection을 별도 React island로 렌더
  -> official calendar quality와 regular-session phase 확인
  -> OPEN 또는 EOD waiting/running일 때만 portfolio island 15초 heartbeat
     -> context, clock, actions와 전체 Streamlit app은 다시 만들지 않음
  ...
  -> CLOSED
     ...
     -> EOD confirmed이면 portfolio heartbeat도 중단
```

## 의미 계약

- 15초는 화면 heartbeat이지 provider 수집 주기가 아니다. provider attempt는 group별 300초보다 자주 실행되지 않는다.
- 15초 heartbeat는 Today 전체가 아니라 `today_portfolio_island`만 실행한다. 장 마감·휴일·주말·개장 전과 EOD confirmed 상태에는 periodic Python run이 없다.
- 뉴욕·한국 현재 시각과 countdown의 1초 변화는 `MarketSessionClock`의 local React state이며 Python/Streamlit rerun을 만들지 않는다. phase가 실제로 바뀔 때만 allowlisted event로 app run 한 번을 요청한다.
...
- portfolio island 장애 시 static market context와 navigation은 유지되며, 별도 API/SSE/WebSocket은 사용하지 않는다.
