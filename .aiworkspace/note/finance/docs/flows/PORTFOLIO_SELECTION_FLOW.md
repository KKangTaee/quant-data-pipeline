...
- Backtest Analysis의 Handoff panel이 source 등록 action과 entry judgment를 소유한다. Policy Signals owns evidence detail and does not repeat the same entry-readiness hero. React custom components own the visible Handoff card / button and the `검증 기준 상세` first-stage evidence board, while Python keeps source registration write / rerun, policy classification, and persistence.
