...
- Step 1은 후보 선택과 검증 관점을 `1A / 1B`로 분리한다. Step 2는 current-session latest runtime replay를 fragment rerun으로 실행해 상단 후보/정책 context를 유지한다. Step 3은 verified, measured caution, validated caution, resolve-now, engineering-required, Final Review handoff를 분리하고 `상세 검증 근거`를 disclosure로 제공한다. Step 4는 audit save와 eligible save-and-move를 제공하며 Final Review route 이동만 app rerun을 사용한다.
...
- blocked Level2에서는 handoff를 `검증 통과 후 Final Review에서 확인할 항목`으로만
  예고한다. eligible validation에서만 `Final Review에서 이어서 판단할 항목`으로
  승격하고, Final Review 본문은 이를 `최종 판단 입력 / 인수한 검증 한계 /
  Monitoring 이관 조건` 세 lane으로 root issue당 한 번만 표시한다.
