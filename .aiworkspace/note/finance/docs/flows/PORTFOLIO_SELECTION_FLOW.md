...
- Flow 3 `2차 검증 결론 / Fix Queue`는 Final Review 이동 가능 여부, 먼저 해결할 blocker, 다음 단계, compact 기준 요약을 먼저 보여주는 first-read surface다. 이 surface는 `workspace_panel.py`가 소유하고, `practical_validation_fix_queue` React component는 read-only 카드로 Final Review 이동 판단 / top fix items / 기준 요약을 보여준다. 저장 / 이동 button은 Flow 5가 소유하므로 Flow 3에는 만들지 않는다.
...
- Flow 4 `근거 Workbench`는 먼저 `Final Review 이동 기준 상세` 보드로 Source Readiness / Validation Readiness / Final Review Readiness Preview의 판정 근거와 보강 위치를 보여준 뒤, 검증 근거 보드, Applied Validation Map, Provider Action Center, Look-through Board, Robustness Lab, detailed diagnostics를 담는다. 화면 board는 검증 module과 1:1 개념이 아니므로 module gate와 evidence board를 분리해서 읽는다.
