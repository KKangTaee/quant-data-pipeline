...
- Final Review는 상세 evidence table보다 `Final Review 투자 검토서`와 Decision Cockpit을 먼저 보여준다. 투자 검토서는 Python service가 만든 read model을 React로 렌더링하며, 추천 / 보류 / 탈락 / Monitoring 후보, 점수 체계, 강점 / 약점, Level2 REVIEW 처리 결과, 저장 / Monitoring handoff 경계, 약점 개선안을 사람이 읽는 검토서 형태로 보여준다.
