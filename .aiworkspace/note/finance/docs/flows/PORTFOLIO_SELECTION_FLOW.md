...
- Flow 3 `2차 검증 결론 / Fix Queue`는 Final Review 이동 가능 여부, 먼저 해결할 blocker, 다음 단계, compact 기준 요약을 먼저 보여주는 first-read surface다. 이 surface는 `workspace_panel.py`가 소유하고, `practical_validation_fix_queue` React component는 read-only 카드로 Final Review 이동 판단 / top fix items / 기준 요약을 보여준다. 각 blocker는 `무엇을 검증했나 / 부족한 점 / 해야 할 일 / 왜 중요한가`로 읽히며, `NEEDS_INPUT` / `NOT_RUN` 같은 raw status는 `기술 기준` 보조 tag로 남긴다. 저장 / 이동 button은 Flow 5가 소유하므로 Flow 3에는 만들지 않는다.
...
- Flow 4 `근거 Workbench`는 먼저 `Final Review로 넘기기 전 확인 기준` 보드로 Source / Validation / Final Review readiness 기준의 판정 근거와 보강 위치를 보여준 뒤, 검증 근거 보드, Applied Validation Map, Provider Action Center, Look-through Board, Robustness Lab, detailed diagnostics를 담는다. 이 보드는 새 검증 단계가 아니라 Flow 3 결론이 어떤 확인 기준에서 나왔는지 풀어쓴 기준 상세이며, 화면 board는 검증 module과 1:1 개념이 아니므로 module gate와 evidence board를 분리해서 읽는다.
