...
- Practical Validation은 Control Center에서 후보 / profile / latest replay / gate를 먼저 요약한다.
...
- Practical Validation의 `Latest Runtime Replay`는 별도 audit board가 아니라 `3. 최신 데이터 기준 전략 재검증` 섹션에서 재검증을 실행해 해소한다.
