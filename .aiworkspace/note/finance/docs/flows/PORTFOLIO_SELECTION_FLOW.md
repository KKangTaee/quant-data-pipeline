...
| 3 | Final Review | Decision Desk에서 후보 현황과 우선순위를 먼저 본다. visible Review Queue table은 없고, stable key 기반 `검토 대상` selector와 접힌 `후보 비교 상세`를 사용한다. selector 변경만으로 후보별 판단 surface를 열지 않으며 `최종 검토서 확인`으로 현재 후보를 확정한다. 확정 후보와 selector가 다르면 stale 안내와 함께 투자 검토서 / Decision Cockpit / Final Decision Action을 숨긴다. 투자 검토서는 candidate meta, 선택 판단 요약, 강점 / 확인 지점, `Final Review 확인 필요`, 해석을 first-read로 보여준다. `Final Review 확인 필요`는 `데이터 주의`, `2단계 실용성 주의`, `최종 판단 참고`, `Monitoring 추적`, `저장 전 보강`을 `점수에 반영됨 / 저장 전 확인 / Monitoring 조건으로 넘김 / blocker`로 분리한다. 상세 점수 / 저장 및 Monitoring handoff / 약점 개선안 / Monitoring 조건은 lower tabs에서 확인한다. Python service가 score, cap, gate, REVIEW 행동, 저장 / Monitoring handoff를 소유하고 React는 표시만 한다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, selection gate를 통과한 selected route만 Portfolio Monitoring 후보 handoff 대상이다. 확인 버튼은 새 검증, provider fetch, DB 수집, registry write를 수행하지 않는다. Final Review는 live approval, broker order, auto rebalance가 아니다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, Decision Desk 우선순위 요약, stable key 기반 검토 대상 선택, 명시적 `최종 검토서 확인`, stale report 차단, 후보 비교 상세, 투자 검토서의 선택 판단 요약 / 강점 / 확인 지점 / Level2 REVIEW 행동 / 해석, Decision Cockpit, Final Review 판단 record, selected-route gate 기반 Portfolio Monitoring 후보 handoff, Evidence Appendix와 Saved Decision Review | visible Review Queue table, 별도 `Step 1 / Candidate Board`, selector 변경만으로 report 자동 렌더, 새 비중 실험, 약점 개선 portfolio 자동 생성 / 자동 backtest / 자동 저장, provider data 수집, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, auto rebalance |
