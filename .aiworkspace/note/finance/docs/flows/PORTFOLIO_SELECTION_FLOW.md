...
| 3 | Final Review | Candidate Board의 review priority / first-review candidate와 Decision Cockpit으로 selected-route 상태를 보고 Decision Record Checklist로 최종 select / hold / reject / re-review 판단 저장 조건을 확인하며, 상세 Practical Validation evidence는 Appendix에서 read-only로 확인한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS_V2.jsonl` |
