...
- Step 1은 후보 선택과 검증 관점을 `1A / 1B`로 분리한다. Step 2는 current-session latest runtime replay intent를 declared component `on_change`에서 먼저 소비해 같은 fragment rerun 안에서 새 projection을 만들고, 별도 `st.rerun`을 호출하지 않아 상단 후보/정책 context와 one-shell을 유지한다. Step 3은 verified, measured caution, validated caution, resolve-now, engineering-required, Final Review handoff를 분리하고 `상세 검증 근거`를 disclosure로 제공한다. Step 4는 audit save와 eligible save-and-move를 제공하며 Final Review route 이동만 app rerun을 사용한다.
