...
- `상세 검증 근거`는 별도 Flow 5가 아니다. 사용자 설명은 검증명 / 확인 결과 / 의미 / 다음 행동 순서로 보여주고, raw 함수 경로와 내부 status는 secondary `technical_trace`에 둔다. 데이터/편향, 검증 방법, 포트폴리오 구성, 실전 현실성, 스트레스/강건성 5개 category 중 하나만 펼친다. custom profile, advanced replay mode, full source snapshot, raw JSON만 `고급 설정과 원본 근거`에 둔다.
