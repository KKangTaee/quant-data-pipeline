...
| 3 | Final Review | Decision Desk에서 후보 현황과 우선순위를 먼저 본다. visible Review Queue table은 없고, stable key 기반 `검토 대상` selector와 접힌 `후보 비교 상세`를 사용한다. selector 변경만으로 후보별 판단 surface를 열지 않으며 `최종 검토서 확인`으로 현재 후보를 확정한다. 확정 후보와 selector가 다르면 stale 안내와 함께 투자 검토서 / 판단 기록을 숨긴다. 투자 검토서는 단일 판단 상태, 투자 매력도, 확인 필요 수, `투자 매력도 / 근거 신뢰도 / Monitoring 준비도`, 총평, 강점 / 약점, 저장 전 확인 질문, 10개 조건부 Monitoring 패턴, `Final Review 확인 필요`를 first-read로 보여준다. 하단은 연결된 `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어` shell이다. REVIEW는 module별 stored audit에서 관측 / 판단 근거 / source / 기준일 / score policy를 연결하고 measured / derived / qualitative / missing contract를 구분하며, open 개수만으로 투자 매력도를 감점하거나 cap하지 않는다. 다음 실험은 적용 가능한 상위 3개 가설만 표시하고 자동 실행하지 않는다. standalone Decision Cockpit 없이 `판단 기록`이 상태 / 권장 판단 / route / 사유 / 저장 CTA를 소유한다. Python service가 score, gate, REVIEW trace, 패턴 support state, 저장 / Monitoring handoff를 소유하고 React는 표시만 한다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, selection gate를 통과한 selected route만 Portfolio Monitoring 후보 handoff 대상이다. 확인 버튼은 새 검증, provider fetch, DB 수집, registry write를 수행하지 않는다. Final Review는 live approval, broker order, auto rebalance가 아니다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, Decision Desk 우선순위 요약, stable key 기반 검토 대상 선택, 명시적 `최종 검토서 확인`, stale report 차단, 총평 / 강점과 약점 / 세 점수 축 / stored audit REVIEW trace / 10개 조건부 Monitoring 패턴 / 상위 3개 다음 실험 아이디어, 통합 판단 기록, selected-route gate 기반 Portfolio Monitoring 후보 handoff, Evidence Appendix와 Saved Decision Review | visible Review Queue table, 별도 `Step 1 / Candidate Board` / standalone Decision Cockpit, selector 변경만으로 report 자동 렌더, REVIEW 개수 기반 투자 매력도 감점 / cap, 재검증 없는 예상 점수 상승, 새 비중 실험, 약점 개선 portfolio 자동 생성 / 자동 backtest / 자동 저장, provider data 수집, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, auto rebalance |
...
- Final Review는 상세 evidence table보다 `Final Review 투자 검토서`를 먼저 보여준다. 투자 검토서는 Python service가 만든 read model을 React로 렌더링하며, first-read에서는 단일 판단 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평, 강점과 약점, 저장 전 확인 질문, 10개 조건부 Monitoring 패턴을 보여준다. 하단은 연결된 `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어` shell이고, 저장 / Monitoring handoff는 바로 이어지는 통합 `판단 기록`에서 확인한다.
...
- Final Review의 주 action은 투자 검토서 다음에 나오는 통합 `판단 기록`이다. 이 구간은 상태 / 차단 수 / 권장 판단, route, 판단 사유, 저장 CTA를 먼저 보여주고 checklist / evidence checks / 저장 ID는 접힌 `판단 근거와 저장 경계`로 낮춘다. 상세 Practical Validation / Robustness / Paper Observation / Investability Packet 표는 중복 검증이 아니라 이전 검증 결과를 추적하는 Evidence Appendix다. route는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토를 선택할 수 있다.
