...
이 문서는 Backtest에서 후보를 만들고, Practical Validation에서 검증 근거를 만들고, Final Review에서 Portfolio Monitoring 후보 여부를 판단한 뒤 Portfolio > Portfolio Monitoring에서 사후 확인하는 현재 사용자 흐름을 설명한다.

파일 / helper 이름은 여전히 legacy `Selected Portfolio Dashboard`를 포함할 수 있지만, 현재 사용자-facing route와 stage 이름은 `Portfolio > Portfolio Monitoring`이다.

현재 화면은 `portfolio_monitoring_workspace_v2` React Command Center이며, 구조와 migration 경계는 `../architecture/PORTFOLIO_MONITORING_REACT_COMMAND_CENTER.md`와 `../runbooks/PORTFOLIO_MONITORING_MIGRATION_AND_QA.md`를 따른다. `종목·전략 결과` 행 선택은 React local navigation으로 개별 추적 상세만 바꾸고, 그룹 전환 또는 실제 저장·수집 command에서 Streamlit server rerun을 사용한다.
...
| Monitoring Check | Portfolio > Portfolio Monitoring | 모니터링 이후 recheck readiness, freshness, provider evidence, review signal 확인 |
...
PORTFOLIO_SELECTION_SOURCES
  -> PRACTICAL_VALIDATION_RESULTS
    -> FINAL_PORTFOLIO_SELECTION_DECISIONS
      -> Portfolio > Portfolio Monitoring read-only monitoring
```
...
- `Portfolio > Portfolio Monitoring`은 선정 후 추적을 수행하는 Portfolio navigation의 monitoring 화면이며 Backtest 후보 생성 단계가 아니다. 수집 실행 결과와 다음 행동은 `Data > Data Operations > 실행 이력`에서 확인하고, Portfolio Monitoring에는 raw run/log 진단 패널을 두지 않는다.
...
- Portfolio Monitoring의 직접 미국 주식·ETF 중 `보유 수량`으로 시작한 항목은 선택 상세에서 최초 설정 정정, 추가매수, 일부매도, 거래 수정·취소를 수행한다. 최초 설정 정정의 날짜·수량은 화면에서 먼저 편집하고 명시적 `변경값 확인`으로 DB 적용일·종가·최초 투자금을 조회하며, 현재 입력과 일치하는 확인값이 있을 때만 저장한다. 거래일의 정확한 저장 종가가 기본 체결가이고 실제 체결가로 바꿀 수 있다. 추가매수는 외부 입금, 일부매도 순대금은 외부 출금으로 성과에 반영한다. fixed-notional 주식·ETF, selected strategy, quant backtest 결과는 이 거래 원장을 사용하지 않는다.
- Portfolio Monitoring의 공통 기준일이 최근 완료 NYSE 거래일보다 오래되면 선택 그룹의 활성 direct stock·ETF별 저장 최신일과 `보유 종목 가격 최신화` action을 보여준다. 사용자가 누른 경우에만 기존 daily OHLCV ingestion을 실행하고 수집 후 DB 최신성을 재검증해 공통 기준일과 가치곡선을 다시 계산한다. selected strategy와 종료 항목은 이 action에 포함하지 않으며 사용자-facing 수집 기록은 Data Operations 실행 이력이 소유한다.
...
- visible flow는 `후보와 검증 기준 확인 -> 최신 데이터 기준 재검증 -> 결과 해석과 해결 구분 -> 저장하고 Final Review로 이동`의 4단계다. 저장/이동 intent는 custom component callback이 선소비하지 않고 fragment 본문의 Python consumer가 처리하며, 성공한 이동은 full-app rerun으로 root route owner에 전달한다. 같은 stable `validation_id`는 다시 append하지 않지만 이미 저장된 validation도 정상 handoff할 수 있고, Final Review에는 `practical_validation_result:<validation_id>` active key를 넘겨 방금 인계한 후보를 연다.
- Step 1은 후보 선택과 검증 관점을 `1A / 1B`로 분리하고 replay fragment 밖의 `context` surface에 둔다. 고정 hero 아래 Step 1 summary가 현재 후보와 판정 기준을 함께 보여주고, 후보 목록은 `후보 변경`에서만 펼치는 compact selector다. 검증 관점은 데스크톱 한 행 5열, 760px 2열 줄바꿈이며 마지막 홀수 option은 전체 열을 쓴다. Step 2~4는 별도 `decision` surface와 fragment가 소유한다. latest runtime replay intent는 declared component `on_change`에서 먼저 소비하고 별도 fragment rerun을 추가하지 않으며, 계산 중에도 상단 context와 기존 하단 pending shell을 유지한 뒤 결과만 새 projection으로 바꾼다. 외부 자료 보강을 실행한 source는 Step 2 상단에서 `자료 보강 -> 재검증 -> 새 결과 저장 -> Final Review 확인` lifecycle을 계속 보여주며, 부분 성공이나 실패를 전체 완료로 표시하지 않는다. replay 전 primary CTA는 `보강된 데이터로 재검증`이고, replay 뒤 Gate에 따라 새 결과 저장 또는 남은 blocker 확인으로 바뀐다. Step 3은 verified, measured caution, validated caution, resolve-now, engineering-required, Final Review handoff를 분리하고 `상세 검증 근거`를 disclosure로 제공한다. Step 4는 audit save와 eligible save-and-move를 제공하며 후보/profile 변경과 Final Review route 이동만 app rerun을 사용한다.
- `상세 검증 근거`는 별도 Flow 5가 아니다. 사용자 설명은 검증명 / 확인 결과 / 의미 / 다음 행동 순서로 보여주고, raw 함수 경로와 내부 status는 secondary `technical_trace`에 둔다. 데이터/편향, 검증 방법, 포트폴리오 구성, 실전 현실성, 스트레스/강건성 5개 category 중 하나만 펼친다. source 핵심 사실은 Step 1 `검증 대상 요약`, profile 질문과 threshold는 Step 1 `판정 기준 세부 조정`, advanced replay mode와 실제 기간 provenance는 Step 2 `재검증 범위 / 재검증 기록`, 현재 validation identity는 Step 4 `검증 기록`이 소유한다. full source / replay / validation snapshot은 runtime과 registry에 보존하지만 current Level 2 화면에는 렌더링하지 않는다.
- current Final Review eligible 후보는 unresolved actionable / critical engineering / missing contract가 모두 0이어야 한다. 계산된 Level2 REVIEW는 `validated_caution`으로 현재 단계에서 종결하고, 실제 미검증 필수 방법론은 engineering blocker로 유지한다. accepted limit / final decision / monitoring transfer는 측정값이 있어도 Level2 수리 목록으로 바꾸지 않고 Final Review handoff를 유지한다.
- blocked Level2에서는 handoff를 `검증 통과 후 Final Review에서 확인할 항목`으로만
  예고한다. eligible validation에서만 compact `Final Review 인계 준비 완료`로
  승격하고 class count와 root-dedup 항목만 표시한다. Final Review 본문은 이를
  `최종 판단 입력 / 인수한 검증 한계 / Monitoring 이관 조건`으로 소비하며,
  0건 lane은 렌더링하지 않는다.
- Monitoring transfer는 observation, threshold, cadence, re-review action,
  evidence reference가 모두 있는 구조화 조건만 허용한다. 불완전한 REVIEW는
  Final Review 숙제가 아니라 Level2 engineering blocker다.
- 실제 callable Python handler가 있는 action만 `지금 해야 할 일` CTA를 가진다. action 0건이면 빈 action board를 렌더링하지 않는다.
- React는 presentation과 intent만 소유한다. Python이 finding classification, applicability, root dedup, Gate, handler validation, replay, save, Final Review handoff를 소유한다.
...
- Final Review React surface는 `Research > Market Research`의 질문 중심 projection과 visual language를 함께 따른다. blue-gray palette, rounded panel, soft shadow, compact heading, 20/17/14px radius hierarchy를 사용하며 760px에서 header/chart/finding이 단일 열로 접힌다. 이 규칙은 presentation contract이고 eligibility, Gate, route, save, Monitoring 계산은 계속 Python 소유다.
- 누적 성과와 Underwater는 저장 curve 종료일만 조용히 보여주지 않는다. Decision Workspace는 `현재 차트 / 최신 완료 시장일 / DB 공통일 / 제한 종목`을 compact freshness strip으로 표시한다. 목표일까지 자동 수집 가능한 가격 gap이 있으면 `계속 추적` 선정만 잠그고, `최신 데이터로 다시 계산` intent가 기존 OHLCV 수집 → 동일 source replay → 새 Practical Validation append를 Python에서 실행한다. 기존 validation row는 rewrite하지 않으며 non-select 판단 route는 기록 가능하다.
- Final Review first-read에서는 CNN / AAII 시장심리 패널을 렌더링하지 않는다. 자세한 심리 해석은 `Research > Market Research > 시장 환경 > 심리`에서 확인하며, 시장심리는 후보 우선순위, selected-route gate, Final Decision save readiness, registry 저장, live approval / broker order / auto rebalance에 영향을 주지 않는다.
...
- Final Review의 주 action은 one-shell 하단의 `최종 판단과 사유`다. canonical route는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, `HOLD_FOR_MORE_PAPER_TRACKING`, `REJECT_FOR_PRACTICAL_USE`, `RE_REVIEW_REQUIRED` 그대로 저장하며 사용자 label만 `계속 추적`, `관찰 후 재검토`, `추적 대상에서 제외`, `Level2로 돌려보내기`로 표시한다. accepted limit가 있으면 각 root마다 `한계를 인수하고 계속` 또는 `Level2로 되돌리기`를 먼저 선택해야 하며, 반환 선택은 `RE_REVIEW_REQUIRED` route와만 함께 저장할 수 있다. 사유를 직접 작성하고 모든 한계를 처리해야 CTA가 활성화되며 Decision ID / constraints / next action은 Python이 자동 생성한다.
...
- Final Review는 Saved Decisions ledger, Dossier, Evidence Packet, Raw JSON을 렌더링하지 않는다. selected row의 운영 확인은 Portfolio > Portfolio Monitoring이 맡고, hold / reject / re-review row는 append-only audit data로 보존한다.
...
- Final Review는 current 후보의 `선정 전 미해결 항목 0`을 확인한 뒤 accepted limit / final decision / Monitoring transfer를 기존 route와 판단 사유로 종결한다. final decision row에는 기존 `evidence_closure_snapshot`과 함께 chart point를 제외한 verdict, finding ids, structured Monitoring conditions, accepted root ids, source gaps, normalized `accepted_limit_acknowledgements`를 `decision_brief_snapshot_v1`으로 저장한다. Python이 expected/duplicate/unknown root와 allowed decision, return route 일관성을 검증하며 React payload만으로 우회할 수 없다. selected route는 기존 Gate와 closed closure를 모두 통과해야 `monitoring_candidate=True`가 된다.
...
- `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`은 Final Review 판단 record를 append-only로 담는다. 새 row는 `monitoring_candidate` / `monitoring_handoff_state`, `evidence_closure_snapshot`, compact `decision_brief_snapshot`을 포함한다. snapshot의 `accepted_limit_acknowledgements`는 Final Review가 각 Level2 한계를 인수했는지 반환했는지를 root 기준으로 보존한다. Portfolio Monitoring은 selected row의 structured `decision_brief_snapshot.monitoring_conditions`를 우선 읽고, snapshot 없는 legacy row는 `paper_tracking_snapshot.review_triggers`를 fallback으로 읽는다. 기존 row는 재작성하지 않는다.
- `SELECTED_DASHBOARD_PORTFOLIOS.jsonl`은 Portfolio > Portfolio Monitoring 전용 saved setup이다. File name은 legacy dashboard term을 유지한다. Final Review 판단, monitoring evidence, approval record가 아니며 user-created portfolio 이름, 설명, selected decision strategy slot, start / latest-end mode / balance / memo만 보존한다.
- `SELECTED_PORTFOLIO_MONITORING_LOG.jsonl`는 사용자가 명시적으로 남기는 optional monitoring check record이며 자동 저장 대상이 아니다.
- `finance_meta.monitoring_security_position_event`는 direct-stock fixed-shares 거래와 revision을 append-only로 보존한다. 이 가상 입출금 원장은 saved/registry JSONL이나 broker/account transaction을 대체하지 않는다.
...
| Portfolio > Portfolio Monitoring | `app/web/final_selected_portfolio_dashboard*.py`, `app/runtime/backtest/read_models/final_selected_portfolios.py` |
...
- Portfolio > Portfolio Monitoring이 read-only monitoring 경계를 넘어서거나 저장 경계가 바뀔 때
