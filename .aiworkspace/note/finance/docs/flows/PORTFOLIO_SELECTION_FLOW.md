...
- Flow 3 `2차 검증 결론 / Fix Queue`는 Final Review 이동 가능 여부와 `Final Review 이동을 막는 이슈`를 먼저 보여주는 first-read surface다. 이 surface는 `workspace_panel.py`가 소유하고, `practical_validation_fix_queue` React component는 read-only 카드로 Final Review 이동 판단 / top blocking issues / criteria status summary를 보여준다. 각 blocker는 `현재 문제 / 완료 기준 / 보강 위치 / 영향`으로 읽히며, `NEEDS_INPUT` / `NOT_RUN` 같은 raw status는 `기술 기준` 보조 tag로 남긴다. 저장 / 이동 button은 Flow 5가 소유하므로 Flow 3에는 만들지 않는다.
...
- Flow 4 `근거 Workbench`는 먼저 `Final Review로 넘기기 전 확인 기준` 보드로 Source / Validation / Final Review readiness 기준별 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약한 뒤, 검증 근거 보드, Applied Validation Map, Provider Action Center, Look-through Board, Robustness Lab, detailed diagnostics를 담는다. 이 보드는 새 검증 단계가 아니라 Flow 3 결론이 어떤 확인 기준에서 나왔는지 요약하는 기준 상태판이며, 화면 board는 검증 module과 1:1 개념이 아니므로 module gate와 evidence board를 분리해서 읽는다.
