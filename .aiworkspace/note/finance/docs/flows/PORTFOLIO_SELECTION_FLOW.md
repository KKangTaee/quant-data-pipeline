...
| 3 | Final Review | Decision Desk command center로 오늘의 판단 상태를 먼저 보고, CNN / AAII market sentiment context overlay를 시장 배경으로만 확인한다. Candidate Board에서 수익성 / benchmark / 후보 간 우선순위 / profile 적합성을 비교한 뒤 `Final Review 투자 검토서`에서 추천 / 보류 / 탈락 / Monitoring 후보, 점수와 이유, 강점 / 약점, Level2 REVIEW 처리 결과, 저장 / Monitoring handoff 경계, 약점 개선안을 먼저 읽는다. Decision Cockpit은 같은 evidence packet의 selection-readiness 상태를 확인하는 실행 전 점검판이다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 Portfolio Monitoring 후보 handoff 대상이다. Practical Validation의 데이터 / 실용성 REVIEW는 다시 검증하지 않고 read-only warning으로 소비하며, Final Review / Monitoring role 항목만 최종 판단 / 추적 handoff로 이어진다. 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 보존한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
