...
- Final Review source picker는 Practical Validation Gate와 selected-route preflight를 통과한 current result만 표시한다. 새 pre-final enrichment 계약에서 blocking인 row는 저장 기록으로 남아도 후보에서 숨긴다. 계약 도입 전 legacy row와 저장 후 다시 stale해진 row는 audit 보존을 위해 복구 후보로 열람할 수 있지만 Decision Desk / recommendation / Final Decision Action은 `2단계 재검증 필요`로 표시하고 모든 판단 저장을 비활성화한다.
