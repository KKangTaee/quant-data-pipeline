...
| 3 | Final Review | Decision Desk에서 후보 현황과 우선순위를 먼저 보고 stable key 기반 `검토 대상`과 `최종 검토서 확인`으로 현재 후보를 확정한다. selector가 바뀌면 stale 안내와 함께 report / 판단 intent를 숨긴다. 투자 검토서는 단일 판단 상태와 `투자 매력도 / 근거 신뢰도 / Monitoring 준비도`, 총평, 4행 해석 직후에 `모니터링 후보 선정 / 보류 / 탈락 / 재검토` route와 사용자가 직접 작성하는 판단 사유, gate 기반 활성 / 비활성 CTA를 보여준다. 이후 강점 / 약점, 저장 전 확인 질문, 10개 조건부 Monitoring 패턴, `Final Review 확인 필요`, `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어`가 이어진다. React는 report 표시와 route / reason intent만 전달하고 Python이 score, gate, save evaluation, 자동 Decision ID, row 작성, append, Monitoring handoff를 소유한다. Evidence Appendix와 Saved Decisions ledger는 main flow에 렌더링하지 않으며 selected row 운영 확인은 Operations > Portfolio Monitoring이 소유한다. 기존 Final Decision row는 삭제하거나 재작성하지 않는다. Final Review는 live approval, broker order, auto rebalance가 아니다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, Decision Desk 우선순위 요약, stable key 기반 검토 대상 선택, 명시적 `최종 검토서 확인`, stale report 차단, 총평 / 세 점수 축 / 4행 해석 직후 최종 판단 route·사유·CTA, 강점과 약점 / stored audit REVIEW trace / 10개 조건부 Monitoring 패턴 / 상위 3개 다음 실험 아이디어, selected-route gate 기반 Portfolio Monitoring 후보 handoff | visible Review Queue table, 별도 `Step 1 / Candidate Board` / standalone Decision Cockpit, selector 변경만으로 report 자동 렌더, Decision ID / storage path / 운영 조건 사용자 편집, Evidence Appendix, Saved Decisions ledger, REVIEW 개수 기반 투자 매력도 감점 / cap, 재검증 없는 예상 점수 상승, 새 비중 실험, 약점 개선 portfolio 자동 생성 / 자동 backtest / 자동 저장, provider data 수집, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, auto rebalance |
...
- Final Review source picker는 source별 최신 Practical Validation row를 먼저 고른 뒤 Gate와 selected-route preflight eligibility를 적용한다. 최신 row가 blocking이면 저장 기록으로 남아도 후보에서 숨기며, 과거 eligible row로 fallback하지 않는다. 계약 도입 전 legacy row와 저장 후 다시 stale해진 row는 audit 보존을 위해 복구 후보로 열람할 수 있지만 Decision Desk / recommendation / Final Decision Action은 `2단계 재검증 필요`로 표시하고 모든 판단 저장을 비활성화한다. Practical Validation의 명시적인 save-and-move가 성공하면 새 `practical_validation_result:<validation_id>` stable key를 selector와 confirmed state에 함께 전달해 새 검토서를 바로 연다.
...
- Final Review는 상세 evidence table보다 `Final Review 투자 검토서`를 먼저 보여준다. 투자 검토서는 Python service가 만든 read model을 React로 렌더링하며, 단일 판단 상태, 투자 매력도 / 근거 신뢰도 / Monitoring 준비도, 총평과 4행 해석 직후에 최종 판단 route와 판단 사유, gate 기반 CTA를 배치한다. React는 report 표시와 decision intent만 전달하고 Python이 save evaluation / 자동 Decision ID / append를 소유한다. 이후 강점과 약점, 저장 전 확인 질문, 10개 조건부 Monitoring 패턴과 `점수 근거 / 남은 판단 근거 / 다음 실험 아이디어` shell이 이어진다.
...
- Final Review의 주 action은 투자 검토서 총평과 4행 해석 직후의 `최종 판단`이다. `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토 route를 고르고 사용자가 판단 사유를 직접 작성해야 CTA가 활성화된다. Decision ID, 운영 조건, 다음 행동은 Python이 자동 생성해 row에 저장하며 사용자 화면에는 노출하지 않는다.
...
- Final Review는 current 후보의 `선정 전 미해결 항목 0`을 확인한 뒤 accepted limit / final decision / Monitoring transfer를 기존 route와 판단 사유로 종결하고, 결과를 final decision row의 `evidence_closure_snapshot`에 저장한다. 정성 role count에는 고정 숫자 감점을 적용하지 않는다.
