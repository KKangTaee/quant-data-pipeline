...
| 3 | Final Review | Decision Desk command center로 후보 현황과 다음 판단을 먼저 본다. Final Review first-read에서는 CNN / AAII 시장심리 패널을 렌더링하지 않는다. 자세한 심리 해석은 `Workspace > Overview > Sentiment`에서 확인한다. Candidate Board에서 수익성 / benchmark / 후보 간 우선순위 / profile 적합성을 비교한 뒤 `Final Review 투자 검토서`에서 추천 / 보류 / 탈락 / Monitoring 후보, 최종 선택 사유, 세부 점수, score cap, Level2 REVIEW 점수 영향, 강점 / 약점, 저장 / Monitoring handoff 경계, 약점 개선안, 판단 저장 전 메모를 먼저 읽는다. Scorecard는 `Investment`, `Risk`, `Readiness`, `Evidence Quality`, `Monitoring Suitability` weighted dimension을 쓰며, hard blocker / selected-route not-ready / gate review-required / 과도한 open review는 높은 원점수라도 추천권으로 과대평가되지 않게 cap을 만든다. Decision Cockpit은 같은 evidence packet의 selection-readiness 상태를 확인하는 실행 전 점검판이다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 Portfolio Monitoring 후보 handoff 대상이다. Practical Validation의 데이터 / 실용성 REVIEW는 다시 검증하지 않고 score impact와 read-only warning으로 소비하며, Final Review / Monitoring role 항목만 최종 판단 / 추적 handoff로 이어진다. 시장심리 timing / rebalance 활용은 별도 리서치와 look-ahead-safe 검증 전까지 Final Review gate나 Portfolio Monitoring signal로 쓰지 않는다. 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 보존한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, 수익성 / benchmark / 후보 간 우선순위 / profile 적합성 판단, Decision Desk command center, Candidate Board review priority / queue, 투자 검토서에서 추천 / 보류 / 탈락 / Monitoring 후보와 점수 / 강점 / 약점 / Level2 REVIEW 처리 / 약점 개선안 요약, Decision Cockpit에서 모니터링 후보 가능 / 보류 / 재검토 / 거절 상태 안내, Decision Record Checklist와 판단 문안으로 Final Review 판단 record 저장, selected-route gate가 통과한 row만 Portfolio Monitoring 후보 handoff, Saved Decision Review ledger로 저장된 판단 기록 재확인, Portfolio Monitoring handoff로 dashboard 대상 row / monitorable / blocked 상태 확인, Evidence Appendix에서 investability evidence packet / construction risk / risk contribution / component role weight / validation method strength / data coverage / backtest realism 근거 read-only 확인, selection-readiness gate, Final Review / Monitoring role handoff, deployment readiness policy snapshot 보존, saved decision dossier export | 새 비중 실험, 약점 개선 portfolio 자동 생성 / 자동 backtest / 자동 저장, provider data 수집, sentiment 기반 selection gate 변경, sentiment timing / rebalance signal 생성, Final Review first-read 시장심리 패널, 사용자 메모용 반복 저장, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, 자동 report 파일 생성, dashboard monitoring 자동 저장 |
...
- Final Review first-read에서는 CNN / AAII 시장심리 패널을 렌더링하지 않는다. 자세한 심리 해석은 `Workspace > Overview > Sentiment`에서 확인하며, 시장심리는 Candidate Board priority, selected-route gate, Final Decision save readiness, registry 저장, live approval / broker order / auto rebalance에 영향을 주지 않는다.
