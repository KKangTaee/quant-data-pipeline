...
| 3 | Final Review | Decision Desk command center로 후보 현황과 다음 판단을 먼저 본다. Final Review first-read에서는 별도 `Step 1 / Candidate Board`와 CNN / AAII 시장심리 패널을 렌더링하지 않는다. 자세한 심리 해석은 `Workspace > Overview > Sentiment`에서 확인한다. Decision Desk 아래에서 Review Queue, `검토 대상` selector, 접힌 후보 비교 상세로 Gate 통과 후보의 수익성 / benchmark / 후보 간 우선순위 / profile 적합성 근거를 고른 뒤 `Final Review 투자 검토서`에서 `선택 판단 요약`, Monitoring 후보 여부, 종합 / 원점수 / score cap, 가장 강한 근거, 가장 큰 확인 지점, Level2 REVIEW 반영, 강점, 확인 지점, 실제 해석 카드를 먼저 읽는다. old first-read `판단 저장 전 메모` block은 반복 표시하지 않지만, 저장에 필요한 note contract는 Python service payload와 Final Review 저장 action에서 유지한다. Scorecard는 `Investment`, `Risk`, `Readiness`, `Evidence Quality`, `Monitoring Suitability` weighted dimension을 쓰며, hard blocker / selected-route not-ready / gate review-required / 과도한 open review는 높은 원점수라도 추천권으로 과대평가되지 않게 cap을 만든다. Decision Cockpit은 같은 evidence packet의 selection-readiness 상태를 확인하는 실행 전 점검판이다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 Portfolio Monitoring 후보 handoff 대상이다. Practical Validation의 데이터 / 실용성 REVIEW는 다시 검증하지 않고 score impact와 read-only warning으로 소비하며, Final Review / Monitoring role 항목만 최종 판단 / 추적 handoff로 이어진다. 시장심리 timing / rebalance 활용은 별도 리서치와 look-ahead-safe 검증 전까지 Final Review gate나 Portfolio Monitoring signal로 쓰지 않는다. 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 보존한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
- Final Review는 상세 evidence table보다 `Final Review 투자 검토서`와 Decision Cockpit을 먼저 보여준다. 투자 검토서는 Python service가 만든 read model을 React로 렌더링하며, `선택 판단 요약`, Monitoring 후보 여부, 점수 체계, 강점, 확인 지점, 실제 해석 카드, Level2 REVIEW 처리 결과, 저장 / Monitoring handoff 경계, 약점 개선안을 사람이 읽는 검토서 형태로 보여준다.
