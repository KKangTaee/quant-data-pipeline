...
- `Operations > Portfolio Monitoring`은 선정 후 추적을 바로 수행하는 Operations의 유일한 사용자-facing 화면이며 Backtest 후보 생성 단계가 아니다. 수집 실행 결과와 다음 행동은 `Data > Data Operations > 실행 이력`에서 확인하고, Portfolio Monitoring에는 raw run/log 진단 패널을 두지 않는다.
...
- Final Review React surface는 `Workspace > Overview > 시장 맥락`의 질문 중심 projection과 visual language를 함께 따른다. blue-gray palette, rounded panel, soft shadow, compact heading, 20/17/14px radius hierarchy를 사용하며 760px에서 header/chart/finding이 단일 열로 접힌다. 이 규칙은 presentation contract이고 eligibility, Gate, route, save, Monitoring 계산은 계속 Python 소유다.
