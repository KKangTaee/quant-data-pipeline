...
- Backtest Analysis의 Data Trust Summary는 DB 가격 기준일이 최신 완료 거래일보다 오래된 경우, 현재 후보 ticker만 대상으로 OHLCV 가격 이력 업데이트 action을 제공한다. 이 action은 데이터 보강만 수행하고 결과 재계산 / source 등록 / 2차 검증 전송은 사용자가 별도로 실행한다.
