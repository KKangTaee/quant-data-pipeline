...
| 3 | Final Review | Decision Desk command center로 오늘의 판단 상태를 먼저 보고, CNN / AAII market sentiment context overlay를 시장 배경으로만 확인한다. Candidate Board에서 수익성 / benchmark / 후보 간 우선순위 / profile 적합성을 비교하고 Decision Cockpit으로 selection-readiness 상태를 확인한다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 Portfolio Monitoring 후보 handoff 대상이다. Practical Validation의 데이터 / 실용성 REVIEW는 다시 검증하지 않고 Evidence Appendix에서 read-only로 소비하며, Final Review / Monitoring role 항목만 최종 판단 / 추적 handoff로 이어진다. 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 보존한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, 수익성 / benchmark / 후보 간 우선순위 / profile 적합성 판단, Decision Desk command center / flow rail, CNN / AAII market sentiment context overlay, Candidate Board review priority / queue, Decision Cockpit에서 모니터링 후보 가능 / 보류 / 재검토 / 거절 상태 안내, Decision Record Checklist와 판단 문안으로 Final Review 판단 record 저장, selected-route gate가 통과한 row만 Portfolio Monitoring 후보 handoff, Saved Decision Review ledger로 저장된 판단 기록 재확인, Portfolio Monitoring handoff로 dashboard 대상 row / monitorable / blocked 상태 확인, Evidence Appendix에서 investability evidence packet / construction risk / risk contribution / component role weight / validation method strength / data coverage / backtest realism 근거 read-only 확인, selection-readiness gate, Final Review / Monitoring role handoff, deployment readiness policy snapshot 보존, saved decision dossier export | 새 비중 실험, provider data 수집, sentiment 기반 selection gate 변경, 사용자 메모용 반복 저장, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, 자동 report 파일 생성, dashboard monitoring 자동 저장 |
