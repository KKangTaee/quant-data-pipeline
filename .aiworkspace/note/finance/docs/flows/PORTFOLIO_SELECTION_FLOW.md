...
| 3 | Final Review | Decision Desk command center로 후보 현황과 다음 판단을 먼저 본다. Final Review first-read에서는 별도 `Step 1 / Candidate Board`와 CNN / AAII 시장심리 패널을 렌더링하지 않는다. 자세한 심리 해석은 `Workspace > Overview > Sentiment`에서 확인한다. Decision Desk 아래에서 Review Queue, `검토 대상` selector, 접힌 후보 비교 상세로 Gate 통과 후보의 수익성 / benchmark / 후보 간 우선순위 / profile 적합성 근거를 고른 뒤 `Final Review 투자 검토서`를 한 장의 판단서처럼 읽는다. 투자 검토서 first-read는 candidate meta strip, `왜 후보인가` / `무엇을 확인해야 하나` decision brief, 강점 / 확인 지점 evidence rows, 실제 해석 rows를 먼저 보여주고, 상세 점수 체계 / Level2 REVIEW / 저장 및 Monitoring handoff / 약점 개선안 / Monitoring 조건은 lower tabbed detail panel에서 `근거 상세`, `저장 경계`, `개선 후보`, `Review 처리`, `Monitoring` 탭으로 확인한다. old first-read `판단 저장 전 메모` block은 반복 표시하지 않지만, 저장에 필요한 note contract는 Python service payload와 Final Review 저장 action에서 유지한다. Scorecard는 `Investment`, `Risk`, `Readiness`, `Evidence Quality`, `Monitoring Suitability` weighted dimension을 쓰며, hard blocker / selected-route not-ready / gate review-required / 과도한 open review는 높은 원점수라도 추천권으로 과대평가되지 않게 cap을 만든다. Decision Cockpit은 같은 evidence packet의 selection-readiness 상태를 확인하는 실행 전 점검판이다. 선택 / 보류 / 거절 / 재검토는 Final Review 판단 record로 저장할 수 있으며, `selection_gate_policy_snapshot`이 통과한 `SELECT_FOR_PRACTICAL_PORTFOLIO` row만 Portfolio Monitoring 후보 handoff 대상이다. Practical Validation의 데이터 / 실용성 REVIEW는 다시 검증하지 않고 score impact와 read-only warning으로 소비하며, Final Review / Monitoring role 항목만 최종 판단 / 추적 handoff로 이어진다. 시장심리 timing / rebalance 활용은 별도 리서치와 look-ahead-safe 검증 전까지 Final Review gate나 Portfolio Monitoring signal로 쓰지 않는다. 더 엄격한 live 투입 감사 성격의 판정은 `deployment_readiness_policy_snapshot`으로 보존한다 | `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl` |
...
| Final Review | Gate-passed 후보 비교, 수익성 / benchmark / 후보 간 우선순위 / profile 적합성 판단, Decision Desk command center와 통합된 Review Queue / 검토 대상 selector / 후보 비교 상세, 투자 검토서에서 meta strip, `왜 후보인가` / `무엇을 확인해야 하나`, Monitoring 후보 여부, 강점, 확인 지점, 실제 해석 rows를 먼저 확인하고 상세 점수 / Level2 REVIEW / handoff / 개선안 / Monitoring 조건은 disclosure로 확인, Decision Cockpit에서 모니터링 후보 가능 / 보류 / 재검토 / 거절 상태 안내, Decision Record Checklist와 판단 문안으로 Final Review 판단 record 저장, selected-route gate가 통과한 row만 Portfolio Monitoring 후보 handoff, Saved Decision Review ledger로 저장된 판단 기록 재확인, Portfolio Monitoring handoff로 dashboard 대상 row / monitorable / blocked 상태 확인, Evidence Appendix에서 investability evidence packet / construction risk / risk contribution / component role weight / validation method strength / data coverage / backtest realism 근거 read-only 확인, selection-readiness gate, Final Review / Monitoring role handoff, deployment readiness policy snapshot 보존, saved decision dossier export | 별도 `Step 1 / Candidate Board` 섹션, 새 비중 실험, 약점 개선 portfolio 자동 생성 / 자동 backtest / 자동 저장, provider data 수집, sentiment 기반 selection gate 변경, sentiment timing / rebalance signal 생성, Final Review first-read 시장심리 패널, 사용자 메모용 반복 저장, Practical Validation 재실행, 실제 자금 투입 승인, broker order, account sync, 자동 report 파일 생성, dashboard monitoring 자동 저장 |
...
- Final Review 상단은 Decision Desk command center로 오늘의 판단 상태, 후보 수, 숨겨진 Gate 미통과 기록, 저장된 Final Review 판단 기록, Portfolio Monitoring 연결 후보를 먼저 보여준다. 이 shell은 UI 표시 계층이며 gate / persistence logic을 바꾸지 않는다.
...
- Final Review는 Decision Desk 아래 후보 선택 패널에서 Gate 통과 후보의 decision state, suggested decision, blocker / open review 수, 주요 audit route를 비교한다.
- Review Queue는 후보를 select-ready, hold / re-review, blocked 순서로 정렬하고, first-review candidate, primary reason, next action을 표시한다. 이 priority는 새 투자 점수가 아니라 기존 evidence를 보기 쉽게 정렬하는 read-only 표시다.
- Final Review는 상세 evidence table보다 `Final Review 투자 검토서`와 Decision Cockpit을 먼저 보여준다. 투자 검토서는 Python service가 만든 read model을 React로 렌더링하며, first-read에서는 candidate meta strip, `왜 후보인가` / `무엇을 확인해야 하나`, 강점 / 확인 지점 row, 실제 해석 rows를 사람이 읽는 검토서 형태로 보여준다. 상세 점수 체계, Level2 REVIEW 처리 결과, 저장 / Monitoring handoff 경계, 약점 개선안, Monitoring 조건은 lower tabbed detail panel에서 한 섹션씩 확인한다.
- Level2 REVIEW는 Final Review에서 모두 다시 검증하지 않는다. `final_readiness_blocker`와 selected-route blocking status는 blocker, `pv_data_caution` / `pv_practical_caution`은 warning, `final_decision_input`은 open review, `monitoring_followup`은 Monitoring follow-up으로 분리해 최종 판단과 추적 조건에 반영한다.
...
- Decision Cockpit은 selection-readiness state, suggested decision, Must Fix, open review items, monitoring seed를 같은 evidence packet에서 읽는다.
- Final Review의 주 action은 Decision Cockpit 다음에 나오는 `Final Review 판단 저장`이다. 이 구간은 Decision Record Checklist, selected-route gate, 판단 사유 / 운영 전 조건 / 다음 행동 문안을 보여주며, 상세 Practical Validation / Robustness / Paper Observation / Investability Packet 표는 중복 검증이 아니라 이전 검증 결과를 추적하는 Evidence Appendix다. route는 `SELECT_FOR_PRACTICAL_PORTFOLIO`, 보류, 거절, 재검토를 선택할 수 있다.
- 저장된 Final Review row는 Saved Decision Review ledger에서 selected count, latest selection, detail tabs, Selected Dashboard handoff, Decision Dossier export로 다시 확인한다. 새 row는 `monitoring_candidate`와 `monitoring_handoff_state`를 통해 Monitoring handoff 여부를 명시한다. 이 review는 과거 판단을 읽는 화면이며 새 report 파일, approval, order를 만들지 않는다.
- Saved Decision Review의 Selected Dashboard handoff는 selected rows, dashboard rows, monitorable / blocked counts, checklist, destination을 보여준다. 이는 기존 selected-route gate나 Selected Dashboard continuity check를 다시 계산하는 중복 검증이 아니라, 저장된 Final Review 판단 중 Monitoring 후보로 표시된 기록이 다음 operations 화면으로 연결 가능한지 보여주는 read-only 연결 상태다.
...
- `FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`은 Final Review 판단 record를 append-only로 담는다. v3 row는 `monitoring_candidate` / `monitoring_handoff_state`를 포함하며, Portfolio Monitoring은 명시적으로 Monitoring 후보인 row만 selected pool로 읽는다. 기존 v2 row는 재작성하지 않는다.
