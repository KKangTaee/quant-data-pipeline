...
- Step 1은 후보 선택과 검증 관점을 `1A / 1B`로 분리하고 replay fragment 밖의 `context` surface에 둔다. 고정 hero 아래 Step 1 summary가 현재 후보와 판정 기준을 함께 보여주고, 후보 목록은 `후보 변경`에서만 펼치는 compact selector다. 검증 관점은 데스크톱 한 행 5열, 760px 2열 줄바꿈이며 마지막 홀수 option은 전체 열을 쓴다. Step 2~4는 별도 `decision` surface와 fragment가 소유한다. latest runtime replay intent는 declared component `on_change`에서 먼저 소비하고 별도 fragment rerun을 추가하지 않으며, 계산 중에도 상단 context와 기존 하단 pending shell을 유지한 뒤 결과만 새 projection으로 바꾼다. Step 3은 verified, measured caution, validated caution, resolve-now, engineering-required, Final Review handoff를 분리하고 `상세 검증 근거`를 disclosure로 제공한다. Step 4는 audit save와 eligible save-and-move를 제공하며 후보/profile 변경과 Final Review route 이동만 app rerun을 사용한다.
