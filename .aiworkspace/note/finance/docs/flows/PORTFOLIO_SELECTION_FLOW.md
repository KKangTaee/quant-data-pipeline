...
- Flow 4 `근거 Workbench`는 먼저 `카테고리별 검증 결과` 보드로 Source & Replay, Data Quality / Bias Control, Comparison Validity, Realism / Tradability, Validation Strength / Robustness, Portfolio Construction, Conditional Evidence의 `상태 / 통과한 기준 / 남은 문제 / 판정`을 요약한다. `selected_route_preflight`는 검증 category가 아니라 `Final Review 이동 요약`으로 분리해 저장 전 막힐 gap만 보조로 보여준다. 이후 검증 근거 보드, Applied Validation Map, Provider Action Center, Look-through Board, Robustness Lab, detailed diagnostics를 이어서 확인한다.
