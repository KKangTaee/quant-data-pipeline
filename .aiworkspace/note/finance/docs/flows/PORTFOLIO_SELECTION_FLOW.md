...
- Practical Validation의 `4. Final Review Gate / 검증 모듈`은 통과 / 차단 여부와 적용 module만 먼저 보여준다. 화면 board는 검증 module과 1:1 개념이 아니므로 `5. 검증 근거 보드`와 `6. 보강 액션`으로 분리한다.
- `Applied Validation Map`은 각 board가 어떤 module의 evidence인지, 현재 후보에 적용되는지, 적용되지 않으면 왜 빠졌는지를 보여준다.
- Practical Validation의 blocker table은 해결 위치와 액션을 같이 보여준다. `Latest Runtime Replay`는 별도 audit board가 아니라 `3. 최신 데이터 기준 전략 재검증` 섹션에서 재검증을 실행해 해소한다.
