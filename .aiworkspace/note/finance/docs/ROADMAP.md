...
| Market Research | confirmed 현재 국면·최근 변화·전환압력·조건부 다음 국면 분포를 포함한 경기 국면, 독립 물가·정책 경로, 선물 매크로 장중 관측·재가격화·반대 근거·조건부 시나리오, 심리의 1W·1M 실제 관측 변화, 일정, S&P 500, Market Movers, 미국 개별 종목의 3-family / 8-view research | context-only, 특정 전환 월·고정 순환·매매 신호가 아니며 Futures Macro 5D 확률·가격 목표와 심리 미래 확률도 validation / monitoring signal로 공개하지 않음 |
| Institutional Holdings | content-first header, bounded manager picker, horizontal destination tabs, 연속 manager 전환 상태 보존, 로컬 제출기한 기반 조건부 수동 갱신, amendment-aware 기관 portfolio·holdings·sector·security detail·identifier coverage, 과거 분기 전환과 두 covered-sleeve proxy | delayed reported-long-holdings research이며 actual fund NAV·추천이 아님; 탭 진입 시 SEC 호출 없음 |
...
|---|---|---|
| [CNN / AAII Sentiment expansion](../tasks/active/overview-sentiment-cnn-aaii-v1-20260719/STATUS.md) | 전체 잠정 `3/4차`. current evidence, immutable collection-time snapshot, common-period history와 CNN 5/20개·AAII Spread 1/4개 관측 간격의 1W·1M 실제 변화 구현 완료 | 미래 확률이 계속 필요하면 target·episode independence·chronological Point-in-Time validation과 공개 기준을 별도 승인 |
