...
| Data Operations | universe, price, statement, factor, macro, provider와 13F의 explicit DB collection / repair | UI direct fetch 없음 |
...
4. **Strategy governance** — daily swing lane을 validation / review / monitoring에
   연결할 실제 필요가 확인되면 연다.
5. **Maintenance / platform work** — refactor, scheduler, UI split과 archive migration은
   제품 가치나 운영 병목이 확인된 범위로만 연다.
