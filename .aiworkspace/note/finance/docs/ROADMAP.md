...
| Track | Status | Notes |
|---|---|---|
| Phase 12 Selected Monitoring / Recheck Operations | Active | `.aiworkspace/note/finance/phases/active/phase12-selected-monitoring-recheck-operations/`; next task is `selected-provider-evidence-staleness-contract-v1` |
...
- Phase 12 is active. `phase12-board-open` created the selected monitoring / recheck operations board, `selected-monitoring-source-map-v1` confirmed existing read-only dashboard evidence can seed implementation, and `recheck-readiness-freshness-contract-v1` added read-only recheck operations preflight. Main remaining gaps are provider evidence staleness / coverage, Review Signals / Recheck Comparison policy duplication, optional allocation drift boundary, and session-only monitoring evidence clarity. Next target is `selected-provider-evidence-staleness-contract-v1`.
