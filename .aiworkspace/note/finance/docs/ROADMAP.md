...
|---|---|---|
| Backtest Analysis Level1 Decision Workspace V1 | Complete | Fixed Level1 question, purpose-grouped Single catalog, new / saved Mix inner mode, Python-owned maturity / fingerprint / Gate / handler validation, two-surface React/fallback, decision-first result, stale-result preservation, distinct Mix save / Level2 handoff를 완료했다. 7차에서 9개 Single choice / 12개 primary concrete variant를 Python schema-driven React settings로 통일하고 legacy native form / Quality Snapshot을 replay compatibility로 격리했다. 760px one-column, actual Equal Weight / GTAA / Quality+Value Annual 실행을 확인했다. Strategy runtime, provider, DB schema, Level2 / Level3 route semantics는 변경하지 않았다. |
