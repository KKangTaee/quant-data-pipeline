...
- Latest completed task: `.aiworkspace/note/finance/tasks/active/overview-market-context-brief-context-absorption-v6-20260620/`
- 목적: `Workspace > Overview > Market Context`의 V5 `브리프 신뢰도`가 여전히 가이드/주의사항처럼 보이고 시장맥락 자체의 필요 정보로 읽히지 않는다는 사용자 피드백에 따라, 별도 신뢰도 섹션을 제거하고 의미 있는 부분만 시장 브리프 결론으로 흡수했다.
- 주요 변경: `brief_caveats` / `브리프 신뢰도`를 제거했다. `brief_rows`는 움직임, 확산, Futures/Macro 배경, optional 이벤트 배경을 표시한다. Events REVIEW / estimate는 `이벤트 배경`에서 직접 원인 근거가 약한지 보여주고, Futures / OHLCV data-health 항목이 있을 때만 `Futures/Macro 배경`을 `장중 macro 해석 보류`로 낮춘다. 상세 source / freshness는 하단 `근거: 자료 기준 / 출처 상태`가 소유한다.
...
- Latest completed product task: `.aiworkspace/note/finance/tasks/active/overview-market-context-brief-context-absorption-v6-20260620/`
