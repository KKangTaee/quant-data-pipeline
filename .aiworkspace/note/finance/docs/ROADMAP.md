...
| Track | Status | Notes |
|---|---|---|
| Phase 11 Portfolio Construction Risk Controls | Active | `.aiworkspace/note/finance/phases/active/phase11-portfolio-construction-risk-controls/`; next task is `phase11-integrated-qa-closeout` |
...
- Phase 11 is active. `phase11-board-open` created the official portfolio construction risk controls board, `construction-risk-source-map-v1` confirmed existing evidence can be reused, `concentration-overlap-exposure-contract-v1` added read-only Construction Risk Audit V1, `correlation-risk-contribution-contract-v1` added read-only Risk Contribution Audit V1, `component-role-weight-discipline-v1` added read-only Component Role / Weight Audit V1, and `construction-risk-gate-policy-v1` connected those audits to selected-route gate policy. Next task is `phase11-integrated-qa-closeout`.
