...
Current Market Movers redesign task는 `.aiworkspace/note/finance/tasks/active/market-movers-read-model-hardening-v1-20260720/`다.

- 목적: 승인된 결정형 워크벤치의 UI 전환 전에 sector/filter, collection readiness, group flow, bellwether, selected research 의미를 안정된 read model로 고정한다.
- 현재 상태: 전체 `3/5차` 완료. canonical 11-sector, return/volume/market-cap 유효 분모, `COMPLETE/PARTIAL/BLOCKED`, sector/industry current flow, latest market-cap Top 3, financial factor series, PIT-correct current PER gate와 `market_movers_decision_payload_v1`을 구현했다.
- 다음: 4차에서 기존 visible 구조를 React one-shell로 교체하고 ranking-selected state, breadth context, 가격/재무/뉴스·공시 조사 UI를 연결한다. 5차 sector conditional outlook은 historical episode와 OOS publication gate를 통과하기 전까지 수치를 노출하지 않는다.
