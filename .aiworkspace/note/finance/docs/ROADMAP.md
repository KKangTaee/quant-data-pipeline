...
- Latest completed task: `.aiworkspace/note/finance/tasks/active/overview-market-context-context-findings-v3-20260620/`
...
- 주요 변경: user-facing `다음 맥락 체크`를 `맥락 검토 결과`로 전환하고, 가격 움직임 / Futures-Macro / Events / 자료 신뢰도 caveat를 `결론`, `해석 영향`, `자료 기준` 행으로 표시한다. `next_checks`는 compatibility payload로 남겼지만 사용자 행동 checklist로 쓰지 않는다.
...
- Latest completed product task: `.aiworkspace/note/finance/tasks/active/overview-market-context-context-findings-v3-20260620/`
...
| Overview Market Context Context Findings V3 | Complete | Market Context now renders `맥락 검토 결과` instead of a user-action `다음 맥락 체크` checklist. The cockpit read model adds `context_findings` for price movement, Futures / Macro, Events, and data-health caveat, each with conclusion, interpretation impact, evidence, source area, freshness, and repair hint when needed. `next_checks` remains compatibility payload only. |
