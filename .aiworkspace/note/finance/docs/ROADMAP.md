...
| Track | Status | Notes |
|---|---|---|
| Phase 10 Walk-forward / OOS / Regime Validation | Active | `.aiworkspace/note/finance/phases/active/phase10-walkforward-oos-regime-validation/`; next task is `walkforward-split-contract-v1` |
...
- Phase 10 is active. `walkforward-oos-source-map-v1` confirmed that Practical Validation already has curve / benchmark / replay source plumbing, but explicit walk-forward / OOS / regime audit rows are not yet first-class gate evidence. Next task is `walkforward-split-contract-v1`.
