...
| Product Research Plugin Stage 5 | Complete | `.aiworkspace/note/finance/tasks/active/product-research-plugin-stage5/`; product research orchestration skill과 bundle bootstrap/check helper를 기존 plugin에 추가 완료 |
...
반복 product research run은 `finance-product-research-workflow`가 전체 순서를 조정한다.
새 research bundle은 `.aiworkspace/plugins/quant-finance-workflow/scripts/bootstrap_product_research_bundle.py`로 만들 수 있고, 산출물 구조는 `check_product_research_bundle.py`로 검증한다.
