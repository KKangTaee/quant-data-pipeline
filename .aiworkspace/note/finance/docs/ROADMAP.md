...
Current Market Movers redesign task는 `.aiworkspace/note/finance/tasks/active/market-movers-react-one-shell-v1-20260720/`다.

- 목적: 승인된 결정형 워크벤치에서 종목 발견, sector/industry 확산 확인, 선택 종목 조사를 한 화면과 selected state로 끝낸다.
- 현재 상태: 전체 `4/5차` 완료. 3차 canonical read model 위에 React-first ranking/breadth 62/38 workbench, bellwether Top 3, quick research, 가격·재무·뉴스·공시 상세와 bounded selected-symbol handoff를 연결했다. 재무는 보고 주기·영역·factor를 분리하고 단일 factor/axis와 70/30 chart/readout을 사용한다.
