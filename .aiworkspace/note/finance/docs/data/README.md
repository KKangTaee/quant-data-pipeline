...
| `fomc_sep_projection` | Federal Reserve SEP GDP/PCE values stored by release vintage, target year, and statistic. 1년 reconstruction에 필요한 2025-06 이후 history를 backfill하며, daily latest discovery가 이후 release를 append한다. Prior vintages are not overwritten. |
