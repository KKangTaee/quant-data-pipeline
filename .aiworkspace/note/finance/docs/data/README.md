...
| `market_intraday_snapshot` | Overview daily market movers intraday previous-close snapshot for S&P 500 / Top1000 / Top2000 |
| `market_event_calendar` | Overview Events calendar snapshot for FOMC, macro releases, earnings, and other market events. Macro rows use official BLS / BEA schedule sources, with BLS `.ics` import fallback when backend requests are blocked; earnings rows carry source validation and lifecycle status metadata. |
