...
- Market Movers Weekly / Monthly return eligibility는 lifetime row count가 아니라 선택 기간의 required start와 symbol별 실제 첫 가격일을 비교한다. stale limited-history symbol은 bounded overlap으로 재수집하고, 첫 가격일이 required start 뒤면 해당 기간에서만 명시적으로 제외한다.
...
- SEC Form 13F holdings are delayed reporting evidence and can be filed up to 45 days after quarter-end. Reported changes are not live buy / sell signals and do not show shorts, cash, derivatives, hedge structure, or full trading intent. `13F-HR/A`는 amendment type이 명확할 때만 replacement/addition으로 합성하고 불명확하면 last unambiguous base를 보존한다. Bulk manager pointer도 holdings가 있는 unambiguous base/restatement만 승격하며 notice, empty filing, additive-only/unknown amendment는 승격하지 않는다. Quarter-end와 filing-to-filing 수익은 actual fund NAV가 아니라 이전 reported-long value weight의 covered-sleeve proxy다. `amount_type=SH` common-equity와 저장 `adj_close`가 모두 있는 position만 계산하고 PRN/채권성·우선주성 class, option, raw-close-only, missing price/identifier weight는 coverage에서 제외하며 0% return으로 대체하지 않는다. Current identity enrichment accepts only one OpenFIGI US Equity ticker/composite FIGI, suppresses ambiguous mappings, and falls back only to legacy exact issuer-name matches. It is current-state identity evidence, not a historical PIT security master.
