...
- `practical-validation-required-taxonomy-refactor-v1` 이후 Practical Validation과 Final Review는 PIT / look-ahead / survivorship / provider freshness를 Data Coverage 계열 owner에서 보고, Validation Method Strength는 temporal / OOS / regime 방법론 evidence만 본다.
  Historical universe / delisting evidence가 명시되지 않으면 survivorship / universe row는 Data Coverage에서 `REVIEW`로 남긴다.
