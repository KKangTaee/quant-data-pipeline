...
- 현재 main UI에서는 `Backtest > Final Review`의 최종 선정 저장과 `Evidence Appendix`에서 benchmark, cadence, trigger를 확인하고 final review record 안에 남긴다.
...
현재 stage ownership 기준에서는 Selected Portfolio Dashboard가 선정 이후 monitoring / recheck를 담당한다.
...
### 기본 설명
단일 current candidate 또는 saved Portfolio Proposal을 최종 실전 후보로 선정할지 판단하는 전용 화면이다.
...
그래서 Final Review는 "초안 작성"과 "최종 선정 저장"을 분리해,
...
- 최종 선정 route는 `SELECT_FOR_PRACTICAL_PORTFOLIO`이며, selected-route gate가 통과됐을 때만 정식 저장한다.
- `최종 후보로 선정`을 누르면 `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS_V2.jsonl`에 append-only row가 남는다.
...
후보나 proposal을 최종 실전 후보로 `선정`, `보류`, `거절`, `재검토` 중 어디에 둘지 판단하는 근거 묶음이다.

### 왜 사용되는지
paper observation 기준이 준비됐거나 legacy paper ledger가 `READY_FOR_FINAL_SELECTION_REVIEW`로 읽히더라도
그 자체가 자동 선정이나 주문 지시는 아니다.
최종 후보 판단은 백테스트 성과, validation blocker, stress gap, paper observation 기준,
