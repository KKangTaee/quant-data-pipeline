...
- `Phase 22`에서는 이 결과를 바로 모니터링 후보로 승격하지 않고,
...
### 기본 설명
legacy workflow에서 후보 여러 개를 하나의 포트폴리오 제안 초안으로 묶던 기록이다.
...
현재 primary workflow에서는 Portfolio Proposal을 필수 단계로 사용하지 않는다.
새 후보 묶음은 `Backtest Analysis > Portfolio Mix Builder`에서 source로 만들고,
`Practical Validation -> Final Review -> Operations > Portfolio Monitoring`으로 이어진다.

### 왜 사용되는지
최종 목표는 단일 전략 후보를 많이 쌓는 것이 아니라,
사용자가 실제로 검토할 수 있는 포트폴리오 구성안과 가이드를 제시하는 것이다.

Portfolio Proposal은 현재 archive / recovery compatibility record로 남아 있으며,
main source chain이나 Final Review selected-route gate를 대체하지 않는다.
...
- Phase 30의 `Backtest > Portfolio Proposal`과 `.aiworkspace/note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl` row는 보존하지만, 현재 primary navigation에서는 직접 dispatch하지 않는다. 이 저장과 review는 live trading 승인이나 주문 지시와는 분리한다.
...
### 예시 / 필요 상황
- Phase 32에서는 Validation Pack 계약으로 robustness preview, stress summary, Phase33 handoff를 보여준다. Phase 34 보정 이후 현재 main UI에서는 Final Review가 이 검증 언어를 재사용한다.
...
### 예시 / 필요 상황
- Phase 32의 `Suggested Sweeps`와 `Stress / Sensitivity Summary`는 실제 stress sweep 결과가 아니라, 다음에 실행할 검증 질문과 result row 계약이다.
...
## Paper Portfolio Tracking Ledger

### 기본 설명
실제 돈을 넣기 전에 후보나 Portfolio Proposal을 paper 상태로 관찰하기 위한 장부다.

단순히 과거 snapshot을 비교하는 것이 아니라,
시작일, target weight, tracking benchmark, review cadence, 재검토 기준을 가진 운영 기록을 뜻한다.

Phase 34 보정 이후 사용자-facing main flow에서는 별도 ledger 저장을 필수 단계로 요구하지 않고,
Final Review record 안에 paper observation 기준을 함께 남긴다.
이 ledger는 기존 Phase33 QA 기록이나 별도 운영 장부가 필요한 경우의 compatibility artifact로 유지한다.

### 왜 사용되는지
최종 실전 포트폴리오 선정 전에는 백테스트 결과뿐 아니라,
실제 운용을 가정한 관찰 기간과 상태 변화 기록이 필요하기 때문이다.
...
- 현재 main UI에서는 `Backtest > Final Review`의 모니터링 후보 선정 저장과 `Evidence Appendix`에서 benchmark, cadence, trigger를 확인하고 final review record 안에 남긴다.
...
후보 handoff policy와 운용성 해석에 직접 연결되는 입력값 묶음이다.

### 왜 사용되는지
기본 백테스트 숫자만으로는
benchmark 대비 의미가 있는지, 비용 / 유동성 가정이 어느 정도인지,
다음 검증 단계로 넘길 수 있는지까지 판단하기 어렵기 때문이다.

### 예시 / 필요 상황
- 위치:
  - `Backtest > 전략 선택 > Advanced Inputs > Promotion Policy Signal`
- 이 안에서는 보통
  - investability filter
  - transaction cost
  - benchmark contract
...
실전형 승격 판단에 필요한 benchmark, policy, execution preview 상태까지 한 번에 보기 어렵기 때문이다.

### 예시 / 필요 상황
- `Backtest > Single Strategy`에서 전략 실행 후
  결과 상단 탭 중 `Real-Money`를 열면
  `Promotion Decision`, `Promotion Suggested Route`, `Next Validation Focus`, `Execution Preview` 같은 섹션을 볼 수 있다.
...
### 기본 설명
현재 백테스트 결과를 기준으로,
이 전략을 모니터링 후보로 어느 정도까지 해석할 수 있는지를 요약한 상위 판단값이다.

### 왜 사용되는지
전략이 존재하고 백테스트가 돌아간다고 해서
곧바로 투자 후보나 live 승인 대상이 되는 것은 아니기 때문이다.
...
## Promotion Suggested Route

### 기본 설명
현재 사용자-facing Real-Money 화면에서는 독립 검증 단계가 아니라,
`Promotion Decision` 아래의 `Suggested Route`로 표시되는 후보 경로 값이다.
즉 "Promotion 결과상 다음에 어디에 둘 것인가?"를 말한다.

### 왜 사용되는지
실제 검증 흐름에서는 전략을 바로 투자하지 않고,
watchlist review, paper observation, small-capital review 같은 후속 검토 경로로 나눠 관리하는 편이 현실적이기 때문이다.
다만 이 값은 Promotion보다 중요한 별도 검증이 아니라 Promotion의 추천 경로다.

### 예시 / 필요 상황
- 어떤 전략이 `real_money_candidate`여도
  바로 live가 아니라 `Paper Observation Candidate`로 먼저 둘 수 있다.
- annual strict 중 조건이 더 잘 맞는 전략은 `Small-Capital Review Candidate`까지 검토할 수 있다.

---

## Promotion Route Status

### 기본 설명
현재 전략이 Promotion 결과상 어느 추천 경로에 있는지를 나타내는 내부 metadata 값이다.
기존 metadata 이름은 `shortlist_status`지만, 사용자 화면에서는 `Promotion Suggested Route`로 읽는다.

### 왜 사용되는지
같은 후보 전략이라도
지금은 단순 재검토 대상인지,
paper observation 조건을 확인할 후보인지,
소액 검토 가능성까지 볼 후보인지가 다르기 때문이다.

### 예시 / 필요 상황
- `watchlist`
  - 아직은 후보 재검토 단계
- `paper_probation`
  - 다음 단계에서 paper observation 조건을 확인할 후보
- `small_capital_trial`
  - 다음 단계에서 소액 검토 가능성까지 확인할 후보
- `hold`
  - 지금은 Promotion route를 Hold / Review로 두는 단계

---

## Promotion Route Next Step

### 기본 설명
현재 Promotion 추천 경로에서 다음으로 해야 할 실무 행동을 짧게 적어둔 내부 metadata 값이다.
기존 metadata 이름은 `shortlist_next_step`지만, 사용자 화면에서는 `Promotion Route Next Step`으로 읽는다.
...
### 예시 / 필요 상황
- `manual_review_then_practical_validation_gate`
  - 먼저 수동 검토를 하고, Practical Validation으로 보낼 수 있는지 본다
- `send_to_practical_validation_for_paper_observation_check`
  - paper observation 요건을 다음 검증 단계에서 확인한다
- `send_to_practical_validation_for_small_capital_review`
  - 소액 검토 가능성을 다음 검증 단계에서 확인한다
...
Backtest Analysis에서는 이 상태를 실제로 시작한 것처럼 표시하지 않고, 후속 검증 경로 또는 paper observation 조건으로만 읽는다.

### 왜 사용되는지
좋아 보이는 전략도 바로 live로 올리기보다,
Final Review 이후의 paper observation이나 소액 검토 조건을 거쳐 검증하는 편이 현실적이기 때문이다.
...
Backtest Analysis의 1차 후보 선별 화면에서는 주기를 확정하지 않는다.
...
Backtest Analysis에서 확정하는 값이 아니라, Final Review / Portfolio Monitoring 쪽에서 다루는 운영 기준이다.
...
Backtest Analysis에서는 이 값을 실제 monitoring 항목이 아니라 `Next Validation Focus`로 표시한다.
...
Backtest Analysis에서는 실제 breach가 아니라 다음 검증에서 재확인할 `Validation Review Signals`로 표시한다.

### 왜 사용되는지
실전 관찰 단계에서는 어떤 신호가 재발하면 더 보수적으로 볼지 미리 정해두는 편이 안전하기 때문이다.
...
## Split-Period Check / Legacy Out-of-Sample Review

### 기본 설명
Backtest Real-Money 1차 화면에서 전후반 구간을 나눠 성과가 특정 시기 우연에 크게 의존하지 않았는지 보는 간이 점검이다.
legacy metadata 이름은 `out_of_sample_review_status`로 남아 있지만, 사용자 화면에서는 formal holdout 검증처럼 보이지 않도록 `Split-Period Check`로 표시한다.

### 왜 사용되는지
한 구간에서만 좋았던 전략을 더 보수적으로 해석하기 위해서다.
다만 이 값은 Practical Validation의 walk-forward / OOS / regime evidence를 대체하지 않으며, Backtest 단계에서는 review signal로만 읽는다.
...
## Front-Half Excess / Legacy In-Sample Excess
...
### 예시 / 필요 상황
- 전반부는 매우 좋고 후반부는 급격히 나빠졌다면 split-period 해석이 더 보수적일 수 있다.

---

## Back-Half Excess / Legacy Out-Sample Excess
...
### 예시 / 필요 상황
- 후반부 excess가 급격히 악화되면 split-period check가 `watch` 또는 `caution`으로 읽힐 수 있다.
...
## Execution Preview / Legacy Deployment Readiness

### 기본 설명
legacy metadata에서는 실제 배치 직전 checklist처럼 쓰였지만,
Backtest Analysis 화면에서는 `Execution Preview`로 낮춰 표시한다.
즉 지금 배치를 열 수 있다는 뜻이 아니라 비용, 유동성, 운용 가능성 같은 실행 부담을 미리 보는 값이다.

### 왜 사용되는지
`Promotion`은 후보 평가와 추천 경로를 함께 보여주고,
`Execution Preview`는 다음 검증으로 넘기기 전에 실행 부담 blocker가 큰지 확인하기 위해서다.
Backtest Analysis에서는 legacy `Probation`, `Monitoring`, `Shortlist` 파생값을 Execution Preview 점수에 다시 반영하지 않는다.
...
live 투입 전에는 paper tracking 기간, 재검토 날짜, 중단 조건, 데이터 재수집 필요 여부를 별도로 기록해야 한다.
...
### 왜 사용되는지
Pre-Live 상태값만 있으면 Real-Money promotion route와 거의 비슷하게 보일 수 있다.
다음 행동 기록이 있어야
...
## Execution Preview Status Count

### 기본 설명
execution preview 안에서 `Pass / Watch / Fail / Unavailable`가 각각 몇 개인지 세어 보여주는 요약이다.
source checks만 세며, 후속 단계인 probation / monitoring 상태를 재평가하지 않는다.

### 왜 사용되는지
한 항목씩 다 읽기 전에,
다음 검증으로 넘기기 전 실행 부담 blocker가 전체적으로 어느 정도인지 빠르게 스캔할 수 있기 때문이다.

### 예시 / 필요 상황
- `Pass`
  - 기준을 무난하게 충족한 항목 수
- `Watch`
  - 다음 검증에서 더 보수적으로 봐야 하는 항목 수
- `Fail`
  - 다음 검증 전 확인이 필요한 항목 수
- `Unavailable`
  - 판단 근거가 부족한 항목 수
...
어떤 overlay를 켜는지에 따라
`Promotion`, `Execution Preview`, `Validation` 해석이 달라질 수 있기 때문이다.
...
- helper:
  - `.aiworkspace/plugins/quant-finance-workflow/scripts/manage_pre_live_candidate_registry.py`
...
### 기본 설명
legacy workflow에서 백테스트 후보를 바로 투자 추천으로 확정하기 전에,
왜 후보로 남아 있는지와 다음에 무엇을 할지 검토하던 단계다.

현재 primary workflow에서는 Candidate Review를 필수 단계로 사용하지 않는다.
새 후보는 `Backtest Analysis`에서 Practical Validation source로 보내고,
검증과 최종 판단은 `Practical Validation`과 `Final Review`가 맡는다.

### 왜 사용되는지
current candidate, near miss, scenario 후보가 섞이면
사용자가 성과표만 보고 바로 투자 후보나 승인 대상으로 오해할 수 있다.
Candidate Review registry와 helper는 archive / recovery compatibility로 남긴다.

### 예시 / 필요 상황
- legacy `Backtest > Candidate Review`
- `Candidate Board`
- `Inspect Candidate`
- `Suggested Next Step`
...
### 기본 설명
legacy `Latest Backtest Run` 또는 `History` 결과를 후보 검토 화면에서 읽기 위한 임시 초안이다.
현재 candidate registry에 저장된 후보가 아니다.

현재 `Operations > Archive: Backtest Runs`의 정식 action은 legacy candidate draft queue가 아니라 Practical Validation source handoff다.
...
### 예시 / 필요 상황
- legacy `Latest Backtest Run > Candidate Review Handoff > Review As Candidate Draft`
- legacy `History > Selected History Run > Review As Candidate Draft`
- legacy `Candidate Review > Candidate Intake Draft`
...
### 기본 설명
Practical Validation Gate를 통과한 source를 Operations > Portfolio Monitoring 후보로 선정할지 판단하는 전용 화면이다.

`Backtest > Final Review`는 validation, robustness / stress 질문, paper observation 기준, operator judgment를 하나로 묶어 최종 검토 결과를 기록한다.

### 왜 사용되는지
Backtest Analysis / Practical Validation 중간 단계에 최종 저장 버튼을 계속 붙이면 사용자가 각 저장의 의미를 이해하기 어렵다.

그래서 Final Review는 "초안 작성"과 "모니터링 후보 선정 저장"을 분리해,
최종 판단이 live approval이나 주문 지시가 아니라는 경계를 더 선명하게 만든다.

### 예시 / 필요 상황
- Practical Validation Gate를 통과한 source를 선택해 blocker, robustness 질문, paper observation 기준을 한 화면에서 확인한다.
- 모니터링 후보 선정 route는 `SELECT_FOR_PRACTICAL_PORTFOLIO`이며, selected-route gate가 통과됐을 때만 정식 저장한다.
- `모니터링 후보로 선정`을 누르면 `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`에 append-only row가 남는다.
- 이 기록은 Phase35 운영 가이드의 입력이며, live approval이나 broker order가 아니다.

---

## Final Portfolio Selection Decision Pack
...
그 자체가 자동 선정이나 주문 지시는 아니다.
모니터링 후보 판단은 백테스트 성과, validation blocker, stress gap, paper observation 기준,
operator reason을 함께 읽고 사람이 명시적으로 남겨야 한다.
...
- `선정`도 "Portfolio Monitoring 후보로 선택했다"는 검토 기록이지, 실제 주문 실행이 아니다.
...
### 왜 사용되는지
`SELECT_FOR_PRACTICAL_PORTFOLIO`는 "Portfolio Monitoring 후보로 고르겠다"는 검토 기록이지,
...
- live approval이나 broker order와 모니터링 후보 선정 기록을 구분해야 할 때
- 별도 저장 위치:
  - 없음
- 원본 판단 위치:
  - `.aiworkspace/note/finance/registries/FINAL_PORTFOLIO_SELECTION_DECISIONS.jsonl`
...
- Phase35는 모니터링 후보를 다시 저장하는 단계가 아니라 workflow를 Final Review에서 끝내도록 정리한 단계다.
...
### 기본 설명
전략을 실행하거나 여러 후보를 비교해서 Practical Validation으로 보낼 selection source를 만드는 단계다.

### 왜 사용되는지
최종 판단 전에 후보가 어떤 설정과 성과 근거에서 나온 것인지 분리해서 남겨야 하기 때문이다.

### 예시 / 필요 상황
- 단일 전략을 실행해 후보로 저장할 때
- Compare에서 여러 전략이나 ETF mix를 비교할 때
- saved portfolio mix를 다시 실행해 검증 후보로 보낼 때
...
현재 사용자-facing 이름은 `Operations > Portfolio Monitoring`이다.
`Selected Portfolio Dashboard`는 일부 구현 파일과 saved JSONL 이름에 남은 legacy 명칭이다.
...
### 기본 설명
Backtest Analysis에서 Practical Validation으로 넘기는 후보 입력 기록이다.

### 왜 사용되는지
단일 전략, compare 결과, saved mix replay가 같은 검증 단계에서 읽히려면 입력 계약이 하나로 정리되어야 하기 때문이다.

### 예시 / 필요 상황
- `PORTFOLIO_SELECTION_SOURCES.jsonl`에 저장된 row
- `selection_source_id`로 Practical Validation result와 Final Review decision을 연결할 때
...
### 기본 설명
Practical Validation에서 후보를 확인하는 개별 검증 항목이다.

### 왜 사용되는지
포트폴리오를 하나의 점수로만 보지 않고, 데이터 신뢰도, 자산 배분, 비용, macro, stress 같은 원인을 분리해서 보기 위해 사용한다.

### 예시 / 필요 상황
- 12개 Practical Diagnostics 중 `Operability / Cost / Liquidity`를 따로 확인할 때
- `NOT_RUN`이 어떤 진단에서 발생했는지 구분할 때
...
### 기본 설명
provider, FRED, DB snapshot처럼 해당 진단 목적에 직접 맞는 실제 근거다.

### 왜 사용되는지
추정이나 보조 근거가 아니라 검증 항목 자체를 판단할 수 있는 가장 강한 근거를 구분하기 위해 사용한다.

### 예시 / 필요 상황
- issuer official holdings file에서 ETF 구성 종목을 읽은 경우
- FRED series로 VIX, yield curve, credit spread를 읽은 경우
...
### 기본 설명
원래 목적은 다르지만 이미 DB에 있어서 보조 근거로 사용할 수 있는 데이터다.

### 왜 사용되는지
official provider data가 비어 있을 때도 기존 price history, asset profile 같은 데이터로 일부 진단을 계산할 수 있기 때문이다.

### 예시 / 필요 상황
- `nyse_price_history` 거래대금으로 ADV를 추정할 때
- `nyse_asset_profile`의 AUM / expense ratio를 operability 보조 근거로 쓸 때
...
### 기본 설명
직접 데이터가 아니라 ticker 이름, benchmark 움직임, 단순 분류 규칙처럼 추정에 가까운 근거다.

### 왜 사용되는지
검증을 완전히 비워두기보다 약한 신호를 표시할 수 있지만, 실제 통과처럼 오해되지 않게 origin을 구분해야 하기 때문이다.

### 예시 / 필요 상황
- holdings가 없어서 ticker family만 보고 asset class를 추정할 때
- 실제 macro series 없이 benchmark drawdown만 보고 regime을 추정할 때
...
### 기본 설명
데이터나 구현이 없어 진단을 실행하지 못한 상태다.

### 왜 사용되는지
검증을 통과한 것이 아니라 아직 확인하지 못한 항목임을 명확히 표시하기 위해 사용한다.

### 예시 / 필요 상황
- ETF holdings connector가 없는 종목이 포함된 경우
- stress replay가 필요한 daily curve를 아직 계산하지 못한 경우
...
### 기본 설명
근거가 부분적이거나 사람이 확인해야 하는 상태다.

### 왜 사용되는지
실패는 아니지만 자동으로 통과 처리하기에는 설명이 부족한 경우를 분리하기 위해 사용한다.

### 예시 / 필요 상황
- operability snapshot은 있으나 official row와 DB bridge를 섞어 해석해야 하는 경우
- exposure concentration이 높지만 profile에 따라 허용될 수도 있는 경우
...
### 기본 설명
가격 부재, 실행 경계 위반, 핵심 데이터 오류처럼 검증을 계속하면 위험한 상태다.

### 왜 사용되는지
Final Review로 넘기기 전에 반드시 멈추고 원인 화면으로 돌아가야 하는 문제를 구분하기 위해 사용한다.

### 예시 / 필요 상황
- 핵심 ETF price curve가 없는 경우
- target allocation 합계가 정상 범위를 벗어난 경우
- profile과 명백히 충돌하는 leveraged / inverse exposure가 큰 경우
...
### 기본 설명
ETF issuer, FRED, DB bridge에서 수집해 DB에 저장한 기준일 데이터다.

### 왜 사용되는지
Practical Validation이 UI에서 직접 원격 호출하지 않고 loader를 통해 재사용 가능한 검증 근거를 읽게 하기 위해 사용한다.
...
### 기본 설명
ETF ticker만 보는 것이 아니라 ETF 내부 구성 종목과 exposure를 들여다보는 방식이다.

### 왜 사용되는지
겉으로는 여러 ETF를 들고 있어도 내부 holdings가 같은 대형주에 겹칠 수 있기 때문이다.

### 예시 / 필요 상황
- QQQ, SOXX, MTUM이 함께 있을 때 기술주 노출이 과도하게 겹치는지 확인할 때
- ETF 내부 국가 / sector / asset class 비중을 계산할 때
...
### 기본 설명
phase 설계, 작업 분해, 통합, 문서 체계 관리를 담당하는 주 작업 세션이다.

### 왜 사용되는지
여러 병렬 작업이 섞일 때 전체 방향과 완료 기준을 한곳에서 관리해야 하기 때문이다.

### 예시 / 필요 상황
- 이 `codex/main-dev` worktree처럼 phase 계획과 문서 구조 개편을 담당하는 세션
...
### 기본 설명
Main Worktree가 분리한 개별 task를 수행하는 작업 세션이다.

### 왜 사용되는지
후보 탐색, UX polish, 특정 connector 개발처럼 독립적인 작업을 병렬로 진행하기 위해 사용한다.

### 예시 / 필요 상황
- `codex/research`에서 후보 탐색과 재검증을 수행하는 경우
- `codex/sub-dev`에서 phase 범위 밖 UX polish를 수행하는 경우
...
### 기본 설명
실제 구현, 조사, 문서 정리, QA를 수행하는 실행 단위다.

### 왜 사용되는지
phase보다 작고 명확한 작업을 `PLAN`, `DESIGN`, `STATUS`, `RUNS` 같은 문서 안에서 끝까지 추적하기 위해 사용한다.

### 예시 / 필요 상황
- Practical Validation V2 provider connector 개발
