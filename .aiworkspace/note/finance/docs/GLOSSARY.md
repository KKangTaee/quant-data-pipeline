...
- Phase 30에서는 `Backtest > Portfolio Proposal`에서 proposal draft를 만들고 `.aiworkspace/note/finance/registries/PORTFOLIO_PROPOSAL_REGISTRY.jsonl`에 append-only로 저장한다. `Monitoring Review`에서는 저장된 proposal의 blocker와 review gap을 다시 확인하고, `Pre-Live Feedback`에서는 proposal snapshot과 현재 Pre-Live 상태를 비교하며, `Paper Tracking Feedback`에서는 proposal 저장 당시 evidence snapshot과 현재 Pre-Live result snapshot의 CAGR / MDD 변화를 비교한다. Phase 34 보정 이후 최종 검토와 판단 기록은 `Backtest > Final Review`가 맡는다. 이 저장과 review는 live trading 승인이나 주문 지시와는 분리한다.
...
Backtest Analysis에서 확정하는 값이 아니라, Final Review / Selected Dashboard 쪽에서 다루는 운영 기준이다.
...
### 기본 설명
단일 current candidate 또는 saved Portfolio Proposal을 Selected Dashboard 모니터링 후보로 선정할지 판단하는 전용 화면이다.
...
### 왜 사용되는지
Portfolio Proposal 탭 안에 `Save Paper Tracking Ledger`, `Save Final Selection Decision` 같은 저장 버튼을 계속 붙이면 사용자가 각 저장의 의미를 이해하기 어렵다.
...
후보나 proposal을 Selected Dashboard 모니터링 후보 `선정`, `보류`, `거절`, `재검토` 중 어디에 둘지 판단하는 근거 묶음이다.
...
- 후보나 portfolio proposal을 Selected Dashboard 모니터링 후보로 선정할지, 더 관찰할지, 거절할지 기록해야 할 때
- 저장 위치:
...
- `선정`도 "Selected Dashboard 모니터링 후보로 선택했다"는 검토 기록이지, 실제 주문 실행이 아니다.
...
### 왜 사용되는지
`SELECT_FOR_PRACTICAL_PORTFOLIO`는 "Selected Dashboard 모니터링 후보로 고르겠다"는 검토 기록이지,
