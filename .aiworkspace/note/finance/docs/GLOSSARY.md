...
### 기본 설명
리밸런싱 날짜마다 그 시점 기준으로 후보 종목 집합을 다시 계산하는 legacy 모집군 방식이다.
...
### 예시 / 필요 상황
- `Quality Snapshot` legacy broad path처럼
  방향성은 맞지만 strict candidate 계약이 완전히 붙지 않은 상태를 뜻한다.
...
- `Phase 21`에서는 다음 조건을 같은 validation frame으로 고정했다.
  ...
  - universe:
    ...
    - `Historical Dynamic PIT Universe` legacy internal contract
  - 후보 묶음:
