...
2. `Workspace > Overview > Market Movers`
   ...
   - `Why It Moved`에서 Return Rank 또는 Volume Rank ticker를 선택하면 먼저 선택 ticker의 movement summary header를 확인한다. Header는 Symbol / Name / Sector / Industry / Market Cap, period / coverage / rank context, return / previous return / momentum delta / volume / dollar volume을 묶어서 보여준다.
   - Movement header 아래 metadata status strip에서 `Lookup status`, News row count 또는 failure, SEC row count 또는 failure, `fetched_at`, `Session-only` boundary를 확인한다. `PARTIAL`은 warning 상태이며 complete success가 아니다.
   ...
   - `Investigation Leads`에서 `News Metadata`, `SEC Filings`, `External Searches`를 확인한다. Compact news metadata는 title, source / publisher, published time, URL만 표시한다. Compact SEC metadata는 form, filing date, title / description, URL만 표시하고, SEC table은 form / filing date / title / `Open` 중심으로 표시한다. metadata table의 URL은 클릭 가능한 `Open` link로 표시한다. 조회 상태는 `not requested`, `OK`, `partial`, `no metadata`, `failed`를 구분한다. `partial`은 한 provider가 실패했지만 다른 provider row가 있는 상태이며 complete lookup으로 해석하지 않는다.
   - Yahoo Finance / Google News / SEC company search / IR earnings / Google News KR / Naver News outbound search는 `Investigation Leads > External Searches` expander의 클릭 가능한 `Open` link table에서 확인한다. 이 링크들은 앱 내부 조회나 저장을 실행하지 않는 보조 research start point이며, 기본 상태는 collapsed다.
...
- Market Movers `Why It Moved` renders a ticker selector sourced from Return Rank and Volume Rank rows, then shows a movement summary header with selected ticker identity, rank / coverage / period context, return / previous return / momentum delta, volume, and dollar volume.
- Market Movers `Why It Moved` shows a metadata status strip before the fetch button: lookup status, News rows / failure, SEC rows / failure, fetched timestamp, and `Session-only` storage boundary. `PARTIAL` is warning-level evidence, not a complete lookup.
- Market Movers `Why It Moved` uses `Investigation Leads` sections for `News Metadata`, `SEC Filings`, and collapsed `External Searches`. Yahoo Finance, Google News, SEC company search, IR / earnings, Google News KR, and Naver News stay as collapsed external rows with clickable `Open` URLs instead of primary action buttons.
...
- `Fetch compact metadata` is button-only and selected-ticker-only. The not-yet-run state is visible before lookup; `OK`, `PARTIAL`, no metadata, and failed lookup states are separate. Metadata URL columns render as clickable `Open` links. SEC filings are displayed as form / filing date / title / open and sorted by date with form-priority tie-breaks.
