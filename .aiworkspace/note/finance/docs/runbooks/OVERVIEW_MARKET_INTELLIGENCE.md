...
4. `Workspace > Overview > Futures Monitor`
   ...
   - `Macro Thermometer` 탭은 core 16개 선물의 저장된 1d OHLCV를 읽어 Risk-On, Growth, Rate Pressure, Dollar Pressure, Safe Haven, Inflation Pressure score와 오늘의 시장 해석을 표시한다.
   - Macro Thermometer가 비어 있으면 `Refresh Daily Macro OHLCV`를 눌러 core 16개 `1y / 1d` backfill을 실행하거나, `Workspace > Ingestion > 선물 OHLCV 수집`에서 `Period=1y`, `Interval=1d`로 수동 실행한다.
...
- Futures Macro Thermometer shows six standardized daily score cards, scenario summary, evidence tickers, score components, symbol-level 1D / 3D / 5D / 20D / 60D returns, 60D volatility standardized move, 252D position, and caution copy.
