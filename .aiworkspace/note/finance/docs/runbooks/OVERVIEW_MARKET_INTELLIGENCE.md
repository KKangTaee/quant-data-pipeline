...
4. `Workspace > Overview > Futures Monitor`
   ...
   - `과거 점검` 결과는 `판정`, `5거래일 표본`, `20거래일 표본`, `자산군 해석`으로 읽는다. 이 문구는 validation metrics에 있는 표본 수, 평균 수익률, 방향 일관성, target family, hit rule만 사용하며, 계산값 없는 매수/매도 추천 문구를 만들지 않는다.
