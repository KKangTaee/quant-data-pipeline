...
2. `Workspace > Overview > Market Movers`
   ...
   - Movement header 아래 metadata status strip에서 `조회 상태`, 뉴스 row count 또는 failure, SEC row count 또는 failure, `조회 시각`, `세션 전용` boundary를 확인한다. `PARTIAL`은 warning 상태이며 complete success가 아니다.
   ...
   - `조사 단서`에서 `뉴스 메타데이터`, `SEC 공시`, `외부 검색`을 확인한다. Compact news metadata는 제목, 출처 / publisher, 게시 시각, URL만 표시한다. Compact SEC metadata는 양식, 공시일, 제목 / description, URL만 표시하고, SEC table은 양식 / 공시일 / 제목 / `열기` 중심으로 표시한다. metadata table의 URL은 클릭 가능한 `열기` link로 표시한다. 조회 상태는 `조회 전`, `완료`, `부분 완료`, `메타데이터 없음`, `실패`를 구분한다. `부분 완료`는 한 provider가 실패했지만 다른 provider row가 있는 상태이며 complete lookup으로 해석하지 않는다.
   - `SEC 공시`에서 특정 filing 1건의 원문 단서가 필요하면 `원문 미리보기 대상 공시`를 선택한 뒤 `선택 공시 원문 미리보기`를 누른다. Selectbox 변경만으로 외부 조회는 실행되지 않는다. 미리보기는 이번 Streamlit session state에만 남고, form / filing date / title / accession / document / official SEC `열기` link, fetch status, fetched_at, session-only boundary, `공시 Digest`, bounded section snippets만 표시한다. 8-K는 Item heading card와 Exhibit 단서 중심이며, 10-Q / 10-K는 Table of Contents, Part / Item, MD&A, Risk Factors, Financial Statements 위치 단서와 작은 bounded table preview 중심이다. Digest의 `earnings_or_results`, `regulation_fd`, `risk_factors`, `exhibits_or_attachments` 같은 값은 위치 탐색용 힌트이며 원인 판정이 아니다. 문서가 너무 크거나 HTML/iXBRL 파싱이 실패하면 official SEC `열기` link와 실패 상태를 확인한다.
...
- Market Movers `Why It Moved` shows a metadata status strip before the fetch button: `조회 상태`, 뉴스 rows / failure, SEC rows / failure, `조회 시각`, and `세션 전용` storage boundary. `PARTIAL` / `부분 완료` is warning-level evidence, not a complete lookup.
- Market Movers `Why It Moved` uses `조사 단서` sections for `뉴스 메타데이터`, `SEC 공시`, and collapsed `외부 검색`. Yahoo Finance, Google News, SEC company search, IR / earnings, Google News KR, and Naver News stay as collapsed external rows with clickable `열기` URLs instead of primary action buttons.
- Market Movers `Why It Moved > SEC 공시` preserves the compact SEC table as `양식 / 공시일 / 제목 / 열기`. If SEC metadata rows exist, a separate filing selector and `선택 공시 원문 미리보기` button appear. The selector does not fetch by itself; the button fetches only the selected filing and renders a session-only bounded preview with form, filing date, title, accession, document, official SEC `열기`, fetch status, fetched_at, `공시 Digest`, and 8-K Item / Exhibit or 10-Q / 10-K section-location snippets / bounded table clues.
