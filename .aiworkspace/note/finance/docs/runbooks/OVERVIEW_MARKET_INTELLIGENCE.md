...
2. `Workspace > Overview > Market Movers`
   ...
   - `SEC 공시`에서 특정 filing 1건의 원문 단서가 필요하면 `원문 미리보기 대상 공시`를 선택한 뒤 `선택 공시 원문 미리보기`를 누른다. Selectbox 변경만으로 외부 조회는 실행되지 않는다. 미리보기는 이번 Streamlit session state에만 남고, form / filing date / title / accession / document / official SEC `열기` link, fetch status, fetched_at, session-only boundary, bounded section snippets만 표시한다. 8-K는 Item heading 주변 snippet, 10-Q / 10-K는 Table of Contents, Part / Item, MD&A, Risk Factors, Financial Statements 위치 단서 중심이다. 문서가 너무 크거나 HTML/iXBRL 파싱이 실패하면 official SEC `열기` link와 실패 상태를 확인한다.
...
- Market Movers `Why It Moved > SEC 공시` preserves the compact SEC table as `양식 / 공시일 / 제목 / 열기`. If SEC metadata rows exist, a separate filing selector and `선택 공시 원문 미리보기` button appear. The selector does not fetch by itself; the button fetches only the selected filing and renders a session-only bounded preview with form, filing date, title, accession, document, official SEC `열기`, fetch status, fetched_at, and 8-K Item or 10-Q / 10-K section-location snippets.
