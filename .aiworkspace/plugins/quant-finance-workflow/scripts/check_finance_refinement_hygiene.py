...
GENERATED_PATTERNS = [
    ...
    ".aiworkspace/note/finance/phases/active/phase12/_tmp_gtaa_*.csv",
...
REGISTRY_DOCS = {
    ...
    ".aiworkspace/note/finance/registries/EXPERIMENT_REGISTRY.jsonl",
}
...
def _classify(paths: list[str]) -> dict[str, list[str]]:
    ...
    for path in paths:
        ...
        elif path.startswith(".aiworkspace/note/finance/reports/backtests/strategies/") and path.endswith(".md"):
            name = Path(path).name
            if name in {
...
def _build_checks(groups: dict[str, list[str]]) -> list[dict[str, str]]:
    ...
    if changed_strategy_families or any("CURRENT_PRACTICAL_CANDIDATES_SUMMARY.md" in path for path in other_docs):
        has_candidate_registry = ".aiworkspace/note/finance/registries/CURRENT_CANDIDATE_REGISTRY.jsonl" in registries
        checks.append(
            {
